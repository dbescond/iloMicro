...
* Note that according to the definition, the highest level being CONCLUDED is being considered
	...
	gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if b5p1a==1  			// No schooling
		replace ilo_edu_isced11=2 if b5p1a==2  			// Early childhood education
		replace ilo_edu_isced11=3 if b5p1a==3 	// Primary education
		replace ilo_edu_isced11=4 if inlist(b5p1a,4,5)   	// Lower secondary education
		replace ilo_edu_isced11=5 if inlist(b5p1a,6,7)   // Upper secondary education
		*replace ilo_edu_isced11=6 if 					  	// Post-secondary non-tertiary
		replace ilo_edu_isced11=7 if inlist(b5p1a,8,9)  // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if b5p1a==10  			// Bachelor or equivalent
		replace ilo_edu_isced11=9 if b5p1a==11  			// Master's or equivalent level
	...
	/* NOTE: the category Master's or equivalent also includes the Doctoral or equivalent level as there is no distinction between 
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=2 if (ilo_lfs!=1 & b5p24!=2 & (b5p4==1 | b5p5==1 | b5p23==2)) 	// Unemployed
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (b5p2a1==1 | b5p3==1)   									// Employed
		replace ilo_lfs=2 if (ilo_lfs!=1 & b5p23!=2 & (b5p4==1 | b5p5==1 | b5p22==2)) 	// Unemployed
		replace ilo_lfs=3 if (ilo_lfs!=1 & ilo_lfs!=2 & ilo_wap==1)  					// Outside the labour force
			}
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if b5p17==2
		replace ilo_mjh=2 if b5p17==1			
			}
			
	else {
	
	gen ilo_mjh=.
		replace ilo_mjh=1 if b5p16==2
		replace ilo_mjh=2 if b5p16==1
			}
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inlist(b5p10a,4,5,6)	// Employees
			replace ilo_job1_ste_icse93=2 if b5p10a==3				// Employers
			replace ilo_job1_ste_icse93=3 if inlist(b5p10a,1,2)		// Own-account workers
			*replace ilo_job1_ste_icse93=4 if						// Producer cooperatives
			replace ilo_job1_ste_icse93=5 if b5p10a==7				// Contributing family workers
...
* Use ISIC Rev. 3.1 
	...
	gen indu_code_prim=int(b5p7/1000) if ilo_lfs==1 & b5p7!=99999
	...
	* Primary activity
	...
	gen ilo_job1_eco_aggregate=.
		...
		* Secondary activity
		...
		gen indu_code_sec=int(b5p18/1000) if ilo_mjh==2 & b5p18!=99999
		
			}
		
		else {
		
		gen indu_code_sec=int(b5p17/1000) if ilo_mjh==2 & b5p17!=99999
...
* -------------------------------------------------------------------------------------------		
	...
	/* National classification of occupations (KBJI 2002) is based on ISCO 88.
	   ...
	   NOTE: the missing values are coded as 0, which is the same code as the occupation
		...
		gen occ_code_prim=int(b5p8/100) if ilo_lfs==1
...
* -------------------------------------------------------------------------------------------		
	...
	* Only hours actually worked available
		...
		* All jobs:
			gen ilo_joball_how_actual=b5p6b
...
* -------------------------------------------------------------------------------------------			
	...
	/* NOTES:
		...
		* Main job
			...
			* Employees
				egen ilo_job1_lri_ees =rowtotal(b5p12a b5p12b) if ilo_job1_ste_aggregate==1, m
							replace ilo_job1_lri_ees=. if ilo_job1_lri_ees<0
						lab var ilo_job1_lri_ees "Monthly earnings of employees in main job"	
			
			* Self-employed:
				egen ilo_job1_lri_slf =rowtotal(b5p12a b5p12b) if ilo_job1_ste_aggregate==2, m
...
* -------------------------------------------------------------------------------------------
	...
	if time=="2009Q1" | year<2009{
		...
		gen ilo_cat_une=. 
			replace ilo_cat_une=1 if ilo_lfs==2 & b5p25==1		// Previously employed
			replace ilo_cat_une=2 if ilo_lfs==2 & b5p25==2		// Seeking first job
			replace ilo_cat_une=3 if ilo_lfs==2 & ilo_cat_une!=1 & ilo_cat_une!=2	//Unknown
				}
	else {
		gen ilo_cat_une=.
			replace ilo_cat_une=1 if ilo_lfs==2 & b5p24==1		// Previously employed
			replace ilo_cat_une=2 if ilo_lfs==2 & b5p24==2		// Seeking first job
			replace ilo_cat_une=3 if ilo_lfs==2 & ilo_cat_une!=1 & ilo_cat_une!=2	//Unknown
				}
...
* -------------------------------------------------------------------------------------------
	...
	/* NOTE: variable b5_r21a indicates number of years spent looking for a job
		...
		* generate a new variable indicating the total number of months looking for a job:
			...
			gen tot_month_une=.
				replace tot_month_une = 12*b5p21a + b5p21b if ilo_lfs==2 & b5p21a!=99 & b5p21b!=99
			...
			gen tot_month_une=.
				replace tot_month_une = 12*b5p20a + b5p20b if ilo_lfs==2 & b5p20a!=99 & b5p20b!=99
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_reason=.		
		replace ilo_olf_reason=1 if	(b5p23==1 & ilo_lfs==3)				//Labour market
		replace ilo_olf_reason=2 if	inlist(b5p23,3,4,7) & ilo_lfs==3	//Personal / Family-related
		replace ilo_olf_reason=3 if	(b5p23==6 & ilo_lfs==3)				//Does not need/want to work
		replace ilo_olf_reason=4 if (b5p23==8 | b5p23==.) & ilo_lfs==3	//Not elsewhere classified		
			 }
	...
	gen ilo_olf_reason=.		
		replace ilo_olf_reason=1 if	(b5p22==1 & ilo_lfs==3)				//Labour market
		replace ilo_olf_reason=2 if	inlist(b5p22,3,4,7) & ilo_lfs==3	//Personal / Family-related
		replace ilo_olf_reason=3 if	(b5p22==6 & ilo_lfs==3)				//Does not need/want to work
		replace ilo_olf_reason=4 if (b5p22==8 | b5p22==.) & ilo_lfs==3	//Not elsewhere classified
				}
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_dis=1 if ilo_lfs==3 & b5p24==1 & ilo_olf_reason==1
		}
		
	else {
	
	gen ilo_dis=1 if ilo_lfs==3 & b5p23==1 & ilo_olf_reason==1
		}			
