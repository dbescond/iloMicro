...
* DATASET USED: Mexico QLFS 2015Q4
* NOTES: 
...
cd "$inpath"
	...
	* for the definition of our variables, don't consider observations that correspond to "no interview" or absent person  from household
	...
	gen considered=1 if R_DEF=="00" & inlist(C_RES,"1","3")	
...
* Comment: completed level of education always to be considered!
	...
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if CS_P13_1=="00" | (CS_P13_1=="01" & educ_years<2) 															// No schooling
		replace ilo_edu_isced97=2 if (CS_P13_1=="01" & inlist(educ_years,2,3)) | (CS_P13_1=="02" & inrange(educ_years,1,5)) 					// Pre-primary education
		replace ilo_edu_isced97=3 if (CS_P13_1=="02" & educ_years==6) | (CS_P13_1=="03" & inrange(educ_years,1,2))								// Primary education
		replace ilo_edu_isced97=4 if (CS_P13_1=="03" & educ_years==3) | (CS_P13_1=="04" & inrange(educ_years,1,2))								// Lower secondary education
		replace ilo_edu_isced97=5 if (CS_P13_1=="04" & educ_years==3) | (inlist(CS_P13_1,"05","06","07") & inrange(educ_years,1,3)) | ///
												(CS_P13_1=="08" & educ_years==1)																// Upper secondary education
		/* replace ilo_edu_isced97=6 if */																										// Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if (inlist(CS_P13_1,"05","06","07") & educ_years>=4 & educ_years!=.) | (CS_P13_1=="08" & educ_years>=2 & educ_years!=.) |	///
										(CS_P13_1=="09" & inrange(educ_years,1,2))																// First stage of tertiary education 
		replace ilo_edu_isced97=8 if CS_P13_1=="09" & educ_years>=3 & educ_years!=.																// Second stage of tertiary education
...
* Comment: variables corresponding to the set of questions required to identify the status in employment are not being
			* available in the dataset --> therefore the variable defined by the NSO has to be used
				* People outside the labour force: according to their definition, people being available and those not being available both in this category 
					* (but not seeking a job)
 
	gen ilo_lfs=.
		replace ilo_lfs=1 if CLASE2==1
		replace ilo_lfs=2 if CLASE2==2
...
* Comment: No direct question asked about having a secondary job
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if T_TRA==1
		replace ilo_mjh=2 if T_TRA==2
...
* Comment: note that the variable is based on the variable created by the NSO --> original questions (i.e. variables) not
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if POS_OCU==1
			replace ilo_job1_ste_icse93=2 if POS_OCU==2
			replace ilo_job1_ste_icse93=3 if POS_OCU==3
			/*replace ilo_job1_ste_icse93=4 if  */
			replace ilo_job1_ste_icse93=5 if POS_OCU==4
...
* -------------------------------------------------------------------------------------------
*
* Comment: variable included in dataset only allows to define the aggregate level
...
* -------------------------------------------------------------------------------------------
*
* Comment: LFS uses SINCO (National classification system) and only most aggregate categories available --> doesn't allow to align it
			* to any ISCO classification 
			
			* Also, the aggregation level used by the LFS doesn't allow to align it to our aggregate classification (e.g. problem with agricultural workers, as 
				* they don't distinguish between skilled and unskilled agricultural workers, while our aggregate classification does...)
...
* Comment: 
	...
	* Primary occupation
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if TUE3==1
		replace ilo_job1_ins_sector=2 if TUE3!=1
...
* Comment: variable HRSOCUP used, being based on question 5b-c which are referring to main job 

				* use same number for "joball" variable, in order to be able to define hour bands

	* Actual hours worked 
		
		* Primary job
		
		gen ilo_job1_how_actual=HRSOCUP 
...
* Comment: legal maximum: 48 h per week, 8 h per day
			* reference used in technical document (reconstrucción variables): 35 h per week
...
* Comment: not written contracts considered as temporary

	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if TIP_CON==3
		replace ilo_job1_job_contract=2 if inlist(TIP_CON,2,4,5)
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Informal/Formal economy: Unit of production (ilo_job1_ife_prod) [use predefined variable]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------

* Comment: necessary info for defining variable not available --> predefined variable being used: TUE_PPAL

			* note that original variable only considering "informal sector" and "outside the informal sector"
				* --> therefore households are being included in "outside the formal sector", here being labelled as "formal sector"

	
	* Useful questions: TUE3: Institutional sector 
					*	[no question]: Destination of production 
					*	[no question]: Bookkeeping
					*	[no question]: Registration
					*	[no question]: Location of workplace
					* 	EMPLE7C: Size of the establishment
	...
	* Social security coverage: person considered as having a social security coverage when:
	...
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=1 if TUE_PPAL==1
		replace ilo_job1_ife_prod=2 if TUE_PPAL==2
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Informal/formal economy: nature of job (ilo_job1_ife_nature) [use predefined variable]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------

		* predefined variable: EMP_PPAL

	gen ilo_job1_ife_nature=.
		replace ilo_job1_ife_nature=1 if EMP_PPAL==1
		replace ilo_job1_ife_nature=2 if EMP_PPAL==2
...
* Comment: 
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if D_ANT_LAB==1
		replace ilo_cat_une=2 if D_ANT_LAB==2
...
* Comment: variables DISPO and NODISPO do not have value labels and there is no description in the 
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Discouraged job-seekers ('ilo_dis') [check with Yves]
* -------------------------------------------------------------------------------------------
* ------------------------------------------------------------------------------------------- 

* Comment: * use directly people that are considered as "available for work but not looking for work as considering that there are no possibilities" 
				* ("disponible para trabajar que no busca empleo por considerar que no tiene posibilidades")

		gen ilo_dis=1 if ilo_lfs==3 & PNEA_EST==2
