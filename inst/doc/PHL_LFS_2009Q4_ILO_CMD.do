...
global source "LFS"
global time "2009Q4"
global inputFile "PHL_LFS_2009Q4"
...
cd "$inpath"
	...
	gen considered=1 if !inlist(c10_cnwr,1,2,3)	
...
*******************************************************************************************
	...
	* rename a set of variables for given time periods
	
		if inlist(year,2004,2006) | time=="2005Q1" | (year==2003 & inlist(quarter,3,4)) {
...
* Comment: 
	...
	if inrange(year,2007,2009) | (year==2005 & quarter!=1) {	
		
		if year==2008 & inlist(quarter,1,3) {
	...
	if year==2004 | (year==2005 & quarter==1) | (year==2003 & inlist(quarter,3,4)) {	
		gen ilo_wgt=cfwgt		
			}
	
	if year<2003 | (year==2003 & inlist(quarter,1,2)) {			
		gen ilo_wgt=rfadj
			}
...
* Comment: 
	...
	if year<=2002 | (year==2003 & inlist(quarter,1,2)) {
		gen ilo_sex=c04_sex 
			}
		else {
		gen ilo_sex=c06_sex
...
* Comment: Note that according to the definition, the highest level being CONCLUDED is being considered
		...
		* programs starting with "6" (e.g. 689) are not labelled... which makes category "unknown" quite large 
			* approximate with question on vocational training (check with Yves whether procedure correct)
...
/*
	...
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if j12c09_grade==0 
		replace ilo_edu_isced97=2 if inrange(j12c09_grade,10,260) | inrange(j12c09_grade,501,599)
		replace ilo_edu_isced97=3 if inrange(j12c09_grade,270,320)
		replace ilo_edu_isced97=4 if j12c09_grade==330
		replace ilo_edu_isced97=5 if inlist(j12c09_grade,340,350,410,420,810,820)
...
* Comment: c10_cursch
	...
	if year>=2005 & time!="2005Q1" {
	
	if year==2005 & inlist(quarter,2,3,4) {
	
		gen ilo_edu_attendance=c10_cursch
			}
			...
			gen ilo_edu_attendance=acursch
			
				}
		else {
		gen ilo_edu_attendance=a02_csch
			}
			}		
...
* Comment: 
	...
	if inrange(year,2007,2009) {
	
	gen ilo_lfs=.
		replace ilo_lfs=1 if c13_work==1 | c14_job==1
		replace ilo_lfs=2 if ilo_lfs!=1 & c38_lokw==1 & c37_avil==1
	...
	if year==2005 & inlist(quarter,2,3,4) {
		...
		gen ilo_lfs=.
			replace ilo_lfs=1 if c12_work==1 | c13_job==1
			replace ilo_lfs=2 if ilo_lfs!=1 & c31_lookw==1 & c37_avail==1
			replace ilo_lfs=3 if !inlist(ilo_lfs,1,2)
				replace ilo_lfs=. if ilo_wap!=1	
			}
		
	if (year==2003 & inlist(quarter,3,4))| year==2004 | time=="2005Q1" {
	...
	if year<=2002 | (year==2003 & inlist(quarter,1,2)) {
		...
		gen ilo_lfs=.
		replace ilo_lfs=1 if c11_work==1 | c12_job==1
		replace ilo_lfs=2 if ilo_lfs!=1 & c36_lookw==1 & c35_avail==1
		replace ilo_lfs=3 if !inlist(ilo_lfs,1,2)
			replace ilo_lfs=. if ilo_wap!=1			
...
* Comment: 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if c27_ojob==2
		replace ilo_mjh=2 if c27_ojob==1
		...
		gen ilo_mjh=.
		replace ilo_mjh=1 if ojind==2
		replace ilo_mjh=2 if ojind==1
		replace ilo_mjh=. if ilo_lfs!=1
...
* Comment: 
		...
		if inlist(year,2004,2006) | (year==2005 & quarter==1) | (year==2003 & inlist(quarter,3,4)) {
			clonevar c19pclas = c19_pclass			
				}
				
		if year==2005 & inlist(quarter,2,3,4) {		
		clonevar c19pclas = c24_pclass
			}
			
			
		if year<=2002 | (year==2003 & inlist(quarter,1,2)) {
				
		clonevar c19pclas = c17_pclass
				}
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inlist(c19pclas,0,1,2)
			replace ilo_job1_ste_icse93=2 if c19pclas==4
			replace ilo_job1_ste_icse93=3 if c19pclas==3
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if inlist(c19pclas,5,6)
...
* -------------------------------------------------------------------------------------------
*
* Comment: ISIC Rev. 3.1 being reported - find mapping as national classification being indicated in original dataset
	...
	gen indu_code_prim=c17f2_pkb if ilo_lfs==1
		}
		...
		gen indu_code_prim=c16a_pkb if ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
*
* Comment: secondary occupation: j02_otocc - no observations --> variable for secondary occupation 
										* can't be defined
	...
	* ISCO-08 classification used
	...
	if year==2005 & inlist(quarter,2,3,4) {
		...
		gen occ_code_prim=c15f2_procc if ilo_lfs==1 
		...
		if year<=2002 | (year==2003 & inlist(quarter,1,2)) {
			...
			gen occ_code_prim=c14a_procc if ilo_lfs==1
...
* Comment: 
	...
	* Primary occupation
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if c19pclas==2
	...
	* Actual hours worked 
		...
		* Primary job
		...
		if year<=2006 {
			...
			if year==2005 & inlist(quarter,2,3,4) {
			gen ilo_job1_how_actual=c20_phours
				}
			else {
			...
			gen ilo_job1_how_actual=c22_phrs
				}
		...
		* All jobs
		...
		if inrange(year,2007,2009) {
			gen ilo_joball_how_actual=a04_thrs
				}
		
		if year==2006 {
			gen ilo_joball_how_actual=athours 
				}
			
		if year==2005 & inlist(quarter,2,3,4) {
			gen ilo_joball_how_actual=c29_thours
				}
		
		if year<2005 | (year==2005 & quarter==1) {
			
			gen ilo_joball_how_actual=ilo_job1_how_actual 
						* check whether same variable should really be taken
				}
...
* Comment:
	...
	if year<=2006 {
			...
			if year==2005 & inlist(quarter,2,3,4) {
			gen ilo_job1_job_contract=c18_natem
				}
			else {
			...
			gen ilo_job1_job_contract=c20_natem
				}
				}
				}
				
		else {
			gen ilo_job1_job_contract=c20_ntem
				}
...
* Comment: 
	...
	* Useful questions: c19pclas: Institutional sector
...
* Comment:
	...
	* Primary employment 
		...
		if year<=2006 {
			if year==2005 & inlist(quarter,2,3,4) {
			gen ilo_job1_lri_ees=c26_pbasic*(365/12)
				}
			...
			gen ilo_job1_lri_ees=c27_pbsc*(365/12)
				}
...
* Comment:	Question asks how much time ther person has spent looking for another job
		...
		if year==2005 & inlist(quarter,2,3,4) {
		clonevar c40_wks = c34_weeks
			}
			
		if inlist(year,2004,2006) | (year==2005 & quarter==1) | (year==2003 & inlist(quarter,3,4)) {
		clonevar c40_wks = c40_weeks
			}
			
		if year<=2002 | (year==2003 & inlist(quarter,1,2)) {
		
		clonevar c40_wks = c38_weeks
			}			
		
	
	gen ilo_dur_details=.
		replace ilo_dur_details=1 if inrange(c40_wks,1,3)
		replace ilo_dur_details=2 if inrange(c40_wks,4,12)
		replace ilo_dur_details=3 if inrange(c40_wks,13,25)
		replace ilo_dur_details=4 if inrange(c40_wks,26,51)
		replace ilo_dur_details=5 if inrange(c40_wks,52,103)
		replace ilo_dur_details=6 if c40_wks>103 & c40_wks!=.
...
* Comment:
	...
	if year==2005 & inlist(quarter,2,3,4) {
		...
		gen ilo_cat_une=c41_lbef
			}
			
		else {		
		gen ilo_cat_une=c43_lbef
			}
			}	
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Previous occupation ('ilo_prevocu_isco08') [in progress]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			
* Comment: use ISCO-08 classification

	if year==2005 & inlist(quarter,2,3,4) {
		gen prevocu_cod=int(c41f2_pocc/10) if ilo_lfs==2 & ilo_cat_une==1
			}
		...
		if year<=2002 | (year==2003 & inlist(quarter,1,2)){
			...
			gen prevocu_cod=int(c43a_pocc/10) if ilo_lfs==2 & ilo_cat_une==1
...
* Comment: 
	...
	if year<=2006 {
		if year==2005 & inlist(quarter,2,3,4) {
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(c35_wynot,1,2,5)
		replace ilo_olf_reason=2 if inlist(c35_wynot,3,7,8)
			}
		if inlist(year,2004,2006) | (year==2005 & quarter==1) | (year==2003 & inlist(quarter,3,4)) {
	gen ilo_olf_reason=.
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(c40_wynot,1,2,5)
		replace ilo_olf_reason=2 if inlist(c40_wynot,3,7,8)
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(c42_wynt,1,2,5)
		replace ilo_olf_reason=2 if inlist(c42_wynt,3,7,8)
			}
...
* Comment: 	
	...
	if year<=2006 {
		if year==2005 & inlist(quarter,2,3,4) {
		gen ilo_dis=1 if ilo_lfs==3 & c35_wynot==1 & c37_avail==1
			}
		if inlist(year,2004,2006) | (year==2005 & quarter==1) | (year==2003 & inlist(quarter,3,4)) {
		...
		if year<=2002 | (year==2003 & inlist(quarter,1,2)) {
		gen ilo_dis=1 if ilo_lfs==3 & c40_wynot==1 & c35_avail==1		
			}
			}
			
	else {		
		gen ilo_dis=1 if ilo_lfs==3 & c42_wynt==1 & c37_avil==1
			}
