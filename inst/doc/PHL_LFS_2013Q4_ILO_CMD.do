...
global source "LFS"
global time "2013Q4"
global inputFile "LFS October 2013"
...
* Comment: 
	...
	gen ilo_wgt=pwgt*1000
	
		}

		else {
	gen ilo_wgt=pwgt/10000
		}
...
* Comment: Note that according to the definition, the highest level being CONCLUDED is being considered
		...
		* programs starting with "6" (e.g. 689) are not labelled... which makes category "unknown" quite large 
			* approximate with question on vocational training
			
			* variables for education not being kept, as large share falling into "level not stated" and many of the value labels
				* of the original variables are not being indicated
...
* Comment: 
		...
		gen ilo_edu_attendance=a02_cursch
...
* Comment: compare our numbers with variable "newempstat" (other variable considering labour force status, "cempst1", does not take into 
	...
	* Reconstruct "newempstat" variable (variables names won't be the same for 2012Q2) - refer to technical note in report for 2013Q4 (October 2013)
	
	if year==2013 | (year==2012 & inlist(quarter,3,4)) {
	
		gen nat_lfs=.
			replace nat_lfs=1 if c13_work==1 | c14_job==1
			replace nat_lfs=2 if nat_lfs!=1 & c37_avail==1 & (c38_lookw==1 | inlist(c42_wynot,1,2,3,4,5))
			replace nat_lfs=3 if !inlist(nat_lfs,1,2) & !inlist(c10_conwr,1,2,3)
				...
				* c10_conwr allows to exclude overseas workers, workers other than overseas workers and employees in Phil. embassies and consulates
				
				* nat_lfs coincides perfectly with "newempstat" and allows to reproduce numbers in national report (numbers compared for 2013Q4)
...
* Comment: ISIC Rev. 4 being used after (and including) 2012Q3
	...
	* Import value labels
	...
	di in red "National classification being used for ${time} - find mapping and define variables for economic activity accordingly"
...
* Comment: 
	...
	* ISCO-08 classification used
	...
	if year==2012 & quarter==2 {
		...
		gen occ_code_prim=c16_proc if ilo_lfs==1 & !inlist(c16_proc,2,9)
	...
	* Skill level
		...
		* Primary occupation
		...
		gen occ_code_prim=int(c16_procc/100) if ilo_lfs==1 
		...
		gen ilo_job1_ocu_isco08_2digits=occ_code_prim
			...
			replace ilo_job1_ocu_isco08_2digits=63 if inlist(occ_code_prim,64)
				...
				* category 64 considers different type of fishermen (Fish-farm cultivators, Seaweeds cultivators, Other aqua products cultivators, 
											...
											* to original variable (c16_procc)
...
* Comment: use ISCO-08 classification (for periods after and including 2012Q3 )
	...
	if year==2012 & quarter==2 {
		...
		gen prevocu_cod=int(c45_pocc/10) if ilo_lfs==2 & !inlist(c45_pocc,2,9)
	...
	* Skill level
		...
		gen prevocu_cod=int(c45_pocc/1000) if ilo_lfs==2
