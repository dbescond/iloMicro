...
* If quarterly data - keep only data corresponding to considered quarter
	...
	keep if substr(time,6,1)==substr(quarter,5,1)
...
* Comment: different weight considered depending on whether we take quarterly or annual data
	...
	gen ilo_wgt=yearly_weights_rev
		lab var ilo_wgt "Sample weight"
...
* Comment: Note that according to the definition, the highest level being CONCLUDED is being considered
	...
	gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if q128==1
		/* replace ilo_edu_isced11=2 if */
		replace ilo_edu_isced11=3 if q128==2
		replace ilo_edu_isced11=4 if q128==3
		replace ilo_edu_isced11=5 if inlist(q128,4,5,6)
		/* replace ilo_edu_isced11=6 if */
		/* replace ilo_edu_isced11=7 if */
		replace ilo_edu_isced11=8 if inlist(q128,7,8,9)
		replace ilo_edu_isced11=9 if inlist(q128,10,11)
		replace ilo_edu_isced11=10 if inlist(q128,12,13)
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_edu_attendance=.
			replace ilo_edu_attendance=1 if inlist(q123,1,2)
			replace ilo_edu_attendance=2 if q123==3
...
* Comment: the variable presents a high number of missing values in 2012 (especially for Q1 and Q2), therefore the variable is not computed in that year.
...
* -------------------------------------------------------------------------------------------
*            	 
* Comment: 	note that for unemployed, reference time for seeking a job is of four weeks 

		* variable included in original dataset: wstatut

	gen ilo_lfs=.
		replace ilo_lfs=1 if inrange(q21,1,5) |  inlist(nowkreas,11,12) | (nowkreas==1 & inlist(q24,1,2,3)) | (q26==1 & inlist(nowkreas,2,4,5,6,7,8,9,10) & q25==1) | q27==1 | ///
							(inlist(q26,2,3) & inlist(q25,1,2) & nowkreas!=3) | (q26==4 & q25==1 & nowkreas!=3)	
		replace ilo_lfs=2 if ilo_lfs!=1 & (q106==1 | q107==1) & q117==1 & q112!=1
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if q79==2
		replace ilo_mjh=2 if q79==1
...
* ------------------------------------------------------------------------------------------- 
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if q26==1 | q31==1
			replace ilo_job1_ste_icse93=2 if q26==2 | q31==2
			replace ilo_job1_ste_icse93=3 if q26==3 | q31==3
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if q26==4 | q31==4 
	  ...
	  * Secondary activity
		...
		* don't define variables for the moment
		
			gen ilo_job2_ste_icse93=.
				replace ilo_job2_ste_icse93=1 if q82==1
				replace ilo_job2_ste_icse93=2 if q82==2
				replace ilo_job2_ste_icse93=3 if q82==3
				/* replace ilo_job2_ste_icse93=4 if */
				replace ilo_job2_ste_icse93=5 if q82==4
...
* Comment: ISIC Rev. 3.1 (NACE Rev. 1.1)  being used and initially indicated on 2-digits level 
	...
	* economic activity for secondary activity only indicated using descriptions in Albanian - don't capture information
	...
	capture confirm variable nace_rev1 
	
	if !_rc {
	
	gen indu_code_prim=nace_rev1 if ilo_lfs==1 & nace_rev1!=9999
	
		}
		
	else {
	
	gen indu_code_prim=nace2j2d if ilo_lfs==1 & nace2j2d!=9999
		...
		* Primary activity
		...
		gen ilo_job1_eco_isic3_2digits=indu_code_prim
			replace ilo_job1_eco_isic3_2digits=. if ilo_lfs!=1
				...
				lab values ilo_job1_eco_isic3 eco_isic3_2digits
...
* -------------------------------------------------------------------------------------------
	...
	* Primary occupation
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(q37,1,2,3)
...
* --------------------------------------------------------------------------------------------------
* 
* Comment: consider first working time associated with each job (if available) and then consider the sum (i.e. the time dedicated to all working activities during the week)
				
	* Actual hours worked are only being indicated for all jobs together
	
		* Primary job
		
		gen ilo_job1_how_actual=q55 if q55!=99
			replace ilo_job1_how_actual=. if ilo_lfs!=1
			lab var ilo_job1_how_actual "Weekly hours actually worked in main job"
			
		* Secondary job
		
		gen ilo_job2_how_actual=q83 if q83!=99
	...
	* Hours usually worked (only info about main job)
		...
		* Primary job
		...
		gen ilo_job1_how_usual=q54 if q54!=99
...
* Comment: pre-defined variable being used
		...
		gen ilo_job1_job_time=.
			replace ilo_job1_job_time=1 if q51==2
			replace ilo_job1_job_time=2 if q51==1
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_job1_job_contract=.
			replace ilo_job1_job_contract=1 if q33==2
			replace ilo_job1_job_contract=2 if q33==1
...
* -------------------------------------------------------------------------------------------
	...
	* Useful questions: q37: Institutional sector
					* q21: Destination of production (use question used for identification of people in labour force)
					* [no info]: Bookkeeping
					* [no info]: Registration
					* q38 and q41: Social security
					* q50: Location of workplace
					* sizefirm and q47: Size of enterprise
					
				* new version:
					* q42 and q75_3: tax payment (proxy for registration) 
					
		gen soc_sec=1 if q38==1 | q41==1											
					
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=1 if (!inlist(q37,1,2,3,6) & q21!=5 & ((ilo_job1_ste_aggregate==1 & q42==2) | (ilo_job1_ste_aggregate==2 & q75_3==0)))| (!inlist(q37,1,2,3,6) & q21!=5 & ///
				((ilo_job1_ste_aggregate==1 & !inlist(q42,1,2)) | (ilo_job1_ste_aggregate==2 & q75_3==.) | !inlist(ilo_job1_ste_aggregate,1,2)) &  ///
				((ilo_job1_ste_aggregate==1 & soc_sec!=1) | ilo_job1_ste_aggregate!=1) & (q50!=4 | (q50==4 & (!inlist(sizefirm,2,3,4,6) | q47<6))))
		replace ilo_job1_ife_prod=2 if inlist(q37,1,2,3) | (!inlist(q37,1,2,3,6) & q21!=5 & ((ilo_job1_ste_aggregate==1 & q42==1) | (ilo_job1_ste_aggregate==2 & !inlist(q75_3,0,.)))) | ///
			(!inlist(q37,1,2,3,6) & q21!=5 & ilo_job1_ste_aggregate==1 & !inlist(q42,1,2) & soc_sec==1) | (!inlist(q37,1,2,3,6) & q21!=5 & ((ilo_job1_ste_aggregate==1 & !inlist(q42,1,2)) | (ilo_job1_ste_aggregate==2 & q75_3==.) | ///
			!inlist(ilo_job1_ste_aggregate,1,2)) & ((ilo_job1_ste_aggregate==1 & soc_sec!=1) | ilo_job1_ste_aggregate!=1) & (q50==4 & (inlist(sizefirm,2,3,4,6) | (q47>5 & q47!=.))))
		replace ilo_job1_ife_prod=3 if q37==6 | (!inlist(q37,1,2,3,6) & q21==5) | ilo_job1_ocu_isco08_2digits==63 | ilo_job1_eco_isic3_2digits==95
...
* Comment: consider "actual" payment (and not usual payment)

	* no earnings from secondary employment being indicated
	...
	* Primary employment 
			...
			* Monthly earnings of employees
			...
			gen net_payment=.
				replace net_payment=q65 if q66==1
				replace net_payment=(q65*2) if q66==2
				replace net_payment=(q65*4) if q66==3
				replace net_payment=(q65*365/12) if q66==4
		...
		* note that the revenue related to self-employment includes also the value of own consumption 
		...
		egen ilo_job1_lri_slf=rowtotal(q77 loss), m
...
* Comment: question on availability not included in dataset - people having answered to question 87a considered as being available (additional number of hours
	...
	gen ilo_joball_tru=1 if ilo_job1_job_time==1 & q85==1 & !inlist(q87,0,.)
...
* Comment:	Question asks how much time ther person has spent looking for another job
	...
	gen ilo_dur_details=.
		replace ilo_dur_details=1 if q112==2
		replace ilo_dur_details=2 if q112==3
		replace ilo_dur_details=3 if q112==4
		replace ilo_dur_details=4 if q112==5
		replace ilo_dur_details=5 if inlist(q112,6,7)
		replace ilo_dur_details=6 if inlist(q112,8,9)
...
* -------------------------------------------------------------------------------------------
	...
	capture confirm variable q100 
	
		if !_rc {
	
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if q100==1
		replace ilo_cat_une=2 if q100==2
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if p97==1
		replace ilo_cat_une=2 if p97==2
		replace ilo_cat_une=3 if ilo_cat_une==. & ilo_lfs==2
				replace ilo_cat_une=. if ilo_lfs!=2
			lab def cat_une_lab 1 "1 - Unemployed previously employed" 2 "2 - Unemployed seeking their first job" 3 "3 - Unknown"
			lab val ilo_cat_une cat_une_lab
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(seekreas,6,7)
		replace ilo_olf_reason=2 if inlist(seekreas,1,2,3,4)
		replace ilo_olf_reason=3 if inlist(seekreas,5,8)
...
* ------------------------------------------------------------------------------------------- 
		...
		gen ilo_dis=1 if ilo_lfs==3 & inlist(seekreas,6,7) & q117==1
...
cd "$outpath"
		...
		drop indu_code_prim occ_code_prim occ_code_prim_1dig bonus loss yr soc_sec
