* TITLE OF DO FILE: ILO Microdata Preprocessing code template - United Arab Emirates, 2017
* DATASET USED: ARE, LFS, 2017
* NOTES: 
* Files created: Standard variables ARE_LFS_2017_FULL.dta and ARE_LFS_2017_ILO.dta
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 09 July 2018
...
global source "LFS"
global time "2017"
global inputFile "ARE_LFS_2017.dta"
...
* Comment: The variable does not exist in the Microdata. Not produced.
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if gender==1		// Male
		replace ilo_sex=2 if gender==2		// Female
...
* ------------------------------------------------------------------------------ 
    ...
    * Age groups
	gen ilo_age_5yrbands=.
		replace ilo_age_5yrbands=1 if age5_dv==1
		replace ilo_age_5yrbands=2 if age5_dv==2
		replace ilo_age_5yrbands=3 if age5_dv==3
		replace ilo_age_5yrbands=4 if age5_dv==4
		replace ilo_age_5yrbands=5 if age5_dv==5
		replace ilo_age_5yrbands=6 if age5_dv==6
		replace ilo_age_5yrbands=7 if age5_dv==7
		replace ilo_age_5yrbands=8 if age5_dv==8
		replace ilo_age_5yrbands=9 if age5_dv==9
		replace ilo_age_5yrbands=10 if age5_dv==10
		replace ilo_age_5yrbands=11 if age5_dv==11
		replace ilo_age_5yrbands=12 if age5_dv==12
		replace ilo_age_5yrbands=13 if age5_dv==13
		replace ilo_age_5yrbands=14 if age5_dv==14
...
* Comment: Note that according to the definition, the highest level CONCLUDED is considered.
    ...
    * Detailed				
	gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if enrolment==3  						// No schooling
		replace ilo_edu_isced11=1 if edu_level_completed==1 			// No schooling
		replace ilo_edu_isced11=2 if edu_level_completed==2				// Early childhood education
		replace ilo_edu_isced11=3 if edu_level_completed==3 			// Primary education
		replace ilo_edu_isced11=4 if edu_level_completed==4			  	// Lower secondary education
		replace ilo_edu_isced11=5 if edu_level_completed==6		    	// Upper secondary education
		* replace ilo_edu_isced11=6 if 									// Short-cycle tertiary education
		replace ilo_edu_isced11=7 if inlist(edu_level_completed,7,8)	// Post-secondary non-tertiary
		replace ilo_edu_isced11=8 if edu_level_completed==9				// Bachelor or equivalent
		replace ilo_edu_isced11=9 if inlist(edu_level_completed,10,11)	// Master's or equivalent level
		replace ilo_edu_isced11=10 if edu_level_completed==12     		// Doctoral or equivalent level
		replace ilo_edu_isced11=11 if ilo_edu_isced11==.				// Not elsewhere classified
			    label def ilo_isced_11_lab 1 "X - No schooling" 2 "0 - Early childhood education" 3 "1 - Primary education" 4 "2 - Lower secondary education" ///
...
* ------------------------------------------------------------------------------
 	...
 	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if enrolment==1			// Attending
		replace ilo_edu_attendance=2 if enrolment!=1			// Not attending
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=2 if marital_status==3			// Married
...
* Comment: The variable does not exist in the questionnaire.
...
* Comment: reason_prevent_look_work is missing in the microdata. Need to take the national pre-defined variable for Unemployment. 
	
	gen ilo_lfs=.
        destring availability_flag, replace
		replace ilo_lfs=1 if work_flag==1 & ilo_wap==1															// Employed: ILO definition
		replace ilo_lfs=1 if inlist(work_absent_reason,1,2,3,4,5) & ilo_wap==1									// Employed: temporary absent
		replace ilo_lfs=2 if unemployed_dv==1 & ilo_lfs!=1 & ilo_wap==1											// Unemployed: national definition
...
* ------------------------------------------------------------------------------
 	...
 	gen ilo_mjh=.
		replace ilo_mjh=1 if (second_work_flag==2 & ilo_lfs==1)
		replace ilo_mjh=2 if (second_work_flag==1 & ilo_lfs==1)
...
* ------------------------------------------------------------------------------
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if work_type==3 & ilo_lfs==1          	// Employees
		 replace ilo_job1_ste_icse93=2 if work_type==1 & ilo_lfs==1          	// Employers
		 replace ilo_job1_ste_icse93=3 if work_type==2 & ilo_lfs==1          	// Own-account workers
		 replace ilo_job1_ste_icse93=4 if work_type==4 & ilo_lfs==1          	// Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if work_type==6 & ilo_lfs==1          	// Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits=. 
	    replace ilo_job1_eco_isic4_2digits=int(eco_activity_code/100) if (eco_activity_code!=0 & eco_activity_code>=100 & ilo_lfs==1)
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits=. 
	    replace ilo_job1_ocu_isco08_2digits=int(occupation_code/10000) if (occupation_code!=0 & occupation_code>=10000 & ilo_lfs==1)
...
* ------------------------------------------------------------------------------ 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(est_sector,1,2,6) & ilo_lfs==1				// Public
		replace ilo_job1_ins_sector=2 if inlist(est_sector,3,4,5,7,8,9,10) & ilo_lfs==1		// Private
...
* Comment: Question directly asked for all jobs
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual=.
	    replace ilo_joball_how_usual=work_hours_usual if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual=.
	    replace ilo_joball_how_actual=work_hours_actual if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if work_contract_type==1 & ilo_lfs==1				// Permanent
		replace ilo_job1_job_contract=2 if inlist(work_contract_type,2,3,4) & ilo_lfs==1	// Temporary
...
/* Useful questions: Not enough information to be defined
          - Institutional sector: EST_SECTOR_A
		  - Destination of production:
		  - Bookkeeping:
		  - Registration:
		  - Status in employment: WORK_TYPE
		  - Social security contribution (Proxy: pension funds):
		  - Place of work:
		  - Size:
		  - Paid annual leave: PAID_LEAVE
		  - Paid sick leave: SICK_LEAVE
*/
    * Social Security:
...
* Comment: Only scale : Do not produce based on variable "salary_main"
...
/*	* Employees
	gen ilo_job1_lri_ees=.
		replace ilo_job1_lri_ees=0 if (salary_main==1 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=500 if (salary_main==2 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=1750 if (salary_main==3 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=3750 if (salary_main==4 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=7500 if (salary_main==5 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=15000 if (salary_main==6 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=27500 if (salary_main==7 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=42500 if (salary_main==8 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=50000 if (salary_main==9 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=0 if (ilo_job1_lri_ees==. & ilo_job1_ste_aggregate==1)
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
		replace ilo_job1_lri_slf=0 if (salary_main==1 & ilo_job1_ste_aggregate)
		replace ilo_job1_lri_slf=500 if (salary_main==2 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=1750 if (salary_main==3 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=3750 if (salary_main==4 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=7500 if (salary_main==5 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=15000 if (salary_main==6 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=27500 if (salary_main==7 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=42500 if (salary_main==8 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=50000 if (salary_main==9 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=0 if (ilo_job1_lri_slf==. & ilo_job1_ste_aggregate==2)
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
		replace ilo_dur_details=1 if (look_months==0 & ilo_lfs==2)
		replace ilo_dur_details=2 if (inlist(look_months,1,2) & ilo_lfs==2)
		replace ilo_dur_details=3 if (inlist(look_months,3,4,5) & ilo_lfs==2)
		replace ilo_dur_details=4 if (inlist(look_months,6,7,8,9,10,11) & ilo_lfs==2)
		replace ilo_dur_details=5 if (inrange(look_months,12,23) & ilo_lfs==2)
		replace ilo_dur_details=6 if (inrange(look_months,24,50) & ilo_lfs==2)
...
* Comment: Missing reason_prevent_look_work. Not produced as only category 1 could be processed.
...
* Comment: Missing reason_prevent_look_work therefore not feasible to produce
