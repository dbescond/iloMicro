...
global source "HILCS"
global time "2014"
global inputFile "Present_members_of_household_2014"
...
*            available future job starters.
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=1 if (d1_2==1) & ilo_wap==1					                                        // Employed: temporary absent
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=2 if (d1_12==1) & ilo_lfs==1              // More than one job	
...
*            workers.
*          - No information in the second job.

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(d1_5,1,2) & ilo_lfs==1         // Employees
		 replace ilo_job1_ste_icse93=2 if d1_5==3 & ilo_lfs==1                  // Employers
		 replace ilo_job1_ste_icse93=3 if inlist(d1_5,4,5) & ilo_lfs==1         // Own-account workers
		 * replace ilo_job1_ste_icse93=4 if                                     // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if d1_5==6 & ilo_lfs==1                  // Contributing family workers
...
* Comment: 
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = d1_4 if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - No information available in the dataset even though the question is
*            asked.			
...
* Comment: - Public: State and municipal.
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(d1_7,1,2) & ilo_lfs==1          // Public
...
*            job available.
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = d1_11 if ilo_lfs==1
...
*            (based on a self-assessment question)
	...
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if inlist(d1_10,1,3)  & ilo_lfs==1          // Full-time
		replace ilo_job1_job_time=1 if d1_10==2 & ilo_lfs==1                    // Part-time
...
* Comment: - Temporary includes temporary, seasonal, ocassional or one-time
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if d1_9==1 & ilo_lfs==1               	  // Permanent
		replace ilo_job1_job_contract=2 if inlist(d1_9,2,3,4) & ilo_lfs==1        // Temporary
...
/* Useful questions:
          - Institutional sector: d1_7
		  - Private household identification: ilo_job1_eco_isic4_2digits==97
		  - Destination of production: d1_8
