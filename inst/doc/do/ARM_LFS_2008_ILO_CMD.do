* TITLE OF DO FILE: ILO Microdata Preprocessing code template - ARMENIA, 2008
* DATASET USED: ARMENIA, LFS, 2008
* Files created: Standard variables ARM_LFS_2008_FULL.dta and ARM_LFS_2008_ILO.dta
...
global source "LFS"
global time "2008"
global inputFile "Survey_on_Informal_Employment_TACIS_2008"
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if b10==1            // Male
		replace ilo_sex=2 if b10==2            // Female
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only asked to those aged 6 years old or more.
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if b12==1                                     // No schooling
		* replace ilo_edu_isced97=2 if                                          // Pre-primary education
		replace ilo_edu_isced97=3 if b12==2                                     // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if b12==3                                     // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if b12==4                                     // Upper secondary education
		replace ilo_edu_isced97=6 if inlist(b12,5,6)                            // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if inlist(b12,7,8)                            // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if inlist(b12,9,10)                           // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only asked to those aged 16 years and over.
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if b13==1                                    // Single
		replace ilo_mrts_details=2 if b13==2                                    // Married
		replace ilo_mrts_details=3 if b13==3                                    // Union / cohabiting
		replace ilo_mrts_details=4 if b13==5                                    // Widowed
		replace ilo_mrts_details=5 if b13==4                                    // Divorced / separated
...
* Comment: 
    ...
    * Aggregate  	
	gen ilo_dsb_aggregate=.
	    replace ilo_dsb_aggregate=2 if b14==1                                   // Persons with disability
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - the employment part of the questionnaire is filled in for household
*            members aged 15-75 (T2:84_T3:240)	
