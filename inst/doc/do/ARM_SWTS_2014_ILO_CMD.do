...
global country "ARM" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2014"  /*time*/
global inputFile "ARM_SWTS_2014.dta" /*name of the input file in stata format*/
...
* Comment: variable highestlevel_comp different as the one mentionned in C12 of the questionaire
	...
	* Detailed
	gen ilo_edu_isced97=.
		...
		replace ilo_edu_isced97=5 if    highestlevel_comp ==5    // Upper secondary education
		replace ilo_edu_isced97=6 if   highestlevel_comp ==6    // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if   highestlevel_comp ==7     // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  highestlevel_comp ==8      // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: marital==2 ->"engaged to be married" 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: No disability variable. Questions on attendance at work that can be linked with disability
...
*          Unemployed: available future starters not complete enough? Starting new job variable?
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if employed==1& ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if emp_temp_absent==1 & ilo_wap==1	// Employed: temporary absent
		replace ilo_lfs=2 if seekingjob==1 & availability==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if foundjob==1& availability==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
//				
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	*comment: code_occ_main=99 for armed forces
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = code_occ_main    if ilo_lfs==1
		replace ilo_job1_ocu_isco08_2digits = .   if ilo_lfs!=1
		replace ilo_job1_ocu_isco08_2digits = 1   if ilo_job1_ocu_isco08_2digits==99
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment:  Did not find variable on the hours usually worked. Used the hours actually work         
	...
	*Hours USUALLY worked
	...
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = hours_week_m      if ilo_lfs==1
...
//     * ALL JOBS:
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual = .
	    replace ilo_joball_how_usual =hours_week_m       if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual = .
	    replace ilo_joball_how_actual =  hours_week_m     if ilo_lfs==1
...
/* Useful questions:
		  ...
		  - Private household identification: hhnb
		  - Destination of production: prodhh_selfemp
		  - Bookkeeping:
		  - Registration: register_actv
		  - Status in employment: statusinemp
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & ///
		                               inlist(ilo_job1_eco_isic4_2digits,95,96,97,98) 
																   
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   register_actv==1 | ///
									   (inlist(register_actv,2,4) & statusinemp==1 & social_security==1) | ///
									   (inlist(register_actv,2,4) & inlist(statusinemp,2,3,4,5,6) & inlist(nb_persons,2,3,4,5,6,7)))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (statusinemp==1 & benefit_servd==1 & benefit_serve==1) | ///
										 (inlist(statusinemp,2,4)&ilo_job1_ife_prod==2) | ///
										 (inlist(statusinemp,3)&ilo_job1_ife_prod==2))
...
* ------------------------------------------------------------------------------
* Comment: 
*Questionaire D14, total_amount_wage and cash_amount are reversed
* D 18 "payment period" -> if wage for one week, should i *4? etc..
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Choose 35 as a threshold

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<=35  & likeworkmore==1 & availability==1 & ilo_lfs==1
...
*-------------------------------------------------------------------------------
*-------------------------------------------------------------------------------
* Comment: did not find variable on the questionaire
/*
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  seekingjob==1 &  availability==2 & ilo_lfs==3      // Seeking, not available
...
* Comment: "reason_notseeking"
		...
		gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=3 if inlist(reason_notseeking,4,5,6,12)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if reason_notseeking==99 & reason_notavailable==6 & ilo_lfs==3             // Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & step_last12months==2 & ilo_olf_reason==1
...
cd "$outpath"
	...
	save ${ARM}_${SWTS}_${2012}_FULL,  replace		
	
	*Save file only containing ilo_* variables
	keep ilo*
	save ${ARM}_${SWTS}_${2012}_ILO, replace
