...
* Authors: ILO / Department of Statistics / DPAU 15/05/2018
...
global country "BEL" /*ref_area: ISO 3 Code from the workflow*/
global source "EUSILC"  /*survey: Acronym from the workflow*/
global time "2016"  /*time*/
global inputFile "BEL_EUSILC_2016.dta" /*name of the input file in stata format*/
...
* Comment: PX040 respondant status (coverage 16+ and answered)
	...
	gen ilo_wgt=.
	    replace ilo_wgt= PB040 if PX040 == 1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: RX020 age at the end of the income reference period
*          survey coverage for income is 16+ so age and wap define only for 16+		
	gen ilo_age=.
	    replace ilo_age= PX020 if PX020 > 15 & PX040 == 1
		        lab var ilo_age "Age"
...
* Comment: 	
	...
	gen ilo_wap=.
		replace ilo_wap=1 if ilo_age>=16 & ilo_age!=. & PX040 == 1
...
/* Useful questions:
		  ...
		  - Paid sick leave:
*/
	* proxy income tax for self-employed
	gen incTaxSE = 0 if ilo_lfs == 1
	replace incTaxSE = 1 if (PY050G - PY050N) > 0
		lab def incTax_lab 0 "No" 1 "Yes"
		lab val incTaxSE incTax_lab
		lab var incTaxSE "Tax on profit or income for Self-employed"

	* proxy income tax for employees
	gen incTaxWA = 0 if ilo_lfs == 1
	replace incTaxWA = 1 if (PY010G - PY010N) > 0 | (PY020G - PY020N) > 0

		lab val incTaxWA incTax_lab
		lab var incTaxWA "Tax on profit or income for Employees"
		
	* proxy Establishment size	
	gen estSize = 0 of ilo_lfs == 1
	replace estSize PL130 > 5 | PL130 ==. // missing assign to 1
		lab def estSize_lab 0 "1-5 workers" 1 "6 workers and more"
		lab val estSize estSize_lab
		lab var estSize "Establishment size"
		
	* registration of entreprise NO VARIABLE
	
	* Written account NO VARIABLE
	
	* tax payment 'test'
	gen TaxPaiment = 0 if ilo_lfs == 1
	replace TaxPaiment = 1 if (PY030G>0 | PY031G > 0 | PY035G > 0)
		lab def TaxPaiment_lab 0 "No tax payment" 1 "Tax payment"
		lab val TaxPaiment TaxPaiment_lab
		lab var TaxPaiment "No tax payment"
		
	* Institutional sector
		* proxy of nace PL111 (2008 + PL111 == 9 L-N public  + education and health
		* no identification of households !!!
	gen Public = 2 if ilo_lfs == 1
	replace Public = 1 if (PL111 == "l - n")
		lab def Public_lab 1 "State / Government" 2 "Private"
		lab val Public Public_lab
		lab var Public "Public / private"
...
*/	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=2 if Public == 2 & ()
...
/*
	...
	* Employees
	gen ilo_job1_lri_ees = .
		* Gross employee cash or near cash income
		gen Gincome = PY010G if ilo_job1_ste_aggregate==1
		replace Gincome  = 0 if Gincome == .
		* Company car
		gen Ccar = PY021G if ilo_job1_ste_aggregate==1
		replace Ccar = 0 if Ccar == .
		
	    replace ilo_job1_lri_ees = (Gincome + Ccar) / 12      if ilo_job1_ste_aggregate==1
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
		* Company car
		gen Ccar = PY021G if ilo_job1_ste_aggregate==2
		replace Ccar = 0 if Ccar == .
		* Gross cash benefits or losses from self-employment (including royalties)
		gen Ccar = PY050G if ilo_job1_ste_aggregate==2
		replace Ccar = 0 if Ccar == .
	
	    replace ilo_job1_lri_slf =   (Gincome + Ccar) / 12      if ilo_job1_ste_aggregate==2
