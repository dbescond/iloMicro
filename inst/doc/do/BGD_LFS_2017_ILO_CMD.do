* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Bangladesh, 2017
* DATASET USED: Bangladesh, LFS, 2017
* NOTES: 
* Files created: Standard variables BGD_LFS_2017_FULL.dta and BGD_LFS_2017_ILO.dta
...
global source "LFS"
global time "2017"
global inputFile "BGD_QLFS_2016-17"
...
* ------------------------------------------------------------------------------
	...
	destring todrop_*, replace force
	...
	gen ilo_wgt=.
	    if `Q' == -9999{
	       replace ilo_wgt=wgt_fy2017
		}
		else{
		   replace ilo_wgt=wgt_fq2017
		}
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if urbm==2         // Urban 
		replace ilo_geo=2 if urbm==1         // Rural
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 99 indicates 99 years old or more.
...
*          - The original question is not answered by those aged 4 years old or
*            below and therefore they are classified under "Not elsewhere classified".
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if q24==0                                     // No schooling
		replace ilo_edu_isced11=2 if inlist(q24,1,2,3)                          // Early childhood education
		replace ilo_edu_isced11=3 if inlist(q24,4,5,99)                         // Primary education
		replace ilo_edu_isced11=4 if inlist(q24,6,7,8)                          // Lower secondary education
		replace ilo_edu_isced11=5 if inlist(q24,9,10,11)                        // Upper secondary education
		* replace ilo_edu_isced11=6 if                                          // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if q24==12                                    // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if q24==13                                    // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if q24==14                                    // Master's or equivalent level
		replace ilo_edu_isced11=10 if q24==15                                   // Doctoral or equivalent level
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - The original question is not answered by those aged 4 years old or
*            below and therefore they are classified under "Not elsewhere classified".
			
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if q22==1                                  // Attending
		replace ilo_edu_attendance=2 if inlist(q22,2,3)                         // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if q20==1                                    // Single
		replace ilo_mrts_details=2 if q20==2                                    // Married
		*replace ilo_mrts_details=3 if                                          // Union / cohabiting
		replace ilo_mrts_details=4 if q20==3                                    // Widowed
		replace ilo_mrts_details=5 if inlist(q20,4,5)                           // Divorced / separated
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: -  Unemployment comprises those who satisfy the ILO's three criteria
*            (not in employment, looking and available to work); it also includes
*            available future starters (awaiting for joining or awaiting for
*            setting self business). It differs from the variable already included
*            into the dataset due to the "available future starters". 
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q31==1 & inlist(q35,1,2)) & ilo_wap==1			    // Employed: ILO definition
		replace ilo_lfs=1 if (q33==1 & inlist(q35,1,2)) & ilo_wap==1			    // Employed: ILO definition
		replace ilo_lfs=1 if ((q32==1 | q34==1) & inlist(q35,1,2)) & ilo_wap==1	    // Employed: temporary absent
        replace ilo_lfs=1 if inlist(q35,1,2) & ilo_wap==1                           // Employed: answering question about the intention of work (pay/profit/hh cons)
		replace ilo_lfs=2 if ilo_lfs!=1 & (q77==1 & q81==1) & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if ilo_lfs!=1 & (inlist(q80,1,4) & q81==1) & ilo_wap==1	// Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=2 if (q55==1) & ilo_lfs==1             // More than one job    
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Employees include: paid employee, day labourer, apprentices/interns/
*            trainees (if paid) and domestic workers.

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(q49,4,5,6,7) & ilo_lfs==1      // Employees
		 replace ilo_job1_ste_icse93=2 if q49==1 & ilo_lfs==1                   // Employers
		 replace ilo_job1_ste_icse93=3 if q49==2 & ilo_lfs==1                   // Own-account workers
		 * replace ilo_job1_ste_icse93=4 if                                     // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if q49==3 & ilo_lfs==1                   // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if inlist(q60,4,5,6,7) & ilo_mjh==2     // Employees
		  replace ilo_job2_ste_icse93=2 if q60==1 & ilo_mjh==2                  // Employers
		  replace ilo_job2_ste_icse93=3 if q60==2 & ilo_mjh==2                  // Own-account workers
		  * replace ilo_job2_ste_icse93=4 if                                    // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if q60==3 & ilo_mjh==2                  // Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* ISIC REV 4
	*---------------------------------------------------------------------------
	destring q37bsic, replace
	gen eco_act_prim = int(q37bsic/100)
	
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = eco_act_prim if ilo_lfs==1
   ...
   * Aggregate level
	...
	* ISIC REV 4
	*---------------------------------------------------------------------------
	destring q57b, replace
	gen eco_act_secu = int(q57b/100)	
	
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = eco_act_secu if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring q43bsco, replace
	gen occu_prim = int(q43bsco/100)
	
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = occu_prim  if ilo_lfs==1
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
	    replace ilo_job1_ocu_isco08=11 if inlist(ilo_job1_ocu_isco08_2digits,46,49,85,86,99,.) & ilo_lfs==1          // Not elsewhere classified
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring q58a, replace
    gen occu_secu = int(q58a/100)
	
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = occu_secu if ilo_mjh==2
	...
	* 1-digit level 				
	gen ilo_job2_ocu_isco08=.
	    replace ilo_job2_ocu_isco08=11 if inlist(ilo_job2_ocu_isco08_2digits,0,6,9,64,86,99,.) & ilo_mjh==2          // Not elsewhere classified
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Public: Government and local government.
		
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(q39,1,3) & ilo_lfs==1           // Public
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - No information available on the hours actually work (second job).
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = q48 if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	egen ilo_job1_how_actual = rowtotal(q46a q46b q46c q46d q46e q46f q46g), m
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual = .
	    replace ilo_job2_how_actual = q59 if ilo_mjh==2
...
* Comment: - Defined based on a self-assessment question.
	...
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if q45==1 & ilo_lfs==1                      // Full-time
		replace ilo_job1_job_time=1 if q45==2 & ilo_lfs==1                      // Part-time
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if q44==1 & ilo_lfs==1								// Permanent
		replace ilo_job1_job_contract=2 if q44==2 & ilo_lfs==1               				// Temporary
...
* ------------------------------------------------------------------------------
/* Useful questions:
          - Institutional sector: q39
		  - Private household identification: q39 (option 6)
		  - Destination of production: q35 (not used)/ q84g (not used)
		  - Bookkeeping: q40. even though it is asked, it is not used due to the lack of information on for whom the accounting is made for => not used
		  - Registration: not asked.
		  - Status in employment: ilo_job1_ste_icse93
		  - Social security contribution (Proxy: pension or retirement fund): q52a
		  - Place of work: q41 (option 3)
		  - Size: q38
		  - Paid annual leave: not asked.
		  - Paid sick leave: q52c
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if q52a==1 & ilo_lfs==1          // Social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(q39,1,3,4)) | ///
									   (inlist(q39,2,5,7,9,.) & ilo_job1_ste_icse93==1 & social_security==1) | ///
									   (inlist(q39,2,5,7,9,.) & ilo_job1_ste_icse93==1 & social_security!=1 & q41==3 & inlist(q38,3,4,5,6,7)) | ///
									   (inlist(q39,2,5,7,9,.) & ilo_job1_ste_icse93!=1 & q41==3 & inlist(q38,3,4,5,6,7)))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - In cash and in kind. No information available for self-employed in 
*            main job/second job.
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = q54c if ilo_job1_ste_aggregate==1
	...
	* Employees
	gen ilo_job2_lri_ees = .
	    replace ilo_job2_lri_ees = q62c if ilo_job2_ste_aggregate==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: -  National threshold is set at 40 hours actually worked per week.

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_actual<=40  & q70==1 & q71>0 & q72==1 & ilo_lfs==1
...
*-------------------------------------------------------------------------------
*-------------------------------------------------------------------------------
* Comment: - Injuries or illness in the last 12 months.

	 gen ilo_joball_oi_case = .
		 replace ilo_joball_oi_case =  q66 if ilo_lfs==1
...
*-------------------------------------------------------------------------------
*-------------------------------------------------------------------------------
* Comment: - days lost during the last 12 months. If it is impossible 
*            to return to work due to accident then "99" is replaced by  "365" days.

	gen ilo_joball_oi_day = .
		replace ilo_joball_oi_day = q67 if ilo_lfs==1
		replace ilo_joball_oi_day = 365 if ilo_joball_oi_day==99 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - category 1 to 3 months (category 2) includes 3 to 6 months (category
*            3) (note to value)

	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if q82==1 & ilo_lfs==2                        // Less than 1 month
		replace ilo_dur_details=2 if q82==2 & ilo_lfs==2                        // 1 to 3 months (including 3 to 6 months -> C7:3906)
		* replace ilo_dur_details=3 if                                          // 3 to 6 months
		replace ilo_dur_details=4 if q82==3 & ilo_lfs==2                        // 6 to 12 months
		replace ilo_dur_details=5 if q82==4 & ilo_lfs==2                        // 12 to 24 months
		replace ilo_dur_details=6 if q82==5 & ilo_lfs==2                        // 24 months or more
...
*            3 and 4 are not possible to be produced.
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if q77==1 & q81==2 & ilo_lfs==3                // Seeking, not available
		replace ilo_olf_dlma = 2 if q77==2 & q81==1 & ilo_lfs==3	            // Not seeking, available
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(q80,2,8) & ilo_lfs==3            // Labour market 
			replace ilo_olf_reason=2 if q80==3 & ilo_lfs==3                     // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(q80,5,6,7) & ilo_lfs==3          // Personal/Family-related
			replace ilo_olf_reason=4 if q80==9 & ilo_lfs==3                     // Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & q81==1 & ilo_olf_reason==1
...
cd "$outpath"
	...
	drop todrop todrop_1 todrop_2 eco_act_prim eco_act_secu occu_prim occu_secu social_security
	compress
