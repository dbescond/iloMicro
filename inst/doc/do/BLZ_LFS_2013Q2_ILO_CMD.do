* TITLE OF DO FILE: ILO Microdata Preprocessing code template - BELIZE, 2013Q2
* DATASET USED: BLZ, LFS, 2013Q2
* Files created: Standard variables BLZ_LFS_2013Q2_FULL.dta and BLZ_LFS_2013Q2_ILO.dta
...
global source "LFS"
global time "2013Q2"
global inputFile "APRIL_2013_ANONYMISED"
...
*            below this aged are classified under "level not stated".
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if (inlist(ed5new,17,18))                                           // No schooling
		replace ilo_edu_isced97=2 if (inlist(ed4new,1,2,3,4,5,6))                                     // Pre-primary education
		replace ilo_edu_isced97=3 if (inlist(ed4new,7,8,9,10)) | (inlist(ed5new,1,2,3,4,5,6))         // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if (inlist(ed4new,11,12)) | (inlist(ed5new,7,8,9,10))               // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if (inlist(ed4new,13,14)) | (inlist(ed5new,11,12))                  // Upper secondary education
		* replace ilo_edu_isced97=6 if                                                                // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if (ed4new==15) | (inlist(ed5new,13,14))                            // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if (ed5new==15)                                                     // Second stage of tertiary education (leading to an advanced research qualification)                               
...
*            below are classified under "not elsewhere classified".
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if ed3new==1                                  // Attending
		replace ilo_edu_attendance=2 if ed3new==2                                  // Not attending
...
* Comment:    
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(catmain,3,4,5,6) & ilo_lfs==1  // Employees
		 replace ilo_job1_ste_icse93=2 if catmain==1                            // Employers
		 replace ilo_job1_ste_icse93=3 if catmain==2 & ilo_lfs==1               // Own-account workers
		 * replace ilo_job1_ste_icse93=4 if                                     // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if catmain==7 & ilo_lfs==1               // Contributing family workers
...
*          - H including J [C5:1022]
	...
	* 1-digit level
    gen ilo_job1_eco_isic4=.
	    replace ilo_job1_eco_isic4=1 if inlist(ea21_mainnew,1,2,3)
	    replace ilo_job1_eco_isic4=2 if ea21_mainnew==4
	    replace ilo_job1_eco_isic4=3 if ea21_mainnew==5
	    replace ilo_job1_eco_isic4=4 if ea21_mainnew==6                         // [Including E (C5:1019)]
	    * replace ilo_job1_eco_isic4=5 if 
	    replace ilo_job1_eco_isic4=6 if ea21_mainnew==7
	    replace ilo_job1_eco_isic4=7 if ea21_mainnew==8
	    replace ilo_job1_eco_isic4=8 if ea21_mainnew==10                        // [Including J (C5:1022)] 
	    ...
	    * replace ilo_job1_eco_isic4=13 if 
	    replace ilo_job1_eco_isic4=14 if ea21_mainnew==9
	    replace ilo_job1_eco_isic4=15 if ea21_mainnew==13
        * replace ilo_job1_eco_isic4=16 if 
	    replace ilo_job1_eco_isic4=17 if ea21_mainnew==14
   ...
   * Aggregate level
	...
	* 1-digit level
    gen ilo_job2_eco_isic4=.
	    replace ilo_job2_eco_isic4=1 if inlist(ea21_othernew,1,2,3)
	    replace ilo_job2_eco_isic4=2 if ea21_othernew==4
	    replace ilo_job2_eco_isic4=3 if ea21_othernew==5
	    replace ilo_job2_eco_isic4=4 if ea21_othernew==6                        // [Including E (C5:1019)]
	    * replace ilo_job2_eco_isic4=5 if 
	    replace ilo_job2_eco_isic4=6 if ea21_othernew==7
	    replace ilo_job2_eco_isic4=7 if ea21_othernew==8
	    replace ilo_job2_eco_isic4=8 if ea21_othernew==10                       // [Including J (C5:1022)] 
	    ...
	    * replace ilo_job2_eco_isic4=13 if 
	    replace ilo_job2_eco_isic4=14 if ea21_othernew==9
	    replace ilo_job2_eco_isic4=15 if ea21_othernew==13
        * replace ilo_job2_eco_isic4=16 if 
	    replace ilo_job2_eco_isic4=17 if ea21_othernew==14
...
* Comment: 
	...
	if !_rc{
	   replace ilo_job1_ins_sector=1 if catmain==3 & ilo_lfs==1            // Public
...
* Comment: - Only availalbe total hours usually/actually worked per week.
	...
	* Hours USUALLY worked
	...
	capture confirm variable ea25_main
	if !_rc{
	    replace ilo_joball_how_usual = ea25_main if ilo_lfs==1
		replace ilo_joball_how_usual = . if ilo_joball_how_usual==999
	}
	else{
	    replace ilo_joball_how_usual = ea25_totalnew if ilo_lfs==1
		replace ilo_joball_how_usual = . if ilo_joball_how_usual==999
	}	
	...
	* Hours ACTUALLY worked
	...
	capture confirm variable ea26_main
	if !_rc{
	    replace ilo_joball_how_actual = ea26_main if ilo_lfs==1
		replace ilo_joball_how_actual = . if ilo_joball_how_actual==999
	}
    else{
	    replace ilo_joball_how_actual = ea26_totalnew if ilo_lfs==1
		replace ilo_joball_how_actual = . if ilo_joball_how_actual==999
	}
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Original information is only given by income group (without stating
*            the labels of the groups) and therefore it is not possible to
*            define it.
...
*            [T35:4143]
	...
	gen ilo_joball_tru = .
		capture confirm variable ea32new
	    if !_rc{
	       replace ilo_joball_tru = 1 if ilo_joball_how_usual<=35  & ea32new==1 & ilo_lfs==1
		}
	    else{
	       replace ilo_joball_tru = 1 if ilo_joball_how_usual<=35  & ea32==1 & ilo_lfs==1
		}
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if ea14merge<1 & ea14merge!=. & ilo_lfs==2    // Less than 1 month
		replace ilo_dur_details=2 if ea14merge>=1 & ea14merge<3 & ilo_lfs==2    // 1 to 3 months
		replace ilo_dur_details=3 if ea14merge>=3 & ea14merge<6 & ilo_lfs==2    // 3 to 6 months
		replace ilo_dur_details=4 if ea14merge>=6 & ea14merge<12 & ilo_lfs==2   // 6 to 12 months
		replace ilo_dur_details=5 if ea14merge>=12 & ea14merge<24 & ilo_lfs==2  // 12 to 24 months
		replace ilo_dur_details=6 if ea14merge>=24 & ea14merge!=. & ilo_lfs==2  // 24 months or more
...
* Comment:            
	...
	* 1-digit level
    gen ilo_preveco_isic4=.
	    replace ilo_preveco_isic4=1 if inlist(ea21_previousnew,1,2,3)
	    replace ilo_preveco_isic4=2 if ea21_previousnew==4
	    replace ilo_preveco_isic4=3 if ea21_previousnew==5
	    replace ilo_preveco_isic4=4 if ea21_previousnew==6                      // [Including E (C5:1019)]
	    * replace ilo_preveco_isic4=5 if 
	    replace ilo_preveco_isic4=6 if ea21_previousnew==7
	    replace ilo_preveco_isic4=7 if ea21_previousnew==8
	    replace ilo_preveco_isic4=8 if ea21_previousnew==10                     // [Including J (C5:1022)] 
	    ...
	    * replace ilo_preveco_isic4=13 if 	
	    replace ilo_preveco_isic4=14 if ea21_previousnew==9
	    replace ilo_preveco_isic4=15 if ea21_previousnew==13
        * replace ilo_preveco_isic4=16 if 
	    replace ilo_preveco_isic4=17 if ea21_previousnew==14
