...
* Starting Date: 07 November 2018
...
global source "LFS"
global time "2015Q2"
global inputFile "APRIL_2015_ANONYMISED"
...
* Comment:
	...
	*---------------------------------------------------------------------------
	...
	capture confirm variable ea20_3
	if !_rc{
	   replace ocu_pri = int(ea20_3/100)
	   replace ocu_pri =. if ocu_pri<0
	}
	else{
	   replace ocu_pri = int(ea20_main/100)
	   replace ocu_pri =. if ocu_pri<0
	}
	...
	*---------------------------------------------------------------------------
	gen ocu_sec =.
	capture confirm variable ea20_6
	if !_rc{
	   replace ocu_sec = int(ea20_6/100)
	   replace ocu_sec =. if ocu_sec<0
	}
	else{
	   replace ocu_sec = int(ea20_other/100)
	   replace ocu_sec =. if ocu_sec<0
	}	
...
* Comment: 
	...
	capture confirm variable ea18_1
	if !_rc{
...
* Comment: - Only availalbe total hours usually/actually worked per week.
	...
	* Hours USUALLY worked
	...
	capture confirm variable ea22_3new
	if !_rc{
	...
	* Hours ACTUALLY worked
	...
	capture confirm variable ea23_3new
	if !_rc{
...
*           - Before taxes and deductions.
	...
	* Employees
	...
	if !_rc{
	    ...
	    replace ilo_job1_lri_ees = ea34_value if (ea35==4 & ilo_job1_ste_aggregate==1)               // Monthly
		replace ilo_job1_lri_ees = ea34_value*(365/12) if (ea35==1 & ilo_job1_ste_aggregate==1)      // Daily
		replace ilo_job1_lri_ees = ea34_value*(52/12) if (ea35==2 & ilo_job1_ste_aggregate==1)       // Weekly
		replace ilo_job1_lri_ees = ea34_value*(2) if (ea35==3 & ilo_job1_ste_aggregate==1)           // Every two weeks
		replace ilo_job1_lri_ees = ea34_value*(1/12) if (ea35==5 & ilo_job1_ste_aggregate==1)        // Yearly
	    replace ilo_job1_lri_ees = .  if ilo_lfs!=1
	}
	...
	* Self-employed
	...
	if !_rc{	
	    ...
	    replace ilo_job1_lri_slf = ea34_value if (ea35==4 & ilo_job1_ste_aggregate==2)               // Monthly
		replace ilo_job1_lri_slf = ea34_value*(365/12) if (ea35==1 & ilo_job1_ste_aggregate==2)      // Daily
		replace ilo_job1_lri_slf = ea34_value*(52/12) if (ea35==2 & ilo_job1_ste_aggregate==2)       // Weekly
		replace ilo_job1_lri_slf = ea34_value*(2) if (ea35==3 & ilo_job1_ste_aggregate==2)           // Every two weeks
		replace ilo_job1_lri_slf = ea34_value*(1/12) if (ea35==5 & ilo_job1_ste_aggregate==2)        // Yearly
	    replace ilo_job1_lri_slf = .  if ilo_lfs!=1
	}
...
* Comment: 
	...
	*---------------------------------------------------------------------------
	gen ocu_pre =. 
	capture confirm variable ea20_9
	if !_rc{
	   replace ocu_pre = int(ea20_9/100)
	   replace ocu_pre =. if ocu_pre<0
	}
	else{
	   replace ocu_pre = int(ea20_previous/100)
	   replace ocu_pre =. if ocu_pre<0
	}	
