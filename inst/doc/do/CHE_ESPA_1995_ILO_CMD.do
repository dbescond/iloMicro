...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: missing variables on geographical coverage
...
* Comment: ISCED 97 until 2013. From 2014 onwards ISCED 2014
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  iq21==0 & !inrange(bqu2,11,10)                    // No schooling
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if inlist(bdu1,18)                       // Attending
		replace ilo_edu_attendance=2 if ilo_edu_attendance==.                      // Not attending
...
* Check question bd02
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=1 if ((bd03==1 & inlist(bd04,1,2,3,4) | (bd03==1 & inrange(bd04,5,15) & inlist(bd05k,1)))) & ilo_wap==1						// Employed: temporary absent
...
* Comment: 
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if (inlist(eg01,2) | b0000==2)  & ilo_lfs==1   // Employees
		 replace ilo_job1_ste_icse93=2 if (eg01==3) & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if (eg01==1) & ilo_lfs==1          // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if !inlist(ilo_job1_ste_icse93,1,2,3) & bdu1==7 & ilo_lfs==1          // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if inlist(eg01n,2) & ilo_mjh==2          // Employees
		  replace ilo_job2_ste_icse93=2 if (eg01n==3 ) & ilo_mjh==2          // Employers
		  replace ilo_job2_ste_icse93=3 if (eg01==1)  & ilo_mjh==2          // Own-account workers
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Classification NOGA 2008 used by the Swiss, See under internal 
*          before 2009 REV 3.1, for years after 2008, REV 4
	...
	* 1-digit level
    gen ilo_job1_eco_isic4=.
	    replace ilo_job1_eco_isic4=bmu3 if ilo_lfs==1 & bmu3>0
   ...
   * Aggregate level
	...
	* 2-digit level
	gen ilo_job2_eco_isic3_2digits = .
	    replace ilo_job2_eco_isic3_2digits = code_ind_sec  if ilo_mjh==2 & code_ind_sec>0
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: apprentices classified in temporary. Independant, family workers classified as permanent
	
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if (eh01==2 | inlist(teh,1,992,993)) & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if (eh01==1 | bhu1==1 | inlist(teh,2,3,4,991)) & ilo_lfs==1               				// Temporary
...
* Currency: Swiss Francs (CHF)
    ...
    * SECOND JOB:
    *************
	* Swiss social cotisation estimated at 10% from gross to net 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment:  before 2009 REV 3.1, for years after 2008, REV 4
...
if to_drop_1<=2008{
	...
	* 2-digit level
	gen ilo_preveco_isic3_2digits = .
	    replace ilo_preveco_isic3_2digits = code_ind_prev  if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
if to_drop_1<=2009{
	...
	* 2-digit level 
    gen ilo_prevocu_isco88_2digits = . 
	    replace ilo_prevocu_isco88_2digits = code_occ_prev    if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: As the questions on seeking employment is not asked to retired individuals, this group of people is forced to "ilo_olf_dlma = 4 - Not seeking, not available, not willing" through the following code (inlist(ne11,5,6,7,10) | bdu1==20))

if to_drop_1>=2010{
capture confirm variable ne11 // reasons for leaving previous job
	if !_rc{

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  (bd08==1 | nn17t>0) & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7))            & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  (bd08==2) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2) & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if  (bd08==2 & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7)) & inlist(nn01,-1,2)) | (inlist(ne11,5,6,7,10) | bdu1==20)		      & ilo_lfs==3      // Not seeking, not available, not willing
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 2 if  (bd08==2) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2) & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if  (bd08==2 & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7)) & inlist(nn01,-1,2))  | bdu1==20	      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
...
if inrange(to_drop_1,2006,2009){
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  (bd08==1 ) & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7))            & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  (bd08==2 ) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 ) & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if  (bd08==2 & (inlist(bd12,4,5,6) | inlist(nn02,-1,4,5,6,7)) & inlist(nn01,-1,2)) | (inlist(ne11,5,6,7,10) | bdu1==20)		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
...
capture confirm variable ne11
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 4 if  (bd08==2 & inlist(bd12,4,5,6) & inlist(nn01,-1,2)) | (inlist(ne11,5,6,7,10) | bdu1==20)		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 2 if  (bd08==2 ) & (inlist(bd12,1,2,3) )            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 ) & (inlist(bd12,4,5,6) ) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if  (bd08==2 & inlist(bd12,4,5,6) & inlist(nn01,-1,2)) | bdu1==20		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: military service classified as "Other labour market reasons"
* Questions not asked to all retired individuals, therefore this group is forced to "ilo_olf_reason=4 - Does not need/want to work" through bdu1==20
		...
		gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=3 if (inlist(bd131,5,9,10,15,17,16,18) | inlist(bd132,5,9,10,15,17,16,18) | inlist(bd133,5,9,10,15,17,16,18))        & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if (inlist(bd131,7,8) | inlist(bd132,7,8) | inlist(bd133,7,8) | bdu1==20 )          & ilo_lfs==3             // Does not need/want to work
