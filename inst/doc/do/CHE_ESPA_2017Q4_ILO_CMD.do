...
global country "CHE" /*ref_area: ISO 3 Code from the workflow*/
global source "ESPA"  /*survey: Acronym from the workflow*/
global time "2017Q4"  /*time*/
global inputFile "${country}_${source}_${time}.dta" /*name of the input file in stata format*/
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: different variable whether its annual or quaterly



if (to_drop_2 == -9999 & to_drop_1>=2010){ // annual

	gen ilo_wgt=.
	    replace ilo_wgt= ixpxhj
		lab var ilo_wgt "Sample weight"
}
if (inlist(to_drop_2,1,2,3,4) & to_drop_1>=2010){ // quaterly
	gen ilo_wgt=.
	    replace ilo_wgt= ixpxh
		lab var ilo_wgt "Sample weight"
}
	
if to_drop_1<=2009{ // annual
	gen ilo_wgt=.
	    replace ilo_wgt= ixpxh
		lab var ilo_wgt "Sample weight"
}
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: missing variables on geographical coverage between 2006 and 2009
	
if to_drop_1>=2010{
capture confirm variable b028_2000 
if !_rc{

	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(b028_2000,1,2,3)        // Urban 
		replace ilo_geo=2 if b028_2000==4         // Rural
...
if to_drop_1<=2009{
	capture confirm variable b028
if !_rc{

	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(b028,1,2)        // Urban 
		replace ilo_geo=2 if b028==3         // Rural
		        lab def ilo_geo_lab 1 "1 - Urban" 2 "2 - Rural"
		        lab val ilo_geo ilo_geo_lab
		        lab var ilo_geo "Geographical coverage"	
	}
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if is01==1            // Male
		replace ilo_sex=2 if is01==2            // Female
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: ISCED 97 until 2013. From 2014 onwards ISCED 2014
	...
	* Detailed
	gen ilo_edu_isced97=.
		...
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		replace ilo_edu_isced97=3 if bqu2==1                   // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if inlist(bqu2,11,12)                        // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(bqu2,2,3,4,5)                       // Upper secondary education
		replace ilo_edu_isced97=6 if inlist(bqu2,6)                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if  inlist(bqu2,7,9,8)                      // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if bqu2==10                      // Second stage of tertiary education (leading to an advanced research qualification)                               
...
if 	inrange(to_drop_2,2001,2013) {
	...
	* Detailed
	gen ilo_edu_isced97=.
		*replace ilo_edu_isced97=1 if                        // No schooling
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		replace ilo_edu_isced97=3 if bqu2i==10                       // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if bqu2i==11                        // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(bqu2i,21,31,32,33,34,35)                       // Upper secondary education
		replace ilo_edu_isced97=6 if bqu2i==41                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if  inlist(bqu2i,51,52)                      // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if bqu2i==61                       // Second stage of tertiary education (leading to an advanced research qualification)                               
		replace ilo_edu_isced97=9 if ilo_edu_isced97==.     // Level not stated 
			    label def isced_97_lab 1 "X - No schooling" 2 "0 - Pre-primary education" 3 "1 - Primary education or first stage of basic education" 4 "2 - Lower secondary education or second stage of basic education" ///
							           5 "3 - Upper secondary education" 6 "4 - Post-secondary non-tertiary education" 7 "5 - First stage of tertiary education (not leading directly to an advanced research qualification)" ///
							           8 "6 - Second stage of tertiary education (leading to an advanced research qualification)" 9 "UNK - Level not stated"
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		*replace ilo_edu_isced11=1 if                        // No schooling
		replace ilo_edu_isced11=2 if    bqu2i==0                     // Early childhood education
		replace ilo_edu_isced11=3 if   bqu2i==100                      // Primary education
		replace ilo_edu_isced11=4 if  bqu2i==244                 // Lower secondary education
		replace ilo_edu_isced11=5 if  inlist(bqu2i,354,353,344,343)                        // Upper secondary education
		...
		*replace ilo_edu_isced11=7 if                        // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if  inlist(bqu2i,640,650)                        // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if  inlist(bqu2i,740,750)                      // Master's or equivalent level
		replace ilo_edu_isced11=10 if bqu2i==840                      // Doctoral or equivalent level
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if inlist(iq26,1,2,3)                       // Attending
		replace ilo_edu_attendance=2 if inlist(iq26,0,-9)                       // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if  is03==1                                         // Single
		replace ilo_mrts_details=2 if  is03==2                                         // Married
		replace ilo_mrts_details=3 if  is03==6                                         // Union / cohabiting
		replace ilo_mrts_details=4 if  is03==5                                         // Widowed
		replace ilo_mrts_details=5 if  inlist(is03,3,7)                                         // Divorced / separated
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: iz40, iz41, iz42, on the general health state but no questions on see
/*
    * Detailed
	gen ilo_dsb_details=.
...
* Comment: 
*			
*			Employed: temporary absent -> (bd03==1 & inrange(bd04,5,15) & inlist(bd05,1,2,3)) testing the length of absence (no more than 3 weeks)for these reasons         								
*			Unemployed: three criteria -> criteria on availability is maximum 4 weeks
*
* Slight differences between ilo_lfs following stric ILO definitions, and ilo_lfs based on the derived variable "b0000"
* Check question bd02

	gen ilo_lfs=.
        replace ilo_lfs=1 if (bd01==1) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if ((bd03==1 & inlist(bd04,1,2,3,4) | (bd03==1 & inrange(bd04,5,15) & inlist(bd05,1,2,3)))) & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if (bd08==1 & inlist(bd12,1,2,3)) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (bd08==2 & inlist(bd12,1,2,3) & (bd131==11 | bd132==11 | bd133==11) & inlist(bd16,1,2,3,4)) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
/*
		gen ilo_lfs=.
        replace ilo_lfs=1 if inlist(b0000,1,2 ) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=2 if (b000==4) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
	    replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1		// Outside the labour force
				label define label_ilo_lfs 1 "1 - Employed" 2 "2 - Unemployed" 3 "3 - Outside Labour Force"
				label value ilo_lfs label_ilo_lfs
				label var ilo_lfs "Labour Force Status" 
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (ex01==1 | lx01==3 | ilo_mjh==.) & ilo_lfs==1             // One job only     
		replace ilo_mjh=2 if (ex01>1 | inlist(lx01,1,2)) & ilo_lfs==1             // More than one job
...
* Comment: 
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if (inlist(eg11,5) | inlist(eg12,5) | ((eg11==1 | eg12==1) & inlist(eg13,2,3)) | b0000==2)  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if (eg11==2 | eg12==2) & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if (eg11==3 | eg12==3 | ((eg11==1 | eg12==1) & eg13==1)) & ilo_lfs==1          // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if (eg11==4  | eg12==4) & ilo_lfs==1          // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if (inlist(eg11n,5) | inlist(eg12n,5) | ((eg11n==1 | eg12n==1) & inlist(eg13n,2,3))) & ilo_mjh==2          // Employees
		  replace ilo_job2_ste_icse93=2 if (eg11n==2 | eg12n==2  ) & ilo_mjh==2          // Employers
		  replace ilo_job2_ste_icse93=3 if (eg11n==3 | eg12n==3 | ((eg11n==1 | eg12n==1) & eg13n==1))  & ilo_mjh==2          // Own-account workers
		  *replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2          // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if (eg11n==4 | eg12n==4) & ilo_mjh==2          // Contributing family workers
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Classification NOGA 2008 used by the Swiss, similar as ISIC REV 4. See under internal 
    ...
    ***********
	if to_drop_1 >2008{
	gen code_ind_main =. 
		replace code_ind_main= int(em03/10000) if ilo_lfs==1
		replace code_ind_main = int(lm03/10000) if ilo_lfs==1 & b0000==2 // for apprentices
	}
	if to_drop_1<=2008{
	gen code_ind_main =. 
		replace code_ind_main= int(em03/100) if ilo_lfs==1
		replace code_ind_main = int(lm03/100) if ilo_lfs==1 & b0000==2 // for apprentices
	}
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = code_ind_main    if ilo_lfs==1 & code_ind_main>0
   ...
   * Aggregate level
    ...
    *************
	if to_drop_1>2008{
		gen code_ind_sec =. 
		replace code_ind_sec= int(em03n/10000) if ilo_lfs==1
	}
if to_drop_1<=2008{
		gen code_ind_sec =. 
		replace code_ind_sec= int(em03n/100) if ilo_lfs==1
	}
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits =  code_ind_sec    if ilo_mjh==2 & code_ind_sec>0
...
* Comment:
    ...
    ***********
	gen code_occ_main =. 
		replace code_occ_main= int(bfu5i/100) if ilo_lfs==1
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits =  code_occ_main   if ilo_lfs==1 & code_occ_main>0
    ...
    * SECOND JOB:
    *************
	/* coding does not follow isco08
	
	gen code_occ_sec =. 
		replace code_occ_sec= int(ef03n/1000) if ilo_lfs==1
    *---------------------------------------------------------------------------
	* ISCO 08
	*---------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = code_occ_sec    if ilo_mjh==2 & code_occ_sec>0
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if bmu8==2                       & ilo_lfs==1    // Public
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = ek01      if ilo_lfs==1 & ek01>=0
		replace ilo_job1_how_usual = ek17      if ilo_lfs==1 & ek17>=0 // teachers
		replace ilo_job1_how_usual = lk011      if ilo_lfs==1 & lk011>=0 // apprentices 
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual =ek08       if ilo_lfs==1 & ek08>=0
		replace ilo_job1_how_actual =lk081       if ilo_lfs==1 & lk081>=0
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual = .
	    replace ilo_job2_how_usual =  ek01n     if ilo_mjh==2 & ek01n>=0
		replace ilo_job2_how_usual =  ek17n     if ilo_mjh==2 & ek17n>=0
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual = .
	    replace ilo_job2_how_actual = ek08n      if ilo_mjh==2 & ek08n>=0
	...
	* Hours USUALLY worked
    egen ilo_joball_how_usual = rowtotal(ilo_job1_how_usual-ilo_job2_how_usual) if ilo_lfs==1
	    ...
	    *replace ilo_joball_how_usual = rowtotal(ilo_job1_how_usual, ilo_job2_how_usual)     if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: apprentices classified in temporary
	
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if eh01==2 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if (eh01==1 | bhu1==1) & ilo_lfs==1               				// Temporary
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: questions only on annual questionnaire
* Currency: Swiss Francs (CHF)
	...
	* Employees
	    ...
	    replace ilo_job1_lri_ees = int(bwu1/12)    if ilo_job1_ste_aggregate==1
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = int(bwu1/12)      if ilo_job1_ste_aggregate==2
	    replace ilo_job1_lri_slf = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_slf "Monthly labour related income of self-employed - main job"
				
	capture confirm variable iw04n
	if !_rc{
    *************
    * SECOND JOB:
    *************
	* Swiss cotisation estimated at 10% from gross to net 
	
	* Employees
	gen ilo_job2_lri_ees = .
	    replace ilo_job2_lri_ees = iw04n      if ilo_job2_ste_aggregate==1
		replace ilo_job2_lri_ees = (ilo_job2_lri_ees/12) if ilo_job2_ste_aggregate==1 & iw04n>0 & iw02==2 // gross year
		replace ilo_job2_lri_ees = (ilo_job2_lri_ees*8*20) if ilo_job2_ste_aggregate==1 & iw04n>0 & iw02==3 // gross hourly
		replace ilo_job2_lri_ees = (ilo_job2_lri_ees*1.1) if ilo_job2_ste_aggregate==1 & iw04n>0 & iw02==4 // net monthly
		replace ilo_job2_lri_ees = (ilo_job2_lri_ees/12*1.1) if ilo_job2_ste_aggregate==1 & iw04n>0 & iw02==5 // net yearly
		replace ilo_job2_lri_ees = (ilo_job2_lri_ees/8*20*1.1) if ilo_job2_ste_aggregate==1 & iw04n>0 & iw02==6 // net yearly
    ...
    * Self-employed		
	gen ilo_job2_lri_slf = .
	    replace ilo_job2_lri_slf =  iw04n     if ilo_job2_ste_aggregate==2
		replace ilo_job2_lri_slf = (ilo_job2_lri_slf/12) if ilo_job2_ste_aggregate==2 & iw04n>0 & iw02==2 // gross year
		replace ilo_job2_lri_slf = (ilo_job2_lri_slf*8*20) if ilo_job2_ste_aggregate==2 & iw04n>0 & iw02==3 // gross hourly
		replace ilo_job2_lri_slf = (ilo_job2_lri_slf*1.1) if ilo_job2_ste_aggregate==2 & iw04n>0 & iw02==4 // net monthly
		replace ilo_job2_lri_slf = (ilo_job2_lri_slf/12*1.1) if ilo_job2_ste_aggregate==2 & iw04n>0 & iw02==5 // net yearly
		replace ilo_job2_lri_slf = (ilo_job2_lri_slf/8*20*1.1) if ilo_job2_ste_aggregate==2 & iw04n>0 & iw02==6 // net yearly
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Threshold set at 35 hours

capture confirm variable ek21
	if !_rc{

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<=35  & ek21==1  & inlist(ek23,1,2,3)  & ilo_lfs==1
...
*-------------------------------------------------------------------------------
*-------------------------------------------------------------------------------
* Comment: days lost in the last four weeks
capture confirm variable ek24t
	if !_rc{
	gen ilo_joball_oi_day = .
		replace ilo_joball_oi_day = ek24t       if ilo_lfs==1 & ek24t>=0
		replace ilo_joball_oi_day = lk24t       if ilo_lfs==1 & lk24t>=0
...
* Comment: 
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if ax01==1  & ilo_lfs==2                     // Previously employed       
		replace ilo_cat_une=2 if ax01==2    & ilo_lfs==2                     // Seeking for the first time
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if  ae08k==1     & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if  ae08k==2     & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if  ae08k==3     & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if  ae08k==4     & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if  inlist(ae08k,5,6)     & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if  ae08k>=7     & ilo_lfs==2                 // 24 months or more
...
if to_drop_1>2008{
	gen code_ind_prev =. 
		replace code_ind_prev= int(am03/10000) if ilo_lfs==2 & ilo_cat_une==1
		replace code_ind_prev = int(lm03/10000) if ilo_lfs==2 & ilo_cat_une==1 & b0000==2 // for apprentices
}
if to_drop_1<=2008{
	gen code_ind_prev =. 
		replace code_ind_prev= int(am03/100) if ilo_lfs==2 & ilo_cat_une==1
		replace code_ind_prev = int(lm03/100) if ilo_lfs==2 & ilo_cat_une==1 & b0000==2 // for apprentices
}
    *---------------------------------------------------------------------------
	* ISIC REV 4
	*---------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = code_ind_prev     if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* Comment: 
	gen code_occ_prev =. 
		replace code_occ_prev= int(bfu5i/100) if ilo_lfs==2 & ilo_cat_une==1
	...
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits = . 
	    replace ilo_prevocu_isco08_2digits = code_occ_prev    if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: 60% goes to "Not elswhere classified".
* ------------------------------------------------------------------------------
*		Degree of labour market attachment ('ilo_olf_dlma') 	               * 
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: 60% goes to "Not elswhere classified".
if to_drop_1>=2010{
capture confirm variable ne11
	if !_rc{

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  (bd08==1 | inrange(nn17k,0,8)) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7))            & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  (bd08==2 | !inrange(nn17k,0,8)) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 | !inrange(nn17k,0,8)) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (((bd08==2 | !inrange(nn17k,0,8)) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)) | (inlist(ne11,5,6,7,10)))) & nn01==2		      & ilo_lfs==3      // Not seeking, not available, not willing
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 2 if  (bd08==2 | !inrange(nn17k,0,8)) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 | !inrange(nn17k,0,8)) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (((bd08==2 | !inrange(nn17k,0,8)) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)))) & nn01==2		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
...
if inrange(to_drop_1,2006,2009){
capture confirm variable ne11
	if !_rc{

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  (bd08==1 ) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7))            & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  (bd08==2 ) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 ) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (((bd08==2 ) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)) | (inlist(ne11,5,6,7,10)))) & nn01==2		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 2 if  (bd08==2 ) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 ) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (((bd08==2 ) & (inlist(bd12,4,5,6) | inlist(nn02,4,5,6,7)))) & nn01==2		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
...
capture confirm variable ne11
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  (bd08==1 ) & (inlist(bd12,4,5,6) )            & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  (bd08==2 ) & (inlist(bd12,1,2,3) )            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 ) & (inlist(bd12,4,5,6) ) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (((bd08==2 ) & (inlist(bd12,4,5,6)) | (inlist(ne11,5,6,7,10)))) & nn01==2		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 2 if  (bd08==2 ) & (inlist(bd12,1,2,3) )            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if  (bd08==2 ) & (inlist(bd12,4,5,6) ) & nn01==1 	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (bd08==2 ) & inlist(bd12,4,5,6)  & nn01==2		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: 36 % percent goes to "not elsewhere classified"
if inrange(to_drop_1,2006,2009){
capture confirm variable nn02

		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if (inlist(bd131,12 ) | inlist(bd132,12 ) | inlist(bd133,12 ))     & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if (inlist(bd131,13,3) | inlist(bd132,13,3 ) | inlist(bd133,13,3 ))         & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if (inlist(bd131,9,10,15,17,16,18) | inlist(bd132,9,10,15,17,16,18) | inlist(bd133,9,10,15,17,16,18))        & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if (inlist(bd131,5,7,8) | inlist(bd132,5,7,8) | inlist(bd133,5,7,8) | nn02==7)          & ilo_lfs==3             // Does not need/want to work
...
capture confirm variable nn02
		...
		gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=2 if (inlist(bd131,13,3) | inlist(bd132,13,3 ) | inlist(bd133,13,3 ))         & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if (inlist(bd131,9,10,15,17,16,18) | inlist(bd132,9,10,15,17,16,18) | inlist(bd133,9,10,15,17,16,18))        & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if (inlist(bd131,5,7,8) | inlist(bd132,5,7,8) | inlist(bd133,5,7,8))          & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if ilo_olf_reason==. & ilo_lfs==3     // Not elsewhere classified
 			        lab def lab_olf_reason 1 "1 - Labour market" 2 " 2 - Other labour market reasons" 3 "3 - Personal/Family-related"  ///
				                           4 "4 - Does not need/want to work" 5 "5 - Not elsewhere classified"
					lab val ilo_olf_reason lab_olf_reason
...
capture confirm variable nn02
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & (inlist(bd12,1,2,3) | inlist(nn02,1,2,3))      & ilo_olf_reason==1
		...
		gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & inlist(bd12,1,2,3)      & ilo_olf_reason==1
			    lab def ilo_dis_lab 1 "Discouraged job-seekers" 
			    lab val ilo_dis ilo_dis_lab
			    lab var ilo_dis "Discouraged job-seekers"	
	}
