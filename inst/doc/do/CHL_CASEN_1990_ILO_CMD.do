...
global source "CASEN"
global time "1990"
global inputFile "casen1990.dta"
...
/*
gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if e4==9                                                             // X - No schooling
		replace ilo_edu_isced97=2 if e4==1                                                           // 0 - Pre-primary education
		*replace ilo_edu_isced97=3 if e4==3                                                           // 1 - Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if e4==2                                                           // 2 - Lower secondary or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(e4,3,4)                                                // 3 - Upper secondary education
		replace ilo_edu_isced97=6 if e4==6                                               // 4 - Post-secondary non-tertiary education
    	replace ilo_edu_isced97=7 if inlist(e4,5,7)                                              // 5 - First stage of tertiary education
 		*replace ilo_edu_isced97=8 if e4==10                                                            // 6 - Second stage of tertiary education
...
* -------------------------------------------------------------------------------------------
 		...
 		gen ilo_edu_attendance=.
			replace ilo_edu_attendance=1 if e2==1
			replace ilo_edu_attendance=2 if e2!=1
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=5 if inrange(ecivil,3,4)                                // Divorced / separated
...
*/
		...
		* Primary activity
		...
		gen ilo_job1_eco_aggregate=.
			replace ilo_job1_eco_aggregate=1 if rama==0 & ilo_lfs==1
			replace ilo_job1_eco_aggregate=2 if rama==2 & ilo_lfs==1
			replace ilo_job1_eco_aggregate=3 if rama==3 & ilo_lfs==1
			replace ilo_job1_eco_aggregate=4 if rama==1 & ilo_lfs==1
			replace ilo_job1_eco_aggregate=5 if inlist(rama,4,8) & ilo_lfs==1
			replace ilo_job1_eco_aggregate=6 if inrange(rama,5,7) & ilo_lfs==1
...
** Comment: the results do not make sense with more recent years. 
		** Comment:  ISCO-88 classification used

 
			** Comment: it is only possible to compute at 1-digit level

/*
	gen occ_code_prim=int(o5/10) if ilo_lfs==1 & o5>=10
		replace occ_code_prim=o5 if ilo_lfs==1 & inrange(o5,0,7)
 	

    * 1-digit level
	gen ilo_job1_ocu_isco88 =.
	
		replace ilo_job1_ocu_isco88 = 1 if occ_code_prim==1 & ilo_lfs==1  // "1 - Legislators, senior officials and managers"
		replace ilo_job1_ocu_isco88 = 2 if occ_code_prim==2 & ilo_lfs==1  // "2 - Professionals"
		replace ilo_job1_ocu_isco88 = 3 if occ_code_prim==3 & ilo_lfs==1  // "3 - Technicians and associate professionals"
		replace ilo_job1_ocu_isco88 = 4 if occ_code_prim==4 & ilo_lfs==1  // "4 - Clerks"
		replace ilo_job1_ocu_isco88 = 5 if occ_code_prim==5 & ilo_lfs==1  // "5 - Service workers and shop and market sales workers"
		replace ilo_job1_ocu_isco88 = 6 if occ_code_prim==6  & ilo_lfs==1  // "6 - Skilled agricultural and fishery workers"
        replace ilo_job1_ocu_isco88 = 7 if occ_code_prim==7  & ilo_lfs==1  // "7 - Craft and related trades workers"
        replace ilo_job1_ocu_isco88 = 8 if occ_code_prim==8  & ilo_lfs==1  // "8 - Plant and machine operators and assemblers"
        replace ilo_job1_ocu_isco88 = 9 if occ_code_prim==9  & ilo_lfs==1  // "9 - Elementary occupations"
...
gen ilo_job1_how_actual =  jh if ilo_lfs == 1 
...
* Comment: There is no information about availability to work additional hours  
...
*--------------------------------------------------------------------------------------------
/*
 ** COMMENT: there is no information to 
...
** Comment: work-related illness or accident
			...
			gen ilo_joball_oi_case = .
				replace ilo_joball_oi_case = 1 if s9==2
...
/*	 gen ilo_dur_details=.
		replace ilo_dur_details=1 if inrange(o4,0,3)
		replace ilo_dur_details=2 if inrange(o4,4,11)
		replace ilo_dur_details=3 if inrange(o4,12,23)
		replace ilo_dur_details=4 if inrange(o4,24,47)
		replace ilo_dur_details=5 if inrange(o4,48,95)
		replace ilo_dur_details=6 if o4>=96 & o4!=999
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if o12==1
		replace ilo_cat_une=2 if o12==2
