...
global source "CASEN"
global time "1996"
global inputFile "casen1996.dta"
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
** Comment: variable e6 is not categorized as it appears in the questionnaire. 


gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if e6==0                                                             // X - No schooling
		replace ilo_edu_isced97=2 if e6==1                                                           // 0 - Pre-primary education
		replace ilo_edu_isced97=3 if e6==2                                                           // 1 - Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if inlist(e6,3,4)                                                          // 2 - Lower secondary or second stage of basic education
		replace ilo_edu_isced97=5 if inrange(e6,5,9) | e6==11                                               // 3 - Upper secondary education
		replace ilo_edu_isced97=6 if inlist(e6,10,12,13)                                              // 4 - Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if e6==14                                                // 5 - First stage of tertiary education
 		replace ilo_edu_isced97=8 if e6==15                                                           // 6 - Second stage of tertiary education
...
* -------------------------------------------------------------------------------------------
 		...
 		gen ilo_edu_attendance=.
			...
			replace ilo_edu_attendance=2 if e2==2
...
* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inrange(o8,3,6) | o8==8
			replace ilo_job1_ste_icse93=2 if o8==1
			replace ilo_job1_ste_icse93=3 if o8==2
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if o8==7
...
* -------------------------------------------------------------------------------------------
	...
	* Two-digits level
	...
	gen occ_code_prim=int(o6/100) if ilo_lfs==1 & o6!=9999
...
* -------------------------------------------------------------------------------------------
	...
	* Primary occupation
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(o8,3,8)
...
** Comment: o19 is a monthly variable.

gen ilo_job1_how_actual =  o19/4 if ilo_lfs == 1 & o19!=999
 ...
 ** Comment: variable related to the type of contract (o12) is not included in the database, therefore
**				it has been used variable o9 which provides some information in this regard, and however, the results are
**				not exactly in line with what have been obtained in recent years. 
	
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if o9==1
		replace ilo_job1_job_contract=2 if inrange(o9,2,5)
...
* Comment: Useful questions:	* o8: Status in employment
								* o8: Institutional sector 
								* [no useful question]: Destination of production 
								* no info: Bookkeeping
								* no info: Registration						
								* o11: Location of workplace 
								* o10: Size of the establishment					
 ...
 ** COMMENT: There is no infomation related to occupational injuty in the database
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if o27==1
		replace ilo_cat_une=2 if o27==2
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if o5==8
        replace ilo_olf_reason=2 if o5==7
        replace ilo_olf_reason=3 if inrange(o5,1,4) 
		replace ilo_olf_reason=4 if inlist(o5,5,6) 
