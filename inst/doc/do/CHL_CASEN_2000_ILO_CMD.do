...
global source "CASEN"
global time "2000"
global inputFile "casen2000_Stata.dta"
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_edu_isced97=.
		...
		replace ilo_edu_isced97=4 if inlist(educ,4,6)                                                           // 2 - Lower secondary or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(educ,5,7,8)                                                // 3 - Upper secondary education
		replace ilo_edu_isced97=6 if inlist(educ,9,10)                                               // 4 - Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if educ==11                                                // 5 - First stage of tertiary education
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if inlist(o29,2,9)
		replace ilo_mjh=2 if o29==1
...
* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inrange(o10,3,7) | o10==9
			replace ilo_job1_ste_icse93=2 if o10==1
			replace ilo_job1_ste_icse93=3 if o10==2
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if o10==8
...
* -------------------------------------------------------------------------------------------
	...
	* Two-digits level
	...
	gen occ_code_prim=int(o8/100) if ilo_lfs==1 & o8!=9999
...
* -------------------------------------------------------------------------------------------
	...
	* Primary occupation
	 ...
	 gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(o10,3,4) | o10==9
...
** Comment: The variable o12 refers to the number of working hours according to the working contract	
** Comment: According to the Chile labour law "Codigo del Trabajo", part-time job is equivalent to <= 30 hours/week
...
* -------------------------------------------------------------------------------------------	
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if o15==1
		replace ilo_job1_job_contract=2 if inrange(o15,2,5)
...
* Comment: Useful questions:	* o10: Status in employment
								* o10: Institutional sector 
...
* Comment: It is not possible to compute this variable. There is no information about availability to work additional hours  
 ...
 ** COMMENT: It is not possible to compute this variable. There is no information. 
...
* Comment.- variable used: o4 number of weeks looking for a job
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if o5==1
		replace ilo_cat_une=2 if o5==2
...
** Comment: question o7 refers to the last two months

	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if o7==8
        replace ilo_olf_reason=2 if o7==7
        replace ilo_olf_reason=3 if inrange(o7,1,4) 
		replace ilo_olf_reason=4 if inlist(o7,5,6) | o7==9
