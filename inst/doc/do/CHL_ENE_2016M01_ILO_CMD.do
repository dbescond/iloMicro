...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Completed level of education considered.
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if est_conyugal==3                           // Single
		replace ilo_mrts_details=2 if est_conyugal==1                           // Married
		replace ilo_mrts_details=3 if est_conyugal==2                           // Union / cohabiting
		replace ilo_mrts_details=4 if est_conyugal==4                           // Widowed
		replace ilo_mrts_details=5 if inlist(est_conyugal,5,6)                  // Divorced / separated
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Job seeking: reference period 4 weeks
*          - Availability to start a job: reference period current week
 
	gen ilo_lfs=.
		replace ilo_lfs=1 if (((a1==1 | a2==1) & a3==1) | a4==1 | inlist(a6,1,2,3,4,5) | a7==1 | a8==1) & ilo_wap==1   // Employed
		replace ilo_lfs=2 if (ilo_lfs!=1 & ((!inlist(e3_total,0,12,.) & e5==1) | e10==1) & e11==1) & ilo_wap==1        // Unemployed
		replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1                                                         // Outside labour force
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if b19==2 & ilo_lfs==1
		replace ilo_mjh=2 if b19==1 & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if (b2==2) & ilo_lfs==1
			replace ilo_job1_ste_icse93=2 if (b2==1 & b4==1) & ilo_lfs==1
			replace ilo_job1_ste_icse93=3 if (b2==1 & b4==2) & ilo_lfs==1
			/*replace ilo_job1_ste_icse93=4 if  */
			replace ilo_job1_ste_icse93=5 if (b2==3) & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: -Info only indicated at one-digit level
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment:  - Information indicated only at one-digit level
			
	* 1 digit level
	gen occ_code_prim_1dig=b1 if ilo_lfs==1 
	
	* Determine whether ISCO88 or ISCO08 should be used
...
* -------------------------------------------------------------------------------------------
	...
	* Primary occupation
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if b5==1 & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
* Comment: - Question directly asked.
	
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=1 if c1==2 & ilo_lfs==1
		replace ilo_job1_job_time=2 if c1==1 & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------	
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if b9==1 & ilo_job1_ste_aggregate==1
		replace ilo_job1_job_contract=2 if b9==2 & ilo_job1_ste_aggregate==1
...
* Comment: 
	...
	* Useful questions: ilo_job1_ste_aggregate: Status in employment
					*	b5: Institutional sector 
					...
					* Social security: 
						...
						* [b7_1: Paid annual leave]
						* [b7_2: Paid sick leave]
						* [b7_4: Health insurance provided by employer]
						* [b7_5: Unemployment insurance]
		
	capture gen b7_3 = b7a_1
	capture gen b7_1 = b7b_1
	capture gen b7_2 = b7b_2
	...
	destring b15_2, replace
		...
		gen ilo_job1_ife_prod=.
			...
			replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
			                               ((b5==1) | ///
										   (inlist(b5,2,.) & ilo_job1_ste_aggregate==1 & b7_3==1) | ///
										   (inlist(b5,2,.) & ilo_job1_ste_aggregate==1 & b7_3!=1 & b16==1 & inrange(b15_1,2,5)) | ///
										   (inlist(b5,2,.) & ilo_job1_ste_aggregate!=1 & b16==1 & inrange(b15_1,2,5)))
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((ilo_job1_ste_icse93==1 & b7_3==1) | ///
										 (ilo_job1_ste_icse93==1 & !inlist(b7_3,1,2) & b7_1==1 & b7_2==1) | ///
										 (inlist(ilo_job1_ste_icse93,2,3) & ilo_job1_ife_prod==2) | ///
										 (ilo_job1_ste_icse93==4 & ilo_job1_ife_prod==2))
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Earnings ('ilo_ear_ees' and 'ilo_ear_slf') [No info]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - No information available.
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_joball_tru=1 if ilo_job1_job_time==1 & c10==1 & c11==1 & ilo_lfs==1
	...
	gen ilo_dur_details=.
		...
		replace ilo_dur_details=. if ilo_lfs!=2
			    lab def ilo_unemp_det 1 "Less than 1 month" 2 "1 month to less than 3 months" 3 "3 months to less than 6 months" ///
				                      4 "6 months to less than 12 months" 5 "12 months to less than 24 months" ///
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Note that the question asks whether the person had previously a job lasting at 
*            least one month. This applies consequently to all questions concerning the 
*            previous employment

	gen ilo_cat_une=.
		replace ilo_cat_une=1 if e13==1 & ilo_lfs==2
		replace ilo_cat_une=2 if e13==2 & ilo_lfs==2
	...
	* Aggregate level 
    ...
    gen ilo_prevocu_skill=.
		...
		gen ilo_prevocu_isco08=prevocu_cod
		    ...
		    replace ilo_prevocu_isco08=11 if (ilo_prevocu_isco08==. & ilo_cat_une==1 & ilo_lfs==2) | prevocu_cod==10
		            * value label already defined
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Unemployment benefits schemes ('ilo_soc_aggregate') [No info]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: No information available.
...
* -------------------------------------------------------------------------------------------
	 ...
	 gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if ((inrange(e3_total,1,11) | e10==1) & e11==2) & ilo_lfs==3
		replace ilo_olf_dlma=2 if ((!inrange(e3_total,1,11) | e10==2) & e11==1) & ilo_lfs==3
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(e9,15,16,17,18,19,20) & ilo_lfs==3
		replace ilo_olf_reason=2 if inlist(e9,9,13) & ilo_lfs==3
		replace ilo_olf_reason=3 if inlist(e9,3,4,8,10,11,12,14) & ilo_lfs==3
		replace ilo_olf_reason=4 if inlist(e9,5,6,7,21) & ilo_lfs==3
...
* ------------------------------------------------------------------------------------------- 
	...
	gen ilo_dis=1 if !inlist(ilo_lfs,1,2) & e11==1 & ilo_olf_reason==1
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Youth not in education, employment or training (NEETs) ('ilo_neet') [No info]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: No information avaialble.
...
cd "$outpath"
	...
	drop indu_code_* occ_code_*  prev*_cod indu_code_prim occ_code_prim_1dig diff_month diff_year preveco_cod prevocu_cod
	compress 
