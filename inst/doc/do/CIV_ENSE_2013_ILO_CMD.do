...
* Note that according to the definition, the highest level CONCLUDED is considered.
	...
	gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if inlist(m15,2,9)		// No schooling
		replace ilo_edu_isced11=2 if inrange(m21,1,5)  		// Early childhood education
		replace ilo_edu_isced11=3 if inrange(m21,6,9)  		// Primary education
		replace ilo_edu_isced11=4 if inrange(m21,10,12)  	// Lower secondary education
		replace ilo_edu_isced11=5 if inrange(m21,13,15)     // Upper secondary education
		* replace ilo_edu_isced11=6 if 						// Post-secondary non-tertiary
		replace ilo_edu_isced11=8 if inrange(m21,16,17)		// Bachelor or equivalent
		replace ilo_edu_isced11=9 if inrange(m21,18,19)     // Master's or equivalent level
		replace ilo_edu_isced11=10 if m21==20				// Doctoral or equivalent level
		replace ilo_edu_isced11=11 if m21==99				// Not elsewhere classified
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if (m22==1)				// Attending
		replace ilo_edu_attendance=2 if (m22==2 | m22==9)		// Not attending
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if ((se3==1 | inrange(se4,1,8) | inrange(se6,1,7) | se7==1) & ilo_wap==1)		// Employment
		replace ilo_lfs=2 if (ilo_wap==1 & ilo_lfs!=1 & se8==1 & inlist(se10,1,2))						// Unemployment
  ...
  * MAIN JOB:
	...
	* Detailed categories:
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if (inlist(ap3,1,4,5) & ilo_lfs==1)	// Employees
			replace ilo_job1_ste_icse93=2 if (ap3==2 & ilo_lfs==1)				// Employers
			replace ilo_job1_ste_icse93=3 if (inlist(ap3,6,8) & ilo_lfs==1)		// Own-account workers
			* replace ilo_job1_ste_icse93=4 if									// Producers cooperatives
			replace ilo_job1_ste_icse93=5 if (ap3==7 & ilo_lfs==1)				// Contributing family workers
			replace ilo_job1_ste_icse93=6 if (inlist(ap3,3,9,.) & ilo_lfs==1)	// Not classifiable
  ...
  * SECOND JOB:
	...
	* Detailed categories:
		...
		gen ilo_job2_ste_icse93=.
			replace ilo_job2_ste_icse93=1 if (inlist(as3,1,4,5) & ilo_mjh==2)	// Employees
			replace ilo_job2_ste_icse93=2 if (as3==2 & ilo_mjh==2)				// Employers
			replace ilo_job2_ste_icse93=3 if (inlist(as3,6,8) & ilo_mjh==2)		// Own-account workers
			* replace ilo_job2_ste_icse93=4 if									// Producers cooperatives
			replace ilo_job2_ste_icse93=5 if (as3==7 & ilo_mjh==2)				// Contributing family workers
			replace ilo_job2_ste_icse93=6 if (inlist(as3,3,9,.) & ilo_mjh==2)  	// Not classifiable
...
* -------------------------------------------------------------------------------------------
	...
	* MAIN JOB:
	gen indu_code_prim=.
	    replace indu_code_prim=int(ap2b_impute/100)
...
* -------------------------------------------------------------------------------------------		
		...
		gen ilo_job1_ins_sector=.
			...
			replace ilo_job1_ins_sector=2 if (inlist(ap4,3,4,6,7,8,9) & ilo_lfs==1)	// Private
...
* -------------------------------------------------------------------------------------------			
			...
			gen ilo_job1_job_time=.
				replace ilo_job1_job_time=1 if (ap8c<40 & ilo_lfs==1) 	// Part-time
				replace ilo_job1_job_time=2 if (ap8c>39 & ilo_lfs==1)	// Full-time
...
* -------------------------------------------------------------------------------------------			
			...
			gen ilo_job1_job_contract=.
				replace ilo_job1_job_contract=1 if (ap7a==1 & ilo_lfs==1)
				replace ilo_job1_job_contract=2 if (inlist(ap7a,2,9) & ilo_lfs==1)
...
* Main job:
	...
	* 1) Weekly hours ACTUALLY worked
			...
			gen ilo_job1_how_actual=ap8c if (ilo_lfs==1)
				replace ilo_job1_how_actual=0 if (ap8c==. & ilo_lfs==1)
...
* Second job:
	...
	* 1) Weekly hours ACTUALLY worked:
			...
			gen ilo_job2_how_actual=as5b if (ilo_mjh==2 & ilo_lfs==1)
				replace ilo_job2_how_actual=0 if (as5b==. & ilo_mjh==2 & ilo_lfs==1)
...
* All jobs:
	...
	* 1) Weekly hours ACTUALLY worked:
			...
			gen ilo_joball_how_actual=(ap8c + as5b) if (ilo_mjh==2 & ilo_lfs==1) 
				replace ilo_joball_how_actual=ap8c if (ilo_mjh==1 & ilo_lfs==1)
			...
			* All jobs
				...
				* Employees
					gen ilo_job1_lri_ees=.
						replace ilo_job1_lri_ees=ap18a*365/12 if (ap18a1==1 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=ap18a*52/12 if (ap18a1==2 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=ap18a*26/12 if (ap18a1==3 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=ap18a if (ap18a1==4 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=ap18a/3 if (ap18a1==5 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=ap18a/6 if (ap18a1==6 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=ap18a/12 if (ap18a1==7 & ilo_job1_ste_aggregate==1)
						replace ilo_job1_lri_ees=10000000 if (ilo_job1_lri_ees>=10000000 & ilo_job1_lri_ees!=.)
							lab var ilo_job1_lri_ees "Monthly earnings of employees in main job"	
				
				* Self-employed
					gen ilo_job1_lri_slf=.
						replace ilo_job1_lri_slf=ap18a*365/12 if (ap18a1==1 & inlist(ilo_job1_ste_icse93,2,3,4,6))
						replace ilo_job1_lri_slf=ap18a*52/12 if (ap18a1==2 & inlist(ilo_job1_ste_icse93,2,3,4,6))
						replace ilo_job1_lri_slf=ap18a*26/12 if (ap18a1==3 & inlist(ilo_job1_ste_icse93,2,3,4,6))
						replace ilo_job1_lri_slf=ap18a if (ap18a1==4 & inlist(ilo_job1_ste_icse93,2,3,4,6))
						replace ilo_job1_lri_slf=ap18a/3 if (ap18a1==5 & inlist(ilo_job1_ste_icse93,2,3,4,6))
						replace ilo_job1_lri_slf=ap18a/6 if (ap18a1==6 & inlist(ilo_job1_ste_icse93,2,3,4,6))
						replace ilo_job1_lri_slf=ap18a/12 if (ap18a1==7 & inlist(ilo_job1_ste_icse93,2,3,4,6))
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if (se8==1 & inlist(se10,3,4) & ilo_lfs==3) 	// Seeking, not available
		replace ilo_olf_dlma = 2 if (se8==2 & inlist(se10,1,2) & ilo_lfs==3)	// Not seeking, available
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(se9,2,3,4,9,11) & ilo_lfs==3
		replace ilo_olf_reason=2 if inlist(se9,6,7,8) & ilo_lfs==3
		replace ilo_olf_reason=3 if inlist(se9,1,5) & ilo_lfs==3
		replace ilo_olf_reason=4 if inlist(se9,10) & ilo_lfs==3
