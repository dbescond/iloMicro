...
*********************************************************************************************
*********************************************************************************************
* Original datasets are divided by geographical coverage (urban, rural and main cities) and 
* modules of the GEIH's questionnaire. In order to obtain the final dataset to be used, thereÃ¢â‚¬â„¢s 
* a consolidation process carried out beforehand; from reading the related text files and saving 
...
*--- 1.1 As of March 2018, the NSO started publishing datasets directly in stata
*---     format and therefore it is not necessary to import and save from txt to
*---     dta; in that case it is only important to cut the file's name in order 
*---     to proceed with the merge and append the necessary files.

* Check if the txt folder exists:
capture confirm file "`M'.txt/nul"
if _rc==0 {
  cd "$inpath\Internal\\`M'.txt"
  local text_files: dir . files "*.txt"

  foreach file_in of local text_files{
     /*reading all text files in the folder*/
     import delimited "`file_in'", clear
     /*string function*/
     local file_out0 = subinstr("`file_in'", ".txt", "", .) 
     /*local the name of the files only the first 15 characters*/
     local file_out = substr("`file_out0'",1,15)
     /*changing directory*/
     cd "$inpath\Internal\\`M'_dta"
     /*saving dta files with 15 characters*/
     save "`file_out'", replace
     /*changing to the txt directory again*/
     cd "$inpath\Internal\\`M'.txt"
  }   
  cd "$inpath\Internal\\`M'_dta"
}
  ...
  cd "$inpath\Internal\\`M'.dta"
  local dta_files: dir . files "*.dta"

  foreach file_in of local dta_files{
     /*reading all dta files in the folder*/
     use "`file_in'", clear
	 /*renaming all in lower case*/
	 rename *, lower
     /*string function*/
     local file_out0 = subinstr("`file_in'", ".dta", "", .) 
     /*local the name of the files only the first 15 characters*/
     local file_out = substr("`file_out0'",1,15)
     /*changing directory*/
     cd "$inpath\Internal\\`M'_dta"
...
cd "$inpath\Internal\\`M'_dta"
   use "Cabecera - Cara", clear /*CaracterÃƒ­sticas generales*/
   merge 1:1 directorio secuencia_p orden using "Cabecera - Fuer", nogenerate /*Fuerza de trabajo*/
   merge 1:1 directorio secuencia_p orden using "Cabecera - Ocup", nogenerate /*Ocupados*/
   merge 1:1 directorio secuencia_p orden using "Cabecera - Deso", nogenerate /*Desocupados*/
   merge 1:1 directorio secuencia_p orden using "Cabecera - Inac", nogenerate /*Inactivos*/
   merge 1:1 directorio secuencia_p orden using "Cabecera - Otra", nogenerate /*Otras actividades*/
   merge 1:1 directorio secuencia_p orden using "Cabecera - Otro", nogenerate /*Otros ingresos*/
   rename *, lower
   
   *--Variables that must be character (force them to be as character) and those that must be numeric (force them to be numeric)
   
   unab allvars: _all
   unab string_vars: p6290s1 p6240s1 p6310s1 p6430s1 p6480s1 p6765s1 p6780s1 p6410s1 p6810s1 p6830s1 p6880s1 p6980s7a1 p6915s1 p7140s9a1 p7240s1 p7280s1 p7350s1 p7390s1 p7420s7a1 p7450s1 p7458s1 p6980 p7028s1 p7050s1 p7420 
...
clear all
cd "$inpath\Internal\\`M'_dta"
   use "Resto - Caracte", clear   /*CaracterÃƒ­sticas generales*/
   merge 1:1 directorio secuencia_p orden using "Resto - Fuerza ", nogenerate /*Fuerza de trabajo*/
   merge 1:1 directorio secuencia_p orden using "Resto - Ocupado", nogenerate /*Ocupados*/
   merge 1:1 directorio secuencia_p orden using "Resto - Desocup", nogenerate /*Desocupados*/
   merge 1:1 directorio secuencia_p orden using "Resto - Inactiv", nogenerate /*Inactivos*/
   merge 1:1 directorio secuencia_p orden using "Resto - Otras a", nogenerate /*Otras actividades*/
   merge 1:1 directorio secuencia_p orden using "Resto - Otros i", nogenerate /*Otros ingresos*/
   rename *, lower
