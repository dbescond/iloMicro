* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Colombia, 2013
* DATASET USED: Colombia, SWTS, 2013
* NOTES: 
* Files created: Standard variables COL_SWTS_2013_FULL.dta and COL_SWTS_2013_ILO.dta
...
global source "SWTS" 
global time "2013" 
global inputFile "COL_SWTS_2013.dta"
...
* Comment: Geographical coverage is not available
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if   ep726==1     | ep620==2                // No schooling
		replace ilo_edu_isced97=2 if   ep726==2                    // Pre-primary education
		replace ilo_edu_isced97=3 if   ep726==3                      // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if   ep726==4                     // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if    ep726==5                     // Upper secondary education
		replace ilo_edu_isced97=6 if    ep726==6                     // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if    ep726==7                     // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if    ep726==8                     // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if    ep621==1                    // Attending
		replace ilo_edu_attendance=2 if    ep621==2                    // Not attending
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if  ep605==6                                         // Single
		replace ilo_mrts_details=2 if  ep605==3                                         // Married
		replace ilo_mrts_details=3 if  inlist(ep605,1,2)                                // Union / cohabiting
		replace ilo_mrts_details=4 if  ep605==5                                         // Widowed
		replace ilo_mrts_details=5 if  ep605==4                                         // Divorced / separated
...
* Comment: 
    ...
    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=1 if ep617s1==2 & ep617s2==2 & ep617s3==2 & ep617s4==2  // No, no difficulty
		replace ilo_dsb_details=2 if ep617s1a1==1 | ep617s2a1==1 |  ep617s3a1==1 |  ep617s4a1==1  // Yes, some difficulty
		replace ilo_dsb_details=3 if ep617s1a1==2 | ep617s2a1==2 |  ep617s3a1==2 |  ep617s4a1==2    	                // Yes, a lot of difficulty
		replace ilo_dsb_details=4 if ep617s1a1==3 | ep617s2a1==3 |  ep617s3a1==3 |  ep617s4a1==3  		                // Cannot do it at all
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (ep6430~=.|ep6250==1) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if ep6260==1 & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if ep6280==1 & ep744==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (ep6310==1) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: domestic worker included with employees (C2_3881)

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(ep6430,1,2,3)  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if ep6430==5 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if ep6430==4 & ilo_lfs==1          // Own-account workers
		 replace ilo_job1_ste_icse93=4 if ep6430==8 & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if inlist(ep6430,6,7) & ilo_lfs==1          // Contributing family workers
...
* Comment:
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
		...
		*/		
	* 1-digit level 				
gen ilo_job1_ocu_isco08=occu if ilo_lfs==1
		replace ilo_job1_ocu_isco08=11 if !inrange(ilo_job1_ocu_isco08,1,10) & ilo_lfs==1                          // Not elsewhere classified// Armed forces
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = ep658      if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if ep6460==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if ep6460==2 & ilo_lfs==1               				// Temporary
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds): ep657s10, ep657s5(pension) 
		  - Place of work:
		  - Size:ep6870
		  - Paid annual leave:ep657s3
		  - Paid sick leave:ep657s4
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if ep657s10==1   & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if ep657s10==2    & ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((publicadmin==1) | ///
									   (ep654==1) | ///
									   (ep654==4 & ilo_job1_ste_icse93==1 & social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (ilo_job1_ste_icse93==1 & ep657s3==1 & ep657s4==1 & inlist(ep657s10,9,.)) | ///
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_usual<=35  & ep7090==1  & ep7100>0 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(ep702,1,2)      & ilo_lfs==2        // Less than 1 month
		replace ilo_dur_details=2 if ep702==3      & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if ep702==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if ep702==5      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if ep702==6      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if ep702==7      & ilo_lfs==2                 // 24 months or more
...
*Comment: no information on willing to work
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if ep6280==1 & ep744==2 & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  ep6280==2 & ep744==1             & ilo_lfs==3	    // Not seeking, available
		*replace ilo_olf_dlma = 3 if ep6280==2 & ep744==2 &  & ilo_lfs==3      // Not seeking, not available, willing
		*replace ilo_olf_dlma = 4 if ep6280==2 & ep744==2 &  & ilo_lfs==3      // Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if  inlist(ep6310,2,4,5,6,7,8,9)     & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if  inlist(ep6310,1,3)         & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if  inlist(ep6310,10,11,12)         & ilo_lfs==3             // Personal/Family-related
			*replace ilo_olf_reason=4 if ep6300==2          & ilo_lfs==3             // Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & ep744==1     & ilo_olf_reason==1
