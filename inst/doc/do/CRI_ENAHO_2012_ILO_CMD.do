...
* Comment: question made to people aged 10 years or above.
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if a25==6                                     // Single
		replace ilo_mrts_details=2 if a25==2                                     // Married
		replace ilo_mrts_details=3 if a25==1                                     // Union / cohabiting
		replace ilo_mrts_details=4 if a25==5                                     // Widowed
		replace ilo_mrts_details=5 if inlist(a25,3,4)                            // Divorced / separated
...
**         because they were waiting for an answer after an application (b8==3) or in a seasonal break (b8==2). 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (b1==1|b2!=0|b3==1) & ilo_wap==1				    // Employed: ILO definition
		replace ilo_lfs=1 if inrange(b5,1,5) & ilo_wap==1                       // Employed: Temporarily Absent
		
		replace ilo_lfs=2 if inlist(b6,8,9) & (b7a==1|b7b==2|b7c==3|b7d==4|b7e==5|b7f==6|b7g==7|b7h==8|b7i==9|b7j==10|b7k==11) & ilo_lfs!=1 & ilo_wap==1 // // Unemployed: three criteria
   ...
   * ICSE 1993
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=. 
		  replace ilo_job2_ste_icse93=1 if inlist(f7,3,4) & ilo_mjh==2          // Employees
		  replace ilo_job2_ste_icse93=2 if f7==1 & ilo_mjh==2                   // Employers
		  replace ilo_job2_ste_icse93=3 if f7==2 & ilo_mjh==2                   // Own-account workers
		  *replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2                       // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if f7==5 & ilo_mjh==2                   // Contributing family workers
...
*** CAECR – 2011 corresponds to ISIC Rev.4
	...
	* 1-digit level
    gen ilo_job1_eco_isic4=.
		replace ilo_job1_eco_isic4=ramaemppri if ilo_lfs==1
	    replace ilo_job1_eco_isic4=22 if ramaemppri==99 & ilo_lfs==1
   ...
   * Aggregate level
	...
	* 1-digit level
    gen ilo_job2_eco_isic4=.
		replace ilo_job2_eco_isic4=ramaempsec if ilo_mjh==2
	    replace ilo_job2_eco_isic4=22 if ramaempsec==99 & ilo_mjh==2
...
*          The COCR-2010 is the national classification which follows the ISCO-08 classification
...
* Comment: In Costa Rica there are not armed forces. 
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
		replace ilo_job1_ocu_isco08=ocupemppri if ilo_lfs==1
	    replace ilo_job1_ocu_isco08=11 if ocupemppri==10                         // Not elsewhere classified
   ...
   *---------------------------------------------------------------------------
	...
	* 1-digit level 				
	gen ilo_job2_ocu_isco08=.
		replace ilo_job2_ocu_isco08=ocupempsec if ilo_mjh==2 
	    replace ilo_job2_ocu_isco08=11 if ocupempsec==10 & ilo_mjh==2           // Not elsewhere classified
                * labels already defined for main job
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if estabili==10 & ilo_lfs==1             // Permanent
		replace ilo_job1_job_contract=2 if inrange(estabili,21,30) & ilo_lfs==1  // Temporary
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=1 if ilo_lfs==1 & ///
										 ...
										 (inlist(ilo_job1_ste_icse93,1,6) & social_security==. & e9c==6) | ///
										 (inlist(ilo_job1_ste_icse93,1,6) & social_security==. & e9c==5 & e9b==4) | ///
...
* Comment: the national threshold is in 40 hrs. 
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<=39  & c3==1 &  inlist(c4a,1,2) & ilo_lfs==1
...
* Comment: 
	...
	* 1-digit level
    gen ilo_preveco_isic4=.
		replace ilo_preveco_isic4=ramaultempcesant if ilo_lfs==2 & ilo_cat_une==1
	    replace ilo_preveco_isic4=22 if ilo_preveco_isic4==. & inlist(ramaultempcesant,.,99) & ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* Comment: 
	...
	* 1-digit level 				
	gen ilo_prevocu_isco08=.
		replace ilo_prevocu_isco08=ocupultempcesant if ilo_lfs==2 & ilo_cat_une==1 
	    replace ilo_prevocu_isco08=11 if inlist(ocupultempcesant,.,10) & ilo_lfs==2 & ilo_cat_une==1
                  * labels already defined for main job
