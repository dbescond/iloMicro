* TITLE OF DO FILE: ILO Microdata Preprocessing code template - DOMINICAN REPUBLIC, 2015Q1
* DATASET USED: DOMINICAN REPUBLIC, ENCFT, 2015Q1
* Files created: Standard variables DOM_ENCFT_2015Q1_FULL.dta and DOM_ENCFT_2015Q1_ILO.dta
...
global source "ENCFT"
global time "2015Q1"
global inputFile "Miembros_per_q115"
...
* Comment: 
	...
	gen ilo_wgt=.
	    replace ilo_wgt=factor_expansion 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only answered by those aged 3 years old or more and therefore the
*            rest are classified as not elsewhere classified. 
*          - Category "Quisqueya aprende" is classified as not elsewhere classified
*            given that the program covers several levels not specified here.
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if nivel_ultimo_ano_aprobado==9  | (nivel_ultimo_ano_aprobado==1 & inlist(ultimo_ano_aprobado,1,2))                                                                                                      // No schooling
		replace ilo_edu_isced11=2 if (nivel_ultimo_ano_aprobado==1 & ultimo_ano_aprobado==3) | (nivel_ultimo_ano_aprobado==2 & inrange(ultimo_ano_aprobado,1,5))                                                                           // Early childhood education
		replace ilo_edu_isced11=3 if (nivel_ultimo_ano_aprobado==2 & inrange(ultimo_ano_aprobado,6,7))                                                                                                                                     // Primary education
		replace ilo_edu_isced11=4 if (nivel_ultimo_ano_aprobado==2 & ultimo_ano_aprobado==8) | (inlist(nivel_ultimo_ano_aprobado,3,4) & inrange(ultimo_ano_aprobado,1,3))                                                                  // Lower secondary education
		replace ilo_edu_isced11=5 if (nivel_ultimo_ano_aprobado==3 & ultimo_ano_aprobado==4) | (nivel_ultimo_ano_aprobado==5 & inrange(ultimo_ano_aprobado,1,3))                                                                           // Upper secondary education
		replace ilo_edu_isced11=6 if nivel_ultimo_ano_aprobado==4 & ultimo_ano_aprobado==4                                                                                                                                                 // Post-secondary non-tertiary education
		* replace ilo_edu_isced11=7 if                                                                                                                                                                                                     // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if (nivel_ultimo_ano_aprobado==5 & inlist(ultimo_ano_aprobado,5,6)) |  (nivel_ultimo_ano_aprobado==6 & inrange(ultimo_ano_aprobado,1,4)) | (nivel_ultimo_ano_aprobado==7 & ultimo_ano_aprobado==1)       // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if (nivel_ultimo_ano_aprobado==7 & inrange(ultimo_ano_aprobado,2,4))                                                                                                                                     // Master's or equivalent level
		replace ilo_edu_isced11=10 if nivel_ultimo_ano_aprobado==8                                                                                                                                                                         // Doctoral or equivalent level
...
*          - People currently attending shchool, college, university or vocational training.
...
*            or less.
*          - Unemployment includes people not in employment, who were seeking for
*            a job during the last 4 weeks and were available during the past week.
*            It also includes available future starters. The final figure might 
*            differ from the one reported by the central bank due to the 
*            inclusion of the latter group (available future starters). 
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (realizo_trabajo_secundario==2) & ilo_lfs==1       // One job only     
		replace ilo_mjh=2 if (realizo_trabajo_secundario==1) & ilo_lfs==1       // More than one job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Empleado en empresa de zona franca y empleado en hogar privado are 
...
* Comment: 
	...
	* ISIC REV 4
	*---------------------------------------------------------------------------
	destring rama_principal_cod, replace force
	gen econ_act_prim=.
	    replace econ_act_prim=int(rama_principal_cod/100)
   ...
   * Aggregate level
	...
	* ISIC REV 4
	*---------------------------------------------------------------------------
	destring rama_secundaria_cod, replace force
	gen econ_act_sec=.
	    replace econ_act_sec=int(rama_secundaria_cod/100)
		
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = econ_act_sec if ilo_mjh==2
...
* Comment:
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring ocupacion_principal_cod, replace force
	gen occu_prim=.
	    replace occu_prim=int(ocupacion_principal_cod/100)	
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring ocupacion_secundaria_cod, replace force
	gen occu_secu=.
	    replace occu_secu=int(ocupacion_secundaria_cod/100)	
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(categoria_principal,1,2) & ilo_lfs==1    // Public
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if tipo_contrato==1 & ilo_lfs==1               // Permanent
		replace ilo_job1_job_contract=2 if inlist(tipo_contrato,2,3) & ilo_lfs==1      // Temporary
...
/* Useful questions:
		  ...
		  - Private household identification: categoria_principal==5 / ilo_job2_eco_isic4_2digits==97 / ilo_job2_ocu_isco08_2digits==63 (5.88%)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & ///
									   ...
									   (ilo_job1_eco_isic4_2digits==97 | ilo_job2_ocu_isco08_2digits==63))
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
									   ...
									   (inlist(categoria_principal,3,4,6,7,8,.) & registro_transacciones_empresa!=1 & (empresa_inscrita_rnc==1 | empresa_tiene_licencia==1)))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Cases reported in USD dolars are not ataken into account here.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Three criteria: want to work more hours, currently available to work
*            additional hours and worked less than a threshold. The threshold is 
*            set at 40 hours actually worked for those working in the public 
*            sector, and 44 for those working in the private sector. (note to 
*            table T4:1601).
...
* Comment:
	...
	gen ilo_cat_une=.
		...
		replace ilo_cat_une=2 if trabajo_antes==2 & ilo_lfs==2                  // Seeking for the first time
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Category 2 (1 to 3 months) includes category 3 (3 to 6 months) 
*            (note to value: C7:3906)
*          - Category 5 (12 to 24 months) includes category 6 (24 months or more)
...
* Comment: 
	...
	* 2-digit level
	destring rama_cesantia_cod, replace force
...
* Comment: 
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring ocupacion_cesantia_cod, replace force
...
cd "$outpath"
	...
	drop econ_act_prim econ_act_sec occu_prim occu_secu social_security monthly_earnings1
	drop monthly_earnings2 monthly_earnings3 monthly_earnings4 monthly_earnings5
	drop monthly_earnings6 monthly_earnings7
	compress
