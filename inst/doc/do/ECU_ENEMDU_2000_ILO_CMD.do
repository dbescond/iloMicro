* TITLE OF DO FILE: ILO Microdata Preprocessing code template - ECUADOR, 2000
* DATASET USED: ECUADRO, ENEMDU, 2000
* NOTES: 
* Files created: Standard variables ECU_ENEMDU_2000_FULL.dta and ECU_ENEMDU_2000_ILO.dta
...
global time "2000"
global inputFile "enemdu_2000.dta"
...
*
 ...
 gen ilo_edu_isced97=.
		...
		*replace ilo_edu_isced97=1 if nivinst==3 & inlist(anoinst,0,1,.)
		replace ilo_edu_isced97=1 if nivinst==2 & inlist(anoinst,0,1,.)
		*replace ilo_edu_isced97=2 if nivinst==3 & inrange(anoinst,2,8)
	    replace ilo_edu_isced97=2 if nivinst==3 & inlist(anoinst,0,1,2,.)
		replace ilo_edu_isced97=3 if nivinst==2 & inrange(anoinst,2,8)
	    replace ilo_edu_isced97=3 if nivinst==3 & inrange(anoinst,3,5)
	 	replace ilo_edu_isced97=4 if nivinst==3 & inrange(anoinst,6,8)
	    replace ilo_edu_isced97=4 if nivinst==4 & inlist(anoinst,0,1,2,.)
        replace ilo_edu_isced97=5 if nivinst==4 & inrange(anoinst,3,8)
        replace ilo_edu_isced97=5 if nivinst==4 & inrange(anoinst,3,8)
        replace ilo_edu_isced97=5 if nivinst==5 & inlist(anoinst,0,1,2,.)
       *replace ilo_edu_isced97=6 if nivinst==8 
		replace ilo_edu_isced97=7 if nivinst==5 & inrange(anoinst,3,8)
		replace ilo_edu_isced97=7 if nivinst==6 & inlist(anoinst,0,.)
		replace ilo_edu_isced97=8 if nivinst==6 & inrange(anoinst,1,8)
...
** Comment: there is no information available.
...
* Comment: 	Definition of unemployment doesn't follow international standards (i.e. people being actively
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (trabajo==1 | actayuda!=12) & ilo_wap==1                // 1 - Employed: ILO definition
	   replace ilo_lfs=1 if aunotra==1 & ilo_wap==1                                  // 1 - Employed: temporary absent

		
		
		replace ilo_lfs=2 if (bustrasa==1 | bustrama==1) & ilo_lfs!=1 & ilo_wap==1  // 2 - Unemployed: two criteria
...
* Comment: 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if numtrab==3
		replace ilo_mjh=2 if numtrab==4
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Categories are not defined correctly. 
/*
   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(catetrab,1,2,3,7,8,12) & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if inlist(catetrab,4,9) & ilo_lfs==1              // Employers
		 replace ilo_job1_ste_icse93=3 if inlist(catetrab,5,10)& ilo_lfs==1              // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1                        // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if inlist(catetrab,6,11) & ilo_lfs==1              // Contributing family workers
	...
	* SECOND JOB:
			* ICSE 1993
			  gen ilo_job2_ste_icse93=.
				  replace ilo_job2_ste_icse93=1 if inlist(cates,1,2,3,7,8,12) & ilo_mjh==2          // Employees
				  replace ilo_job2_ste_icse93=2 if inlist(cates,4,9) & ilo_mjh==2              // Employers
		          replace ilo_job2_ste_icse93=3 if inlist(cates,5,10)& ilo_mjh==2              // Own-account workers
		         *replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2                        // Members of producers' cooperatives
		          replace ilo_job2_ste_icse93=5 if inlist(cates,6,11) & ilo_mjh==2              // Contributing family workers
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: variable "rama" has a high number of observation unclassified. Therefore, this variable cannot be computed.
	...
	* 2-digit level
	/*
	gen indu_code_prim=int(rama/100) if ilo_lfs==1 & !inlist(rama,650,999,9999)
	...
	* 2-digit level
	...
	gen indu_code_sec=int(ramas/100) if ilo_lfs==1 & ilo_mjh==2 & !inlist(ramas,650,999,9999)
...
* ------------------------------------------------------------------------------
/*
* Comment: variable "grupo" has a high number of observation unclassified. Therefore, this variable cannot be computed.  
	...
	* Comment: Only 1-digit level classification can be possible. 
	
	* 1-digit level 
	
	gen occ_code_prim=int(grupo/1000) if ilo_lfs==1

		gen ilo_job1_ocu_isco88 = occ_code_prim if ilo_lfs==1
		    replace ilo_job1_ocu_isco88=10 if occ_code_prim==0 & ilo_lfs==1 //0 - Armed forces
			*replace ilo_job1_ocu_isco88=11 if inlist(occ_code_prim,88,99) & ilo_lfs==1
	...
	*---------------------------------------------------------------------------
	...
	gen occ_code_sec=int(grupos/1000) if  ilo_mjh==2
    ...
    * 1-digit level
	    ...
	    *replace ilo_job2_ocu_isco88=11 if inlist(ilo_job2_ocu_isco88_2digits,99,.) & ilo_mjh==2                          // Not elsewhere classified
		replace ilo_job2_ocu_isco88=occ_code_sec if (ilo_job2_ocu_isco88==. & ilo_mjh==2)     // The rest of the occupations
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: variable "catetrab" is not categorised correctly. 
...
** Comment: Information about hours ACTUALLY worked only for ALL JOBS.
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual = .
	    replace ilo_job2_how_usual = hortrahs if ilo_mjh==2 & hortrahs!=99
	...
	* Hours USUALLY worked
    egen ilo_joball_how_usual = rowtotal(hortrahp hortrahs hortraho) if hortrahp!=999 & hortrahs!=99 & hortraho!=99, m
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: These variables are not possible to compute because variable "catetrab" is not caterogised correctly, and because there is not enough information. 
...
*/
 ...
 /*
	* Social Security:
	gen social_security=.
	    replace social_security=1 if pe42e==1 & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if pe42e==2 & ilo_lfs==1          // no social security (proxy)
 ...
 Comment: 
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & // 3 - Household
		
		                               
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & pe50==1            // "2 - Formal"  
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & pe50!=1 & pe51==1 // "2 - Formal"
	    replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & pe50!=1 & pe51!=1 & ilo_job1_ste_icse93==1 & pe42e==1  // "2 - Formal"
	    replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & pe50!=1 & pe51!=1 & pe48==1 & (npertra>=6) // "2 - Formal"
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    ...
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & social_security==2 & pe42c==1 & pe42f==1 // 2 - Persons with formal main job
 ...
 ** Comment: these variables are not possible to compute because the variable ilo_job1_ste_aggregate cannot be defined. 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: variable tiembus is in months of searching.
 
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if tiembus==88 & ilo_lfs==2          // Less than 1 month
		replace ilo_dur_details=2 if inrange(tiembus,1,2) & ilo_lfs==2         // 1 to 3 months
		replace ilo_dur_details=3 if inrange(tiembus,3,5) & ilo_lfs==2        // 3 to 6 months
		replace ilo_dur_details=4 if inrange(tiembus,6,11) & ilo_lfs==2        // 6 to 12 months
		replace ilo_dur_details=5 if inrange(tiembus,12,23) & ilo_lfs==2        // 12 to 24 months
		replace ilo_dur_details=6 if tiembus>=24 & ilo_lfs==2 & tiembus!=99    // 24 months or more
...
* Comment: variable "rama" has a high number of observation unclassified. Therefore, this variable cannot be computed.
	...
	/*
	gen preveco_cod=int(rama/1000) if ilo_lfs==2 &   !inlist(rama,650,999,9999)

	* 1-digit level

		gen ilo_preveco_isic3_2digits = preveco_cod if ilo_lfs==2 &  ilo_cat_une==1
		    replace ilo_preveco_isic3_2digits=10 if preveco_cod==0 & ilo_lfs==2 & ilo_cat_une==1  //0 - Armed forces
...
* Comment: variable "grupo" has a high number of observation unclassified. Therefore, this variable cannot be computed.
	...
	*---------------------------------------------------------------------------
 	...
 	gen prevocu_cod=int(grupo/1000) if ilo_lfs==2
    ...
    * 1-digit level
	gen ilo_prevocu_isco88=.
	    *replace ilo_prevocu_isco88=11 if inlist(ilo_prevocu_isco88_2digits,99,.) & ilo_lfs==2 & ilo_cat_une==1                       // Not elsewhere classified
		replace ilo_prevocu_isco88=prevocu_cod if ilo_lfs==2 & ilo_cat_une==1       // The rest of the occupations
		replace ilo_prevocu_isco88=10 if prevocu_cod==0 & ilo_lfs==2 & ilo_cat_une==1                                      // Armed forces
                * labels already defined for main job
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(motnobus,4,5,6) & ilo_lfs==3    // Labour market 
			replace ilo_olf_reason=2 if inlist(motnobus,7,8) & ilo_lfs==3   // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(motnobus,2,3) & ilo_lfs==3    // Personal/Family-related
			replace ilo_olf_reason=4 if motnobus==1 & ilo_lfs==3                // Does not need/want to work
