...
global time "2001"
global inputFile "enemdu_2001.dta"
...
*
 ...
 gen ilo_edu_isced97=.
		...
		replace ilo_edu_isced97=2 if nivinst==3 & inrange(anoinst,2,9)
	    replace ilo_edu_isced97=2 if nivinst==4 & inlist(anoinst,0,1,2,.)
		replace ilo_edu_isced97=3 if nivinst==2 & inrange(anoinst,2,9)
	    replace ilo_edu_isced97=3 if nivinst==4 & inrange(anoinst,3,5)
	 	replace ilo_edu_isced97=4 if nivinst==4 & inrange(anoinst,6,9)
	    replace ilo_edu_isced97=4 if nivinst==5 & inlist(anoinst,0,1,2,.)
        replace ilo_edu_isced97=5 if nivinst==5 & inrange(anoinst,3,9)
        replace ilo_edu_isced97=5 if nivinst==5 & inrange(anoinst,3,9)
        replace ilo_edu_isced97=5 if nivinst==6 & inlist(anoinst,0,1,2,.)
       *replace ilo_edu_isced97=6 if nivinst==8 
		replace ilo_edu_isced97=7 if nivinst==6 & inrange(anoinst,3,9)
		replace ilo_edu_isced97=7 if nivinst==7 & inlist(anoinst,0,.)
		replace ilo_edu_isced97=8 if nivinst==7 & inrange(anoinst,1,9)
	...
	* looking for a job (pe30a!=9) are not being asked the question whether these are being available for work (deseatra==1))
	...
	gen ilo_lfs=.
	   ...
	   replace ilo_lfs=1 if aunotra==1 & ilo_wap==1                                  // 1 - Employed: temporary absent
		...
		replace ilo_lfs=2 if (bustrasa==1 | bustrama==1) & !inlist(pe30a,0,9,99) & ilo_lfs!=1 & ilo_wap==1  // 2 - Unemployed: two criteria
...
* Comment: 
	...
	* 2-digit level
	...
	gen indu_code_prim=int(rama/100) if ilo_lfs==1 & !inlist(rama,0,1,2,6,420,650,5620,8234,999,9999)
	...
	* 2-digit level
	...
	gen indu_code_sec=int(ramas/100) if ilo_lfs==1 & ilo_mjh==2 & !inlist(ramas,0,1,2,6,420,650,5620,8234,999,9999)
...
* Comment:
	...
	* 1-digit level 
	...
	gen occ_code_prim=int(grupo/1000) if ilo_lfs==1
		...
		gen ilo_job1_ocu_isco88 = occ_code_prim if ilo_lfs==1
			...
			replace ilo_job1_ocu_isco88=11 if occ_code_prim==. & ilo_lfs==1
    ...
    * 1-digit level
	    ...
	    *replace ilo_job2_ocu_isco88=11 if inlist(ilo_job2_ocu_isco88_2digits,99,.) & ilo_mjh==2                          // Not elsewhere classified
		...
		replace ilo_job2_ocu_isco88=11 if occ_code_sec==. & ilo_mjh==2
...
** Comment: Information about hours ACTUALLY worked only for ALL JOBS.
	...
	* Hours USUALLY worked
	...
	gen hours_usu_sj = hortrahs if hortrahs!=99  // usually hours second job
	gen hours_usu_oj = hortraho if hortraho!=99  // usually hours all jobs
...
*/
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & catetrab==12 // 3 - Household
	    ...
	    replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & pe50!=1 & pe51!=1 & pe48==1 & (pertrabn==2 | npertra>=6)  // "2 - Formal"
...
* ------------------------------------------------------------------------------
	...
	* Employees
	egen ilo_job1_lri_ees = rowtotal(pe60 pe62a) if  ilo_job1_ste_aggregate==1 & !inlist(pe60,9999,99999,999999) & !inlist(pe62a,9999,99999,999999),m 
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = pe58 if  ilo_job1_ste_aggregate==2 & !inlist(pe58,9999,99999,999999)   
	...
	* Employees
	gen ilo_job2_lri_ees = pe63 if  ilo_job2_ste_aggregate==1 &  !inlist(pe63,9999,99999,999999)  
		        lab var ilo_job2_lri_ees "Monthly earnings of employees - second job"
		
    * Self-employed		
	gen ilo_job2_lri_slf = pe63 if  ilo_job2_ste_aggregate==1 &  !inlist(pe63,9999,99999,999999)  
...
*
	* Detailed categories		
    gen ilo_dur_details=.
		...
		replace ilo_dur_details=6 if tiembus>=96 & ilo_lfs==2 & tiembus!=999    // 24 months or more
...
* ------------------------------------------------------------------------------
	...
	*---------------------------------------------------------------------------
 	...
 	gen preveco_cod=int(rama/100) if ilo_lfs==2 & !inlist(rama,0,1,2,6,420,650,5620,8234,999,9999)
...
* Comment: 
    ...
    * 1-digit level
	gen ilo_prevocu_isco88=.
		...
		replace ilo_prevocu_isco88=11 if prevocu_cod==. & ilo_lfs==2 & ilo_cat_une==1                                       // Not elsewhere classified
