...
* NOTES: 
* Files created: Standard variables on LFS Ecuador
* Authors: Podjanin
...
global source "LFS"
global time "2016Q2"
global inputFile "201606_EnemduBDD"
...
* Comment: 	Definition of unemployment doesn't follow international standards (i.e. people being actively
	* looking for a job are not being asked the question whether these are being available for work)
		...
		* Among unemployed also including people having said that they were not actively looking for a job, but were available to work and:
				* a) would start a job immediately
				* b) is waiting for a response regarding an own business
				* c) is waiting for a response regarding a job	
 
	gen ilo_lfs=.
		replace ilo_lfs=1 if p20==1 | inrange(p21,1,11) | p22==1
		replace ilo_lfs=2 if inrange(p32,1,10) | (inrange(p34,2,4) & p35==1) 
...
* Comment: ISIC Rev. 4 being used and initially indicated on 4-digits level --> keep only 2 digits level
	...
	* economic activity indicated both for primary and secondary activity --> capture it for both
	...
	gen indu_code_sec=int(p52/100) if ilo_lfs==1 
...
* Comment: Indicate both for primary and secondary activity
	...
	* 1 digit level
	...
	gen occ_code_sec_1dig=int(occ_code_sec/10) if ilo_lfs==1
...
* Comment: 
	...
	* Useful questions: p42: Status in employment --> indicates whether person is working in the public sector, as well as domestic employees
	...
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=3 if p42==10 | ilo_job1_ocu_isco08_2digits==63 | ilo_job1_eco_isic4_2digits==97
...
* Comment: 
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(p34,5,6,7)
		replace ilo_olf_reason=2 if inlist(p34,9,10,11,12)
		replace ilo_olf_reason=3 if inlist(p34,1,8)
...
cd "$outpath"
		...
		drop nat_* social_secur_prim business_expend indu_code_* occ_code_* prevocu_cod preveco_cod lab isco08_2digits isco88_2digits isco08 isco88 isic4_2digits isic4 isic3_2digits isic3 job_earn_*
