...
global source "LFS"
global time "2010"
global inputFile "LFSS_2010.dta"
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Given that there's no weights associated to observations without an answer to the
*            question "relation to the household head", these observations are not considered
*            for the analysis.
...
* Comment:
	...
	* ISCED97
    gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if inlist(educ_st,1,2)                        // No schooling
		replace ilo_edu_isced97=2 if educ_st==3                                 // Pre-primary
		replace ilo_edu_isced97=3 if educ_st==4                                 // Primary education
		* replace ilo_edu_isced97=4 if                                          // Lower secondary
		replace ilo_edu_isced97=5 if inlist(educ_st,5,6)                        // Upper secondary
		replace ilo_edu_isced97=6 if educ_st==7                                 // Post-secondary non-tertiary
		replace ilo_edu_isced97=7 if educ_st==8                                 // First stage of tertiary
		replace ilo_edu_isced97=8 if educ_st==9                                 // Second stage of tertiary
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Employment comprises people engaged in work at least for one hour during the past
*            week and those who answered yes to one of the recovery questions in 119 (activities
*            to earn money).
*          - Unemployment includes those satisfying ILO's three criteria and future job starters.
*            Note that only people answering that they were willing to work are asked whether 
*            they're available to work. All questions asking about any job-seeking activity take
*            as reference period the last three months. Given that no other questions capturing 
*            the same information and covering a shorter time period is available, the latter 
*            are being used in order to identify people in unemployment.
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (crwrkm==2 | job_ref_week==1) & ilo_wap==1                                                                      // Employed
		replace ilo_lfs=2 if (ilo_lfs!=1 & dsr_to_wrk==1 & ready_to_wrk==1 & (inlist(srch,1,2) | seek_job==1 | rsn_ntsrch==1)) & ilo_wap==1  // Unemployed
...
* Comment: 
	...
	gen ilo_mjh=.
	    replace ilo_mjh=2 if scoccup==1 & ilo_lfs==1
...
* Comment: 
	...
	* Primary activity
	gen ilo_job1_ste_icse93=.
		replace ilo_job1_ste_icse93=1 if crempst==1 & ilo_lfs==1                // Employees
		replace ilo_job1_ste_icse93=2 if crempst==2 & ilo_lfs==1                // Employers
		replace ilo_job1_ste_icse93=3 if crempst==3 & ilo_lfs==1                // Own-account workers
		* replace ilo_job1_ste_icse93=4 if                                      // Members of producers' cooperatives
		replace ilo_job1_ste_icse93=5 if crempst==4 & ilo_lfs==1                // Contributing family workers
		replace ilo_job1_ste_icse93=6 if ilo_job1_ste_icse93==. & ilo_lfs==1    // Workers not classifiable by status
				label define label_ilo_ste_icse93 1 "1 - Employees" 2 "2 - Employers" 3 "3 - Own-account workers" ///
...
* Comment: 
	...
	gen indu_code_prim=int(crecact/10000)
...
* Comment:  ISCO-08 classification used
	...
	gen occu_code_prim=int(croccup/10000)
	
	* 2-digit level	
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = occu_code_prim if ilo_lfs==1
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(crsector,1,2) & ilo_lfs==1
...
* --------------------------------------------------------------------------------------------------
* --------------------------------------------------------------------------------------------------
* Comment: - No hours usually worked available.

	* hours ACTUALLY worked
	* Primary job
	gen ilo_job1_how_actual = crwrkhrs if ilo_lfs==1
	...
	* Secondary job
	gen ilo_job2_how_actual = scoccup_hr if ilo_mjh==2
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
* Comment: - The threshold is set at 40 hours per week (legal working-time). Given that there's
*            no information on the usual hours of work, actual hours of work are being used.
	
		gen ilo_job1_job_time=.
			replace ilo_job1_job_time=1 if ilo_joball_how_actual<=39 & ilo_lfs==1
			replace ilo_job1_job_time=2 if ilo_joball_how_actual>39 & ilo_joball_how_actual!=. & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
* Comment: - "Sometimes" is classified under temporary contract.
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds): crsocins (social insurance)
		  - Place of work: wrkplace_typ
		  - Size: employees_no
...
*/									
	...
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod = 2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                                 ((inlist(crsector,1,2)) | ///
										 (!inlist(crsector,1,2) & crsocins==1))
...
* Comment: 
	...
	gen ilo_job1_ife_nature=.
		replace ilo_job1_ife_nature=1 if (ilo_job1_ste_icse93==5 | (inlist(ilo_job1_ste_icse93,2,3,4) & inlist(ilo_job1_ife_prod,1,3)) | (inlist(ilo_job1_ste_icse93,1,6) & crsocins!=1)) & ilo_lfs==1
		replace ilo_job1_ife_nature=2 if ((inlist(ilo_job1_ste_icse93,1,6) & crsocins==1) | (inlist(ilo_job1_ste_icse93,2,3,4) & ilo_job1_ife_prod==2)) & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Permanent and temporary workers (crstabl) are answering to questions on period
*            covered by wages (wg_basic_prd) and amount (wg_basic). Seasonal and casual workers 
*            answer directly to a separate questions regarding average daily wages (wg_avdaily).
*          - To convert daily, weekly and 2-weekly income into monthly income, the following 
*            factors were used:
...
*			 - monthly income = yearly income/12		
	...
	* Employees:
	gen cash_month=.
	    replace cash_month = wg_basic*(365/12) if ilo_job1_ste_aggregate==1 & wg_basic_prd==1           // day
		replace cash_month = wg_basic*((365/7)/12) if ilo_job1_ste_aggregate==1 & wg_basic_prd==2       // week
		replace cash_month = wg_basic if ilo_job1_ste_aggregate==1 & wg_basic_prd==3                    // month
		replace cash_month = wg_basic/3 if ilo_job1_ste_aggregate==1 & wg_basic_prd==4                  // 3 months
		replace cash_month = wg_basic/12 if ilo_job1_ste_aggregate==1 & wg_basic_prd==5                 // year
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if unemp_typ==1 & ilo_lfs==2                      // Previously employed       
		replace ilo_cat_une=2 if unemp_typ==2 & ilo_lfs==2                      // Seeking for the first time
...
* Comment: - ISIC Rev. 4
    ...
    gen preveco_cod = int(unemp_lseco/10000)

	* 2-digit level
	gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = preveco_cod   if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* -------------------------------------------------------------------------------------------
* Comment: - ISCO-08
*          - Zero values correspond to missing/not responded (no armed forces).
	...
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits = . 
	    replace ilo_prevocu_isco08_2digits = int(unemp_lst_occ/10000) if ilo_lfs==2 & ilo_cat_une==1 & unemp_lst_occ!=0
                * labels already defined for main job
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Not feasible due to the skip patterns (only persons ready to work are answering 
*            questions on seeking work).
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - 2 variables are used here: rsn_ntdsr_wrk (reason for not willing to work) and 
*          rsn_ntsrch (reason for not searching)
...
cd "$outpath"
	...
	drop job_ref_week seek_job indu_code_prim occu_code_prim cash_month preveco_cod
	compress
