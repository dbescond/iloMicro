* TITLE OF DO FILE: ILO Microdata Preprocessing code template - EGYPT, 2012
* DATASET USED: Egypt, SWTS, 2012
* NOTES: 
* Files created: Standard variables EGY_SWTS_2012_FULL.dta and EGY_SWTS_2012_ILO.dta
...
global source "SWTS"
global time "2012" 
global inputFile "EGY_SWTS_2012.dta"
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if a504==1 | currently_attend==3                       // No schooling
		replace ilo_edu_isced97=2 if a504==2                          // Pre-primary education
		replace ilo_edu_isced97=3 if a504==3                          // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if inlist(a504,4,5)                          // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(a504,6,7)                           // Upper secondary education
		replace ilo_edu_isced97=6 if a504==8                          // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if a504==9                          // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if a504==10                          // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		...
		replace ilo_edu_attendance=2 if   currently_attend==2                     // Not attending
...
* ------------------------------------------------------------------------------ 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if inlist(marital,1,2)                                          // Single
		replace ilo_mrts_details=2 if marital==4                                          // Married
		replace ilo_mrts_details=3 if marital==3                                          // Union / cohabiting
		replace ilo_mrts_details=4 if marital==6                                          // Widowed
		replace ilo_mrts_details=5 if inlist(marital,5,7)                                          // Divorced / separated
...
* ------------------------------------------------------------------------------
    ...
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (a301==1 & a302==1 & a303==1 & a304==1 )                            
             replace ilo_dsb_details=2 if (a301==2 | a302==2 | a303==2 | a304==2 )                                                 
             replace ilo_dsb_details=3 if (a301==3 | a302==3 | a303==3 | a304==3 )                                                 
             replace ilo_dsb_details=4 if (a301==4 | a302==4 | a303==4 | a304==4 )
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 ...
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if statusinemp==4 & ilo_lfs==1          // Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = code_ind_main      if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = isco_occ_main    if ilo_lfs==1
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   register_actv==1 | ///
									   (inlist(register_actv,4) & statusinemp==1 & social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((statusinemp==1 & social_security==1) | ///
										 (statusinemp==1 & inlist(benefit_servj,3,.)& benefit_servd==1 & benefit_servc==1) | ///
										 (inlist(statusinemp,2,3,4)&ilo_job1_ife_prod==2))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Average wages per class as questionnaire only asks ranges of wages and not exact amount. 
	...
	* Employees
	gen ilo_job1_lri_ees = .
		replace ilo_job1_lri_ees = 300 if a913==1 & ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = (499+300)/2 if a913==2 & ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = (999+500)/2 if a913==3 & ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = (2999+1000)/2 if a913==4 & ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = 3000 if a913==5 & ilo_job1_ste_aggregate==1
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = 1000 if a920==1 & ilo_job1_ste_aggregate==2
		replace ilo_job1_lri_slf = (1000+1999)/2 if a920==2 & ilo_job1_ste_aggregate==2
		replace ilo_job1_lri_slf = (2000+4999)/2 if a920==3 & ilo_job1_ste_aggregate==2
		replace ilo_job1_lri_slf = 5000 if a920==4 & ilo_job1_ste_aggregate==2
...
* ------------------------------------------------------------------------------ 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(a1012,1,2) & ilo_lfs==2             // Less than 1 month
		replace ilo_dur_details=2 if a1012==3  	   & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if  a1012==4     & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if  a1012==5     & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if  a1012==6     & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if  a1012==7     & ilo_lfs==2                 // 24 months or more
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=5 if ((reason_notseeking==99& !reason_notavailable==6) | ilo_olf_reason==. )& ilo_lfs==3     // Not elsewhere classified
