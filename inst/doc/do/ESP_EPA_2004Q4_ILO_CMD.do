...
global source "EPA"
global time "2004Q4"
global inputFile "datos_4t04.dta"
...
gen ilo_edu_aggregate = .
       ...
       replace ilo_edu_aggregate = 1 if inlist(ESTUD_NFORM_1,"AN","P1")		   // Less than basic
		replace ilo_edu_aggregate = 2 if inlist(ESTUD_NFORM_1,"P2","S1")	  // Basic
		replace ilo_edu_aggregate = 3 if inlist(ESTUD_NFORM_1,"SG","SP")      // Intermediate
		replace ilo_edu_aggregate = 4 if ESTUD_NFORM_1=="SU"                 // Advanced
		replace ilo_edu_aggregate = 5 if ESTUD_NFORM_1== ""				     // Level not stated
...
** Comments: - the EPA 1999-2004 has one variable for indicating current enrolment >> CURSR - it refers to official studies 
	...
	gen ilo_edu_attendance = .
		replace ilo_edu_attendance = 1 if inlist(CURSR,1,2)                          // 1 - Attending
		replace ilo_edu_attendance = 2 if CURSR== 3                                 //  2 - Not attending
...
** Comment: - The definition of unemployed includes people who are seeking a job using active methods. There was a change in the 
** interpretation of active search methods in the methodology of 2002.

** Comment: - The definition of unemployed in this code does not include availability. Therefore the results for 2002Q1-2004Q4
**				differ from the number of unemployed people published by INE and the obtained from the microdata of the EULFS. 
** - Comment: The suggestion is that there must be a problem with the variable DISP (availability), because there is not cases where the individual is available and seeking
** - Comment: This code overestimate the final result of the total number of unemployed people. 
...
gen ilo_lfs =.
		...
		** EMPLOYED
		...
		replace ilo_lfs = 1 if AUSENT == 1 & ilo_wap == 1               //Employed (absent)
		
		
				
		** UNEMPLOYED
		
		*** Comment: - Note that AVAILABILITY is not included!
		
		replace ilo_lfs = 2 if ilo_lfs != 1 & (BUSCA == 1 & FOBACT  == 1) & ilo_wap == 1  // Not in employment, seeking (actively)
		replace ilo_lfs = 2 if ilo_lfs != 1 &  NUEVEM == 1 & ilo_wap == 1                 // Not in employment, future job starters
		
	
		
		** OUTSIDE LABOUR FORCE
		replace ilo_lfs = 3 if ilo_lfs != 1   & ilo_lfs != 2 & ilo_wap == 1 
...
* Detailed categories:
		...
		gen ilo_job1_ste_icse93 = .
			...
			replace ilo_job1_ste_icse93 = 2 if inlist(SITU,1,2) & ilo_lfs==1	            // Employers
			
			replace ilo_job1_ste_icse93 = 3 if inlist(SITU,3,4) & ilo_lfs==1      			// Own-account workers
...
* Detailed categories:
		...
		gen ilo_job2_ste_icse93 = .
			...
			replace ilo_job2_ste_icse93 = 2 if inlist(SITPLU,1,2) & ilo_mjh == 2 	            // Employers
...
* Comment: - the national economic activity classification is CNAE
		...
		*	- the variable ilo_job_eco_isic at a disaggregated level.
...
destring ACT1, replace
		...
		* ilo_job1_eco_isic3    ***** (not possible)
	...
	* Classification on an aggregate level
		...
		* Primary activity
		...
		gen ilo_job1_eco_aggregate = .
			replace ilo_job1_eco_aggregate = 1 if ACT1 == 0 & ilo_lfs == 1				// 1 - Agriculture
			replace ilo_job1_eco_aggregate = 2 if inlist(ACT1,1,3) & ilo_lfs == 1			// 2 - Manufacturing
			replace ilo_job1_eco_aggregate = 3 if ACT1 == 4 & ilo_lfs == 1					// 3 - Construction
			replace ilo_job1_eco_aggregate = 4 if ACT1 == 2 & ilo_lfs == 1	           	// 4 - Mining and quarrying; Electricity, gas and water supply
			replace ilo_job1_eco_aggregate = 5 if inrange(ACT1,5,7) & ilo_lfs == 1	    // 5 - Market Services (Trade; Transportation; Accommodation and food; and Business and administrative services)
			replace ilo_job1_eco_aggregate = 6 if inlist(ACT1,8,9) & ilo_lfs == 1		 	// 6 - Non-market services (Public administration; Community, social and other services and activities)
			replace ilo_job1_eco_aggregate = 7 if ACT1 == . & ilo_lfs == 1		    // 7 - Not classifiable by economic activity
...
* Comment: - the national occupation classification used until 2010Q4 is CNO is CNO-94

		* 	- CNO-94 has the same major groups as ISCO-88
...
destring PARCOM, replace

		gen ilo_job1_job_time = .
		replace ilo_job1_job_time = 1 if inrange(PARCOM,2,8) 								// 1 - Part-time
		replace ilo_job1_job_time = 2 if PARCOM  == 1								// 2 - Full-time
...
destring HORASE, replace
   ...
   * ilo_job1_how_actual
		...
		gen ilo_job1_how_actual = HORASE if ilo_lfs == 1
...
destring HORPLU, replace
		...
		gen ilo_job2_how_actual = HORPLU if ilo_mjh==2
...
destring DUCONT, replace

gen ilo_job1_job_contract = .
		replace ilo_job1_job_contract = 1 if inlist(DUCONT,1,2)								// 1 - Permanent
		replace ilo_job1_job_contract = 2 if inrange(DUCONT,3,9)									// 2 - Temporary
...
destring OCUPA, replace
	...
	* 1 digit level  
	...
	gen ilo_prevocu_isco88 = .
		...
		replace ilo_prevocu_isco88 = 1 if OCUPA == 1 & ilo_cat_une == 1  // 1 - Legislators, senior officials and managers
		replace ilo_prevocu_isco88 = 2 if OCUPA == 2 & ilo_cat_une == 1   // 2 - Professionals
		replace ilo_prevocu_isco88 = 3 if OCUPA == 3 & ilo_cat_une == 1   // 3 - Technicians and Associate Professionals
		replace ilo_prevocu_isco88 = 4 if OCUPA == 4 & ilo_cat_une == 1   // 4 - Clerks
		replace ilo_prevocu_isco88 = 5 if OCUPA == 5 & ilo_cat_une == 1   // 5 - Service workers and shop and market sales workers
		replace ilo_prevocu_isco88 = 6 if OCUPA == 6 & ilo_cat_une == 1   // 6 - Skilled agricultural and fishery workers
		replace ilo_prevocu_isco88 = 7 if OCUPA == 7 & ilo_cat_une == 1   // 7 - Craft and Related Trades Workers
		replace ilo_prevocu_isco88 = 8 if OCUPA == 8 & ilo_cat_une == 1   // 8 - Plant and machine operators and assemblers
		replace ilo_prevocu_isco88 = 9 if OCUPA == 9 & ilo_cat_une == 1   // 9 - Elementary Occupations
		replace ilo_prevocu_isco88 = 10 if OCUPA == 0 & ilo_cat_une == 1  // 0 - Armed forces
		replace ilo_prevocu_isco88 = 11 if OCUPA == . & ilo_cat_une == 1  // 11 - Not elsewhere classified

		lab def ilo_prevocu_isco88  1 "1 - Managers" 2 "2 - Professionals" 3 "3 - Technicians and Associate Professionals" 4 "4 - lerical Support Workers" ///
									5 "5 - Services and Sales Workers"  ///
									6 "6 - Skilled Agricultural, Forestry and Fishery Workers" 8 "8 - Plant and Machine Operators and Assemblers" 9 "9 - Elementary Occupations" ///
									10 "0 - Armed forces occupations" 11 "11 - Not elsewhere classified" 		
	...
	* Aggregate
	  gen ilo_prevocu_aggregate = .
		  replace ilo_prevocu_aggregate = 1 if inrange(ilo_prevocu_isco88,1,3)  // 1 - Managers, professionals, and technicians
		  replace ilo_prevocu_aggregate = 2 if inlist(ilo_prevocu_isco88,4,5)	// 2 - Clerical, service and sales workers
		  replace ilo_prevocu_aggregate = 3 if inlist(ilo_prevocu_isco88,6,7)	// 3 - Skilled agricultural and trades workers
		  replace ilo_prevocu_aggregate = 4 if ilo_prevocu_isco88 == 8			// 4 - Plant and machine operators, and assemblers
		  replace ilo_prevocu_aggregate = 5 if ilo_prevocu_isco88 == 9			// 5 - Elementary occupations
		  replace ilo_prevocu_aggregate = 6 if ilo_prevocu_isco88 == 10		// 6 - Armed forces
		  replace ilo_prevocu_aggregate = 7 if ilo_prevocu_isco88 == 11		// 7 - Not elsewhere classified
				  *lab def ocu_aggr_lab 1 "1 - Managers, professionals, and technicians" 2 "2 - Clerical, service and sales workers" 3 "3 - Skilled agricultural and trades workers" ///
									  * 4 "4 - Plant and machine operators, and assemblers" 5 "5 - Elementary occupations" 6 "6 - Armed forces" 7 "7 - Not elsewhere classified"
...
destring DISP, replace
		...
		gen ilo_olf_dlma = .
		replace ilo_olf_dlma = 1 if ilo_lfs == 3 & (BUSCA == 1 & FOBACT == 1  ) & DISP != 1			        // Seeking (actively), not available
		replace ilo_olf_dlma = 2 if ilo_lfs == 3 & BUSCA != 1 & DISP == 1			                            // Not seeking, available
		replace ilo_olf_dlma = 3 if ilo_lfs == 3 & BUSCA == 2 & DISP != 1	                    // Not seeking, not available, willing  
		replace ilo_olf_dlma = 4 if ilo_lfs == 3 & BUSCA == 3 & DISP != 1	                // Not seeking, not available, not willing  
...
*** Comment: waiting for running their own business??

gen ilo_olf_reason = .
			replace ilo_olf_reason = 1 if inrange(NBUSCA,1,6) & ilo_lfs==3	  // Labour market
			replace ilo_olf_reason = 1 if inrange(NBUSCA,7,9) & ilo_lfs==3                                    // Other labour market reasons
			replace ilo_olf_reason = 3 if inrange(NBUSCA,10,12) & ilo_lfs==3    // Personal/Family-related
			replace ilo_olf_reason=4 if inlist(NBUSCA,13,14) & ilo_lfs==3		      // Does not need/want to work
			replace ilo_olf_reason=5 if NBUSCA == 15 & ilo_lfs==3	      // Not elsewhere classified
...
gen ilo_dis = .
		replace ilo_dis=1 if inlist(BUSCA,2,3) & NBUSCA == 1   & ilo_lfs ==3
