...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_geo=.
			replace ilo_geo=1 if inrange(lf04,1,7) 
			replace ilo_geo=2 if lf04==8
...
** doubt people in employment
	gen ilo_lfs=.
		replace ilo_lfs=1 if lf30==1 & lf32==1 & ilo_wap==1 // employed (current work status) -- 4hours or more worked
		replace ilo_lfs=1 if lf34==1 &  ilo_wap==1           // employed (absent) -- I GUESS (because lf34 is almost the same no. of observations)
 		
 		replace ilo_lfs=2 if ilo_lfs!=1 & (lf46==1 & lf49==1) & ilo_wap==1  // unemployed (those who looked for job last weekend were available)	
		replace ilo_lfs=2 if ilo_lfs!=1 &  inlist(lf48,6,9) & ilo_wap==1  // Future job starters 
...
** Comment: according to the questionnaire, it seems that question lf41 refers to multiple job holders, 
**          however, in the microdata set, the only variables related to another job refer to looking for an additional job (lf44 & lf45). 
...
* ------------------------------------------------------------------------------------------- 
	...
	* MAIN JOB
		...
		gen ilo_job1_ste_icse93=.
			...
			replace ilo_job1_ste_icse93=5 if lf38==7 // 5 - Contributing family workers 
...
* ISIC Rev. 3.1
	...
	* MAIN JOB:
 	...
 	* 2-digit level	
	gen ilo_job1_eco_isic3_2digits = int(lf36/10) if ilo_lfs == 1
...
** Comment:  ISCO-88 classification used
	...
	gen occ_code_prim=. if ilo_lfs == 1 
	    replace occ_code_prim=int(lf35/10) if ilo_lfs == 1 & !inlist(lf35,2,20,999)
		replace occ_code_prim=51 if lf35==542
...
* -------------------------------------------------------------------------------------------
	...
	* Primary occupation
	 ...
	 gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(lf38,1,2,4)
...
gen ilo_job1_job_contract=.
		...
		replace ilo_job1_job_contract = 1 if  lf37 == 1  & ilo_job1_ste_aggregate==1 // 1 - Permanent
...
* Comment: Useful questions:	* Status in employment: lf38
								* Institutional sector: lf38 
								* Destination of production: [no info]	
								* Bookkeeping: lf39	
								* Registration: lf41		
								* Location of workplace: [no info]		 
								* Size of the establishment: lf40			
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_dur_details=.
				replace ilo_dur_details=1 if (lf55==0 & ilo_lfs==2)                        // Less than 1 month
				replace ilo_dur_details=2 if (inlist(lf55,1,2) & ilo_lfs==2)               // 1 to 3 months
				replace ilo_dur_details=3 if (inrange(lf55,3,5) & ilo_lfs==2)              // 3 to 6 months
				replace ilo_dur_details=4 if (inrange(lf55,6,11) & ilo_lfs==2)             // 6 to 12 months
				replace ilo_dur_details=5 if (inrange(lf55,12,23) & ilo_lfs==2)            // 12 to 24 months
				replace ilo_dur_details=6 if (lf55>=24 & ilo_lfs==2)                       // 24 months or more
	...
	gen ilo_dur_aggregate=.
		...
		replace ilo_dur_aggregate=3 if (inrange(ilo_dur_details,5,6) & ilo_lfs==2)                 // 12 months or more
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if lf56==1 & ilo_lfs==2 // 1 - Unemployed previously employed
		replace ilo_cat_une=2 if lf56==2 & lf46==1 & ilo_lfs==2  // 2 - Unemployed seeking their first job
...
*** Comment: thre is no information on willigness available. 
		...
		replace ilo_olf_dlma = 1 if ilo_lfs == 3 & lf46==1 & lf49==2 		        // Seeking (actively), not available
		replace ilo_olf_dlma = 2 if ilo_lfs == 3 & lf46==2 & lf49==1	            // Not seeking, available
...
gen ilo_olf_reason = .
			replace ilo_olf_reason = 1 if (inlist(lf48,8,10) & ilo_lfs==3)   		// Labour market
			replace ilo_olf_reason = 2 if  lf48==7 & ilo_lfs==3      				   // Other labour market reasons
			replace ilo_olf_reason = 3 if ((inrange(lf48,1,3) | lf48==5) & ilo_lfs==3)    // Personal/Family-related
			replace ilo_olf_reason = 4 if (lf48==4 & ilo_lfs==3)     // Does not need/want to work
			replace ilo_olf_reason = 5 if (inlist(lf48,11,91,99) & ilo_lfs==3)	      	   	// Not elsewhere classified
...
* ------------------------------------------------------------------------------------------- 
	...
	gen ilo_dis = 1 if ilo_lfs==3 & lf49==1 & ilo_olf_reason==1
