...
global inputFile "FRA_EE_2012.dta" /*name of the input file in stata format*/
...
* Comment: 
	gen ilo_geo=.
	    replace ilo_geo=1 if  inlist(pol10,1,2)        // Urban 
		replace ilo_geo=2 if   inlist(pol10,3,4)           // Rural 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: No enough information to distinguish Master and Doctoral level. 
*		dip11==71 is "sans diplôme", too high share 18%?
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		*replace ilo_edu_isced11=1 if dip11==                       // No schooling
		replace ilo_edu_isced11=2 if  dip11==71                       // Early childhood education
		replace ilo_edu_isced11=3 if  dip11==70                     // Primary education
		replace ilo_edu_isced11=4 if  dip11==60                    // Lower secondary education
		replace ilo_edu_isced11=5 if   dip11==50                    // Upper secondary education
		replace ilo_edu_isced11=6 if  inlist(dip11,41,42)                    // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if  inlist(dip11,30,31,33)                  // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if  dip11==11                     // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if dip11==10                      // Master's or equivalent level
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: no information on disability status
...
* Comment: 
	...
	* 2-digit level
	capture confirm variable  nafg088un
	if !_rc {
   ...
   gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = nafg88un     if ilo_lfs==1 
		}
...
/*	
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
	    replace ilo_job1_ocu_isco08=1 if inlist(cse,23,33,37) & ilo_lfs==1   
	    replace ilo_job1_ocu_isco08=2 if inlist(cse,31,43,52,45,46) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=3 if inlist(cse,34,38,35,42,47,54) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=4 if inlist(cse,44) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=5 if inlist(cse,22,55,56) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=6 if inlist(cse,10,11,12,13,69) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=7 if inlist(cse,21,62,63) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=8 if inlist(cse,48,64,65) & ilo_lfs==1                          
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = hhce      if ilo_lfs==1
...
* Comment: threshold set at 35 hours
	...
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if ilo_job1_how_usual>=35  & ilo_lfs==1                             // Full-time
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		...
		replace ilo_job1_job_contract=2 if inlist(contra,2,3,4,5) & ilo_lfs==1               				// Temporary
...
*		   question dispplc asks whether the individual wants to work more knowing is available to do so

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_usual<= 35 & dispplc==1 & ilo_lfs==1
...
* Comment: 
	...
	* 2-digit level
	capture confirm variable nafant
	if !_rc {
	gen ilo_preveco_isic4_2digits = . 
		...
		gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = nafan   if ilo_lfs==2 & ilo_cat_une==1
	}
...
* Comment: 
	...
	* 1-digit level 				
	gen ilo_prevocu_isco08=.
	    replace ilo_prevocu_isco08=1 if inlist(csa,23,33,37) & ilo_lfs==2 & ilo_cat_une==1                          
		replace ilo_prevocu_isco08=2 if inlist(csa,31,43,52,45,46) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=3 if inlist(csa,34,35,42,47,38,54) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=4 if inlist(csa,44) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=5 if inlist(csa,22,55,56) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=6 if inlist(csa,10,11,12,13,69) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=7 if inlist(csa,21,62,63) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=8 if inlist(csa,48,64,65) & ilo_lfs==2 & ilo_cat_une==1                          
...
if !_rc {
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if dem==1 & dispoc==2              & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if dem==0 & dispoc==1                & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if dem==0 & dispoc==2 & soub==1 		      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if dem==0 & dispoc==2 & soub==2		      & ilo_lfs==3      // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	// Not classified 
	 		    lab def dlma_lab 1 "1 - Seeking, not available (Unavailable jobseekers)" 2 "2 - Not seeking, available (Available potential jobseekers)" ///
							     3 "3 - Not seeking, not available, willing (Willing non-jobseekers)" 4 "4 - Not seeking, not available, not willing" 5 "X - Not elsewhere classified"
			    lab val ilo_olf_dlma dlma_lab 
...
* ------------------------------------------------------------------------------
* Comment: 
capture confirm variable raisnrec
if !_rc {
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if  inlist(raisnrec,6,7,8) | raisnsou==4         & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if  inlist(raisnrec,1)        & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if  inlist(raisnrec,2,3,4,5) | inlist(raisnsou,1,2,3)         & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if  inlist(raisnsou,5,6)        & ilo_lfs==3             // Does not need/want to work
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if  inlist(decr,6,7)        & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if  inlist(decr,1)        & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if  (inlist(decr,2,3,4) | ilo_edu_attendance==1)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if  inlist(decr,5)        & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if ilo_olf_reason==. & ilo_lfs==3     // Not elsewhere classified
 			        lab def lab_olf_reason 1 "1 - Labour market" 2 " 2 - Other labour market reasons" 3 "3 - Personal/Family-related"  ///
				                           4 "4 - Does not need/want to work" 5 "5 - Not elsewhere classified"
					lab val ilo_olf_reason lab_olf_reason
