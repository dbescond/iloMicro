* TITLE OF DO FILE: ILO Microdata Preprocessing code template - FRANCE, 2017
* DATASET USED: FRANCE, EE, 2017
...
global country "FRA" /*ref_area: ISO 3 Code from the workflow*/
global source "EE"  /*survey: Acronym from the workflow*/
global time "2017"  /*time*/
global inputFile "FRA_EE_2017.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if  inlist(catau2010r,1,2)        // Urban 
		replace ilo_geo=2 if   inlist(catau2010r,3,4)           // Rural 
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if sexe==1            // Male
		replace ilo_sex=2 if sexe==2            // Female
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: the variable "age5" only allows to derive aggregated age indicator.
...
** range of AGE5 (0-14;15-29;30-39;40-49;50-59;60+)
	gen ilo_age_aggregate=.
		replace ilo_age_aggregate=1 if age5==0
		replace ilo_age_aggregate=2 if age5==15
		replace ilo_age_aggregate=3 if inlist(age5,30,40)
		replace ilo_age_aggregate=4 if age5==50
		replace ilo_age_aggregate=5 if age5==60 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: high proportion of doctoral level, to be verified
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if dip11==71                       // No schooling
		replace ilo_edu_isced11=2 if  dip11==70                       // Early childhood education
		replace ilo_edu_isced11=3 if  dip11==60                     // Primary education
		replace ilo_edu_isced11=4 if  dip11==50                     // Lower secondary education
		replace ilo_edu_isced11=5 if   dip11==42                     // Upper secondary education
		replace ilo_edu_isced11=6 if  dip11== 41                     // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if   dip11==33                     // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if  inlist(dip11,31,30)                      // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if dip11==11                       // Master's or equivalent level
		replace ilo_edu_isced11=10 if dip11==10                      // Doctoral or equivalent level
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  acteu6==5                      // Attending
		replace ilo_edu_attendance=2 if  inlist(acteu6,1,3,4,6)                      // Not attending
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: no information on marital status. Information on the type of household (typmen7)
	/*
	* Detailed
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: no information on marital status
/*
    * Detailed
	gen ilo_dsb_details=.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: acteu follows the ILO resolutions
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (acteu==1) & ilo_wap==1						// Employed: ILO definition
		*replace ilo_lfs=1 if () & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if (acteu==2) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Interns and apprentices considered as employees. No information on members of producers' cooperatives

   * MAIN JOB
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(statut,21,22,33,34,35,43,44,45)  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if  statut==12 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if statut==11 & ilo_lfs==1          // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if statut==13  & ilo_lfs==1          // Contributing family workers
...
* Comment: 
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = nafg088un     if ilo_lfs==1
...
/*
	* 1-digit level
    gen ilo_job1_eco_isic4=.
	    replace ilo_job1_eco_isic4=1 if nafg021un=="A"
	    replace ilo_job1_eco_isic4=2 if nafg021un=="B"
	    replace ilo_job1_eco_isic4=3 if nafg021un=="C"
	    replace ilo_job1_eco_isic4=4 if nafg021un=="D"
	    replace ilo_job1_eco_isic4=5 if nafg021un=="E"
	    replace ilo_job1_eco_isic4=6 if nafg021un=="F"
	    replace ilo_job1_eco_isic4=7 if nafg021un=="G"
	    replace ilo_job1_eco_isic4=8 if nafg021un=="H"
	    replace ilo_job1_eco_isic4=9 if nafg021un=="I"
	    replace ilo_job1_eco_isic4=10 if nafg021un=="J"
	    replace ilo_job1_eco_isic4=11 if nafg021un=="K"
	    replace ilo_job1_eco_isic4=12 if nafg021un=="L"
	    replace ilo_job1_eco_isic4=13 if nafg021un=="M"		
	    replace ilo_job1_eco_isic4=14 if nafg021un=="N"
	    replace ilo_job1_eco_isic4=15 if nafg021un=="O"
        replace ilo_job1_eco_isic4=16 if nafg021un=="P"
	    replace ilo_job1_eco_isic4=17 if nafg021un=="Q"
	    replace ilo_job1_eco_isic4=18 if nafg021un=="R"
	    replace ilo_job1_eco_isic4=19 if nafg021un=="S"
	    replace ilo_job1_eco_isic4=20 if nafg021un=="T"
	    replace ilo_job1_eco_isic4=21 if nafg021un=="U"
		replace ilo_job1_eco_isic4=22 if ilo_job1_eco_isic4==. & ilo_lfs==1
		        lab def eco_isic4_1digit 1 "A - Agriculture, forestry and fishing"	2 "B - Mining and quarrying"	3 "C - Manufacturing"	4 "D - Electricity, gas, steam and air conditioning supply" ///
                                         5 "E - Water supply; sewerage, waste management and remediation activities"	6 "F - Construction"	7 "G - Wholesale and retail trade; repair of motor vehicles and motorcycles"	8 "H - Transportation and storage" ///
                                         9 "I - Accommodation and food service activities"	10 "J - Information and communication"	11 "K - Financial and insurance activities"	12 "L - Real estate activities" ///
...
/*	
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
	    replace ilo_job1_ocu_isco08=1 if inlist(cse,23,37) & ilo_lfs==1   
	    replace ilo_job1_ocu_isco08=2 if inlist(cse,31,33) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=3 if inlist(cse,34,35,42,47) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=4 if inlist(cse,44) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=5 if inlist(cse,22,43,52,55,54,56) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=6 if inlist(cse,10,11,12,13,69) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=7 if inlist(cse,21,48,62,63) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=8 if inlist(cse,64,65) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=9 if inlist(cse,67,68) & ilo_lfs==1                          
	    replace ilo_job1_ocu_isco08=10 if inlist(cse,53) & ilo_lfs==1                          
	...
	* Skill level
	gen ilo_job1_ocu_skill=.
	    replace ilo_job1_ocu_skill=1 if ilo_job1_ocu_aggregate==9                  // Low
		replace ilo_job1_ocu_skill=2 if inlist(ilo_job1_ocu_aggregate,4,5,6,7,8)   // Medium
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(pub3fp,1,2,3)                       & ilo_lfs==1    // Public
...
* Comment: 
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = empnbh       if ilo_lfs==1
...
* Comment: threshold set at 35 hours
	...
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if ilo_job1_how_actual>=35  & ilo_lfs==1                             // Full-time
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: contract for apprentices?
	
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if contra==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if inlist(contra,2,3,4) & ilo_lfs==1               				// Temporary
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: No information to derive the monthly labour related income indicators
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: threshold set at 35 (full-time also 35)?, 
*		   question dispplc asks whether the individual wants to work more if available to do so

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_actual<= 35 & dispplc==1 & ilo_lfs==1
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if cstotr!=8     & ilo_lfs==2                     // Previously employed       
		replace ilo_cat_une=2 if cstotr==8     & ilo_lfs==2                     // Seeking for the first time
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if ancchom==1      & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if ancchom==2      & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if ancchom==3      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if ancchom==4      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if inlist(ancchom,5,6)      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if inlist(ancchom,7,8)      & ilo_lfs==2                 // 24 months or more
...
* Comment: 
	...
	* 2-digit level
	gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = nafant     if ilo_lfs==2 & ilo_cat_une==1
...
* Comment: 
	...
	* 1-digit level 				
	gen ilo_prevocu_isco08=.
	    replace ilo_prevocu_isco08=1 if inlist(csa,23,37) & ilo_lfs==2 & ilo_cat_une==1                          
		replace ilo_prevocu_isco08=2 if inlist(csa,31,33) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=3 if inlist(csa,34,35,42,47) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=4 if inlist(csa,44) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=5 if inlist(csa,22,43,52,55,54,56) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=6 if inlist(csa,10,11,12,13,69) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=7 if inlist(csa,21,48,62,63) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=8 if inlist(csa,64,65) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=9 if inlist(csa,67,68) & ilo_lfs==2 & ilo_cat_une==1                          
	    replace ilo_prevocu_isco08=10 if inlist(csa,53) & ilo_lfs==2 & ilo_cat_une==1                          
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if dem==1 & dispoc==2              & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if dem==0 & dispoc==1                & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if dem==0 & dispoc==2 & souc==1 		      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if dem==0 & dispoc==2 & souc==2		      & ilo_lfs==3      // Not seeking, not available, not willing
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if  inlist(raisnrec,6,7,8)         & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if  inlist(raisnrec,1)         & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if  inlist(raisnrec,2,3,4,5)         & ilo_lfs==3             // Personal/Family-related
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & dispoc==1      & ilo_olf_reason==1
