...
global source "LFS"
global time "2017Q1"
global inputFile "lfsp_jm17_eul.dta"
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Geographical coverage ('ilo_geo') !!! UNDONE!!!
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
...
gen ilo_lfs =.
 	...
 	*replace ilo_lfs = 1 (Wrking==1) |(JbAway==1) | (TYPSCH12==1) | (TYPSCH12==2) | (TYPSCH12==3) |(TYPSCH12==5) | (TYPSCH12==8) | ((TYPSCH12==9) & (YTETJB==1)) | (YTETJB==1)  & ilo_wap == 1
		...
		*** esto es lo que ellos ponen (ver pregunta difjob)
		*** resulta 31,684,778  mientras que ellos reportan 31.85

		replace ilo_lfs = 1 if wrking == 1 &  ilo_wap == 1  
		replace ilo_lfs = 1 if jbaway == 1 & ilo_wap == 1
		replace ilo_lfs = 1 if inlist(typsch12,1,2,3,5,8) & ilo_wap == 1
		*replace ilo_lfs = 1 if (typsch==9) & ytetjb == 1 & ilo_wap == 1
		...
		** EMPLOYED  
		...
		replace ilo_lfs = 1 if wrking == 1 &  ilo_wap == 1  
		replace ilo_lfs = 1 if jbaway == 1 & ilo_wap == 1
		replace ilo_lfs = 1 if (ownbus == 1 | relbus == 1) & ilo_wap == 1
...
* Detailed categories:
		...
		gen ilo_job1_ste_icse93 = .
			...
			replace ilo_job1_ste_icse93 = 1 if c1 == 1   & ilo_lfs == 1   	        // Employees
			...
			replace ilo_job1_ste_icse93 = 6 if  inlist(c1,2,3) & ilo_lfs==1         // Workers not classifiable by status
...
* Detailed categories:
		...
		gen ilo_job2_ste_icse93 = .
			...
			replace ilo_job2_ste_icse93 = 1 if d4 == 1   & d1==1  	       // Employees
			
			replace ilo_job2_ste_icse93 = 2 if d4 == 4 & d1==1             // Employers
			
			replace ilo_job2_ste_icse93 = 3 if inlist(d4,5,6) & d1==1       // Own-account workers
			
			replace ilo_job2_ste_icse93 = 4 if d4 == 8 & d1==1              // Members of producers cooperatives

			replace ilo_job2_ste_icse93 = 5 if d4 == 7 & d1==1	            // Contributing family workers
			
			replace ilo_job2_ste_icse93 = 6 if  inlist(d4,2,3) & d1==1       // Workers not classifiable by status
...
* Aggregate categories
		...
		gen ilo_job2_ste_aggregate = . 
			...
			replace ilo_job2_ste_aggregate = 1 if ilo_job2_ste_icse93 == 1 & d1==1		                // Employees
			
			replace ilo_job2_ste_aggregate = 2 if inlist(ilo_job2_ste_icse93,2,3,4,5) & ilo_mjh == 2   	// Self-employed
			
			replace ilo_job2_ste_aggregate = 3 if ilo_job2_ste_icse93 == 6 & d1==1	 				    // Not elsewhere classified
			replace ilo_job2_ste_aggregate = 3 if ilo_job2_ste_icse93 == . & d1==1	  			
...
*** SECOND JOB
   ...
   * ilo_job2_how_actual
 		...
 		gen ilo_job2_how_actual =d2 if d1==1 
		...
		gen ilo_job2_how_actual_bands = .
			 ...
			 replace ilo_job2_how_actual_bands = 8 if ilo_job2_how_actual_bands == . &  d1==1 	// Not elsewhere classified
	...
	*** People with more than two jobs
	
	*** Variable to compute the weekly hours actually worked in other jobs:
	
	gen ilo_jobother_how_actual =d2 if d1==2 

		 	replace ilo_jobother_how_actual = . if ilo_jobother_how_actual == 997
			lab var ilo_jobother_how_actual "Weekly hours actually worked in other jobs"	 
 ...
 egen ilo_joball_how_actual = rowtotal(ilo_job1_how_actual ilo_job2_how_actual ilo_jobother_how_actual), m 
