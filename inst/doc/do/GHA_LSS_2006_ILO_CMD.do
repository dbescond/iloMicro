* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Ghana
* DATASET USED: Ghana Living Standards Survey 
* NOTES: 
* Files created: Standard variables on HS Ghana
* Authors: ILO / Department of Statistics / DPAU

* Starting Date: 13 December 2017
...
global source "LSS"
global time "2006"
global inputFile "GHA_LSS_2006_ORI"
...
*********************************************************************************************
	...
	/*cd "${inpath}\GLSS5-Data\GLSS5-Data\stata\parta"
	
	use sec0, clear
	
	merge 1:m hhid using sec1
	
	        drop _merge
	
	foreach file in sec2a sec2b sec2c sec4a sec4b sec4c sec4d sec4e sec4f sec4g sec4h {
	
	merge 1:1 hhid pid using `file'.dta
	...
	* don't consider non-household members 
...
* -------------------------------------------------------------------------------------------
	...
	* info on geographic area missing mainly for individuals aged 2 or less --> fill gaps for variable loc5 (used to define ilo_geo)
	
		egen loc_max=max(loc5), by(hhid)
			replace loc5=loc_max if loc5==.
			drop loc_max
			
		gen ilo_geo=.
			replace ilo_geo=1 if inlist(loc5,1,2)
			replace ilo_geo=2 if inlist(loc5,3,4,5)
...
* -------------------------------------------------------------------------------------------
*
* Comment: note that master and doctoral level are not being distinguished by the original variable - put everything under "Master degree" and put a note in read-me file

	* original question asked to individuals aged 3+

	gen ilo_edu_isced11=.
			* No schooling 
		replace ilo_edu_isced11=1 if s2aq1==2 | s2aq2==0
			* Early childhood education
		replace ilo_edu_isced11=2 if inrange(s2aq2,1,15)
			* Primary education
		replace ilo_edu_isced11=3 if inrange(s2aq2,16,18) | inrange(s2aq2,20,22)
			* Lower secondary education
		replace ilo_edu_isced11=4 if inlist(s2aq2,19,23,24,25,27,28,29,30)
			* Upper secondary education
		replace ilo_edu_isced11=5 if inlist(s2aq2,26,31,32,33,41)
			* Post-secondary non-tertiary education
		/* replace ilo_edu_isced11=6 if */
			* Short-cycle tertiary education
		replace ilo_edu_isced11=7 if inlist(s2aq2,42,43)
			* Bachelor's or equivalent level
		replace ilo_edu_isced11=8 if inlist(s2aq2,51,52) | s2aq3==13 
			* Master's or equivalent level
		replace ilo_edu_isced11=9 if s2aq3==14
			* Doctoral or equivalent level 
		replace ilo_edu_isced11=10 if s2aq3==15
...
* Comment: 
		...
		gen ilo_edu_attendance=.
			replace ilo_edu_attendance=1 if s2aq5==1
			replace ilo_edu_attendance=2 if s2aq4==2 | s2aq5==2 | s2aq1==2
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if s1q6==6                                   // Single
		replace ilo_mrts_details=2 if s1q6==1                                   // Married
		replace ilo_mrts_details=3 if s1q6==2                                   // Union / cohabiting
		replace ilo_mrts_details=4 if s1q6==5                                   // Widowed
		replace ilo_mrts_details=5 if inlist(s1q6,3,4)                          // Divorced / separated
...
* Comment: 
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if s4aq1==1
		replace ilo_lfs=2 if ilo_lfs!=1 & inlist(s4dq1,1,2) & inlist(s4dq2,1,2)
...
* Comment: 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if s4aq2==1
		replace ilo_mjh=2 if inrange(s4aq2,2,4)
...
* Comment:
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inlist(s4aq11,1,8,9) 
			replace ilo_job1_ste_icse93=2 if inlist(s4aq11,2,5)
			replace ilo_job1_ste_icse93=3 if inlist(s4aq11,3,6)
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if inlist(s4aq20,4,7)
...
* Comment: ISIC Rev.3.1 used
	...
	gen indu_code_prim=int(s4aq5/100) if ilo_lfs==1
	gen indu_code_sec=int(s4bq2/100) if ilo_lfs==1 & ilo_mjh==2
...
* Comment:  ISCO-88 classification used
	...
	gen occ_code_prim=int(s4aq4/10) if ilo_lfs==1
	gen occ_code_sec=int(s4bq1/10) if ilo_lfs==1 & ilo_mjh==2
...
* Comment: Note that the question is asked to paid employees only 

	* Primary occupation
	
		gen ilo_job1_ins_sector=.
			replace ilo_job1_ins_sector=1 if inlist(s4aq12,1,2,3,6)
...
* Comment: 
	...
	* Actual hours worked 
		...
		* All jobs 
		...
		gen ilo_joball_how_actual=s4aq3
...
* Comment: set 40 hours per week as threshold 
			* --> use actual hours as usual hours not indicated
...
* Comment: Useful questions:	* s4aq11: Status in employment
								* s4aq12: Institutional sector 
								* [No info]: Destination of production 
								* [No info]: Bookkeeping
								* [No info]: Registration						
								* s4aq23: Location of workplace 
								* s4aq26 : Size of the establishment					
								* Social security 
									* s4aq21: Social security benefits
									* s4aq17: Paid annual leave
									* s4aq18: Paid sick leave
	
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=3 if (ilo_job1_eco_isic3_2digits==95 | ilo_job1_ocu_isco88_2digits==62)
		replace ilo_job1_ife_prod=2 if inlist(s4aq12,1,2,3,4,5,6) | (!inlist(s4aq12,1,2,3,4,5,6) & ilo_job1_ste_aggregate==1 & s4aq30==1) | (!inlist(s4aq12,1,2,3,4,5,6) & ///
					((ilo_job1_ste_aggregate==1 & s4aq21!=1) | ilo_job1_ste_aggregate!=1) & inlist(s4aq23,1,3,4,8,9,14,15) & s4aq26>=6 & !inlist(s4aq26,9999,.))
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_job1_ife_nature=.
		replace ilo_job1_ife_nature=1 if ilo_job1_ste_icse93==5 | (inlist(ilo_job1_ste_icse93,2,3,4) & inlist(ilo_job1_ife_prod,1,3)) | (inlist(ilo_job1_ste_icse93,1,6) & s4aq21!=1) 
		replace ilo_job1_ife_nature=2 if (inlist(ilo_job1_ste_icse93,1,6) & s4aq21==1) | (inlist(ilo_job1_ste_icse93,2,3,4) & ilo_job1_ife_prod==2)
...
* Comment: 
	...
	* Currency in Ghana: Cedi
	
		* convert to monthly values
		
		gen cash_month=.
			replace cash_month=s4aq9a*(365/12) if s4aq9b==1
			replace cash_month=s4aq9a*(52/12) if s4aq9b==2
			replace cash_month=s4aq9a*(26/12) if s4aq9b==3
			replace cash_month=s4aq9a if s4aq9b==4
			replace cash_month=s4aq9a/3 if s4aq9b==5
			replace cash_month=s4aq9a/12 if s4aq9b==6
			
		gen inkind_month=.
			replace inkind_month=s4aq14a*(365/12) if s4aq14b==1
			replace inkind_month=s4aq14a*(52/12) if s4aq14b==2
			replace inkind_month=s4aq14a*(26/12) if s4aq14b==3
			replace inkind_month=s4aq14a if s4aq14b==4
			replace inkind_month=s4aq14a/3 if s4aq14b==5
			replace inkind_month=s4aq14a/12 if s4aq14b==6
		
	* Employees
	
		egen ilo_job1_lri_ees=rowtotal(cash_month inkind_month) if ilo_job1_ste_aggregate==1, m
			replace ilo_job1_lri_ees=. if ilo_job1_ste_aggregate!=1 | ilo_job1_lri_ees<0
			lab var ilo_job1_lri_ees "Monthly earnings of employees in main job"
			
	* Self-employed
	
		egen ilo_job1_lri_slf=rowtotal(cash_month inkind_month) if ilo_job1_ste_aggregate==2, m
...
* Comment: also people available during following 30 days are being considered
			* Note that the criterion "willingness" is being captured by a proxy, namely question s4cq1 asking whether the person was seeking to change its job situation during 
			* the past 7 days
		
		gen ilo_joball_tru=1 if ilo_job1_job_time==1 & s4cq1==1 & inlist(s4cq5,1,2)
...
* Comment: 
	...
	gen ilo_dur_details=.
		replace ilo_dur_details=1 if s4dq7==1
		replace ilo_dur_details=2 if s4dq7==2
		replace ilo_dur_details=3 if s4dq7==3
		replace ilo_dur_details=4 if s4dq7==4
		replace ilo_dur_details=5 if s4dq7==5
		replace ilo_dur_details=6 if inlist(s4dq7,6,7)
...
* -------------------------------------------------------------------------------------------
*
* Comment: question asks only whether the person has been employed during the last 12 months

	gen ilo_cat_une=.
		replace ilo_cat_une=1 if !inlist(s4dq8,0,.)
		replace ilo_cat_une=2 if s4dq8==0
...
* Comment: ISCO-88 classification being used
	...
	* reduce it to the one digit level 
	...
	gen prevocu_cod=int(s4dq8/100) if ilo_cat_une==1
...
* Comment: due to filter questions (unavailable people not asked whether they seek a job or not), this variable cannot be defined
...
* Comment:  
	 ...
	 gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if s4dq3==1
		replace ilo_olf_reason=2 if inlist(s4dq3,2,3,4)
		replace ilo_olf_reason=3 if inlist(s4dq3,5,6,7,8) | inlist(s4dq10,1,2,3,4,5)
		replace ilo_olf_reason=4 if s4dq10==6
...
* Comment: 
	...
	gen ilo_dis=1 if ilo_lfs==3 & ilo_olf_reason==1 & inlist(s4dq1,1,2)
...
cd "$outpath"
		...
		drop indu_code_* occ_code_*  prev*_cod  cash_month inkind_month
