...
global source "LSS"
global time "2013"
global inputFile "GHA_LSS_2013_ORI"
...
* Comment: note that master and doctoral level are not being distinguished by the original variable - put everything under "Master degree" and put a note in read-me file
	...
	* original question asked to individuals aged 3+
	...
	gen ilo_edu_isced11=.
			* No schooling 
		replace ilo_edu_isced11=1 if s2aq1==2 | s2aq3==0
			* Early childhood education
		replace ilo_edu_isced11=2 if s2aq3==1
			* Primary education
		replace ilo_edu_isced11=3 if s2aq3==2
			* Lower secondary education
		replace ilo_edu_isced11=4 if inlist(s2aq3,3,4)
			* Upper secondary education
		replace ilo_edu_isced11=5 if inlist(s2aq3,5,6,7)
			* Post-secondary non-tertiary education
		/* replace ilo_edu_isced11=6 if */
			* Short-cycle tertiary education
		replace ilo_edu_isced11=7 if s2aq3==8
			* Bachelor's or equivalent level
		replace ilo_edu_isced11=8 if inlist(s2aq3,9,10)
			* Master's or equivalent level
		replace ilo_edu_isced11=9 if s2aq3==11
...
* Comment: question asked to individuals aged 5+ 
		...
		gen ilo_edu_attendance=.
			replace ilo_edu_attendance=1 if s2aq4==1
			replace ilo_edu_attendance=2 if s2aq4==2
...
* Comment: 
	...
	gen ilo_dsb_aggregate=.
		replace ilo_dsb_aggregate=1 if s3aq26==2
		replace ilo_dsb_aggregate=2 if s3aq26==1
			* force missing values into category "without disability"
		replace ilo_dsb_aggregate=1 if s3aq26==.
...
* Comment: 
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (s4aq1==1 | s4aq2==1 | s4aq3==1)
		replace ilo_lfs=2 if ilo_lfs!=1 & (s4dq1==1 | s4dq1==2) & (s4dq2==1 | s4dq2==2)
...
* Comment: 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if s4fq1==2
		replace ilo_mjh=2 if s4fq1==1
...
* Comment:
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inlist(s4aq20,1,8,9,10) 
			replace ilo_job1_ste_icse93=2 if inlist(s4aq20,2,5)
			replace ilo_job1_ste_icse93=3 if inlist(s4aq20,3,6)
...
* Comment: ISIC Rev.4 used
	
	gen indu_code_prim=int(s4eq7/100) if ilo_lfs==1
	gen indu_code_sec=int(s4fq3/100) if ilo_lfs==1 & ilo_mjh==2
...
* Comment:  ISCO-08 classification used
	...
	* Two-digits level
	...
	gen occ_code_prim=int(s4eq6/100) if ilo_lfs==1
	gen occ_code_sec=int(s4fq2/100) if ilo_lfs==1 & ilo_mjh==2
...
* Comment: Note that the question is asked to paid employees only 
	...
	* Primary occupation
		...
		gen ilo_job1_ins_sector=.
			replace ilo_job1_ins_sector=1 if inlist(s4aq21,1,2,3,6)
...
* Comment: 
	...
	* Actual hours worked 
		...
		* All jobs 
		...
		gen ilo_joball_how_actual=s4aq5
...
* Comment: only indicated whether the contract is writter or oral --> info can't be used
...
* Comment: Useful questions:	* s4aq20: Status in employment
								* s4aq21: Institutional sector 
								* [No info]: Destination of production 
								* [No info]: Bookkeeping
								* [No info]: Registration						
								* s4aq32: Location of workplace 
								* s4aq35 : Size of the establishment					
								* Social security 
									* s4aq30: Social security benefits
									* s4aq26: Paid annual leave
									* s4aq27: Paid sick leave
	
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=3 if (ilo_job1_eco_isic4_2digits==97 | ilo_job1_ocu_isco08_2digits==63)
		replace ilo_job1_ife_prod=2 if inlist(s4aq21,1,2,3,4,5,6) | (!inlist(s4aq21,1,2,3,4,5,6) &  s4aq30==1) | (!inlist(s4aq21,1,2,3,4,5,6) & s4aq30==. & inlist(s4aq32,1,3,4,8,9,14,15) & (s4aq35>=6 & s4aq35<=9000))
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_job1_ife_nature=.
		replace ilo_job1_ife_nature=1 if ilo_job1_ste_icse93==5 | (inlist(ilo_job1_ste_icse93,2,3,4) & inlist(ilo_job1_ife_prod,1,3)) | (inlist(ilo_job1_ste_icse93,1,6) & s4aq30!=1) 
		replace ilo_job1_ife_nature=2 if (inlist(ilo_job1_ste_icse93,1,6) & s4aq30==1) | (inlist(ilo_job1_ste_icse93,2,3,4) & ilo_job1_ife_prod==2)
...
* Comment: section on labour gives only info about employees' revenue
	...
	* Currency in Ghana: Cedi
		...
		* convert to monthly values
		...
		gen salary_month=.
			replace salary_month=s4aq18a*(365/12) if s4aq18b==1
			replace salary_month=s4aq18a*(52/12) if s4aq18b==2
			replace salary_month=s4aq18a*(26/12) if s4aq18b==3
			replace salary_month=s4aq18a if s4aq18b==4
			replace salary_month=s4aq18a/3 if s4aq18b==5
			replace salary_month=s4aq18a/12 if s4aq18b==6
			
		gen inkind_month=.
			replace inkind_month=s4aq23a*(365/12) if s4aq23b==1
			replace inkind_month=s4aq23a*(52/12) if s4aq23b==2
			replace inkind_month=s4aq23a*(26/12) if s4aq23b==3
			replace inkind_month=s4aq23a if s4aq23b==4
			replace inkind_month=s4aq23a/3 if s4aq23b==5
			replace inkind_month=s4aq23a/12 if s4aq23b==6
...
* Comment: also people available during following 30 days are being considered
	...
	gen ilo_joball_tru=1 if ilo_job1_job_time==1 & inlist(s4cq2,1,2) & inlist(s4cq5,1,2)
...
* Comment: 
	...
	egen ilo_joball_oi_case=anycount(s4hsq1a s4hsq1b s4hsq1c s4hsq1d s4hsq1e s4hsq1f s4hsq1g s4hsq1h s4hsq1i s4hsq1j s4hsq1k s4hsq1l), values(1)
...
/*
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if 
		replace ilo_cat_une=2 if 
...
/*
	 gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma=2 if inlist(s4dq1,1,2) & s4dq2==3
...
* Comment:  
	 ...
	 gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(s4dq4,1,2,3,4,5,17)
		replace ilo_olf_reason=2 if inlist(s4dq4,6,7,19)
		replace ilo_olf_reason=3 if inlist(s4dq4,8,9,10,11,12,13,14,15)
		replace ilo_olf_reason=4 if inlist(s4dq4,16,20)
...
cd "$outpath"
		...
		drop indu_code_* occ_code_*  /* prev*_cod */ salary_month inkind_month
