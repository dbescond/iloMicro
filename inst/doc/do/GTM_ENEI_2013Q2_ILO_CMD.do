...
global source "ENEI"
global time "2013Q2"
global inputFile "ENEI I-2013-Personas.dta"
...
* Comment: Given the definition of the levels of education, ISCED97 is being chosen
	...
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if p03a02==3 
		replace ilo_edu_isced97=2 if p03a03==1 | (p03a03==2 & inrange(p03a04,0,5))
		replace ilo_edu_isced97=3 if (p03a03==2 & p03a04==6) | (p03a03==3 & inrange(p03a04,0,2))
		replace ilo_edu_isced97=4 if (p03a03==3 & p03a04==3) | (p03a03==4 & inrange(p03a04,0,5))
		replace ilo_edu_isced97=5 if (p03a03==4 & inlist(p03a04,6,7)) | (p03a03==5 & inrange(p03a04,0,3))
		/* replace ilo_edu_isced97=6 if */
		replace ilo_edu_isced97=7 if p03a03==5 & inrange(p03a04,4,6)
		replace ilo_edu_isced97=8 if inlist(p03a03,6,7)
		replace ilo_edu_isced97=9 if ilo_edu_isced97==.
			label def isced_97_lab 1 "X - No schooling" 2 "0 - Pre-primary education" 3 "1 - Primary education or first stage of basic education" 4 "2 - Lower secondary education or second stage of basic education" ///
							5 "3 - Upper secondary education" 6 "4 - Post-secondary non-tertiary education" 7 "5 - First stage of tertiary education (not leading directly to an advanced research qualification)" ///
							8 "6 - Second stage of tertiary education (leading to an advanced research qualification)" 9 "UNK - Level not stated"
...
* Comment: 
		...
		gen ilo_edu_attendance=.
			...
			replace ilo_edu_attendance=2 if inlist(p03a02,2,3)
...
* Comment: 	Absence from job - threshold set at one month - if absent up to a month, person still considered as employed --> if more, sent further to section about unemployment
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=2 if ilo_lfs!=1 & ((p04b03!=. & p04b05==2) | inlist(p04b04,1,2))
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if p04a02==1 | p04a03==1 | p04a04==1 | p04a05==1 | inrange(p04a07,1,5) | p04a08==1 | inlist(p04a09,1,2) 
		replace ilo_lfs=2 if ilo_lfs!=1 & ((p04b03!=. & p04b06==2) | inlist(p04b05,1,2))
		replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1
			replace ilo_lfs=. if ilo_wap!=1	
				}
...
* Comment: 
	...
	gen ilo_mjh=.
		...
		replace ilo_mjh=2 if p04c01>1 & p04c01!=.
...
* ------------------------------------------------------------------------------------------- 
*
* Comment: "trabajador no remunerado" (unpaid worker) to be classified among contributing family workers

	* Primary activity
	
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inlist(p04c06,1,2,3,4)
			replace ilo_job1_ste_icse93=2 if inlist(p04c06,6,8) 
			replace ilo_job1_ste_icse93=3 if inlist(p04c06,5,7)  
	  ...
	  * Secondary activity
		...
		gen ilo_job2_ste_icse93=.
			...
			replace ilo_job2_ste_icse93=2 if inlist(p04d05,6,8) & p04d14a==1
			replace ilo_job2_ste_icse93=3 if inlist(p04d05,5,7) & p04d14a!=1
...
* Comment: ISIC Rev. 4 being used 
	...
	* economic activity indicated both for primary and secondary activity 
	...
	gen indu_code_prim=p04c04b if ilo_lfs==1
	
	gen indu_code_sec=p04d03b if ilo_lfs==1 & ilo_mjh==2
	...
	gen indu_code_sec=p04d04a if ilo_lfs==1 & ilo_mjh==2
...
* Comment: Indicate both for primary and secondary activity
	...
	gen occ_code_prim=p04c02b if ilo_lfs==1
	gen occ_code_sec=p04d02b if ilo_lfs==1 & ilo_mjh==2
		}
...
* Comment: variable only defined for primary employment, as question on the nature of the contract
	*		is not being asked for secondary employment
	...
	gen ilo_job1_job_contract=.
		...
		replace ilo_job1_job_contract=3 if p04c07==2 | (ilo_job1_job_contract==. & ilo_job1_ste_aggregate==1)
...
* Comment:  --> no information about bookkeeping and registration available...!
	...
	* Useful questions: p04c06: Status in employment / Institutional sector
	...
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=1 if !inlist(p04c06,1,4) & p04c25a!=1
		replace ilo_job1_ife_prod=2 if p04c06==1 | (!inlist(p04c06,1,4) & p04c25a==1)
...
* Comment: ISIC Rev. 4 classification being used - keep only one digit level
	...
	gen preveco_cod=p04f09b_2d if ilo_cat_une==1
		...
		gen preveco_cod=p04f09b if ilo_cat_une==1
		}
