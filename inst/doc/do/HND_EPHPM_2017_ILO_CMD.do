...
global source "EPHPM"
global time "2017"
global inputFile "Hogares0617.dta"
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if ce447==2 & ilo_lfs==1
		replace ilo_mjh=2 if ce447==1 & ilo_lfs==1
...
gen ilo_job1_eco_isic4_2digits=trunc(ce428cod/100) if ilo_lfs==1
 ...
 replace ilo_job1_eco_isic4_2digits=. if inlist(ilo_job1_eco_isic4_2digits,54,83)
...
if ${time} >2005 {
	...
	* based on the status question, unfortunately no further info on corporate sector is done, therefore all private are treated equally (except household)
	...
	replace todrop_institutional=1 if ce444==2|ce444==3
	...
	replace todrop_producesforsale=0 if 1==0
	...
	* but the answers are somewhat different (still fully compatible, yet it changes the results quite dramatically)
	...
	egen todrop_pro_book=rowtotal(ce442_1-ce442_4)
	...
	gen todrop_registration=1 if ce443<4										// theoretical 3 value node/ +1 registered national level/ 0 not registered national level, other, DK / -1 not asked NA
		replace todrop_registration=-1 if todrop_registration==.


	gen todrop_SS=1 if ce436_1==1|inlist(ce436_2,1,2) 														// theoretical 3 value node/ +1 Social security/ 0 Not asked or don't know NA Other / -1 No social security 
		replace todrop_SS=-1 if todrop_SS==. 
		replace todrop_SS=0 if todrop_SS==.


		
	gen todrop_place=1 if 1==0										// theoretical 2 value node/ +1 fixed visible premises / 0 non fixed premises or others
...
* -------------------------------------------------------------------------------------------			
			...
			* Main job
			gen todrop_wage=ce438 if ce437==1
				replace todrop_wage=438*4.333333333333/2 if ce437==2
				replace todrop_wage=438*4.3333333333333 if ce437==3
				replace todrop_wage=438*4.333333333333*7 if ce437==4
				* Employees
...
* -------------------------------------------------------------------------------------------		
		...
		* notice that the question bunches together willing and available
			replace ilo_joball_tru=1 if ilo_joball_how_actual<35 & ce471==1 & ilo_lfs==1
