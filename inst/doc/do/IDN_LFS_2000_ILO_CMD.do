* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Indonesia, 2000
* DATASET USED: Indonesia, LFS, 2000
* NOTES: 
* Files created: Standard variables IDN_LFS_2000_FULL.dta and IDN_LFS_2000_ILO.dta
	...
	* Detailed
	gen ilo_edu_isced97=.
		...
		* replace ilo_edu_isced97=6 if                                          // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if inlist(b4p1a,8,9,0)                        // First stage of tertiary education (not leading directly to an advanced research qualification)
...
* Comment: - no info on union/cohabiting
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if stat==1                                // Single
		replace ilo_mrts_details=2 if stat==2                                // Married
		*replace ilo_mrts_details=3 if                                          // Union / cohabiting
		replace ilo_mrts_details=4 if stat==4                                // Widowed
		replace ilo_mrts_details=5 if stat==3                                // Divorced / separated
...
*            definition here used is two criteria. (T5:1429)
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (b4p2a1==1 | b4p3==1) & ilo_wap==1	                // Employed: engaged in work or worked for at least 1 hour in the past week
		replace ilo_lfs=1 if (b4p4==1) & ilo_wap==1                             // Employed: temporary absent
		replace ilo_lfs=2 if (ilo_lfs!=1 & b4p5==1) & ilo_wap==1                // Unemployed: two criteria: not in employment and seeking
		replace ilo_lfs=2 if (ilo_lfs!=1 & b4p19==4) & ilo_wap==1               // Unemployed: future starters
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (b4p13==2) & ilo_lfs==1                            // One job only     
		replace ilo_mjh=2 if (b4p13==1) & ilo_lfs==1                            // More than one job
...
*            classified under own-account workers.
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if b4p10==4 & ilo_lfs==1                 // Employees
		 replace ilo_job1_ste_icse93=2 if b4p10==3 & ilo_lfs==1                 // Employers
		 replace ilo_job1_ste_icse93=3 if inlist(b4p10,1,2) & ilo_lfs==1        // Own-account workers
		 * replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1                       // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if b4p10==5 & ilo_lfs==1                 // Contributing family workers
...
*            under code 51 of ISIC 3.1.
	...
	*---------------------------------------------------------------------------
	...
	gen indu_code_sec=int(b4p14/10) if ilo_mjh==2 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification: Indonesian Standard Occupational (KBJI) 2000.
*            Possibility to map it to ISCO-88 at 1-digit level.
	...
	*---------------------------------------------------------------------------
	...
	gen occ_code_prim=int(b4p8/10) if ilo_lfs==1
			
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
	    replace ilo_job1_ocu_isco88=1 if inlist(occ_code_prim,11,12,13,14) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=2 if inlist(occ_code_prim,21,22,23,24,25) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=3 if inlist(occ_code_prim,31,32,33,34,39) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=4 if inlist(occ_code_prim,51,59) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=5 if inlist(occ_code_prim,61,62,63) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=6 if occ_code_prim==46 & ilo_lfs==1
		replace ilo_job1_ocu_isco88=7 if inlist(occ_code_prim,44,45,49) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=8 if inlist(occ_code_prim,41,42,43,71,72,73,79,81) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=9 if inlist(occ_code_prim,82,83,91,92,99) & ilo_lfs==1
		replace ilo_job1_ocu_isco88=10 if inlist(occ_code_prim,1,2,3,9) & ilo_lfs==1
...
*          - No information available for secondary job.
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = b4p9  if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual = .
	    replace ilo_joball_how_actual = b4p6b if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Information only available for main job by employees.
*          - Earnings in cash and/or in kind.
*          - Local currency: Indonesian Rupiah.
    ...
    ***********
	...
	egen earn_cash_kind = rowtotal(b4p11a1 b4p11a2), m
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if b4p21==1 & ilo_lfs==2                          // Previously employed       
		replace ilo_cat_une=2 if b4p21==2 & ilo_lfs==2                          // Seeking for the first time
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if b4p17<1 & b4p17!=. & ilo_lfs==2            // Less than 1 month
		replace ilo_dur_details=2 if b4p17>=1 & b4p17<3 & ilo_lfs==2            // 1 to 3 months
		replace ilo_dur_details=3 if b4p17>=3 & b4p17<6 & ilo_lfs==2            // 3 to 6 months
		replace ilo_dur_details=4 if b4p17>=6 & b4p17<12 & ilo_lfs==2           // 6 to 12 months
		replace ilo_dur_details=5 if b4p17>=12 & b4p17<24 & ilo_lfs==2          // 12 to 24 months
		replace ilo_dur_details=6 if b4p17>=24 & b4p17!=. & ilo_lfs==2          // 24 months or more
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if b4p19==1 & ilo_lfs==3                   // Labour market 
			* replace ilo_olf_reason=2 if                                       // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(b4p19,2,3,6) & ilo_lfs==3        // Personal/Family-related
			replace ilo_olf_reason=4 if b4p19==5 & ilo_lfs==3                   // Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & b4p20==1 & ilo_olf_reason==1
...
cd "$outpath"
	...
	drop year indu_code_prim indu_code_sec occ_code_prim earn_cash_kind
	compress
