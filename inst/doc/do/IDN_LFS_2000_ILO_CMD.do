...
* Note that according to the definition, the highest level being CONCLUDED is being considered
	...
	gen ilo_edu_isced97=.
		...
		* First stage of tertiary education
		replace ilo_edu_isced97=7 if inlist(b4p1a,8,9,0)
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (b4p2a1==1 | b4p3==1 | b4p4==1)							// Employed
		replace ilo_lfs=2 if (ilo_lfs!=1 & b4p20!=2 & (b4p5==1 | b4p19==4)) 			// Unemployed
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if b4p13==2
		replace ilo_mjh=2 if b4p13==1
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_job1_ste_icse93=.
		replace ilo_job1_ste_icse93=1 if b4p10==4									// Employees
		replace ilo_job1_ste_icse93=2 if inlist(b4p10,2,3) 							// Employers
		replace ilo_job1_ste_icse93=3 if b4p10==1									// Own-account workers
		*replace ilo_job1_ste_icse93=4 if											// Producer cooperatives
		replace ilo_job1_ste_icse93=5 if b4p10==5									// Contributing family workers
	...
	/* National classification of occupations (KBJI 2002) is based on ISCO 88.
		...
		* ISCO 88 - 1 digit
			gen ilo_job1_ocu_isco88=.
				replace ilo_job1_ocu_isco88=occ_code_prim_1dig if occ_code_prim_1dig>0
...
* -------------------------------------------------------------------------------------------			
	...
	/* NOTES:
	   ...
	   Unit: local currency - Inonesian rupiah */
		...
		clonevar earn_cash = b4p11a1
		clonevar earn_inkind = b4p11a2
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if ilo_lfs==2 & b4p21==1							// Previously employed
		replace ilo_cat_une=2 if ilo_lfs==2 & b4p21==2							// Seeking first job
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Previous economic activity ('ilo_preveco_isic4')  [don't define variable]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
	
	* due to a filter question, not all people that were previously employed were asked 
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_reason=.		
		...
		replace ilo_olf_reason=2 if	inlist(b4p19,2,3,6) & ilo_lfs==3	//Personal / Family-related
		replace ilo_olf_reason=3 if	(b4p19==5 & ilo_lfs==3)				//Does not need/want to work
...
* -------------------------------------------------------------------------------------------		
		...
		gen ilo_dis=1 if ilo_lfs==3 & b4p20==1 & ilo_olf_reason==1	
