* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Indonesia, 2007
* DATASET USED: Indonesia, LFS, 2007
* NOTES: 
...
* Many variables in 2002's dataset are string -> destring them
  if year==2002 {
	 destring *, replace 
  }
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------		
* Comment: - Different variable's name in 2002 and 2005.

	gen ilo_wgt=.
	    if year==2005 {
		   replace ilo_wgt=timbang
		}
		if year==2002 {
		   replace ilo_wgt=infl
		}
...
* Comment: - Different variable's name in 2002 and 2005.
	...
	gen ilo_geo=.
	    if year==2005 {
		   replace ilo_geo=1 if daerah==1     // Urban
		   replace ilo_geo=2 if daerah==2     // Rural
		}
		if year==2002 {
		   replace ilo_geo=1 if b1r5==1       // Urban
		   replace ilo_geo=2 if b1r5==2       // Rural
		}
		if !inlist(year, 2002, 2005) {
...
* Comment: 
	...
	gen ilo_sex=.
	    if !_rc {
		   ...
		   replace ilo_sex=1 if b3k4==1       // Male
		   replace ilo_sex=2 if b3k4==2       // Female
		}
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Minimum age considered is 10 years old.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Highest level of educaiton complited.
*          - First stage of tertiary education includes second stage of tertiary
*            education (note to value: C3:991).
	...
	* Detailed
	gen ilo_edu_isced97=.
	    if year==2002 {
		   replace ilo_edu_isced97=1 if b4ar1a==1                               // No schooling
		   replace ilo_edu_isced97=2 if b4ar1a==2                               // Pre-primary education
		   replace ilo_edu_isced97=3 if b4ar1a==3                               // Primary education or first stage of basic education
		   replace ilo_edu_isced97=4 if inlist(b4ar1a,4,5)                      // Lower secondary education or second stage of basic education
		   replace ilo_edu_isced97=5 if inlist(b4ar1a,6,7)                      // Upper secondary education
		   * replace ilo_edu_isced97=6 if                                       // Post-secondary non-tertiary education
		   replace ilo_edu_isced97=7 if inlist(b4ar1a,8,9,0)                    // First stage of tertiary education (not leading directly to an advanced research qualification)
		...
		if year<=2006 & year!=2002 {
		   replace ilo_edu_isced97=1 if b4p1a==0                                // No schooling
		   replace ilo_edu_isced97=2 if b4p1a==1                                // Pre-primary education
		   replace ilo_edu_isced97=3 if b4p1a==2                                // Primary education or first stage of basic education
		   replace ilo_edu_isced97=4 if inlist(b4p1a,3,4)                       // Lower secondary education or second stage of basic education
		   replace ilo_edu_isced97=5 if inlist(b4p1a,5,6)                       // Upper secondary education
		   * replace ilo_edu_isced97=6 if                                       // Post-secondary non-tertiary education
		   replace ilo_edu_isced97=7 if inlist(b4p1a,7,8,9)                     // First stage of tertiary education (not leading directly to an advanced research qualification)
		   * replace ilo_edu_isced97=8 if                                       // Second stage of tertiary education (leading to an advanced research qualification)                               
		   replace ilo_edu_isced97=9 if ilo_edu_isced97==.                      // Level not stated 
		}
		if year==2007 {
		   replace ilo_edu_isced97=1 if b4p1a==1                                // No schooling
		   replace ilo_edu_isced97=2 if b4p1a==2                                // Pre-primary education
		   replace ilo_edu_isced97=3 if b4p1a==3                                // Primary education or first stage of basic education
		   replace ilo_edu_isced97=4 if inlist(b4p1a,4,5)                       // Lower secondary education or second stage of basic education
		   replace ilo_edu_isced97=5 if inlist(b4p1a,6,7)                       // Upper secondary education
		   * replace ilo_edu_isced97=6 if                                       // Post-secondary non-tertiary education
		   replace ilo_edu_isced97=7 if inlist(b4p1a,8,9,10)                    // First stage of tertiary education (not leading directly to an advanced research qualification)
		   * replace ilo_edu_isced97=8 if                                       // Second stage of tertiary education (leading to an advanced research qualification)                               
		   replace ilo_edu_isced97=9 if ilo_edu_isced97==.                      // Level not stated 
		}
...
*            definition here used is two criteria. (T5:1429)
	...
	gen ilo_lfs=.
	    if year==2002 {
		   replace ilo_lfs=1 if (b4br2a1==1) & ilo_wap==1	                           // Employed: engaged in work during the reference period
		   replace ilo_lfs=1 if (b4br3==1) & ilo_wap==1                                // Employed: temporary absent
		   replace ilo_lfs=2 if (ilo_lfs!=1 & (b4br4==1 | b4br5==1)) & ilo_wap==1      // Unemployed: two criteria: not in employment and seeking
		   replace ilo_lfs=2 if (ilo_lfs!=1 & b4er21==2) & ilo_wap==1                  // Unemployed: future starters
	       replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1                      // Outside the labour force
		}
		if year<=2006 & year!=2002 {
		   replace ilo_lfs=1 if (b4p2a1==1) & ilo_wap==1	                           // Employed: engaged in work during the reference period
		   replace ilo_lfs=1 if (b4p3==1) & ilo_wap==1                                 // Employed: temporary absent
		   replace ilo_lfs=2 if (ilo_lfs!=1 & (b4p4==1 | b4p5==1)) & ilo_wap==1        // Unemployed: two criteria: not in employment and seeking
		   replace ilo_lfs=2 if (ilo_lfs!=1 & b4p21==2) & ilo_wap==1                  // Unemployed: future starters
	       replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1                      // Outside the labour force
		}
		if year==2007 {
		   replace ilo_lfs=1 if (b4p2a1==1) & ilo_wap==1	                           // Employed: engaged in work during the reference period
		   replace ilo_lfs=1 if (b4p3==1) & ilo_wap==1                                 // Employed: temporary absent
		   replace ilo_lfs=2 if (ilo_lfs!=1 & (b4p4==1 | b4p5==1)) & ilo_wap==1        // Unemployed: two criteria: not in employment and seeking
		   replace ilo_lfs=2 if (ilo_lfs!=1 & b4p23==2) & ilo_wap==1                  // Unemployed: future starters
	       replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1                      // Outside the labour force
		}
...
* Comment: 
    ...
    gen ilo_mjh=.
	    if year==2002 {
		   replace ilo_mjh=1 if b4dr15==2 & ilo_lfs==1                          // One job only
		   replace ilo_mjh=2 if b4dr15==1 & ilo_lfs==1                          // More than one job
		}
		if year<=2006 & year!=2002 {
		   replace ilo_mjh=1 if b4p15==2 & ilo_lfs==1                           // One job only
		   replace ilo_mjh=2 if b4p15==1 & ilo_lfs==1                           // More than one job
		}
		if year==2007 {
		   replace ilo_mjh=1 if b4p17==2 & ilo_lfs==1                           // One job only
		   replace ilo_mjh=2 if b4p17==1 & ilo_lfs==1                           // More than one job
		}
...
*          - Casual employees in agriculture/non-agriculture are classified under employees.
   ...
   * ICSE 1993
   gen empstatus=.
       if year==2002 {
	      replace empstatus = b4cr10a
	   }
	   if year<=2006 & year!=2002 {
	      replace empstatus = b4p10
	   }
	   if year==2007 {
	      replace empstatus = b4p11a 
	   }
...
*            under code 51 of ISIC 3.1.
	...
	*---------------------------------------------------------------------------
	...
	gen indu_code_prim=.
	    if year==2002 {
	       replace indu_code_prim=int(b4cr7/10)
		}
	    if year<=2006 & year!=2002 {
	       replace indu_code_prim=int(b4p7/10)
		}
		if year==2007 {
	       replace indu_code_prim=int(b4p7/1000) if b4p7!=99999
		}
	...
	*---------------------------------------------------------------------------
	...
	gen indu_code_sec=.
    	if year==2002 {
		   replace indu_code_sec = int(b4dr16/10)
	    }	
		if year<=2006 & year!=2002 {
		   replace indu_code_sec = int(b4p16/10)
		}     
	    if year==2007 {
		   replace indu_code_sec = int(b4p18/1000) if b4p18!=99999
		}		
...
*          - 2001-2006: Classification according to the KJI (Occupations
*            Classification) 1982, linked to ISCO-68. Correspondences are made at
*            one-digit level due to the amount of discordances at 2-digit level.
*            * Professionals include technicians and associate professionals (note to value: C4:1002)
*            * Craft and related trades workers include plant and machine 
*              operators and assemblers and elementary occupations (note to value: C4:2162)
*          - No information available for secondary job.
    ...
    ***********
	...
	if year==2007 {
	   replace occ_code_prim=int(b4p8/100)
    ...
    * Skill level				
	...
	if year==2002 {
	replace occ_code_prim=int(b4cr8/100)
	   
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
		replace ilo_job1_ocu_isco88=10 if inlist(b4cr8,1,2,3,4,9) & ilo_lfs==1                             // Armed forces
		replace ilo_job1_ocu_isco88=1 if ilo_job1_ocu_isco88==. & occ_code_prim==2 & ilo_lfs==1            // Legislators, senior officials and managers
		replace ilo_job1_ocu_isco88=2 if ilo_job1_ocu_isco88==. & inlist(occ_code_prim,0,1) & ilo_lfs==1   // Professionals (including technicians) (C4:1002)
		* replace ilo_job1_ocu_isco88=3                                                                    // Technicians and associates
		replace ilo_job1_ocu_isco88=4 if ilo_job1_ocu_isco88==. & occ_code_prim==3 & ilo_lfs==1            // Clerks
		replace ilo_job1_ocu_isco88=5 if ilo_job1_ocu_isco88==. & inlist(occ_code_prim,4,5) & ilo_lfs==1   // Sales and services workers
		replace ilo_job1_ocu_isco88=6 if ilo_job1_ocu_isco88==. & occ_code_prim==6 & ilo_lfs==1            // Skilled agricultural and fishery workers
		replace ilo_job1_ocu_isco88=7 if ilo_job1_ocu_isco88==. & inlist(occ_code_prim,7,8,9) & ilo_lfs==1 // Craft and related trade workers (including 8 & 9) (C4:2162)
		* replace ilo_job1_ocu_isco88=8                                                                    // Plant and machine operators
		* replace ilo_job1_ocu_isco88=9                                                                    // Elementary occupations
    ...
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
		replace ilo_job1_ocu_isco88=10 if inlist(b4p8,1,2,3,4,9) & ilo_lfs==1                              // Armed forces
		replace ilo_job1_ocu_isco88=1 if ilo_job1_ocu_isco88==. & occ_code_prim==2 & ilo_lfs==1            // Legislators, senior officials and managers
		replace ilo_job1_ocu_isco88=2 if ilo_job1_ocu_isco88==. & inlist(occ_code_prim,0,1) & ilo_lfs==1   // Professionals (including technicians) (C4:1002)
		* replace ilo_job1_ocu_isco88=3                                                                    // Technicians and associates
		replace ilo_job1_ocu_isco88=4 if ilo_job1_ocu_isco88==. & occ_code_prim==3 & ilo_lfs==1            // Clerks
...
*          - No information available for secondary job.
	...
	* Hours ACTUALLY worked
	...
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = hawm  if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	...
	gen ilo_joball_how_actual = .
	    replace ilo_joball_how_actual = hawa if ilo_lfs==1
...
*          - Local currency: Indonesian Rupiah.
    ...
    ***********
	if year==2002 {
	   egen earn_cash_kind = rowtotal(b4cr12a b4cr12b), m
	}
	if year<=2006 & year!=2002 {
	   egen earn_cash_kind = rowtotal(b4p12a b4p12b), m
	}
	if year==2007 {
	   egen earn_cash_kind = rowtotal(b4p13a b4p13b), m
	}
...
cd "$outpath"
	...
	drop year empstatus indu_code_prim indu_code_sec occ_code_prim hawm hawa 
