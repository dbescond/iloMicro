...
*            definition here used is two criteria. (T5:1429)
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=2 if (ilo_lfs!=1 & b5p7!=2 & (b5p4==1 | b5p5==1)) & ilo_wap==1   // Unemployed: two criteria: not in employment and seeking
		replace ilo_lfs=2 if (ilo_lfs!=1 & b5p7!=2 & b5p6==2) & ilo_wap==1               // Unemployed: future starters
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 2011Q1: national classification mostly follow ISIC Rev. 3.1. 2-digit
*            Indonesian classification of industries 53 and 54 are exports/imports of goods,
*            except of motor vehicles and motorcycles and therefore are classified
*            under code 51 of ISIC 3.1.
*          - 2011Q3-2013Q3: national classification is based on ISIC Rev.4 at 
*            2-digit level. 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 2011Q1: not possible to map it to neither ISCO-88 nor ISCO-08 and 
*            therefore it is not defined.
*          - 2011Q3-2013Q3: national classification follow the Indonesian Standard
*            Occupational (KBJI) 2002. Possibility to map it to ISCO-88 at 
*            1-digit level. Missing values are coded as "0" which is the same as 
...
*          - No information available for secondary job.
  ...
  if time=="2011Q1" {
     * No possible to map it to neither 2-digit nor 1-digit level.
  }
...
* Comment:
	 ...
	 gen ilo_olf_reason=.
		 replace ilo_olf_reason=1 if b5p6==1 & ilo_lfs==3                       // Labour market 
		 * replace ilo_olf_reason=2 if                                          // Other labour market reasons
		 replace ilo_olf_reason=3 if inlist(b5p6,3,4,7) & ilo_lfs==3            // Personal/Family-related
		 replace ilo_olf_reason=4 if b5p6==6 & ilo_lfs==3                       // Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & b5p7==1 & ilo_olf_reason==1
