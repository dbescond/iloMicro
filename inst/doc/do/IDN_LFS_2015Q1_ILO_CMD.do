* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Indonesia, 2015
* DATASET USED: Indonesia, LFS, 2015
* NOTES: 
...
*          - Category master's or equivalent level includes category doctoral or
*            equivalent level.
*          - Mapping is based on:
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		...
		* replace ilo_edu_isced11=6 if                                          // Post-secondary non-tertiary education
		...
		replace ilo_edu_isced11=8 if educ_level==13                             // Bachelor's or equivalent level
...
*            those answering yes to job/business seeking and who are not "not
*            looking for a job" (given that the question is not answered by all);
*            it also includes future starters.

	gen ilo_lfs=.
		replace ilo_lfs=1 if (b5_r2a1==1) & ilo_wap==1	                                    // Employed: engaged in work during the reference period
		replace ilo_lfs=1 if (b5_r3==1) & ilo_wap==1                                        // Employed: temporary absent
		replace ilo_lfs=2 if (ilo_lfs!=1 & b5_r7!=2 & (b5_r4==1 | b5_r5==1)) & ilo_wap==1   // Unemployed
		replace ilo_lfs=2 if (ilo_lfs!=1 & b5_r7!=2 & b5_r6==2) & ilo_wap==1                // Unemployed: future starters
...
* Comment:
  ...
  capture confirm var b5p17 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if add_job==2 & ilo_lfs==1                                 // One job only
		replace ilo_mjh=2 if add_job==1 & ilo_lfs==1                                 // More than one job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification is based on ISIC Rev.4 at 2-digit level.
	...
	*---------------------------------------------------------------------------
	gen indu_code_prim = .
	    replace indu_code_prim = kbli2009
   ...
   * Aggregate level
	...
	*---------------------------------------------------------------------------
	...
	if !_rc {
	   gen indu_code_sec = int(b5_r18/1000) if b5_r18!=99999
	}
	else {					
	   gen indu_code_sec = int(b5p18/1000) if b5p18!=99999
	}
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification follows the Indonesian Standard
*            Occupational (KBJI) 2002. Possibility to map it to ISCO-88 at 
*            1-digit level. 
*          - missing values are coded as "0" which is the same as 
*            for armed forces; given that it is not possible to identify both 
*            separately, they are classified under "Not elsewhere classified".
*          - No information available for secondary job.
    ...
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
	    replace ilo_job1_ocu_isco88 = kbji2002 if kbji2002>0 & ilo_lfs==1
...
*          - No information available for secondary job.
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    ...
	    if !_rc {
           replace ilo_job1_how_actual=b5p11 if ilo_lfs==1
	    }
	    else {
           replace ilo_job1_how_actual=b5_r11 if ilo_lfs==1
	    }	
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual = .
		...
		if !_rc {
		   replace ilo_joball_how_actual=b5p8b if ilo_lfs==1
		}
		else {
		   replace ilo_joball_how_actual=b5_r8b if ilo_lfs==1
		}
...
*          - Local currency: Indonesian Rupiah.
    ...
    ***********
	...
	if !_rc {
	   egen earn_cash_kind = rowtotal(b5p13a b5p13b), m 
	}
	else {
	   egen earn_cash_kind = rowtotal(b5_r13a b5_r13b), m 
	}
...
*            another to the amount of months.
    ...
    gen tot_month_une=.
        replace tot_month_une = 12*b5_r21a + b5_r21b if b5_r21a!=99 & b5_r21b!=99
...
*            3 and 4 are not possible to be produced. 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if (b5_r4==1 | b5_r5==1) & b5_r7==2 & ilo_lfs==3 			// Seeking, not available
		replace ilo_olf_dlma = 2 if (b5_r4==2 & b5_r5==2) & b5_r7==1 & ilo_lfs==3			// Not seeking, available
...
* Comment:
	 ...
	 gen ilo_olf_reason=.
		 replace ilo_olf_reason=1 if b5_r6==1 & ilo_lfs==3                      // Labour market 
		 * replace ilo_olf_reason=2 if                                          // Other labour market reasons
		 replace ilo_olf_reason=3 if inlist(b5_r6,3,4,7) & ilo_lfs==3           // Personal/Family-related
		 replace ilo_olf_reason=4 if b5_r6==6 & ilo_lfs==3                      // Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & b5_r7==1 & ilo_olf_reason==1
...
cd "$outpath"
	...
	drop time year quarter add_job empstatus indu_code_prim indu_code_sec earn_cash_kind 
	drop cat_une tot_month_une
	drop marital
	compress
