...
* Comment:
	   ...
	   gen ilo_sex=.
	       replace ilo_sex=1 if b4_k4==1      // Male
		   replace ilo_sex=2 if b4_k4==2      // Female
...
*          - No schooling is determined from question 8 in section 4; as for the
*            rest of levels of education question 1 in section 5 is used.
*          - Mapping is based on:
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Those persons answering "little/some" disability are classified as
*            without disability.

	gen ilo_dsb_aggregate=.
		replace ilo_dsb_aggregate=2 if b5_r4a==3                                // Vision
		replace ilo_dsb_aggregate=2 if b5_r4b==6                                // Hearing
		replace ilo_dsb_aggregate=2 if b5_r4c==3                                // Walking/climbing stairs
		replace ilo_dsb_aggregate=2 if b5_r4d==6                                // Using/moving fingers/hand
		replace ilo_dsb_aggregate=2 if b5_r4e==3                                // Talking and understanding/communicating with others
		replace ilo_dsb_aggregate=2 if b5_r4f==6                                // Others
...
*            period.
*          - Unemployment: the national definition includes discouraged job-seekers
*            and does not consider the availability to work as one of the criteria.
*            Following the questionnaire skip pattern, unemployment here includes
*            those answering yes to job/business seeking (previous week in 2016;
*            previous week or previus month in 2017) and who are not "not
*            looking for a job" (given that the question is not answered by all);
*            it also includes future starters.

	gen ilo_lfs=.
	if year==2016 {
		replace ilo_lfs=1 if (b5_r5a1==1) & ilo_wap==1	                                                        // Employed: engaged in work during the reference period
		replace ilo_lfs=1 if (b5_r6==1) & ilo_wap==1                                                            // Employed: temporary absent
		replace ilo_lfs=2 if ilo_lfs!=1 & (b5_r17a!=2 & (b5_r11==1 | b5_r12==1)) & ilo_wap==1                   // Unemployed
		replace ilo_lfs=2 if ilo_lfs!=1 & (b5_r17a!=2 & (inlist(b5_r16a,1,2) | b5_r16b==1)) & ilo_wap==1        // Unemployed: future starters
	    replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1                                                  // Outside the labour force
	}
	else {
		replace ilo_lfs=1 if (b5_r5a1==1) & ilo_wap==1	                                                                      // Employed: engaged in work during the reference period
		replace ilo_lfs=1 if (b5_r6==1) & ilo_wap==1                                                                          // Employed: temporary absent
		replace ilo_lfs=2 if ilo_lfs!=1 & (b5_r21a!=2 & (b5_r15a==1 | b5_r15b==1 | b5_r16a==1 |  b5_r16b==1))  & ilo_wap==1   // Unemployed
		replace ilo_lfs=2 if ilo_lfs!=1 & (b5_r21a!=2 & (inlist(b5_r20a,1,2) | b5_r20b==1)) & ilo_wap==1                      // Unemployed: future starters
	    replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1                                                               // Outside the labour force
	}
...
* Comment:
  ...
  gen ilo_mjh=.
  if year==2016{
	 replace ilo_mjh=1 if b5_r34==2 & ilo_lfs==1                                // One job only
	 replace ilo_mjh=2 if b5_r34==1 & ilo_lfs==1                                // More than one job
  }
  else {
	 replace ilo_mjh=1 if b5_r38a==2 & ilo_lfs==1                               // One job only
	 replace ilo_mjh=2 if b5_r38a==1 & ilo_lfs==1                               // More than one job 
  }
...
*          - Casual employees in agriculture/non-agriculture are classified under employees.
  ...
  capture confirm var b5_r23 
  if !_rc  {
     gen empstatus = b5_r23
  }
  else {
     gen empstatus = b5_r27a
  }
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 2016Q1: national classification of industries is based on ISIC Rev.4
*            at 2-digit level.
*          - 2016Q3-2017Q3: classification available is possible to map it at the
*            aggregate level.
	...
	* ISIC REV 4
	*---------------------------------------------------------------------------
    gen indu_code_prim = int(b5_r19_kbli2015_3/10)
   ...
   * Aggregate level
	...
	* ISIC REV 4
	*---------------------------------------------------------------------------			   
	gen indu_code_sec=int(b5_r35_kbli2015_3/10)
	
		* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = indu_code_sec if indu_code_sec>0 & ilo_mjh==2
   ...
   * Aggregate level
	...
	*---------------------------------------------------------------------------
    if time=="2016Q3" {
	   gen indu_code_prim = b5_r19_17
	}
	if year==2017 {
	   gen indu_code_prim = b5_r23_17
	}
	
	* Aggregated level 
	gen ilo_job1_eco_aggregate=.
		replace ilo_job1_eco_aggregate=1 if indu_code_prim==1 & ilo_lfs==1
		replace ilo_job1_eco_aggregate=2 if indu_code_prim==3 & ilo_lfs==1
		replace ilo_job1_eco_aggregate=3 if indu_code_prim==6 & ilo_lfs==1
		replace ilo_job1_eco_aggregate=4 if inlist(indu_code_prim,2,4,5) & ilo_lfs==1
		replace ilo_job1_eco_aggregate=5 if inrange(indu_code_prim,7,13) & ilo_lfs==1
		replace ilo_job1_eco_aggregate=6 if inrange(indu_code_prim,14,17) & ilo_lfs==1
	...
	*---------------------------------------------------------------------------
	if time=="2016Q3" {
	   gen indu_code_sec = b5_r35_17 
	}
	if year==2017 {
	   gen indu_code_sec = b5_r39_17
	}

	* Aggregated level 
	gen ilo_job2_eco_aggregate=.
		replace ilo_job2_eco_aggregate=1 if indu_code_sec==1 & ilo_mjh==2
		replace ilo_job2_eco_aggregate=2 if indu_code_sec==3 & ilo_mjh==2
		replace ilo_job2_eco_aggregate=3 if indu_code_sec==6 & ilo_mjh==2
		replace ilo_job2_eco_aggregate=4 if inlist(indu_code_sec,2,4,5) & ilo_mjh==2
		replace ilo_job2_eco_aggregate=5 if inrange(indu_code_sec,7,13) & ilo_mjh==2
		replace ilo_job2_eco_aggregate=6 if inrange(indu_code_sec,14,17) & ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 2016Q1: national classification follows the Indonesian Standard
*            Occupational (KBJI) 2002. Possibility to map it to ISCO-88 at 
*            1-digit level. 
*          - 2016Q3-2017Q3: national classification follow the Indonesia Standard
*            Occupational (KBJI 2014). Possiblity to map it to ISCO-08 at 1-digit
*            level.
*          - No information available for secondary job.
  ...
  if time=="2016Q1" {
	...
	* ISCO 88
	*---------------------------------------------------------------------------
	gen occ_code_prim = b5_r20_kbji2002
	
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
		replace ilo_job1_ocu_isco88=occ_code_prim if ilo_lfs==1
		replace ilo_job1_ocu_isco88=10 if occ_code_prim==0 & ilo_lfs==1
	...
	*---------------------------------------------------------------------------
  	if time=="2016Q3" {
	   gen occ_code_prim = b5_r20_201
	}
	if year==2017 {
	   gen occ_code_prim = b5_r24_201
	}
	
	* 1-digit level
	gen ilo_job1_ocu_isco08 = occ_code_prim if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 2017Q1-Q3: Not possible to isolate public and private from category
*            1 in question 35 and therefore the variable is not defined.
*          - 2016Q1-Q3: Public: Government.

      
	if year==2016{
	  * MAIN JOB
	  gen ilo_job1_ins_sector=.
		  replace ilo_job1_ins_sector=1 if (b5_r31==1 & ilo_lfs==1)	                // Public
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    if year==2017 {
	       replace ilo_job1_how_usual = b5_r26b if ilo_lfs==1
		}
		if year==2016 {
		   replace ilo_job1_how_usual = b5_r22b if ilo_lfs==1
		}
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    if year==2017 {
	       replace ilo_job1_how_actual = b5_r26a if ilo_lfs==1
		}
		if year==2016 {
		   replace ilo_job1_how_actual = b5_r22a if ilo_lfs==1
		}
	...
	* Hours ACTUALLY worked
	if year==2017 {
	   gen ilo_job2_how_actual = .
	       replace ilo_job2_how_actual =  b5_r41 if ilo_mjh==2
	...
	* Hours USUALLY worked
	...
	if year==2017 {
	    replace ilo_joball_how_usual = b5_r43b if ilo_lfs==1
	}
	if year==2016 {
	   replace ilo_joball_how_usual = b5_r37b if ilo_lfs==1
	}
	...
	* Hours ACTUALLY worked
	...
	if year==2017 {
	    replace ilo_joball_how_actual = b5_r43a if ilo_lfs==1
	}
	if year==2016 {
	    replace ilo_joball_how_actual = b5_r37a if ilo_lfs==1
	}
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 35 hours per week usually worked in main job is set as a threshold
*            (normal working time in the country). 
			
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if ilo_job1_how_usual>=35 & ilo_job1_how_usual>0 & ilo_lfs==1    // Full-time
		replace ilo_job1_job_time=1 if ilo_job1_how_usual<35 & ilo_job1_how_usual!=. & ilo_lfs==1    // Part-time
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Oral agreements are considered as temporary contracts. Also, if no
*            contract available it is considered as temporary contract.
 
 if year==2016	{
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if b5_r29==1 & ilo_lfs==1                   // Permanent
		replace ilo_job1_job_contract=2 if inlist(b5_r29,2,3,4) & ilo_lfs==1        // Temporary
 ...
 if year==2017 {
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if b5_r33==1 & ilo_lfs==1                   // Permanent
		replace ilo_job1_job_contract=2 if inlist(b5_r33,2,3,4) & ilo_lfs==1        // Temporary
		replace ilo_job1_job_contract=3 if ilo_job1_job_contract==. & ilo_lfs==1  	// Unknown
			    lab def job_contract_lab 1 "1 - Permanent" 2 "2 - Temporary" 3 "3 - Unknown"
			    lab val ilo_job1_job_contract job_contract_lab
			    lab var ilo_job1_job_contract "Job (Type of contract)"
...
* ------------------------------------------------------------------------------
* Comment:
/* 2017 -- Useful questions:
          - Institutional sector: b5_r35 (don't use it for identifying the public sector (first option too aggregate) but only for the identification of households)
		  - Private household identification: (from above)
		  - Destination of production: b5_r12/b5_r13a/b5_r29 (not used, too ambiguous)
		  - Bookkeeping: b5_r28
		  - Registration: not asked.
		  - Status in employment: ilo_job1_ste_aggregate
		  - Social security contribution (Proxy: pension funds): b5_r32d/b5_r32e/b5_r32f (pension plan (lump sum/annuity)/(annual/sick/childbirth leave))
		  - Place of work: b5_r36 (fixed and non-fixed premises are not identifiable. not used.)
		  - Size: not asked.
*/
/* 2016 -- Useful questions:
          - Institutional sector: b5_r31
		  - Private household identification: (from above)
		  - Destination of production: b5_r25 (not used, too ambiguous)
		  - Bookkeeping: b5_r24
		  - Registration: not asked.
		  - Status in employment: ilo_job1_ste_aggregate
		  - Social security contribution (Proxy: pension funds): b5_r28c/b5_r28d/b5_r28f (old-age/pension insurance/(annual/sick/childbirth leave))
		  - Place of work: b5_r32
		  - Size: not asked.
*/
* 
if time=="2017Q3" {    
    ...
    * Social Security:
	gen social_security=.
	    replace social_security=1 if (b5_r32d==4 | b5_r32e==1) | (!inlist(b5_r32d,4,5) & !inlist(b5_r32d,1,2) & b5_r32f==4) & ilo_lfs==1          // social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((b5_r35!=4 & inlist(b5_r28,2,3)) | ///
									   (b5_r35!=4 & !inlist(b5_r28,2,3) & ilo_job1_ste_aggregate==1 & social_security==1))
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & ///
		                               (b5_r35==3)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((b5_r35!=3 & inlist(b5_r28,2,3)) | ///
									   (b5_r35!=3 & !inlist(b5_r28,2,3) & ilo_job1_ste_aggregate==1 & social_security==1))
		replace ilo_job1_ife_prod=1 if ilo_lfs==1 & !inlist(ilo_job1_ife_prod,2,3)
				lab def ilo_ife_prod_lab 1 "1 - Informal" 2 "2 - Formal" 3 "3 - Household" 
				lab val ilo_job1_ife_prod ilo_ife_prod_lab
				lab var ilo_job1_ife_prod "Informal / Formal Economy (Unit of production)"
...
if year==2016 {
    ...
    * Social Security:
	gen social_security=.
	    replace social_security=1 if (b5_r28c==1 | b5_r28d==4) | (!inlist(b5_r28c,1,2) & !inlist(b5_r28d,4,5) & b5_r28f==4) & ilo_lfs==1          // social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(b5_r31,1,2,3)) | ///
									   (!inlist(b5_r31,1,2,3,7) & inlist(b5_r24,2,3)) | ///
									   (!inlist(b5_r31,1,2,3,7) & !inlist(b5_r24,2,3) & ilo_job1_ste_aggregate==1 & social_security==1))
		replace ilo_job1_ife_prod=1 if ilo_lfs==1 & !inlist(ilo_job1_ife_prod,2,3)
				lab def ilo_ife_prod_lab 1 "1 - Informal" 2 "2 - Formal" 3 "3 - Household" 
				lab val ilo_job1_ife_prod ilo_ife_prod_lab
				lab var ilo_job1_ife_prod "Informal / Formal Economy (Unit of production)"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------			
* Comment: - Earnings in cash and/or in kind. Only possible to define for employees.
*          - Local currency: Indonesian Rupiah.

if time=="2017Q3"{
   * Employees
   egen ilo_job1_lri_ees = rowtotal(b5_r30b1 b5_r30b2 b5_r30c11 b5_r30c12 b5_r30c21 b5_r30c22), m
        replace ilo_job1_lri_ees =. if ilo_job1_ste_aggregate!=1
		   	    lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"	

   * Self-employed
   * No information available for all self-employed (only for own-account workers but not for the rest).
}
	
if time=="2017Q1"{
   * Employees
   egen ilo_job1_lri_ees = rowtotal(b5_r30a1 b5_r30a2 b5_r30b11 b5_r30b12 b5_r30b21 b5_r30b22 b5_r30b31 b5_r30b32), m
        replace ilo_job1_lri_ees =. if ilo_job1_ste_aggregate!=1
		   	    lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"	
...
if year==2016 {
   * Employees
   egen ilo_job1_lri_ees = rowtotal(b5_r26a b5_r26b), m
        replace ilo_job1_lri_ees=. if ilo_job1_ste_aggregate!=1
...
* Comment:
 ...
 if year==2017 {
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<35  &  b5_r44a==1 & b5_r44b==1  & ilo_lfs==1
 }
 if year==2016 {
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<35  &  b5_r38a==1 & b5_r38b==1  & ilo_lfs==1
 }
...
* Comment:
  ...
  if year==2016 {
	 ...
	 gen ilo_cat_une=.
		 ...
		 replace ilo_cat_une=2 if cat_une==2 & ilo_lfs==2 						// Seeking first job
		 replace ilo_cat_une=3 if !inlist(ilo_cat_une,1,2) & ilo_cat_une!=1     // Unknown
...
* Comment:
  ...
  if year==2016 {
	 ...
	 gen ilo_dur_details=.
		 replace ilo_dur_details=1 if tot_month_une<1                           // Less than 1 month
	 	 replace ilo_dur_details=2 if tot_month_une>=1 & tot_month_une<3        // 1 to 3 months
		 replace ilo_dur_details=3 if tot_month_une>=3 & tot_month_une<6        // 3 to 6 months
		 replace ilo_dur_details=4 if tot_month_une>=6 & tot_month_une<12       // 6 to 12 months
		 replace ilo_dur_details=5 if tot_month_une>=12 & tot_month_une<24      // 12 to 24 months
		 replace ilo_dur_details=6 if tot_month_une>=24 & tot_month_une!=.      // 24 months or more
		 replace ilo_dur_details=7 if tot_month_une==. & ilo_lfs==2             // Not elsewhere classified
	...
	gen ilo_dur_aggregate=.
		replace ilo_dur_aggregate=1 if inrange(ilo_dur_details,1,3)             // Less than 6 months
		replace ilo_dur_aggregate=2 if ilo_dur_details==4                       // 6 to 12 months
		replace ilo_dur_aggregate=3 if inrange(ilo_dur_details,5,6)             // 12 months or more
		replace ilo_dur_aggregate=4 if ilo_dur_details==7                       // Not elsewhere classified
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Given that variable capturing the economic activitiy related to previous
*            job only considers that were left max 1 year ago and not any previous
*            job, variable is not being defined.
...
* Comment: 				 
  ...
  if year==2017 {
     gen olf_reason = b5_r20a
  }	
  if year==2016 {
     gen olf_reason = b5_r16a
  }
	
	 gen ilo_olf_reason=.		
		 replace ilo_olf_reason=1 if olf_reason==3 & ilo_lfs==3					    // Labour market (discouraged) 
		 * replace ilo_olf_reason=2 if 									            // Other labour market reasons
		 replace ilo_olf_reason=3 if inlist(olf_reason,6,7,8,10,11) & ilo_lfs==3	// Personal / Family-related
		 replace ilo_olf_reason=4 if olf_reason==5 & ilo_lfs==3  					// Does not need/want to work
...
* Comment:
  ...
  if year==2017 {
	 gen ilo_dis=.
	     replace ilo_dis=1 if !inlist(ilo_lfs,1,2) &  b5_r21a==1 & ilo_olf_reason==1
  }
  if year==2016 {
	 gen ilo_dis=.
	     replace ilo_dis=1 if !inlist(ilo_lfs,1,2) &  b5_r17a==1 & ilo_olf_reason==1
  }
...
cd "$outpath"
	...
	drop empstatus indu_code_prim indu_code_sec occ_code_prim social_security cat_une tot_month_une olf_reason
	drop marital
	compress
