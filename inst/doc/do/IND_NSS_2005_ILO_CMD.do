...
cd "$inpath"
	...
	* omit observations not having any information about the sex of the person
...
*			82 did not seek but was available for work (for current weekly status approach)
	...
	gen curr_lfs=.
		replace curr_lfs=1 if inrange(Curr_Day_Act_Status_1,11,72) | Curr_Day_Act_Status_1==98
		replace curr_lfs=2 if curr_lfs!=1 & Curr_Day_Act_Status_1==81
...
/* 
	...
	gen curr_mjh=.
		replace curr_mjh=1 if Curr_Day_Act_Status_2==.
		replace curr_mjh=2 if Curr_Day_Act_Status_2!=. & curr_lfs==1
...
use the same variable that was used to identify people in employment, as it contains information on the status in employment
	...
	gen curr_job1_ste_icse93=.
		replace curr_job1_ste_icse93=1 if inlist(Curr_Day_Act_Status_1,31,71,72)
		replace curr_job1_ste_icse93=2 if Curr_Day_Act_Status_1==12
		replace curr_job1_ste_icse93=3 if inlist(Curr_Day_Act_Status_1,11,41,42,51)
		/*replace curr_job1_ste_icse93=4 if */
		replace curr_job1_ste_icse93=5 if inlist(Curr_Day_Act_Status_1,21,61,62)
...
/* Classification used: ISIC Rev. 3 (or NIC 1998) - given the minimal differences and the unavailability of ISIC Rev. 3 in the system, use ISIC Rev. 3.1 */
	...
	* no info on usual subsidiary activity
	...
	gen indu_code_prim=int(Usual_principal_activity_NIC_5_d/1000) if ilo_lfs==1
...
*		
	...
	* Useful questions: Enterprise_Type: Institutional sector --> use as a proxy the variable Enterprise_Type (in order to identify the public and the household sector)
					...
					*	Location_of_workplace: Location of workplace
		...
		gen soc_sec_cov=inlist(Social_security_benefits,1,4,5,7) | ( !inlist(Social_security_benefits,1,4,5,7) & Eligible_for_paid_leave==1)
			...
			gen ilo_job1_ife_prod=.
				replace ilo_job1_ife_prod=1 if !inlist(Enterprise_type,5,6,7,8) & ((ilo_job1_ste_aggregate==1 & soc_sec_cov!=1) | ilo_job1_ste_aggregate!=1) & ///
						(!inlist(Location_of_workplace,14,16,24,26) | (inlist(Location_of_workplace,14,16,24,26) & !inlist(No_of_workers_in_enterprises,2,3,4)))
				replace ilo_job1_ife_prod=2 if inlist(Enterprise_type,5,6,7) | (!inlist(Enterprise_type,5,6,7,8) & ilo_job1_ste_aggregate==1 & soc_sec_cov==1) | ///
						(!inlist(Enterprise_type,5,6,7,8) & ((ilo_job1_ste_aggregate==1 & soc_sec_cov!=1) | ilo_job1_ste_aggregate!=1) & inlist(Location_of_workplace,14,16,24,26) & inlist(No_of_workers_in_enterprises,2,3,4))
