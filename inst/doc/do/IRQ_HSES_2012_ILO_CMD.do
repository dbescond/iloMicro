* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Iraq, 2012
* DATASET USED: Iraq, Iraq Household Socio Economic Survey (SES), 2012
* NOTES: 
* Files created: Standard variables IRQ_SES_2012_FULL.dta and IRQ_SES_2012_ILO.dta
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if q00_16==1         // Urban 
		replace ilo_geo=2 if q00_16==2         // Rural
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 28 observations without information on sex of the respondent and
*            therefore deleted.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 1 observation without information about age of the respondant and
*            therefore deleted.
	
	gen ilo_age=.
	    replace ilo_age=q0104 
		        lab var ilo_age "Age"
...
*            http://uis.unesco.org/en/isced-mappings
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if (q0502==2)                                                     // No schooling
		replace ilo_edu_isced11=2 if (q0502==1 & q0503==1) | (q0502==3 & q0507_1==1)                // Early childhood education
		replace ilo_edu_isced11=3 if (q0502==1 & q0503==2) | (q0502==3 & inlist(q0507_1,2,3))       // Primary education
		replace ilo_edu_isced11=4 if (q0502==1 & inlist(q0503,3,4)) | (q0502==3 & q0507_1==4)       // Lower secondary education
		replace ilo_edu_isced11=5 if (q0502==1 & inlist(q0503,5,6)) | (q0502==3 & q0507_1==5)       // Upper secondary education
		* replace ilo_edu_isced11=6 if                                                              // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if (q0502==1 & q0503==7) | (q0502==3 & q0507_1==6)                // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if (q0502==1 & inlist(q0503,8,9)) | (q0502==3 & q0507_1==7)       // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if (q0502==1 & q0503==10)                                         // Master's or equivalent level
		replace ilo_edu_isced11=10 if (q0502==1 & q0503==11)                                        // Doctoral or equivalent level
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if q0502==3                                // Attending
		replace ilo_edu_attendance=2 if inlist(q0502,1,2)                       // Not attending
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Original question refers to any "continuous, medically diagnosed
*            disability".
*          - Persons not answering are classified under "No, no difficulty".
*          - For the aggregate version of the indicator, persons with disability
*            are those answering "(3) Yes, a lot of difficulty" or " (4) Cannot 
*            do it at all".

    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=2 if q0602==1 & q0603==1     	                // Yes, some difficulty
		replace ilo_dsb_details=3 if q0602==1 & q0603==2    	                // Yes, a lot of difficulty
		replace ilo_dsb_details=4 if q0602==1 & q0603==3  		                // Cannot do it at all
		replace ilo_dsb_details=1 if ilo_dsb_details==.                         // No, no difficulty
...
*            as a reason for not looking for a job but as a reason for not working)).
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q0801>0 & q0801!=.) & ilo_wap==1					        // Employed: working more than one hour during the reference week.
		replace ilo_lfs=1 if (q0801==0 & q0802==10) & ilo_wap==1				        // Employed: temporary absent
		replace ilo_lfs=2 if (ilo_lfs!=1 & inlist(q0805,1,2) & q0808==1) & ilo_wap==1	// Unemployed: three criteria
		replace ilo_lfs=2 if (ilo_lfs!=1 & q0801==0 & q0802==4) & ilo_wap==1	        // Unemployed: available future starters
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=2 if (jsn_2==2) & ilo_lfs==1                            // More than one job
...
*            employees. non-wage jobs to self-employed.
	...
	* Aggregate categories 
	  gen ilo_job1_ste_aggregate=.
		  replace ilo_job1_ste_aggregate=1 if (q1302_1_1==1 | q1302_2_1==1) & ilo_lfs==1                                // Employees
		  replace ilo_job1_ste_aggregate=2 if ilo_job1_ste_aggregate!=1 & (q1302_3_1==1 | q1302_4_1==1) & ilo_lfs==1    // Self-employed
	...
	* Aggregate categories
	  gen ilo_job2_ste_aggregate=.
		  replace ilo_job2_ste_aggregate=1 if (q1302_1_2==1 | q1302_2_2==1) & ilo_mjh==2                                // Employees
		  replace ilo_job2_ste_aggregate=2 if ilo_job2_ste_aggregate!=1 & (q1302_3_2==1 | q1302_4_2==1) & ilo_mjh==2    // Self-employed
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Given than section 14 is only related to employees (paid job-holder)
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification is mainly following ISCO-08 at 2-digit level.

    ***********
    * MAIN JOB:
    ***********
	destring q1301_occ_1, replace
	gen ocu_code_prim=.
	    replace ocu_code_prim=int(q1301_occ_1/10000)
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = ocu_code_prim if ilo_lfs==1
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
	    replace ilo_job1_ocu_isco08=11 if inlist(ilo_job1_ocu_isco08_2digits,0,6,9,39,77,.) & ilo_lfs==1                          // Not elsewhere classified
    ...
    * SECOND JOB:
    *************
	destring q1301_occ_2, replace
	gen ocu_code_secu=.
	    replace ocu_code_secu=int(q1301_occ_2/10000)
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = ocu_code_secu if ilo_mjh==2
	...
	* 1-digit level 				
	gen ilo_job2_ocu_isco08=.
	    replace ilo_job2_ocu_isco08=11 if inlist(ilo_job2_ocu_isco08_2digits,0,45,56,.) & ilo_mjh==2                 // Not elsewhere classified
...
*          - Public: governmental sector, public sector.
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(q1403_1,1,2) & ilo_lfs==1       // Public
	...
	gen ilo_job2_ins_sector=.
		replace ilo_job2_ins_sector=1 if inlist(q1403_2,1,2) & ilo_mjh==2       // Public
...
*            employed people are classified under "not elsewhere classified".
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = q1410_1 if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	egen ilo_job1_how_actual = rowtotal(q1304_1_1 q1304_2_1 q1304_3_1 q1304_4_1 q1304_5_1 q1304_6_1 q1304_7_1), m
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual = .
	    replace ilo_job2_how_usual = q1410_2 if ilo_mjh==2
	...
	* Hours ACTUALLY worked
    egen ilo_job2_how_actual = rowtotal(q1304_1_2 q1304_2_2 q1304_3_2 q1304_4_2 q1304_5_2 q1304_6_2 q1304_7_2), m
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - From the national report: work is considered as full time work if 
*            the working hours is no less than 6 hours a day (36 hours per week 
*            assuming 6 days per week).
			
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if ilo_joball_how_usual>=36 & ilo_lfs==1                             // Full-time
		replace ilo_job1_job_time=1 if ilo_joball_how_usual<36 & ilo_joball_how_usual!=. & ilo_lfs==1    // Part-time
...
*            defined.
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if inlist(q0805,1,2) &  q0808==2  & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if !inlist(q0805,1,2) & q0808==1  & ilo_lfs==3	     // Not seeking, available
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(q0804,1,6) & ilo_lfs==3          // Labour market 
			* replace ilo_olf_reason=2 if                                       // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(q0804,4,5,8,12) & ilo_lfs==3     // Personal/Family-related
			replace ilo_olf_reason=4 if inlist(q0804,3,10) & ilo_lfs==3         // Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & q0808==1  & ilo_olf_reason==1
