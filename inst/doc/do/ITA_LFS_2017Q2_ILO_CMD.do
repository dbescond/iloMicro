...
**			therefore this geographical units are too big to be able to do the distinction between urban/rural area. 
...
/*
destring stacim, replace
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if stacim==1                                  // Single
		replace ilo_mrts_details=2 if stacim==2                                  // Married
		*replace ilo_mrts_details=3 if                                           // Union / cohabiting
		replace ilo_mrts_details=4 if stacim==6                                  // Widowed
		replace ilo_mrts_details=5 if inlist(stacim,3,4,5)                         // Divorced / separated
...
destring cond3, replace
	...
	gen ilo_lfs =.
	    ...
	    ** UNEMPLOYED 
		** Comment: the questionnaire, for unemployed people (persons answering yes in F8), skips the question about availability (f13). 
		** 			Therefore we have to use the LFS variable computed by ISTAT. 
...
**** Comment: ISTAT is using a national classification for the occupations ("La Classificazione per unitÃ  professionali 2011"). 
 ...
 * MAIN JOB	
 ...
 destring prof3, replace
    ...
    * 1-digit level
		...
		replace ilo_job1_ocu_isco08 = 1 if inrange(occ_code_prim,11,13) & ilo_lfs==1  // "1 - Managers"
		replace ilo_job1_ocu_isco08 = 2 if inrange(occ_code_prim,21,26) & ilo_lfs==1  // "2 - Professional"
		replace ilo_job1_ocu_isco08 = 3 if inrange(occ_code_prim,31,34) & ilo_lfs==1  // "3 - Technicians and associate professionals"
		replace ilo_job1_ocu_isco08 = 4 if inrange(occ_code_prim,41,44) & ilo_lfs==1  // "4 - Clerical support workers"
		replace ilo_job1_ocu_isco08 = 5 if inrange(occ_code_prim,51,54) & ilo_lfs==1  // "5 - Service and sales workers"
		replace ilo_job1_ocu_isco08 = 6 if occ_code_prim == 64  & ilo_lfs==1  // "6 - Skilled agricultural, forestry and fishery workers"
        replace ilo_job1_ocu_isco08 = 7 if inlist(occ_code_prim,61,62,63,65)  & ilo_lfs==1  // "7 - Craft and related trades workers"
        replace ilo_job1_ocu_isco08 = 8 if inrange(occ_code_prim,71,74)  & ilo_lfs==1  // "8 - Plant and machine operators, and assemblers"
        replace ilo_job1_ocu_isco08 = 9 if inrange(occ_code_prim,81,84)  & ilo_lfs==1  // "9 - Elementary occupations"
        replace ilo_job1_ocu_isco08 = 10 if inrange(occ_code_prim,91,93)  & ilo_lfs==1  // "0 - Armed forces occupations"
		replace ilo_job1_ocu_isco08 = 11 if occ_code_prim==.  & ilo_lfs==1  // "X - Not elsewhere classified"
...
destring prof3s, replace
    ...
    * 1-digit level
	gen ilo_job2_ocu_isco08 =.
		...
		replace ilo_job2_ocu_isco08 = 1 if inrange(occ_code_sec,11,13) & ilo_mjh==2  // "1 - Managers"
		replace ilo_job2_ocu_isco08 = 2 if inrange(occ_code_sec,21,26) & ilo_mjh==2  // "2 - Professional"
		replace ilo_job2_ocu_isco08 = 3 if inrange(occ_code_sec,31,34) & ilo_mjh==2  // "3 - Technicians and associate professionals"
		replace ilo_job2_ocu_isco08 = 4 if inrange(occ_code_sec,41,44) & ilo_mjh==2  // "4 - Clerical support workers"
		replace ilo_job2_ocu_isco08 = 5 if inrange(occ_code_sec,51,54) & ilo_mjh==2  // "5 - Service and sales workers"
		replace ilo_job2_ocu_isco08 = 6 if occ_code_sec == 64  & ilo_mjh==2          // "6 - Skilled agricultural, forestry and fishery workers"
        replace ilo_job2_ocu_isco08 = 7 if inlist(occ_code_sec,61,62,63,65) & ilo_mjh==2  // "7 - Craft and related trades workers"
        replace ilo_job2_ocu_isco08 = 8 if inrange(occ_code_sec,71,74)  & ilo_mjh==2  // "8 - Plant and machine operators, and assemblers"
        replace ilo_job2_ocu_isco08 = 9 if inrange(occ_code_sec,81,84)  & ilo_mjh==2  // "9 - Elementary occupations"
        replace ilo_job2_ocu_isco08 = 10 if inrange(occ_code_sec,91,93) & ilo_mjh==2  // "0 - Armed forces occupations"
		replace ilo_job2_ocu_isco08 = 11 if occ_code_sec==.  & ilo_mjh==2  // "X - Not elsewhere classified"
...
** SECOND JOB
   ...
   * ilo_job2_how_actual
 		...
 		gen ilo_job2_how_actual =d2 if inlist(d1,1,2)
		 		replace ilo_job2_how_actual = . if ilo_job2_how_actual == .
...
destring prof3e, replace
    ...
    * 1-digit level
	gen ilo_prevocu_isco08 =.
		replace ilo_prevocu_isco08 = 1 if inrange(occ_code_prev,11,13) & ilo_cat_une==1  // "1 - Managers"
		replace ilo_prevocu_isco08 = 2 if inrange(occ_code_prev,21,26) & ilo_cat_une==1  // "2 - Professional"
		replace ilo_prevocu_isco08 = 3 if inrange(occ_code_prev,31,34) & ilo_cat_une==1  // "3 - Technicians and associate professionals"
		replace ilo_prevocu_isco08 = 4 if inrange(occ_code_prev,41,44) & ilo_cat_une==1  // "4 - Clerical support workers"
		replace ilo_prevocu_isco08 = 5 if inrange(occ_code_prev,51,54) & ilo_cat_une==1  // "5 - Service and sales workers"
		replace ilo_prevocu_isco08 = 6 if occ_code_prev == 64  & ilo_cat_une==1  // "6 - Skilled agricultural, forestry and fishery workers"
        replace ilo_prevocu_isco08 = 7 if inlist(occ_code_prev,61,62,63,65)  & ilo_cat_une==1  // "7 - Craft and related trades workers"
        replace ilo_prevocu_isco08 = 8 if inrange(occ_code_prev,71,74)  & ilo_cat_une==1  // "8 - Plant and machine operators, and assemblers"
        replace ilo_prevocu_isco08 = 9 if inrange(occ_code_prev,81,84)  & ilo_cat_une==1  // "9 - Elementary occupations"
        replace ilo_prevocu_isco08 = 10 if inrange(occ_code_prev,91,93)  & ilo_cat_une==1  // "0 - Armed forces occupations"
		replace ilo_prevocu_isco08 = 11 if occ_code_prev==.  & ilo_cat_une==1  // "X - Not elsewhere classified"
