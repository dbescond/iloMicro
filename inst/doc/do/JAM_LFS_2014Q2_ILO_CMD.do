* TITLE OF DO FILE: ILO Microdata Preprocessing code template - JAMAICA, 2014
* DATASET USED: JAMAICA, SWTS, 2014
* Files created: Standard variables JAM_SWTS_2014_FULL.dta and JAM_SWTS_2014_ILO.dta
...
global country "JAM" /*ref_area: ISO 3 Code from the workflow*/
global source "LFS"  /*survey: Acronym from the workflow*/
global time "2014Q2"  /*time*/
global inputFile "JAM_LFS_2014Q2.DTA" /*name of the input file in stata format*/
...
*-- weights and observations used depending on the quarter/year frequency to be analyzed

*-- annual

	gen ilo_wgt=rfact if to_drop_2==-9999
		lab var ilo_wgt "Sample weight"		

*-- quarter_1                
    if `Q' == 1 {
	    replace ilo_wgt = rfact_`Q'
		lab var ilo_wgt "Sample weight"	
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(urcode,1,2)         // Urban 
		replace ilo_geo=2 if urcode==3         // Rural
...
* Comment:
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if q320==1 | q421==1  | q514==1                    		// No schooling
		replace ilo_edu_isced97=2 if (inlist(q320,2,3,4) & q321==1) | (inlist(q421,2,3,4) & q422==1)  | (inlist(q514,2,3,4) & q515==1)                   		// Pre-primary education
		replace ilo_edu_isced97=3 if (inlist(q320,5,6,7) & q321==1) | (inlist(q421,5,6,7) & q422==1)  | (inlist(q514,5,6,7) & q515==1)                      		// Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if (inlist(q321,2,3,4) & q322==1) | (inlist(q422,2,3,4) & q423==1)  | (inlist(q515,2,3,4) & q516==1)   // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if (inlist(q321,5,6,7) & q322==1) | (inlist(q422,5,6,7) & q423==1)  | (inlist(q515,5,6,7) & q516==1)                 		// Upper secondary education
		replace ilo_edu_isced97=6 if inlist(q322,2,3,4,5) | inlist(q423,2,3,4,5) | inlist(q516,2,3,4,5) // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if inlist(q322,6,7,8) | inlist(q423,6,7,8) | inlist(q516,6,7,8)                  		// First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  inlist(q322,9) | inlist(q423,9) | inlist(q516,9)                     		// Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  q21a==5                      // Attending
		replace ilo_edu_attendance=2 if  !inlist(q21a,5,7)                      // Not attending
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q21==1 | q21a==1 | q21b==1 | q22==1) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if (q23==1) & q31!=5  & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if (q21a==3 | inlist(q43,1,2)) & (q25==1 | inrange(q41,1,8)) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if q25==2 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (q37==1) & ilo_lfs==1             // One job only     
		replace ilo_mjh=2 if (q37>1) & ilo_lfs==1             // More than one job
...
* Comment:
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(q323,1,2,3) & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if q323==5 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if q323==6 & ilo_lfs==1          // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if q323==4 & ilo_lfs==1          // Contributing family workers
...
* Comment: 
	...
	* 2-digit level
	gen ilo_job1_eco_isic3_2digits = .
	    replace ilo_job1_eco_isic3_2digits = int(q39m/100)  if ilo_lfs==1
	...
	* 2-digit level
	gen ilo_job2_eco_isic3_2digits = .
	    replace ilo_job2_eco_isic3_2digits = int(q39s/100)  if ilo_mjh==2
...
* Comment:
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = int(q38m/100)    if ilo_lfs==1
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = int(q38s/100)    if ilo_lfs==1
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if  inlist(q323,1,2)                     & ilo_lfs==1    // Public
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = q33      if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = q35      if ilo_lfs==1 & q35<99
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if q6_3==2 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if q6_3==1  & ilo_lfs==1               				// Temporary
...
* Comment:
/* Useful questions:
          - Institutional sector: infsec1
		  - Private household identification: 
		  - Destination of production: infsec2
		  - Bookkeeping:infsec3
		  - Registration: infsec4
		  - Status in employment:ilo_job1_ste_icse93
		  - Social security contribution (Proxy: pension funds):infsec5
		  - Place of work:infsec6
		  - Size:infsec7
		  - Paid annual leave: infemp2
		  - Paid sick leave:infemp3
*/
    * Social Security:
	*gen social_security=.
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((infsec1==1) | ///
									   (infsec1==2 & inlist(infsec2,1) & infsec3==1) | ///
									   (infsec1==2 & inlist(infsec2,1) & inlist(infsec3,2,99) & infsec4==1) | ///
									   (infsec1==2 & inlist(infsec2,1) & inlist(infsec3,2,99) & inlist(infsec4,99) & ilo_job1_ste_icse93==1 & infsec5==1 ) | ///
									   (infsec1==2 & inlist(infsec2,1) & inlist(infsec3,2,99) & inlist(infsec4,99) & ilo_job1_ste_icse93!=1 & infsec6==1 & infsec7==1 ) | ///
									   (infsec1==2 & inlist(infsec2,1) & inlist(infsec3,2,99) & inlist(infsec4,99) & ilo_job1_ste_icse93==1 & inlist(infsec5,2,99) & infsec6==1 & infsec7==1 ))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((inlist(ilo_job1_ste_icse93,1,6) & infemp1==1 ) | ///
										 (inlist(ilo_job1_ste_icse93,1,6) & infemp1==99 & infemp2==1 & infemp3==1 ) | ///
...
* Comment: 
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = q325a      if ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees= ilo_job1_lri_ees *(52/12) if q325ap==1
		replace ilo_job1_lri_ees= ilo_job1_lri_ees /12 if q325ap==3
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = q325a      if ilo_job1_ste_aggregate==2
		replace ilo_job1_lri_slf= ilo_job1_lri_slf *(52/12) if q325ap==1
		replace ilo_job1_lri_slf= ilo_job1_lri_slf /12 if q325ap==3
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if q45==1     & ilo_lfs==2                     // Previously employed       
		replace ilo_cat_une=2 if q45==2     & ilo_lfs==2                     // Seeking for the first time
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(q41,1,2) & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if q41==3      & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if q41==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if inlist(q41,5,6) & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if q41==7      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if q41==8      & ilo_lfs==2                 // 24 months or more
...
* Comment: 
	...
	* 2-digit level
	gen ilo_preveco_isic3_2digits = .
	    replace ilo_preveco_isic3_2digits = int(q417/100)   if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* Comment: 
	...
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits = . 
	    replace ilo_prevocu_isco08_2digits = int(q416/100)    if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if (q21a==3 | inlist(q43,1,2)) & q25!=1              & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if q21a!=3 & (q25==1 | inrange(q41,1,8))            & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if q21a!=3 & q25!=1 & q24==1 		      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if q21a!=3 & q25!=1 & q24==2 			      & ilo_lfs==3      // Not seeking, not available, not willing
...
*Comment:
	...
	gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(q44,1,2,4)          & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(q44,3,6,8)              & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(q44,0,5,7)              & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if inlist(q44,.,9) & q25==6          & ilo_lfs==3             // Does not need/want to work
...
* Comment:
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & (q25==1 | inrange(q41,1,8))    & ilo_olf_reason==1
...
* Comment:
	...
	gen ilo_sub=.
	    replace ilo_sub=1 if !inlist(ilo_lfs,1,2) & infsec2==2
