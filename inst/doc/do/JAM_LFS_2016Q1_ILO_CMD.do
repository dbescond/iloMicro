* TITLE OF DO FILE: ILO Microdata Preprocessing code template - JAMAICA, 2016
* DATASET USED: JAMAICA, SWTS, 2016
* Files created: Standard variables JAM_SWTS_2016_FULL.dta and JAM_SWTS_2016_ILO.dta
...
global time "2016Q1"  /*time*/
global inputFile "JAM_LFS_2016Q1.dta" /*name of the input file in stata format*/
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: not sure on the weight
...
* Comment:
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if q322==1 | q423==1  | q516==1                    // No schooling
		replace ilo_edu_isced97=2 if q322==2 | q423==2  | q516==2                    // Pre-primary education
		replace ilo_edu_isced97=3 if q322==3 | q423==3  | q516==3                    // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if inlist(q322,4,5) | inlist(q423,4,5) | inlist(q516,4,5)                     // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if q322==6 | q423==6  | q516==6                    // Upper secondary education
		replace ilo_edu_isced97=6 if q322==7 | q423==7 | q516==7                     // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if q322==8 | q423==8 | q516==8                     // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  q322==9 | q423==9 | q516==9                     // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		...
		replace ilo_edu_attendance=2 if  inlist(q21a,1,2,3,4,6)                      // Not attending
...
* Comment: 
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=2 if (q21a==3) | q25==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
...
* Comment: 
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = int(q39m/100)     if ilo_lfs==1
   ...
   * Aggregate level
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = int(q39s/100)      if ilo_mjh==2
...
* Comment:
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = int(q38s/100)    if ilo_mjh==2
...
* Comment: 
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual =  q35     if ilo_lfs==1
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if q62==2 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if q62==1  & ilo_lfs==1               				// Temporary
...
* Comment:
/* Useful questions:
          - Institutional sector: q610
		  - Private household identification: 
		  - Destination of production:
		  - Bookkeeping:
		  - Registration: q611b
		  - Status in employment: ilo_job1_ste_icse93
		  - Social security contribution (Proxy: pension funds):q66
		  - Place of work: q323a
		  - Size: q324
		  - Paid annual leave: q67a
		  - Paid sick leave: q68
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if  q66==1 &   ilo_lfs==1          // social security (proxy)
		replace social_security=2 if  q66==3 &  ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(q610,1,2)) | ///
									   (inlist(q610,3,4,5,6) & q611b==1) | ///
									   (inlist(q610,3,4,5,6) & q611b==4 & ilo_job1_ste_icse93==1 & social_security==1)  | ///
									   (inlist(q610,3,4,5,6) & q611b==4) & ilo_job1_ste_icse93!=1) & inlist(q323a,4,6,8,9) & inlist(q324,3,4,5)
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (inlist(ilo_job1_ste_icse93,1,6) & q67a==1 & q68==1) | ///
...
* Comment: 
	...
	* Employees
	gen ilo_job1_lri_ees = .
		...
		replace ilo_job1_lri_ees = ilo_job1_lri_ees * (52/12)   if ilo_job1_ste_aggregate==1 & q325ap==1 // weekly
		replace ilo_job1_lri_ees = ilo_job1_lri_ees /12   if ilo_job1_ste_aggregate==1 & q325ap==7 //yearly
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
	    ...
	    replace ilo_job1_lri_slf = ilo_job1_lri_slf * (52/12)   if ilo_job1_ste_aggregate==1 & q325ap==1 // weekly
		replace ilo_job1_lri_slf = ilo_job1_lri_slf /12   if ilo_job1_ste_aggregate==1 & q325ap==7 //yearly
...
* Comment: 
	...
	* 2-digit level
	gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = int(q417/100)     if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
