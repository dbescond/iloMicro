* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Jamaica, 2013
* DATASET USED: Jamaica, SWTS, 2013
* NOTES: 
* Files created: Standard variables JAM_SWTS_2013_FULL.dta and JAM_SWTS_2013_ILO.dta
...
global inputFile "JAM_SWTS_2013.dta" /*name of the input file in stata format*/
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: C3:996 (including "2" in primary education)
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  c2a==1                      // Attending
		replace ilo_edu_attendance=2 if  inlist(c2a,2,3)                      // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=2 if  b10==3                                         // Married
		replace ilo_mrts_details=3 if  inlist(b10,2,6)                                         // Union / cohabiting
		replace ilo_mrts_details=4 if  b10==5                                         // Widowed
		replace ilo_mrts_details=5 if  b10==4                                         // Divorced / separated
...
*/
    * Aggregate  	
	gen ilo_dsb_aggregate=.
	    replace ilo_dsb_aggregate=1 if (inlist(b21,1,2)| inlist(b22,1,2)| ///
										inlist(b23,1,2)|inlist(b24,1,2))   // Persons without disability
		replace ilo_dsb_aggregate=2 if (inlist(b21,3,4)| inlist(b22,3,4)| ///
										inlist(b23,3,4)|inlist(b24,3,4))  // Persons with disability
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c33a==1|c33b==1|c33c==1|c33d==1|c33e==1|c33f==1|c33g==1) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if (c34==1) & ilo_wap==1						// Employed: temporary absent
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: used the generated variable industry_group
...
*/
	* 1-digit level	
	...
	/*
		...
		*/
		replace ilo_job1_eco_isic3=industry_group if ilo_lfs==1
		replace ilo_job1_eco_isic3=18 if ilo_job1_eco_isic3==97
		replace ilo_job1_eco_isic3=18 if ilo_job1_eco_isic3==.  & ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: used the generated variable occ_group
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
		...
		*replace ilo_job1_ocu_isco08=10 if (ilo_job1_ocu_isco08==0 & ilo_lfs==1) // Armed forces
		replace ilo_job1_ocu_isco08=occ_group if ilo_lfs==1
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(d6b,1,2,6)                       & ilo_lfs==1    // Public
...
* Comment:
/* Useful questions:
          - Institutional sector: d6b, ilo_job1_ins_sector
...
*/
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & d6b==5
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(d6b,1,2,6)) | ///
									   (!inlist(d6b,1,2,5,6)& d5==1) | ///
									   (!inlist(d6b,1,2,5,6)&!inlist(d5,1,2,3)& ilo_job1_ins_sector==1 &social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (ilo_job1_ste_icse93==1 & inlist(d13j,3,97)& d13c==1 & d13d==1) | ///
...
*         ilo_lri_slf': Monthly labour related self income calculated as the net profit plus the theoretical value of the products taken from the main activity
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = d14_amt      if ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees + d16_amt if ilo_job1_ste_aggregate==1 & d15==2
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(5*52/12) if d14_prd==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(52/12) if d14_prd==2
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(26/12) if d14_prd==3
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = d27a_1      if ilo_job1_ste_aggregate==2
		replace ilo_job1_lri_slf = 0     if ilo_job1_ste_aggregate==2 & d27a_2==2
