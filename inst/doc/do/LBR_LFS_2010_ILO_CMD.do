...
global inputFile "LBR_LFS_2010.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=2 if b6==2                                     // Married
		replace ilo_mrts_details=3 if b6==3                                     // Union / cohabiting
...
* -------------------------------------------------------------------------------------------			
	...
	* 1) Unit of production - Formal / Informal Sector
		...
		/* Generation of a variable indicating the social security coverage (to be dropped afterwards) */
			...
			gen social_security=.
				replace social_security=1 if (e12==1 & ilo_lfs==1)
	
			gen ilo_job1_ife_prod=.
				
				replace ilo_job1_ife_prod=2 if (inlist(e10,1,2,3) | e11==1) & ilo_lfs==1
				
				replace ilo_job1_ife_prod=3 if (e10==4 & ilo_lfs==1)
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if	(inlist(h5,1,2,3) & ilo_lfs==3)							//Labour market
		replace ilo_olf_reason=2 if	(inlist(h5,4,5,6,7) & ilo_lfs==3)						//Other labour market reasons
		replace ilo_olf_reason=3 if	(inlist(h6,1,2,3,4,5) & (h5==0 | h5==9) & ilo_lfs==3)	//Personal/Family-related
		replace ilo_olf_reason=4 if (h5==8 & ilo_lfs==3)									//Does not need/want to work
		replace ilo_olf_reason=5 if (h6==6 & ilo_lfs==3)									//Not elsewhere classified
