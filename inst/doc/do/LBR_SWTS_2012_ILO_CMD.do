* TITLE OF DO FILE: ILO Microdata Preprocessing code template - LIBERIA, 2012
* DATASET USED: Liberia, SWTS, 2012
* NOTES: 
* Files created: Standard variables LBR_SWTS_2012_FULL.dta and LBR_SWTS_2012_ILO.dta
...
global country "LBR" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2012"  /*time*/
global inputFile "LBR_SWTS_2013.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if  a4b==1        // Urban 
		replace ilo_geo=2 if  a4b==2        // Rural
...
* Comment: c7 only for individuals who have completed education. 
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if c7==1 | c1==2                  // No schooling
		replace ilo_edu_isced97=2 if c7==2                    // Pre-primary education
		replace ilo_edu_isced97=3 if c7==3                       // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if c7==4                     // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if c7==5                        // Upper secondary education
		replace ilo_edu_isced97=6 if c7==6                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if c7==7                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if c7==8                       // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		...
		replace ilo_edu_attendance=2 if (c2==2|c2==3)                       // Not attending
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = d4ic      if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = d2ic   if ilo_lfs==1
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	...
	replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (inlist(ilo_job1_eco_isic4_2digits,97) |  ilo_job1_ocu_isco08_2digits==63)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   d5==1 | ///
									   (inlist(d5,4) & ilo_job1_ste_icse93==1 & d13j==1))
...
* ------------------------------------------------------------------------------
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees =  d141     if ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees + d16 if d15==2 & !inlist(d16,2,3)
...
* Comment: D35 (how many additional hours could you have worked) used as a proxy for the current availability of the individual. Threshold set at 35 hours a week. 	
	gen ilo_joball_tru = .
