* TITLE OF DO FILE: ILO Microdata Preprocessing code template - LIBERIA, 2014
* DATASET USED: LIBERIA, SWTS, 2014
* NOTES: 
* Files created: Standard variables LBR_SWTS_2014_FULL.dta and LBR_SWTS_2014_ILO.dta
...
global source "SWTS" 
global time "2014"
global inputFile "LBR_SWTS_2014.dta"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if  urru==1        // Urban 
		replace ilo_geo=2 if urru==2         // Rural
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if b4==2            // Male
		replace ilo_sex=2 if b4==1            // Female
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if c11==1 |c1==2                       // No schooling
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		...
		replace ilo_edu_isced97=4 if c11==3                       // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if c11==4                        // Upper secondary education
		replace ilo_edu_isced97=6 if c11==5                        // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if c11==6                        // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if c11==7                        // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if c3==1                       // Attending
		replace ilo_edu_attendance=2 if  inlist(c3,2,3)                      // Not attending
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=2 if   b9==3                                        // Married
		replace ilo_mrts_details=3 if   b9==2                                        // Union / cohabiting
		replace ilo_mrts_details=4 if   b9==5                                        // Widowed
		replace ilo_mrts_details=5 if   b9==4                                        // Divorced / separated
...
* Comment: 
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (b18==1 & b15==1 & b16==1 & b17==1 )                            
             replace ilo_dsb_details=2 if (b18==2 | b15==2 | b16==2 | b17==2 )                                                 
             replace ilo_dsb_details=3 if (b18==3 | b15==3 | b16==3 | b17==3 )                                                 
             replace ilo_dsb_details=4 if (b18==4 | b15==4 | b16==4 | b17==4 )
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (d13a==1 | d13b==1 | d13c==1 | d13d==1 | ///
		d13e==1 | d13f==1 |d13g==1 ) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if d14==1 & ilo_wap==1										// Employed: temporary absent
		replace ilo_lfs=2 if inlist(f1,1,2) & f9==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (f3a==1 | f3b==1) & f9==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
* Comment:
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if e8==1  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if e8==2 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if e8==3 & ilo_lfs==1          // Own-account workers
		 replace ilo_job1_ste_icse93=4 if e8==4 & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if e8==5 & ilo_lfs==1          // Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = e4isi     if ilo_lfs==1
		replace ilo_job1_eco_isic4_2digits = .    if e4isi==9999 & ilo_lfs!=1
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = e2isc    if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = e28      if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if e10==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if e10==2 & ilo_lfs==1               				// Temporary
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds):e13j
		  - Place of work:
		  - Size:
		  - Paid annual leave: e13c
		  - Paid sick leave: e13d
*/
    * Social Security:
gen social_security=.
	    replace social_security=1 if e13j==1 &    ilo_lfs==1          // social security (proxy)
		replace social_security=2 if e13j==2  &  ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	...
	replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (inlist(ilo_job1_eco_isic4_2digits,97) |  ilo_job1_ocu_isco08_2digits==63)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   e5==1 | ///
									   (inlist(e5,4) & ilo_job1_ste_icse93==1 & e13j==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((ilo_job1_ste_icse93==1 & e13j==1) | ///
										 (ilo_job1_ste_icse93==1 & inlist(e13j,3,997)& e13c==1 & e13d==1) | ///
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees =  e14b     if ilo_job1_ste_aggregate==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees + e16b if e16a==1 & !inlist(e16b,2,3)
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = e24     if ilo_job1_ste_aggregate==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment:Could we use e30 (how many additional hours could you have worked last week) as a proxy for the " current availability" of indidivuals working?

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_actual<= 35 & e29==1  & e30>0  & ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(f8,1,2) & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if f8==3      & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if f8==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if f8==5      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if f8==6      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if f8==7      & ilo_lfs==2                 // 24 months or more
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if inlist(f1,1,2)& f9==2 & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  f1==3  & f9==1           & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if f1==3	&f9==2 & f4==1	      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if f1==3	& f9==2 & f4==2	      & ilo_lfs==3      // Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(f5,7,8,9,10,11)          & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(f5,1,2)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(f5,3,4,5,6)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if f10==6          & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if (ilo_olf_reason==.|f5==99 & !f10==6) & ilo_lfs==3     // Not elsewhere classified
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & f9==1     & ilo_olf_reason==1
