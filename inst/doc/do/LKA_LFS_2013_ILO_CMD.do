* TITLE OF DO FILE: ILO Microdata Preprocessing code template - LKA, 2013
* DATASET USED: LKA LFS 2013
* NOTES:
* Author: Roger Gomis
...
*-- annual
	...
	gen ilo_wgt=weighu / 1000000000   if to_drop_2==-9999
		lab var ilo_wgt "Sample weight"		

*-- quarter_1                
    if `Q' == 1 {
	    replace ilo_wgt = weighu / 1000000000*4
		lab var ilo_wgt "Sample weight"	
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_geo=.
		replace ilo_geo=1 if sector==1
		replace ilo_geo=2 if sector==2|sector==3
...
/* ISCED 2011 mapping	file:///J:\DPAU\MICRO\LKA\LFS\2013\ORI\ISCED_2011_Mapping_EN_Sri%20Lanka.xlsx */
*also very useful from wikipedia
/*
Primary[edit]
...
*remarks, some studying of grade 1 is considered as early childhood, also Masters + PhD are bunched together, the minimum level (masters) is assumed
* post secondary non tertiary is empty
	recode P10 (19=1) (0/4=2) (5/8=3) (9/10=4) (11/14=5) (15=7) (16=9) (. 17=11), gen(ilo_edu_isced11)
...
* note the usage of matriculation in level to proxy for attendance
	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if inlist(P11,1,2,3,4)			// Attending
		replace ilo_edu_attendance=2 if inlist(P11,5)				// Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if P9==1                                     // Single
		replace ilo_mrts_details=2 if P9==2                                     // Married
		*replace ilo_mrts_details=3 if                                          // Union / cohabiting
		replace ilo_mrts_details=4 if P9==3                                     // Widowed
		replace ilo_mrts_details=5 if inlist(P9,4,5)                            // Divorced / separated
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (Q2==1| (Q2==2&Q4==1))  & ilo_wap==1 	// Employed or temporay absent, the latter includes the following reasons (the questionaire brings them to economic activity)
		/* Q5:
		1. Off season/Bad weather .........................................................................
		2. Temporary stopping economic activity
			(disorganization, suspension of work , mechanical, electrical ........
				breakdown, Shortage of raw material, financial difficulties,
				strikes, nothing works
		3. sick/Injury/personal ................................................................................
		4. Leave/holiday/ vacation/educational leave or training ...........................
		5. Personal/ family needs ............................................................................
		9. Others (specify) .......................................................................................
		*/
		replace ilo_lfs=2 if (Q47==3|(Q48==1&Q51==1))&ilo_wap==1 & ilo_lfs!=1	// Unemployed: (Note in employment) AND Will start new Job, Business OR Took steps to find job past 4 weeks AND Ready to accept the job within the next two weeks
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*  Were you engaged in a secondary occupation / economic activity in the last week ? 1 Yes 2 No
	gen ilo_mjh=.
		replace ilo_mjh=1 if Q24==2 & ilo_lfs==1
		replace ilo_mjh=2 if Q24==1 & ilo_lfs==1
  ...
  * MAIN JOB:
	...
	* Detailed categories:
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if Q9==1 & ilo_lfs==1		// Employees
			replace ilo_job1_ste_icse93=2 if Q9==2  & ilo_lfs==1		// Employers
			replace ilo_job1_ste_icse93=3 if Q9==3 & ilo_lfs==1		// Own-account workers
					// Members of producers cooperatives
			replace ilo_job1_ste_icse93=5 if Q9==4 & ilo_lfs==1		// Contributing family workers
...
* IT DOES FOLLOW ISIC  4
		...
		* Primary activity
		...
		gen ilo_job1_eco_isic4_2digits=trunc(Q8/1000) if (ilo_lfs==1)
...
* -------------------------------------------------------------------------------------------		
	...
	/* Classification used: ISCO 08  Warning unknow category isco 08 digits 2 = 64 included on isco08 digit 1 = 6 */
...
* ISCO 08 - 1 digit
		...
		* Remove the case that is wrong at the 2 digit level
replace ilo_job1_ocu_isco08_2digits=. if ilo_job1_ocu_isco08_2digits==64
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
...
* -------------------------------------------------------------------------------------------		
	...
	* if the respondant does not know the variable is not defined
		gen ilo_job1_ins_sector=.
			replace ilo_job1_ins_sector=1 if (inlist(Q14,1,2) & ilo_lfs==1)	// Public (includes semipublic)
			replace ilo_job1_ins_sector=2 if (inlist(Q14,3) & ilo_lfs==1)	// Private
			* Dealing with SE and with employees that has been excluded in the questionaire
			replace ilo_job1_ins_sector=2 if ilo_lfs==1 & ilo_job1_ste_icse93>1&ilo_job1_ste_icse93<6&ilo_job1_ins_sector==.
			replace ilo_job1_ins_sector=2 if ilo_lfs==1 & ilo_job1_ins_sector==. & ilo_job1_ins_sector==. & Q10 ==4
			
			* even with that there are still 11 respondants that do not have data on this question, I set them to private
...
*Notice, respondents with Casual employment or with a no permanent employment are set to temporary
gen ilo_job1_job_contract=.
	replace ilo_job1_job_contract=1 if (Q10==1&ilo_lfs==1& ilo_job1_ste_aggregate==1)
	replace ilo_job1_job_contract=2 if (inlist(Q10,2,3,4) & ilo_lfs==1& ilo_job1_ste_aggregate==1)
...
* -------------------------------------------------------------------------------------------			
	...
	/* Useful questions:
		...
		Q14 public or not, used for ilo_job1_ins_sector=1
		Q18  sector classification
		
		Q19 destination production 
		...
		Employees only
		Q11 SS or pension
		Q12 Anual leave ->Irrelevant since we have the SS
	...
	*** Preparing necessary variables - the variables must be adjusted to fit the data
	...
	recode Q18 (1=1) (2/4 =0) , gen(todrop_institutional) 				// theoretical 3 value node/ +1 Gov, Corporation, NGO, Embassy, IO / 0 Farm or private business, other, not asked, don't know, NA / -1 private household
	replace todrop_institutional=1 if ilo_job1_ins_sector==1
	replace todrop_institutional=-1 if Q17==8 //oddly enough, in the size question the HH sector is identified
	...
	gen todrop_bookkeeping=0 														// theoretical 2 value node/ +1 keeps accounts for GOV/ 0 does not keep accounts or personal use or other, not asked, DK, NA  :::actual node empty (no necessary data available)
		replace todrop_bookkeeping=1 if Q16==1
		
	gen todrop_registration=1 if Q15==1											// theoretical 3 value node/ +1 registered national level/ 0 not registered national level, other, DK / -1 not asked NA
		replace todrop_registration=0 if Q15==2|Q15==3
		replace todrop_registration=-1 if todrop_registration==.
		
	gen todrop_SS=1 if Q11==1												// theoretical 3 value node/ +1 Social security/ 0  Not asked; NA/ -1 No social security or don't know, Other, etc
		replace todrop_SS=-1 if todrop_SS==.
		*we don't have data on premises
	...
	*note 5, is considered into 6 or more due to the nature of the data	, also based on regular employees
	gen todrop_size=1 if Q17>1& Q17<7							// theoretical 2 value node/ +1 more than 6 workers / 0 less than 6 workers
		replace todrop_size=0 if Q17==7|Q17==1
		
		
		*** Important, Paidleave and Sick leave are ONLY used if the question on SS is non-existant
	...
	gen todrop_producesforsale=1 													//  theoretical 2 value node / +1 Yes product for sale, Other, Not asked, Don't Know, NA/ 0 No, not for sale 
		replace todrop_producesforsale=0 if Q19==2
	***********************************************************
...
* 1) Weekly hours ACTUALLY worked - Main job
			...
			gen ilo_job1_how_actual=Q21 if (ilo_lfs==1)

					lab var ilo_job1_how_actual "Weekly hours actually worked in main job"

			* note: 9 respondants that are goverment/or semi public employees instead of skeep just to question 20, presumably by mistake skip to Q40 or Q45 depending on the case
			* I set them to a 40 hours week except for the case that is temporary missing from work, I set it to zero
			replace ilo_job1_how_actual=40 if ilo_job1_how_actual==.&Q2==1&ilo_lfs==1
			replace ilo_job1_how_actual=0 if ilo_job1_how_actual==.&Q2==2&Q4==1&ilo_lfs==1
...
gen ilo_job1_how_usual=Q20 if (ilo_lfs==1)
...
* 1) Weekly hours ACTUALLY worked:
		...
		gen ilo_job2_how_actual=Q39 if (ilo_mjh==2 & ilo_lfs==1)
...
* 2) Weekly hours USUALLY worked 
			...
			gen ilo_job2_how_usual=Q38 if (ilo_mjh==2&ilo_lfs==1)
...
* -------------------------------------------------------------------------------------------			
			...
			* Main job
				...
				* Employees
					egen todrop_monthly=rowtotal(Q45A1 Q45A2 Q45A3)
					replace todrop_monthly =. if todrop_monthly == 0
					* bad coding, apparent that it should have been set to misssing
					replace todrop_monthly =. if todrop_monthly == 999999
					
					gen ilo_job1_lri_ees=.
						replace ilo_job1_lri_ees= todrop_monthly if (ilo_job1_ste_aggregate==1 & Q45==1 )
						replace ilo_job1_lri_ees= Q45B3 if (ilo_job1_ste_aggregate==1 & Q45==1 )&ilo_job1_lri_ees==.
				...
				*SE
					...
					gen ilo_job1_lri_slf=.
						replace ilo_job1_lri_ees= Q45C1 if (ilo_job1_ste_aggregate==1 & Q45==2 )
...
* -------------------------------------------------------------------------------------------		
		...
		gen ilo_joball_tru=.
		*less than 35, would like to work more if given the opportunity, available if given the opportunity in the next two weeks
			replace ilo_joball_tru=1 if (ilo_joball_how_actual<35 & Q41==1 & Q44==1 ) & ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if (Q59==1&ilo_lfs==2)
		replace ilo_cat_une=2 if (Q59==2&ilo_lfs==2)
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_dur_aggregate=.
				replace ilo_dur_aggregate=1 if (Q58==1 & ilo_lfs==2)
				replace ilo_dur_aggregate=2 if (Q58==2 & ilo_lfs==2)
				replace ilo_dur_aggregate=3 if (Q58==3 & ilo_lfs==2)
...
* -------------------------------------------------------------------------------------------	
	...
	* Classification aggregated level
		...
		* Primary activity
		gen todrop=trunc(Q60/100) if (ilo_lfs==2&ilo_cat_une==1)
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* the meaning of the first two answers to Q47 is not immediatetly  clear , but seems to indicate willingnes to work
* in particular the question "Would you expected to do a paid job or to start a self employment?" is answered Yes or No (or already found a job/made arrangements for business)
	
/*Not computed, the question that is closer to willingness to work filters out seeking and availability in case of a negative answer
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if Q48==1&Q51==2&ilo_lfs==3							 					//Seeking, not available
		replace ilo_olf_dlma = 2 if Q48==2&Q51==1&ilo_lfs==3												//Not seeking, available
		replace ilo_olf_dlma =3  if Q48==2&Q51==2&Q47==1&ilo_lfs==3											//Not seeking, not available, willing
		replace ilo_olf_dlma =4  if Q47==2 & ilo_lfs==3 &	ilo_olf_dlma==.										//Not seeking, not available, not willing
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* note that the explanation for not participating in the labour market is segreagated in 2 questions (Q50, Q52) according to the answer to Q47

	gen ilo_olf_reason=.
		
		*** Based on Q52
		replace ilo_olf_reason=3 if	inlist(Q52,1,2,4)  & ilo_lfs==3					//Personal/Family-related
		replace ilo_olf_reason=4 if	inlist(Q52,3)								//Does not Want/Need to work
		replace ilo_olf_reason=5 if inlist(Q52,9)  & ilo_lfs==3						//Not elsewhere classified

		
		*** Based on Q50, additional
		replace ilo_olf_reason=1 if	inlist(Q50,2,3,4) & ilo_lfs==3 & ilo_olf_reason==.						//Labour market
		replace ilo_olf_reason=2 if	inlist(Q50,1) & ilo_lfs==3 & ilo_olf_reason==.						//Other Labour market
		replace ilo_olf_reason=3 if	(inlist(Q50,5,6,7)  & ilo_lfs==3) & ilo_olf_reason==.					//Personal/Family-related
		replace ilo_olf_reason=5 if (inlist(Q50,9)  & ilo_lfs==3) & ilo_olf_reason==.						//Not elsewhere classified
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_dis=1 if ( Q48==2 & ilo_olf_reason==1)
