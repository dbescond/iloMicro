* TITLE OF DO FILE: ILO Microdata Preprocessing code template - SRI LANKA, 2014
* DATASET USED: SRI LANKA, LFS, 2014
* NOTES: 
* Files created: Standard variables LKA_LFS_2014_FULL.dta and LKA_LFS_2014_ILO.dta
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 19 September 2018
...
global source "LFS"
global time "2014"
global inputFile "LKA_LFS_Merged_2014"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Rural includes rural and estate
	
	gen ilo_geo=.
	    replace ilo_geo=1 if sector==1                  // Urban 
		replace ilo_geo=2 if inlist(sector,2,3)         // Rural
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
        replace ilo_sex=1 if p4==1            // Male
	    replace ilo_sex=2 if p4==2            // Female
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Original question refers to the current educational attendance status.
*          - Only asked to those aged 5 years old or more and therefore the rest
*            are classified as "not elsewhere classified".
*          - Master's (7) includes doctoral studies (8) (note to value: C3:4158)
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if p10==19                                    // No schooling
		replace ilo_edu_isced11=2 if inrange(p10,0,4)                           // Early childhood education
		replace ilo_edu_isced11=3 if inrange(p10,5,8)                           // Primary education
		replace ilo_edu_isced11=4 if inlist(p10,9,10)                           // Lower secondary education
		replace ilo_edu_isced11=5 if inrange(p10,11,14)                         // Upper secondary education
		* replace ilo_edu_isced11=6 if                                          // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if p10==15                                    // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if p10==16                                    // Bachelor's or equivalent level
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only asked to those aged 5 years old or more and therefore the rest
*            are classified under "not elsewhere classified".
			
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if inlist(p11,1,2,3,4)                     // Attending
		replace ilo_edu_attendance=2 if p11==5                                  // Not attending
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Unemployment: figures may differ to the ones reported by the national
*            statistical office due to the definition of unemployment used. Here
*            it is used the ILO three criteria definition (not in employment,
*            seeking and available) and future starters.
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q2==1) & ilo_wap==1						        // Employed: ILO definition
		replace ilo_lfs=1 if (q2==2 & q4==1) & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if (q48==1 & q51==1) & ilo_lfs!=1 & ilo_wap==1		// Unemployed: three criteria
		replace ilo_lfs=2 if (q47==3) & ilo_lfs!=1 & ilo_wap==1			        // Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (q24==2) & ilo_lfs==1             // One job only     
		replace ilo_mjh=2 if (q24==1) & ilo_lfs==1             // More than one job
   ...
   * ICSE 1993
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if q27==1 & ilo_mjh==2                  // Employees
		  replace ilo_job2_ste_icse93=2 if q27==2 & ilo_mjh==2                  // Employers
		  replace ilo_job2_ste_icse93=3 if q27==3 & ilo_mjh==2                  // Own-account workers
		  * replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2                      // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if q27==4 & ilo_mjh==2                  // Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	*---------------------------------------------------------------------------
	gen eco_pri = .
	    replace eco_pri = int(q8/1000)
	
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = eco_pri if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Category 64 at 2-digit level is included into category 6 at 1-digit
*            level.
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = occ_pri if ilo_lfs==1
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = occ_sec if ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Public sector includes government and semi-government.
*          - Only asked to paid employees.
		
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(q14,1,2) & ilo_lfs==1           // Public
...
* ------------------------------------------------------------------------------
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = q20 if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual =  q21  if ilo_lfs==1
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual = .
	    replace ilo_job2_how_usual = q38  if ilo_mjh==2
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual = .
	    replace ilo_job2_how_actual = q39  if ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Following the national report: "[...]In Sri Lanka scenario, 35 hours
*            per week is taken as the cut-off point applicable to all workers[...]"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - temporary includes: temporary, casual and no permanent employer.
	
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if q10==1 & ilo_lfs==1               	    // Permanent
		replace ilo_job1_job_contract=2 if inlist(q10,2,3,4) & ilo_lfs==1       	// Temporary
...
/* Useful questions:
          - Institutional sector: q14
		  - Private household identification: => q17 (included even though the question is related
		                                         to the number of regular employees at the institution).
											  => q19 refers to the destination of production (sale or barter).
											  => ilo_job1_eco_isic4_2digits==97 OR ilo_job1_ocu_isco08_2digits==63
		  - Destination of production: q19
		  - Bookkeeping: q15
		  - Registration: q16 (formal is assumed to be registered at national level)
		  - Status in employment: ilo_job1_ste_aggregate
		  - Social security contribution (Proxy: pension funds): q11 (only asked to paid employees answering 1,2,3 to q10)
		  - Place of work: not asked.
		  - Size: q17
		  - Paid annual leave: q12 (only asked to paid employees answering 1,2,3 to q10)
		  - Paid sick leave: not asked.
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if q11==1 & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if q12==2 & ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(q14,1,2)) | ///
									   (inlist(q14,3,.) & inlist(q19,1,3,.) & q15==1) | ///
									   (inlist(q14,3,.) & inlist(q19,1,3,.) & inlist(q15,2,3,.) & q16==1) | ///
									   (inlist(q14,3,.) & inlist(q19,1,3,.) & inlist(q15,2,3,.) & q16==. & ilo_job1_ste_aggregate==1 & social_security==1))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Monthly labour related income for employees include salary earnears
*            and daily wage earners. It includes income in kind. Local currency:
*            Rupees (Rs.)
	...
	* Employees
	egen salary_pri = rowtotal(q45a1 q45a2 q45a3), m                // For month salary earners
	egen wage_pri = rowtotal(q45b3 q45b4), m                        // For daily wage earners (already taken to month)
	
	gen ilo_job1_lri_ees=.
		replace ilo_job1_lri_ees = salary_pri if ilo_job1_ste_aggregate==1
	    replace ilo_job1_lri_ees = wage_pri if ilo_job1_lri_ees==. & ilo_job1_ste_aggregate==1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = q45c1 if ilo_job1_ste_aggregate==2
	...
	* Employees
	egen salary_sec = rowtotal(q46a1 q46a2 q46a3), m                // For month salary earners
	egen wage_sec = rowtotal(q46b3 q46b4), m                        // For daily wage earners (already taken to month)
	
	
	gen ilo_job2_lri_ees = .
	    replace ilo_job2_lri_ees = salary_sec if ilo_job2_ste_aggregate==1
		replace ilo_job2_lri_ees = wage_sec if ilo_job2_lri_ees==. & ilo_job2_ste_aggregate==1
    ...
    * Self-employed		
	gen ilo_job2_lri_slf = .
	    replace ilo_job2_lri_slf = q46c1 if ilo_job2_ste_aggregate==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<=35  & q41==1 & q44==1 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if q59==1 & ilo_lfs==2                            // Previously employed       
		replace ilo_cat_une=2 if q59==2 & ilo_lfs==2                            // Seeking for the first time
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only possible to define at aggregate level.

    * Aggregate categories
	gen ilo_dur_aggregate=.
		replace ilo_dur_aggregate=1 if (q58==1 & ilo_lfs==2)                    // Less than 6 months
		replace ilo_dur_aggregate=2 if (q58==2 & ilo_lfs==2)                    // 6 to 12 months
		replace ilo_dur_aggregate=3 if (q58==3 & ilo_lfs==2)                    // 12 months or more
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - As for the occupation difined for main job, code 64 at 2-digit level
*            is left as it is at 2-digit level and sent to category 6 at 1-digit
*            level.
	...
	*---------------------------------------------------------------------------
	gen occ_prev = .
 	    replace occ_prev = int(q60/100)

	
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits = . 
	    replace ilo_prevocu_isco08_2digits = occ_prev if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Based on questions q50 and q52

		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if (inlist(q50,2,3,4)) & ilo_lfs==3                                 // Labour market 
			replace ilo_olf_reason=2 if (q50==1) & ilo_lfs==3                                            // Other labour market reasons
			replace ilo_olf_reason=3 if (inlist(q50,5,6,7) | inlist(q52,1,2,4)) & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if (q52==3) & ilo_lfs==3                                            // Does not need/want to work
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Not possible to define due to the skip pattern prior to availability
*            question.
...
cd "$outpath"
	...
	drop eco_pri eco_sec occ_pri occ_sec social_security salary_pri wage_pri salary_sec 
	drop wage_sec occ_prev
	compress
