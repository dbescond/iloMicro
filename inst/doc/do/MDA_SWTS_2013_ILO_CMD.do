* TITLE OF DO FILE: ILO Microdata Preprocessing code template - MOLDOVA, REPUBLIC OF, 2013
* DATASET USED: MOLDOVA, REPUBLIC OF, SWTS, 2013
* Files created: Standard variables MDA_SWTS_2013_FULL.dta and MDA_SWTS_2013_ILO.dta
...
global source "SWTS"
global time "2013"
global inputFile "MDA_SWTS_2013.dta"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if urbrural==1         // Urban 
		replace ilo_geo=2 if urbrural==2         // Rural
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: no information on education. 
/*
    *---------------------------------------------------------------------------
	* ISCED 97
	*---------------------------------------------------------------------------
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  tsm25==1                      // Attending
		replace ilo_edu_attendance=2 if  inlist(tsm25,2,3)                      // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if  tsm5==1                                         // Single
		replace ilo_mrts_details=2 if  tsm5==2                                         // Married
		replace ilo_mrts_details=3 if  tsm5==5                                        // Union / cohabiting
		replace ilo_mrts_details=4 if tsm5==3                                          // Widowed
		replace ilo_mrts_details=5 if tsm5==4                                          // Divorced / separated
...
* Comment: 
       ...
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (tsm14==1 & tsm15==1 & tsm16==1 & tsm17==1 & tsm18==1 & tsm19==1)                            
             replace ilo_dsb_details=2 if (tsm14==2 | tsm15==2 | tsm16==2 | tsm17==2 | tsm18==2 | tsm19==2 )                                                 
             replace ilo_dsb_details=3 if (tsm14==3 | tsm15==3 | tsm16==3 | tsm17==3 | tsm18==3 | tsm19==3 )                                                 
             replace ilo_dsb_details=4 if (tsm14==4 | tsm15==4 | tsm16==4 | tsm17==4 | tsm18==4 | tsm19==4)
...
* Comment: 
	...
	gen ilo_lfs=.
		...
		*replace ilo_lfs=1 if () & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if unemployed==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
...
* Comment: 
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = tsma49c      if ilo_job1_ste_aggregate==1
