* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Madagascar, 2015
* DATASET USED: Madagascar ENESI 2015
* Files created: Standard variables on Madagascar ENESI 2015
...
global source "ENESI"
global time "2015"
global inputFile "MDG_ENESI_2015.dta"
...
* Comment: Based on questions M13 and M15. 
	
	gen ilo_edu_aggregate=.
		replace ilo_edu_aggregate=1 if m13==2 | inlist(m15,0,1,2,3,4)  			// Less than basic
		replace ilo_edu_aggregate=2 if inlist(m15,5,6,7,8)						// Basic
		replace ilo_edu_aggregate=3 if inlist(m15,9,10,11)						// Intermediate
		replace ilo_edu_aggregate=4 if inrange(m15,12,22)						// Advanced
		replace ilo_edu_aggregate=5 if m13==.									// Level not stated
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if m17==1				// Attending 
		replace ilo_edu_attendance=2 if m17==2 | m13==2		// Not attending 
		replace ilo_edu_attendance=3 if m13==.				// Not elsewhere classified 
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=2 if m6==1										// Married
		replace ilo_mrts_details=3 if m6==2										// Union / cohabiting
...
* Comment: Availability for unemployment is within the next month. 

	 gen ilo_lfs=.
		replace ilo_lfs=1 if (ea2==1 | inrange(ea3,1,10)) & ilo_wap==1								// Employed
		replace ilo_lfs=1 if (ea4==1 & (inlist(ea5,1,2,3,4) | ea6==1)) & ilo_wap==1					// Employed
		replace ilo_lfs=2 if ilo_lfs!=1 & (ea7a==1 | ea7b==1) & inlist(ea7c,1,2,3) & ilo_wap==1		// Unemployed
	...
	* MAIN JOB
		...
		* Detailed categories
			gen ilo_job1_ste_icse93=.
				replace ilo_job1_ste_icse93=1 if inlist(ap3,1,2,3,4,5)			// Employees
				replace ilo_job1_ste_icse93=2 if ap3==6							// Employers
				replace ilo_job1_ste_icse93=3 if ap3==7							// Own-account workers
				* replace ilo_job1_ste_icse93=4 if 								// Producer cooperatives
				replace ilo_job1_ste_icse93=5 if ap3==9							// Contributing family workers
				replace ilo_job1_ste_icse93=6 if (ap3==8 | ap3==.)				// Not classifiable
	...
	* SECOND JOB
		...
		* Detailed categories
		gen ilo_job2_ste_icse93=.
				replace ilo_job2_ste_icse93=1 if inlist(as4,1,2,3,4,5)			// Employees
				replace ilo_job2_ste_icse93=2 if as4==6							// Employers
				replace ilo_job2_ste_icse93=3 if as4==7							// Own-account workers
				* replace ilo_job2_ste_icse93=4 if								// Producer cooperatives
				replace ilo_job2_ste_icse93=5 if as4==9							// Contributing family workers
				replace ilo_job2_ste_icse93=6 if (as4==8 | as4==.)				// Not classifiable
...
* -------------------------------------------------------------------------------------------
	...
	* MAIN JOB
			gen occup_2dig=int(ap1/10) if ilo_lfs==1 
		...
		* SECOND JOB
			gen occup_2dig_job2=int(as2/100) if ilo_mjh==2 
...
* -------------------------------------------------------------------------------------------		
	...
	* MAIN JOB
		gen ilo_job1_ins_sector=.
			replace ilo_job1_ins_sector=1 if inlist(ap4,1,2,4,5) & ilo_lfs==1		// Public
	...
	* SECOND JOB
		gen ilo_job2_ins_sector=.
			replace ilo_job2_ins_sector=1 if inlist(as5,1,2,4,5) & ilo_mjh==2		// Public
...
* -------------------------------------------------------------------------------------------		
	...
	* Weekly hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    replace ilo_job1_how_actual=(ap10*ap11) if ilo_lfs==1
	...
	* Weekly hours USUALLY worked - based on the month of August
	gen ilo_job1_how_usual=.
	    replace ilo_job1_how_usual=ap110812 if ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_olf_dlma=.
			...
			replace ilo_olf_dlma=4 if c19==2 & c20==2 & c25!=1 & c23==2 & ilo_lfs==3		// Not seeking, not available, not willing
