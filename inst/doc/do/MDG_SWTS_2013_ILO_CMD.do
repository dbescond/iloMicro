* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Madagascar, 2013
* DATASET USED: Madagascar, SWTS, 2013
* NOTES: 
* Files created: Standard variables MGD_SWTS_2013_FULL.dta and MGD_SWTS_2013_ILO.dta
...
global source "SWTS"
global time "2013"
global inputFile "MDG_SWTS_2013.dta"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if b4_sex==1            // Male
		replace ilo_sex=2 if b4_sex==2            // Female
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  c12==1 | c1==2                     // No schooling
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		replace ilo_edu_isced97=3 if  c12==2                           // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if  c12==3                           // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if  c12==4                           // Upper secondary education
		replace ilo_edu_isced97=6 if  c12==5                           // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if  c12==6                           // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  c12==7                           // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if  b10_sitm== 1                              // Single
		replace ilo_mrts_details=2 if   b10_sitm== 3                                         // Married
		replace ilo_mrts_details=3 if  b10_sitm== 2                                          // Union / cohabiting
		replace ilo_mrts_details=4 if   b10_sitm== 5                                         // Widowed
		replace ilo_mrts_details=5 if    b10_sitm== 4                                        // Divorced / separated
...
* Comment: 
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (b18_pb_v==1 & b19_pb_a==1 & b20_pb_m==1 & b21_pb_m==1 )                            
             replace ilo_dsb_details=2 if (b18_pb_v==2 | b19_pb_a==2 | b20_pb_m==2 | b21_pb_m==2 )                                                 
             replace ilo_dsb_details=3 if (b18_pb_v==3 | b19_pb_a==3 | b20_pb_m==3 | b21_pb_m==3 )                                                 
             replace ilo_dsb_details=4 if (b18_pb_v==4 | b19_pb_a==4 | b20_pb_m==4 | b21_pb_m==4 )
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c32_a==1 | c32_b==1 | c32_c==1 | c32_d==1 | ///
		c32_e==1 | c32_f==1 |c32_g==1 ) & ilo_wap==1				   // Employed: ILO definition
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Apprentices included with employees

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(d7,1,6)  & ilo_lfs==1          // Employees
	...
	* 1-digit level
	...
	/*
		...
		*/
	    ...
	    replace ilo_job1_eco_isic4=22 if (ilo_job1_eco_isic4==. | industry==99) & ilo_lfs==1
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds): d13_j
		  - Place of work:
		  - Size:
		  - Paid annual leave: d13_c
		  - Paid sick leave: d13_d
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if d13_j==1  &  ilo_lfs==1          // social security (proxy)
		replace social_security=2 if d13_j==2  &  ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   d5_a==1 | ///
									   (inlist(d5_a,4) & ilo_job1_ste_icse93==1 & d13_j==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((ilo_job1_ste_icse93==1 & d13_j==1) | ///
										 (ilo_job1_ste_icse93==1 & inlist(d13_j,3,997)& d13_c==1 & d13_d==1) | ///
...
* ------------------------------------------------------------------------------
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = d14_b if ilo_job1_ste_aggregate==1
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
		...
		replace ilo_job1_lri_slf = ilo_job1_lri_slf + d29 if ilo_job1_ste_aggregate==2 & d28==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: d35 (how many additional hours could you have worked last week) used as a proxy for the " current availability" of indidivuals working?
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(e12b,1,2)      & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if e12b==3      & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if e12b==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if e12b==5      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if e12b==6      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if e12b==7      & ilo_lfs==2                 // 24 months or more
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=5 if (ilo_olf_reason==.|e5==99 & !e14==6) & ilo_lfs==3     // Not elsewhere classified
