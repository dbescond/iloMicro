* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Maldives, 2016
* DATASET USED: Maldives, HIES, 2016
* NOTES: 
* Files created: Standard variables MDV_HIES_2016_FULL.dta and MDV_HIES_2016_ILO.dta
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 04 May 2018
...
global source "HIES"
global time "2016"
global inputFile "MDV_HIES_2016_Labour_Force_Final"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if q404==2            // Male
		replace ilo_sex=2 if q404==1            // Female
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - If the person is currently studying then s/he is classified 
*            under the previous level to that one currently attending, otherwise
*            s/he is classified following the highest academic/vocational 
*            certificate achieved. In both cases the person will be classified under
*            the highest level attended.
*          - It follows the mapping developed by UNESCO.
*            (http://uis.unesco.org/en/isced-mappings)
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if (q419==2 | q424==8)                                         // No schooling
		replace ilo_edu_isced11=2 if q419==1 & (q422==1 & inrange(q423,7,12))                    // Early childhood education
		replace ilo_edu_isced11=3 if q419==1 & ((q422==1 & q423==13))                            // Primary education
		replace ilo_edu_isced11=4 if q419==1 & ((q422==1 & inlist(q423,14,15)) | (q424==1))      // Lower secondary education
		replace ilo_edu_isced11=5 if q419==1 & ((q422==1 & q423==16) | (inlist(q424,2,3)))       // Upper secondary education
		replace ilo_edu_isced11=6 if q419==1 & (q424==4)                                         // Post-secondary non-tertiary education
		* replace ilo_edu_isced11=7 if q419==1 &                                                 // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if q419==1 & ((q422==1 & inlist(q423,17,19)) | (q424==5))      // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if q419==1 & ((q422==1 & q423==18) | (q424==6))                // Master's or equivalent level
		replace ilo_edu_isced11=10 if q419==1 & (q424==7)                                        // Doctoral or equivalent level
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if q422==1                       // Attending
		replace ilo_edu_attendance=2 if q422==2                       // Not attending
...
* ------------------------------------------------------------------------------ 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if q427==1                                   // Single
		replace ilo_mrts_details=2 if q427==2                                   // Married
		* replace ilo_mrts_details=3 if                                         // Union / cohabiting
		replace ilo_mrts_details=4 if q427==4                                   // Widowed
		replace ilo_mrts_details=5 if q427==3                                   // Divorced / separated
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Following the national report, persons in employment are those aged
*            15 years or more and who during the reference week, were engaged in
*            any activity to produce goods or provide services for pay or profit
*            for one hour or more. It does not include own-use production workers
*            (mostly household consumption) (note to table: S14:4077).
*          - Unemployment includes those satisfying ILO's three criteria and 
*            available future starters.
*          - Note that given there is a small proportion of persons belonging to
*            the working-age population that did not reply to the labour module,
*            these are going to be excluded from now on from the dataset (3% of the
*            total).

    keep if emp_status!=9
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q509==1 | q510==1) & ilo_wap==1				                     // Employed
		replace ilo_lfs=1 if ((q511==1 | q512==1) & inlist(q513,1,2)) & ilo_wap==1               // Employed  (only or mainly for sale/barter)  
		replace ilo_lfs=1 if ((q514==1 | q515==1) & (inlist(q516,1,2,3,4))) & ilo_wap==1		 // Employed: temporary absent
		replace ilo_lfs=1 if ((q517==1 | q518==1)) & ilo_wap==1		                             // Employed: temporary absent coming back within 3 months or less or receiving income
		replace ilo_lfs=2 if ilo_lfs!=1 & (q519==1 & q525==1) & ilo_wap==1                       // Unemployed: three criteria
		replace ilo_lfs=2 if ilo_lfs!=1 & (q522==1 & q525==1) & ilo_wap==1			             // Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=2 if (q626==1) & ilo_lfs==1                             // More than one job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - The original category "group worker" refers to: "[...] Group workers
*            have been added to capture one of the forms in which locals organise
*            their work, mainly in the fishing, construction and small-scale 
*            manufacturing industries. These workers organize themselves in groups
*            whereby the income they get will be distributed among the group, with
*            each member acting as own-account worker, with one person acting as
*            group leader[...]". Thus it is classified as members of producers'
*            cooperatives.
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if q610==1 & ilo_lfs==1                  // Employees
		 replace ilo_job1_ste_icse93=2 if q610==2 & ilo_lfs==1                  // Employers
		 replace ilo_job1_ste_icse93=3 if inlist(q610,3,4) & ilo_lfs==1         // Own-account workers
		 replace ilo_job1_ste_icse93=4 if q610==6 & ilo_lfs==1                  // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if q610==5 & ilo_lfs==1                  // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if q636==1 & ilo_mjh==2                 // Employees
		  replace ilo_job2_ste_icse93=2 if q636==2 & ilo_mjh==2                 // Employers
		  replace ilo_job2_ste_icse93=3 if inlist(q636,3,4) & ilo_mjh==2        // Own-account workers
		  replace ilo_job2_ste_icse93=4 if q636==6 & ilo_mjh==2                 // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if q636==5 & ilo_mjh==2                 // Contributing family workers
...
* ------------------------------------------------------------------------------
    ...
    * MAIN JOB:
    ***********
	gen eco_prim = q6011isic_2
	destring (eco_prim), replace force
   ...
   * Aggregate level
    ...
    * SECOND JOB:
    *************
	gen eco_secu = q6271isic_2
	destring (eco_secu), replace force
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = eco_secu if ilo_mjh==2
...
* ------------------------------------------------------------------------------
    ...
    * MAIN JOB:
    ***********
    gen ocu_prim = q605isco_2
	destring (ocu_prim), replace force
    ...
    * SECOND JOB:
    *************
    gen ocu_secu = q631isco_2
	destring (ocu_secu), replace force	
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = ocu_secu if ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Public includes: govenment office, independent government institution
*            MNDF/Police, state-owned enterprises and embassies or international 
*            bodies.
		
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(q602,1,2,4,5,8) & ilo_lfs==1    // Public
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only hours actually worked last week available for main and second
*            job.
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = (q606*q607) if ilo_lfs==1
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual = .
	    replace ilo_job2_how_actual = (q632*q633) if ilo_mjh==2
	...
	* Hours actually worked in other activities
    gen ilo_job3_how_actual = .
	    replace ilo_job3_how_actual = q6521hrs if ilo_lfs==1 & q652==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Threshold is based on the weighted median of the hours actually 
*            worked in all jobs (48 hours per week).
			
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if ilo_joball_how_actual>=48 & ilo_lfs==1                              // Full-time
		replace ilo_job1_job_time=1 if ilo_joball_how_actual<48 & ilo_joball_how_actual!=. & ilo_lfs==1    // Part-time
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if q619==3 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if inlist(q619,1,2) & ilo_lfs==1               		// Temporary
...
* ------------------------------------------------------------------------------
/* Useful questions:
          - Institutional sector: q602
		  - Private household identification: (ilo_job1_eco_isic4_2digits==97 | ilo_job1_ocu_isco08_2digits==63) / q602 (option 11)
		  - Destination of production: not asked.
		  - Bookkeeping: not asked.
		  - Registration: q603 (only asked to private companies/own enterprises.
		  - Status in employment: ilo_job1_ste_aggregate
		  - Social security contribution (Proxy: pension funds): q621 (pension) / q623 (health)
		  - Place of work: q602 (option 11 for non-fixed premises)
		  - Size: q612
		  - Paid annual leave: q62211get (get annual leave)
		  - Paid sick leave: q62231get (get sick leave)
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if q621==1 & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if q621==2 & ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(q602,1,2,3,4,5,8)) | ///
									   (inlist(q602,6,7,9,10,12,99) & q603==1) | ///
									   (inlist(q602,6,7,9,10,12,99) & inlist(q603,9,.) & ilo_job1_ste_aggregate==1 & social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((inlist(ilo_job1_ste_aggregate,1,6) & social_security==1) | ///
										 (inlist(ilo_job1_ste_aggregate,1,6) & social_security==. & q62211get==1 & q62231get==1) | ///
										 (inlist(ilo_job1_ste_aggregate,2,4) & ilo_job1_ife_prod==2) | ///
										 (ilo_job1_ste_aggregate==3 & ilo_job1_ife_prod==2))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National currency: Rufiyaa.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Following the national report the threshold is set at 48 hours 
*            actually worked per week.

	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_actual<48 & ilo_joball_how_actual!=0 & q654==1 & q655==1 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if q523==1 & ilo_lfs==2                           // Previously employed       
		replace ilo_cat_une=2 if q523==2 & ilo_lfs==2                           // Seeking for the first time
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Category (2): "1 month to less than 3 months" include category (3):
*            "3 months to less than 6 months". (note to value: C7:3906).

	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if q521==1 & ilo_lfs==2                       // Less than 1 month
		replace ilo_dur_details=2 if q521==2 & ilo_lfs==2                       // 1 to 3 months (including 3-6 months)[C7:3906]
		* replace ilo_dur_details=3 if                                          // 3 to 6 months
		replace ilo_dur_details=4 if q521==3 & ilo_lfs==2                       // 6 to 12 months
		replace ilo_dur_details=5 if q521==4 & ilo_lfs==2                       // 12 to 24 months
		replace ilo_dur_details=6 if q521==5 & ilo_lfs==2                       // 24 months or more
...
*            3 and 4 are not possible to be produced. 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if q519==1 & q525==2 & ilo_lfs==3              // Seeking, not available
		replace ilo_olf_dlma = 2 if q519==2 & q525==1 & ilo_lfs==3	            // Not seeking, available
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(q522,9,10,14) & ilo_lfs==3           // Labour market 
			replace ilo_olf_reason=2 if q522==2 & ilo_lfs==3                        // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(q522,3,4,5,6,7,12,13) & ilo_lfs==3   // Personal/Family-related
			replace ilo_olf_reason=4 if inlist(q522,8,11,15) & ilo_lfs==3           // Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & q525==1 & ilo_olf_reason==1
...
cd "$outpath"
	...
	drop eco_prim eco_secu ocu_prim ocu_secu social_security
	compress
