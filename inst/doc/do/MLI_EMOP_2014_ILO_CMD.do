...
* NOTES: 
* Files created: Standard variables on EMOP Mali, 2014
...
* Comment: size : 6+ instead of 5+ considered (given options in original question)
	...
	* Useful questions: ap4: Institutional sector
	...
	gen registration=.
						replace registration=2 if ap6a==2 | ap6b==2 | ap6c==2 | ap6d==2 // not registered
						replace registration=3 if ap6a==3 & ap6b==3 & ap6c==3 & ap6d==3 // doesn't know
					replace registration=1 if ap6a==1 | ap6b==1 | ap6c==1 | ap6d==1 // registered
							
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=1 if (!inlist(ap4,1,2,4,5,6,7) & ap8c!=1 & registration==2) | (!inlist(ap4,1,2,4,5,6,7) & ap8c!=1 & !inlist(registration,1,2) ///
					& ((ilo_job1_ste_aggregate==1 & ap16b6!=1) | ilo_job1_ste_aggregate!=1) & (!inlist(ap7,3,9) | (inlist(ap7,3,9) & !inrange(ap5,4,9))))
		replace ilo_job1_ife_prod=2 if inlist(ap4,1,2,4,5,6) | (!inlist(ap4,1,2,4,5,6,7) & ap8c==1) | (!inlist(ap4,1,2,4,5,6,7) & ap8c!=1 & registration==1) | ///
						(!inlist(ap4,1,2,4,5,6,7) & ap8c!=1 & !inlist(registration,1,2) & ilo_job1_ste_aggregate==1 & ap16b6==1) | (!inlist(ap4,1,2,4,5,6,7) & ap8c!=1 & !inlist(registration,1,2) & ///
						 ((ilo_job1_ste_aggregate==1 & ap16b6!=1) | ilo_job1_ste_aggregate!=1) & inlist(ap7,3,9) & inrange(ap5,4,9))
...
cd "$outpath"
		...
		drop indu_code_prim  occ_code_prim occ_code_prim_1dig nat_lfs bonus
