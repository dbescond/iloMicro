...
* NOTES: 
* Files created: Standard variables on EMOP Mali, 2015
* Authors: ILO / Department of Statistics / DPAU

* Starting Date: 19 April 2017
...
global source "EMOP"
global time "2015"
global inputFile "MLI_2015_EMOP_STATA"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: question made to people aged 12 years and above. 
...
* Comment: 
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if ea2b==1 | inrange(ea3,1,9) | (ea4==1 & (inlist(ea5a,1,2,3) | (ea5a==4 & ea6a==1) | (inrange(ea5a,5,9) & ea6b==1)))
	...
	* reproduce national figures 
		* pre-defined variables: statut_ela2 situat situat2
		* --> given the outcome, even if in technical document "ILO criteria" mentioned, the criteria used do not correspond to the ILO definition at all
			* nat_lfs coincides almost perfectly with statut_ela2, situat and situat2
	
	gen nat_lfs=.
		replace nat_lfs=1 if ea2b==1 | inrange(ea3,1,9) | (ea4==1 & (inlist(ea5a,1,2,3) | (ea5a==4 & ea6a==1) | (inrange(ea5a,5,9) & ea6b==1)))
		replace nat_lfs=2 if nat_lfs!=1 & (ea7a==1 | inlist(ea7c,1,2) | (ea7b==2 & ea8a==1 & ea8b2==1))
		replace nat_lfs=3 if !inlist(nat_lfs,1,2)
			replace nat_lfs=. if !inrange(ilo_age,15,64)
...
* Comment: size : 6+ instead of 5+ considered (given options in original question)
	...
	* Useful questions: ap4: Institutional sector
					  ...
					  * ap6ba ap6bb ap6bc ap6bd : Registration (different types of registration)
	...
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=1 if (!inlist(ap4,1,2,4,5,6,7) & ap8c1!=2 & registration==2) | (!inlist(ap4,1,2,4,5,6,7) & ap8c1!=2 & !inlist(registration,1,2) ///
					& ((ilo_job1_ste_aggregate==1 & ap16b6!=1) | ilo_job1_ste_aggregate!=1) & (!inlist(ap7,3,7,9) | (inlist(ap7,3,7,9) & !inrange(ap5,4,9))))
		replace ilo_job1_ife_prod=2 if inlist(ap4,1,2,4,5,6) | (!inlist(ap4,1,2,4,5,6,7) & ap8c1==2) | (!inlist(ap4,1,2,4,5,6,7) & ap8c1!=2 & registration==1) | ///
						(!inlist(ap4,1,2,4,5,6,7) & ap8c1!=2 & !inlist(registration,1,2) & ilo_job1_ste_aggregate==1 & ap16b6==1) | (!inlist(ap4,1,2,4,5,6,7) & ap8c1!=2 & !inlist(registration,1,2) & ///
...
* -------------------------------------------------------------------------------------------
*
* Comment: can't be defined due to filter questions 
