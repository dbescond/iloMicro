* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Mali, 2016
* DATASET USED: Mali HS 2016
* NOTES: 
* Files created: Standard variables on EMOP Mali, 2016
...
global source "EMOP"
global time "2016"
global inputFile "MLI_EMOP_2016.dta"
...
* -------------------------------------------------------------------------------------------
*
* Comment: Following the ISCED 11 mapping, see "isced_2011_mapping_template_fr_mali_0.xlsx" under Internal
*          High share of "Not elsewhere classified" as question asked for individuals aged 3 and above
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if m17==2 | m18==12                       // No schooling
		replace ilo_edu_isced11=2 if  inrange(m18,0,3)                      // Early childhood education
		replace ilo_edu_isced11=3 if  inrange(m18,4,9)                       // Primary education
		replace ilo_edu_isced11=4 if  inlist(m19,2,3)                      // Lower secondary education
		replace ilo_edu_isced11=5 if  inlist(m19,4,5,6)                       // Upper secondary education
		*replace ilo_edu_isced11=6 if                        // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if inlist(m19,7)                       // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if m19==8                       // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if  m19==9                      // Master's or equivalent level
		replace ilo_edu_isced11=10 if m19==10                      // Doctoral or equivalent level
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: question asked to people aged 12 years and above. 
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if m06==4                                    // Single
		replace ilo_mrts_details=2 if inlist(m06,1,2)                           // Married
		replace ilo_mrts_details=3 if m06==3                                    // Union / cohabiting
		replace ilo_mrts_details=4 if m06==6                                    // Widowed
		replace ilo_mrts_details=5 if m06==5                                    // Divorced / separated
...
* Comment: No question asked on disability status
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (ea2b==1 | inrange(ea3,1,9)) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if (ea4==1 & (inlist(ea5a,1,2,3) | (ea5a==4 & ea6a==1) | (inrange(ea5a,5,9) & ea6b==1))) & ilo_wap==1						// Employed: temporary absent
		replace ilo_lfs=2 if ((ea7a==1 | ea7b==1) & (inlist(ea7c,1,2) | ea8b2==1) ) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (ea8b1==12 &(inlist(ea7c,1,2) | ea8b2==1)) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
* ------------------------------------------------------------------------------------------- 
*
* Comment: apprentices categorized as employee. Associé (associate) categorized as employers (to be checked)

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(ap3,1,2,3,4,5,9)  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if inlist(ap3,6,8) & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if ap3==7 & ilo_lfs==1          // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if ap3==8 & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if inlist(ap3,10) & ilo_lfs==1          // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if inlist(as4,1,2,3,4,5,9)  & ilo_mjh==2          // Employees
		  replace ilo_job2_ste_icse93=2 if inlist(as4,6,8) & ilo_mjh==2          // Employers
		  replace ilo_job2_ste_icse93=3 if  as4==7 & ilo_mjh==2          // Own-account workers
		  *replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2          // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if inlist(as4,10) & ilo_mjh==2          // Contributing family workers
...
gen indu_code_sec=int(as3/10) if ilo_lfs==1 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Low share of "5- Elementary occupations", as farmers are ending into "6- Skilled agricultural, forestry and fishery workers" category because of the original "ap6" coded variable
    ...
    ***********
		...
		replace occ_code_prim=ap1 if ilo_lfs==1
		replace occ_code_prim=110000 if ap1==6 & ilo_lfs==1
		replace occ_code_prim=int(occ_code_prim/10000) if ilo_lfs==1
	...
	* 1 digit level
		...
		replace ilo_job1_ocu_isco08=occ_code_prim if ilo_lfs==1
    ...
    *************	
		...
		gen occ_code_sec=. if ilo_lfs==1
		replace occ_code_sec=as2 if ilo_lfs==1
		replace occ_code_sec=110000 if as2==6 & ilo_mjh==2
		replace occ_code_sec=int(occ_code_sec/10000) if ilo_mjh==2
	...
	* 1-digit level 				
	...
	replace ilo_job2_ocu_isco08=occ_code_sec if ilo_mjh==2
	    replace ilo_job2_ocu_isco08=11 if ilo_job2_ocu_isco08==. & ilo_mjh==2                          // Not elsewhere classified
...
* Comment: 
		...
		replace ilo_job1_ins_sector=1 if  inlist(ap4,1,2,6)                      & ilo_lfs==1    // Public
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = ap13c       if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = ap11      if ilo_lfs==1
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual = .
	    replace ilo_joball_how_usual = as9      if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Using ilo_job1_how_actual rather than ilo_joball_how_usual_bands as there is a high share of not classified
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if ap8e==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if inlist(ap8e,2,3,4) & ilo_lfs==1               				// Temporary
...
/* Useful questions:
          - Institutional sector: ap4
		  - Private household identification: 
		  - Destination of production: ap8c2
		  - Bookkeeping: ap8c1
		  - Registration:  ap6ba ap6bb ap6bc ap6bd
		  - Status in employment: ilo_job1_ste_icse93
		  - Social security contribution (Proxy: pension funds): ap16b6
		  - Place of work: ap7
		  - Size: ap5
		  - Paid annual leave: ap16b4
		  - Paid sick leave: ap16b5
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if  ap16b6==1 &   ilo_lfs==1          // social security (proxy)
		replace social_security=2 if  ap16b6==2 &  ilo_lfs==1          // no social security (proxy)
		
	* Registration -> A "yes" to one of the four questions is enough for being considered registered. Similar for "Not registered"
	gen registration=.
		replace registration=2 if ap6ba==2 | ap6bb==2 | ap6bc==2 | ap6bd==2 // not registered
		replace registration=3 if ap6ba==3 & ap6bb==3 & ap6bc==3 & ap6bd==3 // doesn't know
		replace registration=1 if ap6ba==1 | ap6bb==1 | ap6bc==1 | ap6bd==1 // registered
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(ap4,1,2,6)) | ///
									   (inlist(ap4,3,4,5) & ap8c2!=3 & inlist(ap8c1,2,3)) | ///
									   (inlist(ap4,3,4,5) & ap8c2!=3 & !inlist(ap8c1,2,3) & registration==1) | ///
									   (inlist(ap4,3,4,5) & ap8c2!=3 & !inlist(ap8c1,2,3) & inlist(registration,3,.) & ilo_job1_ste_aggregate==1 & social_security==1 ) | /// 
									   (inlist(ap4,3,4,5) & ap8c2!=3 & !inlist(ap8c1,2,3) & inlist(registration,3,.) & ilo_job1_ste_aggregate==1 & social_security==2 & ap7==9 & inrange(ap5,4,9)))							   
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((inlist(ilo_job1_ste_icse93,1,.) & social_security==1) | ///
										 ((inlist(ilo_job1_ste_icse93,1,.) & social_security==.) & ap16b4==1 & ap16b5==1) | ///
										 ((inlist(ilo_job1_ste_icse93,2,4) & ilo_job1_ife_prod==2)) | ///
										 (inlist(ilo_job1_ste_icse93,3) & inlist(ap8c2,1,2) & ilo_job1_ife_prod==2))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Currency in Mali: CFA Franc
    ...
    * SECOND JOB:
    *************
	capture confirm variable  as11bas2
	if !_rc {
	* Employees
	gen ilo_job2_lri_ees = .
	    replace ilo_job2_lri_ees = as11bas2     if ilo_job2_ste_aggregate==1
	...
	* Self-employed		
	gen ilo_job2_lri_slf = .
	    replace ilo_job2_lri_slf = as11bas2      if ilo_job2_ste_aggregate==2
	...
	* Employees
	gen ilo_job2_lri_ees = .
	    replace ilo_job2_lri_ees = as11ba     if ilo_job2_ste_aggregate==1
		replace ilo_job2_lri_ees = .     if ilo_mjh!=2
		        lab var ilo_job2_lri_ees "Monthly earnings of employees - second job"
				
	* Self-employed		
	gen ilo_job2_lri_slf = .
	    replace ilo_job2_lri_slf = as11ba      if ilo_job2_ste_aggregate==2
		replace ilo_job2_lri_slf = .     if ilo_mjh!=2
		        lab var ilo_job2_lri_slf "Monthly labour related income of self-employed - second job"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: r3 -> likeworkmore, but no information on availability
/*
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<=35  & r3==1 & ilo_lfs==1
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if c2==1     & ilo_lfs==2                     // Previously employed       
		replace ilo_cat_une=2 if c2==2     & ilo_lfs==2                     // Seeking for the first time
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if  inlist(c1_2,0,1)     & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if  inlist(c1_2,2,3)       & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if  inlist(c1_2,4,5,6)       & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if  inlist(c1_2,7,10) & c1_2==11   & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if inlist(c1_1,1,2) & c1_2==11        & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if c1_1>2      & ilo_lfs==2                 // 24 months or more
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Questions on previous economic activity not asked to unemployed individuals
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if (ea7a==1 | ea7b==1) & (!inlist(ea7c,1,2) | ea8b2==2) & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if (ea7a==2 | ea7b==2) & (inlist(ea7c,1,2) | ea8b2==1)              & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if (ea7a==2 | ea7b==2) & (!inlist(ea7c,1,2) | ea8b2==2) & ea8a==1		      & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (ea7a==2 | ea7b==2) & (!inlist(ea7c,1,2) | ea8b2==2) & ea8a==2		      & ilo_lfs==3      // Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Individuals attending education (ilo_edu_attendance==1) classified as "4 - Does not need/want to work"
*          Individuals declaring to be retired classified as "4 - Does not need/want to work"


		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(ea8b1,1,2,3)         & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(ea8b1,4,12)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(ea8b1, 6,7,8,9,10,11,13)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if inlist(ea8b1,5) | (ilo_edu_attendance==1 & !inlist(ilo_olf_reason,1,2,3)) | tp14a==1 | ea8b3==3         & ilo_lfs==3             // Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & (inlist(ea7c,1,2) | ea8b2==1)  & ilo_olf_reason==1
