...
* DATASET USED: Myanmar, Labour Force Survey, 2017 Quarter 1
* NOTES: 
* Files created: Standard variables MMR_LFS_2017Q1_FULL.dta and MMR_LFS_2017Q1_ILO.dta
...
global source "LFS"
global time "2017Q1"
global inputFile "MMR_LFS_2017Q1"
...
* Comment: Question B6 is used (highest level completed for persons aged 5 years and above)
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if b6==1					// No schooling
		replace ilo_edu_isced11=2 if b6==2					// Early childhood education
		replace ilo_edu_isced11=3 if b6==3					// Primary education
		replace ilo_edu_isced11=4 if b6==4					// Lower secondary education
		replace ilo_edu_isced11=5 if b6==5					// Upper secondary education
		* replace ilo_edu_isced11=6 if						// Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if b6==6					// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if b6==7					// Bachelor's or equivalent level
		* replace ilo_edu_isced11=9 if						// Master's or equivalent level
		* replace ilo_edu_isced11=10 if						// Doctoral or equivalent level
		replace ilo_edu_isced11=11 if b6==8					// Not elsewhere classified
...
* ------------------------------------------------------------------------------ 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if b7==1					// Attending
		replace ilo_edu_attendance=2 if inlist(b7,2,8)			// Not attending
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Marital status (b9) question is made to people aged 14 years and above. 
*          - There is no info on union/cohabiting.
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if b9==1                                     // Single
		replace ilo_mrts_details=2 if b9==2                                     // Married
		*replace ilo_mrts_details=3 if                                          // Union / cohabiting
		replace ilo_mrts_details=4 if b9==5                                     // Widowed
		replace ilo_mrts_details=5 if inlist(b9,3,4)                            // Divorced / separated
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=2 if (g1==1 & g2a!=12 & inlist(g6,1,2) & ilo_lfs!=1 & ilo_wap==1)	// Unemployed: three criteria
...
* ------------------------------------------------------------------------------
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual=.
	    replace ilo_joball_how_usual=f3 if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru=.
	    replace ilo_joball_tru=1 if (ilo_joball_how_usual<=40 & f4==1 & f5==1 & ilo_lfs==1)
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if (g1==1 & g6==3 & ilo_lfs==3)			// Seeking, not available
		replace ilo_olf_dlma=2 if (g1==2 & inlist(g6,1,2) & ilo_lfs==3)	// Not seeking, available
		replace ilo_olf_dlma=3 if (g1==2 & g3==1 & g6==3 & ilo_lfs==3)	// Not seeking, not available, willing
		replace ilo_olf_dlma=4 if (g1==2 & g3==2 & g6==3 & ilo_lfs==3)	// Not seeking, not available, not willing
