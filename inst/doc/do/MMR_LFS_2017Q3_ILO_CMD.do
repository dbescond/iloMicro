* TITLE OF DO FILE: ILO Microdata Preprocessing code template - MYANMAR, 2017
* DATASET USED: Myanmar, Labour Force Survey, 2017 Quarter 3
* NOTES: 
...
global source "LFS"
global time "2017Q3"
global inputFile "MMR_LFS_2017Q3"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if u_r==1		// Urban 
		replace ilo_geo=2 if u_r==2		// Rural
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if b4==1		// Male
		replace ilo_sex=2 if b4==2		// Female
...
* Comment: Question B6 is used (highest level completed for persons aged 5 years and above)

	* ISCED 11
				
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if b6==2					// No schooling
		replace ilo_edu_isced11=1 if b7==1					// No schooling
		replace ilo_edu_isced11=2 if b7==2					// Early childhood education
		replace ilo_edu_isced11=3 if b7==3					// Primary education
		replace ilo_edu_isced11=4 if b7==4					// Lower secondary education
		replace ilo_edu_isced11=5 if b7==5					// Upper secondary education
		* replace ilo_edu_isced11=6 if						// Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if b7==6					// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if b7==7					// Bachelor's or equivalent level
		* replace ilo_edu_isced11=9 if						// Master's or equivalent level
		* replace ilo_edu_isced11=10 if						// Doctoral or equivalent level
		replace ilo_edu_isced11=11 if b7==8					// Not elsewhere classified
...
* ------------------------------------------------------------------------------ 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if b8==1					// Attending
		replace ilo_edu_attendance=2 if b8==2					// Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if b10==1					// Single
		replace ilo_mrts_details=2 if b10==2					// Married
		* replace ilo_mrts_details=3 if 						// Union / cohabiting
		replace ilo_mrts_details=4 if b10==5					// Widowed
		replace ilo_mrts_details=5 if inlist(b10,3,4)			// Divorced / separated
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c1==1 | c4==2 | inlist(c5,1,2)) & ilo_wap==1								// Employed: ILO definition
		replace ilo_lfs=1 if (inlist(c7,1,2,3,4) | c8==1 | c9==1) & ilo_wap==1							// Employed: temporary absent
		replace ilo_lfs=2 if ((g1a==1 | g1b==1) & g2a!=12 & inlist(g6,1,2) & ilo_lfs!=1 & ilo_wap==1)	// Unemployed: three criteria
		replace ilo_lfs=2 if (g4==1 & inlist(g6,1,2) & ilo_lfs!=1 & ilo_wap==1)							// Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (e1!=1 & ilo_lfs==1)		// One job only     
		replace ilo_mjh=2 if (e1==1 & ilo_lfs==1)		// More than one job
...
* ------------------------------------------------------------------------------
   ...
   * MAIN JOB - ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if (inlist(d7,1,2,3) & ilo_lfs==1)		// Employees
	...
	* SECOND JOB - ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if (inlist(e4,1,2,3) & ilo_mjh==2)		// Employees
		  replace ilo_job2_ste_icse93=2 if (e4==4 & ilo_mjh==2)					// Employers
		  replace ilo_job2_ste_icse93=3 if (e4==5 & ilo_mjh==2)					// Own-account workers
		  * replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2						// Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if (e4==6 & ilo_mjh==2)					// Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits=. 
	    replace ilo_job1_eco_isic4_2digits=int(d1/100) if ilo_lfs==1
   ...
   * Aggregate level
    ...
    * SECOND JOB - ISIC REV. 4
	
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits=. 
	    replace ilo_job2_eco_isic4_2digits=int(e2/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits=. 
	    replace ilo_job1_ocu_isco08_2digits=int(d2/100) if ilo_lfs==1
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits=. 
	    replace ilo_job2_ocu_isco08_2digits=int(e3/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(d3,1,4,5) & ilo_lfs==1			// Public
...
* ------------------------------------------------------------------------------
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual=.
	    replace ilo_job1_how_usual=d26 if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    replace ilo_job1_how_actual=d25 if ilo_lfs==1
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual=.
	    replace ilo_job2_how_usual=e7 if ilo_mjh==2
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual=.
	    replace ilo_job2_how_actual=e6 if ilo_mjh==2
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual=.
		replace ilo_joball_how_usual=(e7+d26+f2) if (ilo_lfs==1 & ilo_mjh==2)
		replace ilo_joball_how_usual=(e7+d26) if (ilo_lfs==1 & ilo_mjh==2 & ilo_joball_how_usual==.)
	    replace ilo_joball_how_usual=e7 if (ilo_lfs==1 & ilo_mjh==1)
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual=.
	    replace ilo_joball_how_actual=(e6+d25) if (ilo_lfs==1 & ilo_mjh==2)
		replace ilo_joball_how_actual=d25 if (ilo_lfs==1 & ilo_mjh==1)
...
* Comment: Based on 40 hours per week
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if (d9==2 & ilo_lfs==1)						// Permanent
		replace ilo_job1_job_contract=2 if (d9==1 & ilo_lfs==1)						// Temporary
...
/* Useful questions:
          - Institutional sector: D3
		  - Destination of production: C5
		  - Bookkeeping: NA
		  - Registration: D4A
		  - Status in employment: D7
		  - Social security contribution (Proxy: pension funds): D11
		  - Place of work: D6
		  - Size: NA
		  - Paid annual leave: D12
		  - Paid sick leave: D13
*/

    * Social Security:
	gen social_security=.
	    replace social_security=1 if inlist(d11,1,2,3) & ilo_lfs==1		// Social security (proxy)
		replace social_security=2 if inlist(d11,4,5,8) & ilo_lfs==1		// No social security (proxy)
	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (d3==8 | ilo_job1_eco_isic4_2digits==97 | ilo_job1_ocu_isco08_2digits==63)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & (inlist(d3,1,2,3,4,5) | inlist(d4a,1,2,3,4,5,6))
...
* ------------------------------------------------------------------------------
	...
	* Employees
	gen ilo_job1_lri_ees=.
	    replace ilo_job1_lri_ees=d16c if (ilo_job1_ste_aggregate==1 & d17==2)
		replace ilo_job1_lri_ees=(d16c*d18) if (ilo_job1_ste_aggregate==1 & d17==1)
	    replace ilo_job1_lri_ees=. if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf=.
	    replace ilo_job1_lri_slf=d21 if ilo_job1_ste_aggregate==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru=.
	    replace ilo_joball_tru=1 if (ilo_joball_how_usual<=40 & f3==1 & f4==1 & ilo_lfs==1)
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    * replace ilo_dur_details=1 if ilo_lfs==2						// Less than 1 month
		replace ilo_dur_details=2 if (g5==1 & ilo_lfs==2)				// 1 to 3 months
		replace ilo_dur_details=3 if (g5==2 & ilo_lfs==2)				// 3 to 6 months
		replace ilo_dur_details=4 if (g5==3 & ilo_lfs==2)				// 6 to 12 months
		replace ilo_dur_details=5 if (g5==4 & ilo_lfs==2)				// 12 to 24 months
		replace ilo_dur_details=6 if (inlist(g5,5,6) & ilo_lfs==2)		// 24 months or more
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if ((g1a==1 | g1b==1) & g6==3 & ilo_lfs==3)			// Seeking, not available
		replace ilo_olf_dlma=2 if ((g1a==2 & g1b==2) & inlist(g6,1,2) & ilo_lfs==3)	// Not seeking, available
		replace ilo_olf_dlma=3 if ((g1a==2 & g1b==2) & g3==1 & g6==3 & ilo_lfs==3)	// Not seeking, not available, willing
		replace ilo_olf_dlma=4 if ((g1a==2 & g1b==2) & g3==2 & g6==3 & ilo_lfs==3)	// Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if (inlist(g4,7,8,10,11) & ilo_lfs==3)	// Labour market 
			replace ilo_olf_reason=2 if (inlist(g4,1,2,3) & ilo_lfs==3)		// Other labour market reasons
			replace ilo_olf_reason=3 if (inlist(g4,4,5,6) & ilo_lfs==3)		// Personal/Family-related
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if (!inlist(ilo_lfs,1,2) & inlist(g6,1,2) & ilo_olf_reason==1)
