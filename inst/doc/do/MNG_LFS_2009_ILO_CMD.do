* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Mongolia, 2009
...
global source "LFS"
global time "2009"
global inputFile "8. LFS 2008-2009_eng.dta"
...
* -------------------------------------------------------------------------------------------		
	...
	* As we do not know the level of disability, only the aggregated variable can be done.
		...
		gen ilo_dsb_aggregate=.
			replace ilo_dsb_aggregate=1 if i12==2	 // Without disablitity
			replace ilo_dsb_aggregate=2 if i12==1	 // With disability
...
gen ilo_lfs=.
    replace ilo_lfs=1 if ii51>=1 & ii51!=. & ilo_wap==1 				        // Employed
	replace ilo_lfs=1 if ii52==1 &	ilo_wap==1		 					        // Employed
	replace ilo_lfs=2 if ilo_lfs!=1 & (iv104==1 & iv105==1) & ilo_wap==1        // Unemployed	
...
/* NOTE: Question 58 asks the status in employment. One of the answers is "Herder", which does 
         not clearly specify the status in employment. As the country classifies them under 
	...
	* MAIN JOB:
		...
		* Detailled categories:
			gen ilo_job1_ste_icse93=.
				replace ilo_job1_ste_icse93=1 if (ii58==1 & ilo_lfs==1)	             // Employees
				replace ilo_job1_ste_icse93=2 if (ii58==2 & ilo_lfs==1)			     // Employers
				replace ilo_job1_ste_icse93=3 if (inlist(ii58,3,5) & ilo_lfs==1)	 // Own-account workers (including herders)
				replace ilo_job1_ste_icse93=4 if (ii58==4 & ilo_lfs==1)			     // Producer cooperatives
				replace ilo_job1_ste_icse93=5 if (ii58==6 & ilo_lfs==1)			     // Contributing family workers
				replace ilo_job1_ste_icse93=6 if ((ii58==7 | ii58==.) & ilo_lfs==1)  // Not classifiable
	...
	* SECOND JOB:
		...
		* Detailled categories:
			gen ilo_job2_ste_icse93=.
				replace ilo_job2_ste_icse93=1 if (inlist(ii84,1,2) & ilo_mjh==2)	 // Employees
				replace ilo_job2_ste_icse93=2 if (ii84==3 & ilo_mjh==2)			     // Employers
				replace ilo_job2_ste_icse93=3 if (ii84==5 & ilo_mjh==2)	             // Own-account workers
...
* -------------------------------------------------------------------------------------------
	...
	/* ISIC Rev. 4 (2-digits)*/
		...
		* SECOND JOB:
			...
			* ISIC Rev. 4 - 2 digit
						gen isic2dig_job2 = int(ii87/100)
...
* -------------------------------------------------------------------------------------------		
	...
	* MAIN JOB:
			...
			gen occup_2dig=int(ii53/100) if ilo_lfs==1 
...
* -------------------------------------------------------------------------------------------		
	...
	/* Question 62 (88 for the second job) 	
	...
	* MAIN JOB:
		...
		gen ilo_job1_ins_sector=.
			replace ilo_job1_ins_sector=1 if inlist(ii62,40,50,60) & ilo_lfs==1	     	// Public: state owned entrerprises, local government owned enterprises, budget organization
			replace ilo_job1_ins_sector=2 if ilo_job1_ins_sector!=1 & ilo_lfs==1		// Private: non-profit organization, private sector
	...
	* SECOND JOB:
		...
		gen ilo_job2_ins_sector=.
			replace ilo_job2_ins_sector=1 if inlist(ii88,40,50,60)& ilo_mjh==2			// Public 
...
* -------------------------------------------------------------------------------------------		
	...
	/* Here is the information we have in the questionnaire:
			- Question 51: actual hours worked in all jobs 
			- Question 54: usual hours worked in main job
		...
		* MAIN JOB:
			...
			* 2) Weekly hours USUALLY worked:
				gen ilo_job1_how_usual=ii54 if ilo_lfs==1
					lab var ilo_job1_how_usual "Weekly hours usually worked in main job"
					
				gen ilo_job1_how_usual_bands=.
				    replace ilo_job1_how_usual_bands=1 if ilo_job1_how_usual<1
		...
		* SECOND JOB
			...
			* 1) Weekly hours ACTUALLY worked:
				...
				gen ilo_job2_how_actual_bands=.
					replace ilo_job2_how_actual_bands=1 if ilo_job2_how_actual<1
	   ...
	   * ALL JOBS:
			...
			* 1) Weekly hours ACTUALLY worked:
				gen ilo_joball_how_actual=ii51 if ilo_lfs==1
						lab var ilo_joball_how_actual "Weekly hours actually worked in all jobs"
		
				gen ilo_joball_how_actual_bands=.
					replace ilo_joball_how_actual_bands=1 if ilo_joball_how_actual<1
...
* -------------------------------------------------------------------------------------------			
	...
	/* The hours of wor per week shall be up to 40 hours (general limit), accoring to the TRAVAIL 
	database from the ILO*/
    ...
    * Formal/informal sector
		gen ilo_job1_ife_prod=.
			...
			replace ilo_job1_ife_prod=2 if ilo_job1_ife_prod==. & ilo_lfs==1 & ///
										   ...
										   (inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,4,.) & ilo_job1_ste_icse93==1 & social_security!=1 & inlist(ii59,2,3,4,7,8) & inrange(ii71,6,12)) | /// 
										   (inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,4,.) & ilo_job1_ste_icse93!=1 & inlist(ii59,2,3,4,7,8) & inrange(ii71,6,12)))
			replace ilo_job1_ife_prod=1 if ilo_job1_ife_prod==. & ilo_lfs==1 & ///
			                               ((inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,2,3)) | /// 
			                               (inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,4,.) & ilo_job1_ste_icse93==1 & social_security!=1 & inlist(ii59,2,3,4,7,8) & inlist(ii71,1,2,3,4,5,.)) | /// 
										   (inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,4,.) & ilo_job1_ste_icse93==1 & social_security!=1 & inlist(ii59,1,5,6,9,10,11,12,13,14,15,.)) | ///
										   (inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,4,.) & ilo_job1_ste_icse93!=1 & inlist(ii59,1,5,6,9,10,11,12,13,14,15,.)) | ///
										   (inlist(ii62,80,.) & ii68!=3 & ii65!=1 & inlist(ii69,4,.) & ilo_job1_ste_icse93!=1 & inlist(ii59,2,3,4,7,8) & inlist(ii71,1,2,3,4,5,.)))
...
* -------------------------------------------------------------------------------------------			
	   ...
	   * No national threshold  could be found, so the widely used threshold of 35 hour/week is used. 	 	*/
				...
				gen ilo_joball_tru=.
					replace ilo_joball_tru=1 if ilo_lfs==1 & iii100==1 & ilo_joball_how_actual<35
...
* -------------------------------------------------------------------------------------------
	...
	/* NOTE: information on previous activity is only asked to people aged  5-60 (so the persons 
		...
		gen ilo_cat_une=.
			replace ilo_cat_une=1 if ilo_lfs==2 & vi132==1							// Previously employed
			replace ilo_cat_une=2 if ilo_lfs==2 & vi132==2							// Seeking first job
...
/* 	   * Question 108 asks how long the unemployed have been AVAILABLE for work (in STATA it is written
       ...
       * Aggregate:
			...
			gen ilo_dur_aggregate=.
				replace ilo_dur_aggregate=1 if inrange(iv108,1,3) & ilo_lfs==2 	    // Less than 6 months
				replace ilo_dur_aggregate=2 if iv108==4 & ilo_lfs==2				// 6 months to less than 12 months
				replace ilo_dur_aggregate=3 if inlist(iv108,5,6) & ilo_lfs==2		// 12 months or more
				replace ilo_dur_aggregate=4 if iv108==. & ilo_lfs==2				// Not elsewhere classified
...
* -------------------------------------------------------------------------------------------		
					...
					* ISIC Rev. 4 - 2 digit
					gen indu_code_une_2dig=.
						replace indu_code_une_2dig=int(vi134/100) 
...
/* Classification used: ISCO_08 */
		...
		*Correspondences at two digit level
		gen occ_code_previ=int(vi133/100) if (ilo_lfs==2 & ilo_cat_une==1)
...
* -------------------------------------------------------------------------------------------		
	...
	/* Question 106 is the closest to the definition. However, it is asked only to people available 
	   ...
	   In consequence, question 113 ("What was the main reason for not working or not being available 
	   most of the time in the last 12 months") is used to determine olfs_reason. 		*/ 
	
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if	inlist(v113,8) & ilo_lfs==3				    //Labour market
			replace ilo_olf_reason=2 if	inlist(v113,1,2,5,6,7,9) & ilo_lfs==3	    //Personal/Family-related
			replace ilo_olf_reason=3 if inlist(v113,3,4) & ilo_lfs==3				//Does not need/want to work
			replace ilo_olf_reason=4 if inlist(v113,10,.) & ilo_lfs==3				//Not elsewhere classified
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_dis=1 if ilo_lfs==3 & inlist(iv104,0,1) & ilo_olf_reason==1
