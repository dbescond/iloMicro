* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Mongolia, 2011
* DATASET USED: Mongolia LFS 2011
* NOTES: 
...
global source "LFS"
global time "2011"
global inputFile "12. LFS 2011_eng.dta"
...
/* ISCED 97 mapping: Based on UNESCO mapping
	...
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if ii18==1  				// No schooling
		*replace ilo_edu_isced97=2 if						// Pre-primary education
		replace ilo_edu_isced97=3 if ii18==2				// Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if ii18==3				// Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(ii18,4,5) 	 	// Upper secondary education
		replace ilo_edu_isced97=6 if ii18==6	 			// Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if ii18==7  				// First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if ii18==8 				// Second stage of tertiary education (leading to an advanced research qualification)
		replace ilo_edu_isced97=9 if ii18==.				// Not elsewhere classified
...
gen ilo_lfs=.
	...
	replace ilo_lfs=2 if ilo_lfs!=1 & (v90==1 & v92==1) & ilo_wap==1  	        // Unemployed	
...
* -------------------------------------------------------------------------------------------
	...
	/* NOTE: information on previous activity is only asked to people aged  5-60 (so the persons 
		...
		gen ilo_cat_une=.
			replace ilo_cat_une=1 if ilo_lfs==2 & vii119==1							// Previously employed
			replace ilo_cat_une=2 if ilo_lfs==2 & vii119==2							// Seeking first job
	   ...
	   * The category "Less than 6 months" includes as well the unemployed for 6 months. 		 */
		...
		* Aggregate:
			...
			gen ilo_dur_aggregate=.
				replace ilo_dur_aggregate=1 if inrange(v95,1,3) & ilo_lfs==2 	// Less than 6 months
				replace ilo_dur_aggregate=2 if v95==4 & ilo_lfs==2				// 6 months to less than 12 months
				replace ilo_dur_aggregate=3 if inlist(v95,5,6) & ilo_lfs==2		// 12 months or more
				replace ilo_dur_aggregate=4 if v95==. & ilo_lfs==2				// Not elsewhere classified
...
* -------------------------------------------------------------------------------------------		
	...
	* NOTE: information on previous activity is only asked to people aged  5-60. 
	* In 2011 numerica whereas in 2012 is character
	
	capture confirm string variable vii121
     if !_rc{ 
          destring vii121, replace 
     }
	 else{
	   replace vii121=vii121
	 }
	
					* ISIC Rev. 4 - 2 digit
					gen indu_code_une_2dig=.
						replace indu_code_une_2dig=int(vii121/100) 
...
/* Classification used: ISCO_08 */
	...
	capture confirm string variable vii120
     if !_rc{ 
          destring vii120, replace 
     }
	 else{
	   replace vii120=vii120
	 }
	
		*Correspondences at two digit level
		gen occ_code_previ=int(vii120/100) if (ilo_lfs==2 & ilo_cat_une==1)
	    ...
	    * ISCO 08 - 1 digit
		...
		gen ilo_prevocu_isco08=.
			...
			replace ilo_prevocu_isco08=11 if inlist(ilo_prevocu_isco08, 0,.) & ilo_lfs==2 & ilo_cat_une==1
...
* -------------------------------------------------------------------------------------------		
	...
	/* Question 95 is the closest to the definition. However, it is asked only to people available 
	   ...
	   most of the time in the last 12 months") is used to determine olfs_reason. 		*/ 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if	inlist(vi100,8,11) & ilo_lfs==3				//Labour market
			replace ilo_olf_reason=2 if	inlist(vi100,1,2,5,6,7,9,10) & ilo_lfs==3	//Personal/Family-related
			replace ilo_olf_reason=3 if inlist(vi100,3,4) & ilo_lfs==3				//Does not need/want to work
			replace ilo_olf_reason=4 if inlist(vi100,12,.) & ilo_lfs==3				//Not elsewhere classified
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_dis=1 if ilo_lfs==3 & inlist(v90,0,1) & ilo_olf_reason==1
