* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Mauritius, 2001
* DATASET USED: Mauritius, CMPHS, 2001
* NOTES: 
* Files created: Standard variables MUS_CMPHS_2001_FULL.dta and MUS_CMPHS_2001_ILO.dta
...
global source "CMPHS"
global time "2001"
global inputFile "MUS_CMPHS_2001_All"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if q105_sex==1            // Male
		replace ilo_sex=2 if q105_sex==2            // Female
...
* Comment: - Only possible to define aggregate level.
	...
	* Aggregate
	gen ilo_edu_aggregate=.
		replace ilo_edu_aggregate=1 if (q107_schoolatt_a==3 | q108_edu_level==1)                             // Less than basic
		replace ilo_edu_aggregate=2 if inrange(q108_edu_level,11,23)                                         // Basic 
		replace ilo_edu_aggregate=3 if inrange(q108_edu_level,24,26)                                         // Intermediate
		replace ilo_edu_aggregate=4 if (inlist(q109_schoolatt_v,1,2) & inrange(q110_qual_course,500,700))    // Advanced
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if q107_schoolatt_a==1 | q109_schoolatt_v==1                                              // Attending
		replace ilo_edu_attendance=2 if ilo_edu_attendance!=1 & (inlist(q107_schoolatt_a,2,3) | inlist(q109_schoolatt_v,2,3))  // Not attending
...
* Comment: - For the detailed version: married already includes union.
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if q106_mstatus==5                           // Single
		replace ilo_mrts_details=2 if q106_mstatus==1                           // Married
		* replace ilo_mrts_details=3 if                                         // Union / cohabiting
		replace ilo_mrts_details=4 if q106_mstatus==2                           // Widowed
		replace ilo_mrts_details=5 if inlist(q106_mstatus,3,4)                  // Divorced / separated
...
*            available future starters.
	...
	gen ilo_lfs=.
   	    replace ilo_lfs=1 if (q201_did_work==1) & ilo_wap==1					                   // Employed: ILO definition
	    replace ilo_lfs=1 if (q202_absent==1) & ilo_wap==1						                   // Employed: temporary absent
		replace ilo_lfs=2 if ilo_lfs!=1 & (q203_look_work==1 & q211_avail_work==1) & ilo_wap==1	   // Unemployed: three criteria
...
*            for second job.
   ...
   * ICSE 1993
     gen ilo_job1_ste_icse93=.
	     replace ilo_job1_ste_icse93=1 if inlist(q223_emp_status,3,4) & ilo_lfs==1    // Employees
	     replace ilo_job1_ste_icse93=2 if q223_emp_status==1 & ilo_lfs==1             // Employers
	     replace ilo_job1_ste_icse93=3 if q223_emp_status==2 & ilo_lfs==1             // Own-account workers
	     * replace ilo_job1_ste_icse93=4 if                                           // Members of producers' cooperatives
	     replace ilo_job1_ste_icse93=5 if q223_emp_status==5 & ilo_lfs==1             // Contributing family workers
...
*            economic activity in second job.
	...
	* 2-digit level
	gen ilo_job1_eco_isic3_2digits = .
	    replace ilo_job1_eco_isic3_2digits = q221_activity if ilo_lfs==1
...
*            ISCO-88. No information available for second job.
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco88_2digits = .
	    replace ilo_job1_ocu_isco88_2digits = q222_occupation if ilo_lfs==1
    ...
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
	    replace ilo_job1_ocu_isco88=11 if inlist(ilo_job1_ocu_isco88_2digits,45,60,63,85,99,.) & ilo_lfs==1         // Not elsewhere classified
...
* Comment: - No information on the hours usually worked per week in main/all jobs.
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = q228_work_hours if ilo_lfs==1
...
*          - No information for the second job.
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = q227_income if ilo_job1_ste_aggregate==1
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = q227_income if ilo_job1_ste_aggregate==2
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if q213_past_work==1 & ilo_lfs==2                 // Previously employed       
		replace ilo_cat_une=2 if q213_past_work==2 & ilo_lfs==2                 // Seeking for the first time
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if q207_time_lookwk==0 & ilo_lfs==2                           // Less than 1 month
		replace ilo_dur_details=2 if inrange(q207_time_lookwk,1,3) & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if inrange(q207_time_lookwk,4,6) & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if inrange(q207_time_lookwk,7,12) & ilo_lfs==2                // 6 to 12 months
		replace ilo_dur_details=5 if inrange(q207_time_lookwk,13,24) & ilo_lfs==2               // 12 to 24 months
		replace ilo_dur_details=6 if q207_time_lookwk>=25 & q207_time_lookwk!=. & ilo_lfs==2    // 24 months or more
...
*            3 and 4 are not possible to be produced. 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if q203_look_work==1 & q211_avail_work==2 & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if q203_look_work==2 & q211_avail_work==1 & ilo_lfs==3	     // Not seeking, available
