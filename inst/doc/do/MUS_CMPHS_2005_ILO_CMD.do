...
* Comment: - Only possible to define aggregate level.
	...
	* Aggregate
	gen ilo_edu_aggregate=.
	 ...
	 if year==2004 {
		replace ilo_edu_aggregate=1 if (schoolatt==3 | educattain==1)                               // Less than basic
		replace ilo_edu_aggregate=2 if inrange(educattain,11,23)                                    // Basic 
		replace ilo_edu_aggregate=3 if inrange(educattain,24,28)                                    // Intermediate
		replace ilo_edu_aggregate=4 if (inlist(whenstud,1,2,3) & inrange(qualcourse,500,700))       // Advanced
		replace ilo_edu_aggregate=5 if ilo_edu_aggregate==.                                         // Level not stated
	 }
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
	 ...
	 if year==2004 {
		replace ilo_edu_attendance=1 if schoolatt==1 | inlist(whenstud,1,2)                                        // Attending
		replace ilo_edu_attendance=2 if ilo_edu_attendance!=1 & (inlist(schoolatt,2,3) | inlist(whenstud,3,4))     // Not attending
		replace ilo_edu_attendance=3 if ilo_edu_attendance==.                                                      // Not elsewhere classified
	 }
...
* Comment: - For the detailed version: married already includes union.
	...
	* Detailed
	gen ilo_mrts_details=.
	 ...
	 if year==2004 {
	    replace ilo_mrts_details=1 if maritalstatus==5                          // Single
		replace ilo_mrts_details=2 if maritalstatus==1                          // Married
		* replace ilo_mrts_details=3 if                                         // Union / cohabiting
		replace ilo_mrts_details=4 if maritalstatus==2                          // Widowed
		replace ilo_mrts_details=5 if inlist(maritalstatus,3,4)                 // Divorced / separated
		replace ilo_mrts_details=6 if ilo_mrts_details==.			            // Not elsewhere classified
	 }
...
*            starters.  
	...
	gen ilo_lfs=.
	 ...
	 if year==2004 {
   	    replace ilo_lfs=1 if (work==1 | activities==1) & ilo_wap==1					       // Employed: ILO definition
	    replace ilo_lfs=1 if (awayfrwk==1) & ilo_wap==1						               // Employed: temporary absent
		replace ilo_lfs=2 if ilo_lfs!=1 & (lookfrwk==1 & availwk==1) & ilo_wap==1	       // Unemployed: three criteria
		replace ilo_lfs=2 if ilo_lfs!=1 & (whynotlk==8 & availwk==1) & ilo_wap==1	       // Unemployed: available future starters
	    replace ilo_lfs=3 if !inlist(ilo_lfs,1,2) & ilo_wap==1		                       // Outside the labour force
	 }
...
*          - 2004: No information available on status in employment in second job.
...
*          - 2004: No information available on economic activity in second job.
	...
	* 2-digit level
	...
	if year==2004 {
	    replace ilo_job1_eco_isic3_2digits = int(activity/1000)  if ilo_lfs==1
...
*            ISCO-88.
*          - 2004: No information available for the second job.
	...
	* 2-digit level 
	...
	if year==2004 {
	    ...
	    replace ilo_job1_ocu_isco88_2digits = int(occup/1000) if ilo_lfs==1
    ...
    * 1-digit level
	gen ilo_job1_ocu_isco88=.
	    replace ilo_job1_ocu_isco88=11 if inlist(ilo_job1_ocu_isco88_2digits,36,39,45,50,53,60,63,64,65,75,85,90,94,95,99,.) & ilo_lfs==1    // Not elsewhere classified
...
*            3 and 4 are not possible to be produced.
	...
	gen ilo_olf_dlma=.
	 ...
	 if year==2004 {
		replace ilo_olf_dlma = 1 if lookfrwk==1 & availwk==2 & ilo_lfs==3       // Seeking, not available
		replace ilo_olf_dlma = 2 if lookfrwk==2 & availwk==1 & ilo_lfs==3	    // Not seeking, available
		* replace ilo_olf_dlma = 3 if 		                                    // Not seeking, not available, willing
		* replace ilo_olf_dlma = 4 if 		                                    // Not seeking, not available, not willing
		replace ilo_olf_dlma = 5 if	(ilo_olf_dlma==. & ilo_lfs==3)	            // Not classified 
	 }
...
* Comment: 
	...
	gen ilo_olf_reason=.
	 if year==2004 {
		replace ilo_olf_reason=1 if inlist(whynotlk,5,9) & ilo_lfs==3           // Labour market 
		* replace ilo_olf_reason=2 if                                           // Other labour market reasons
		replace ilo_olf_reason=3 if inlist(whynotlk,1,2,4,6,7) & ilo_lfs==3     // Personal/Family-related
		replace ilo_olf_reason=4 if inlist(whynotlk,3,10) & ilo_lfs==3          // Does not need/want to work
...
* Comment: 
	...
	gen ilo_dis=.
	 if year==2004 {
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & availwk==1 & ilo_olf_reason==1
	 }
	 if year==2005 {
