* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Mauritius, 2008
* DATASET USED: Mauritius, CMPHS, 2008
* NOTES: 
...
*            starters.  
	...
	gen ilo_lfs=.
		...
		if year==2006 {
		  replace ilo_lfs=1 if (awayfromwrk==1) & ilo_wap==1						           // Employed: temporary absent
		}
		else {
   ...
   * ICSE 1993
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
	    if year==2006 {
		  replace ilo_job2_ste_icse93=1 if inlist(second_emp_status,3,4) & ilo_mjh==2    // Employees
		  replace ilo_job2_ste_icse93=2 if second_emp_status==1 & ilo_mjh==2             // Employers
		  replace ilo_job2_ste_icse93=3 if second_emp_status==2 & ilo_mjh==2             // Own-account workers
		  * replace ilo_job2_ste_icse93=4 if                                             // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if second_emp_status==5 & ilo_mjh==2             // Contributing family workers
...
*            NSIC Rev.1 is based on ISIC Rev.3
	...
	* 2-digit level
	gen ilo_job1_eco_isic3_2digits = .
	  if year==2006 {
	    replace ilo_job1_eco_isic3_2digits = industry  if ilo_lfs==1
	  }
	  else {
	...
	* 2-digit level
	gen ilo_job2_eco_isic3_2digits = .
	  if year==2006 {
	    replace ilo_job2_eco_isic3_2digits = secondindustry if ilo_mjh==2
	  }
	  else {
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - 2008: Even though there's a variable available referring to the 
*            previous economic activity, there is no mapping available to neither
*            ISIC Rev.4 or 3, and therefore it is not defined.
	...
	* 2-digit level
	gen ilo_preveco_isic3_2digits = .
	  if year==2006 {
	    replace ilo_preveco_isic3_2digits = lastactivity  if ilo_lfs==2 & ilo_cat_une==1
	  }
	  if year==2007 {
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - No information avilable.
