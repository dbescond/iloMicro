* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Mauritius, 2017
* DATASET USED: Mauritius, CMPHS, 2017
* NOTES: 
* Files created: Standard variables MUS_CMPHS_2017_FULL.dta and MUS_CMPHS_2017_ILO.dta
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 30 August 2018
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		...
		replace ilo_edu_attendance=2 if ilo_edu_attendance==. & (inlist(schatt,2,3,4,5,6) | inlist(whenstudied,4,5))     // Not attending
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Employees include: employees (3) and apprentice/internship (4)

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(empstat,3,4) & ilo_lfs==1      // Employees
		 ...
		 * replace ilo_job1_ste_icse93=4 if                                     // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if empstat==5 & ilo_lfs==1               // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if inlist(secemplstat,3,4) & ilo_mjh==2  // Employees
		  replace ilo_job2_ste_icse93=2 if secemplstat==1 & ilo_mjh==2           // Employers
		  replace ilo_job2_ste_icse93=3 if secemplstat==2 & ilo_mjh==2           // Own-account workers
		  * replace ilo_job2_ste_icse93=4 if                                     // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if secemplstat==5 & ilo_mjh==2           // Contributing family workers
   ...
   * Aggregate level
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = int(secondact/1000) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = hrsusual if ilo_lfs==1
	...
	* Hours USUALLY worked
    egen ilo_joball_how_usual = rowtotal(hrsusual hrsusualother), m
...
* Comment: 
	...
	* 2-digit level
	gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = int(lastactivity/1000) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* Comment: 
	...
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits = . 
	    replace ilo_prevocu_isco08_2digits = int(lastocc/1000) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
	...
	* 1-digit level 				
	gen ilo_prevocu_isco08=.
	    replace ilo_prevocu_isco08=11 if inlist(ilo_prevocu_isco08_2digits,77,.) & ilo_lfs==2 & ilo_cat_une==1                          // Not elsewhere classified
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 3 if lookwk==2 & availwk==2 & availabletowk==1 & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if lookwk==2 & availwk==2 & availabletowk==2 & ilo_lfs==3      // Not seeking, not available, not willing
