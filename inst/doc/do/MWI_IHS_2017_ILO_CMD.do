...
global source "IHS"  /*survey: Acronym from the workflow*/
global time "2017"  /*time*/
global inputFile "HH_MOD_B_C_D_E_N1_N2_X_P_A" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if reside == 1       // Urban 
		replace ilo_geo=2 if reside == 2       // Rural
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if hh_b03 == 1            // Male
		replace ilo_sex=2 if hh_b03 == 2            // Female
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: No schooling includes Pre-primary as no distinction made.
*	ISCED 11, additionally not enough information for tertiary education breakdown, so also 
*	grouped in bachelors and masters (excl. early childhood, short cycle, PhD).
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  hh_c09==1   // No schooling
													
		replace ilo_edu_isced97=3 if  hh_c09==2   // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if  hh_c09==3   // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if  hh_c09==4	  // Upper secondary education
		replace ilo_edu_isced97=6 if  hh_c09==5   // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if  hh_c09==6   // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  hh_c09==7   // Second stage of tertiary education (leading to an advanced research qualification)                               
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if hh_c09==1              // No schooling
															// Early childhood education
		replace ilo_edu_isced11=3 if hh_c09==2              // Primary education
		replace ilo_edu_isced11=4 if hh_c09==3              // Lower secondary education
		replace ilo_edu_isced11=5 if hh_c09==4              // Upper secondary education
		replace ilo_edu_isced11=6 if hh_c09==5              // Post-secondary non-tertiary education
															// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if hh_c09==6              // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if hh_c09==7              // Master's or equivalent level
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if hh_c13==1                       // Attending
		replace ilo_edu_attendance=2 if hh_c13==2                       // Not attending
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Union/cohabiting and polygamous is in married.
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if hh_b24 == 6  		 // Single
		replace ilo_mrts_details=2 if inlist(hh_b24,1,2) // Married
														// Union / cohabiting
		replace ilo_mrts_details=4 if hh_b24 == 5        // Widowed
		replace ilo_mrts_details=5 if inlist(hh_b24,3,4) // Divorced / separated
