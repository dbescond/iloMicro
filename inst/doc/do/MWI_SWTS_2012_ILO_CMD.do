* TITLE OF DO FILE: ILO Microdata Preprocessing code template - MALAWI, 2012
* DATASET USED: MALAWI, SWTS, 2012
* NOTES: 
* Files created: Standard variables MWI_SWTS_2012_FULL.dta and MWI_SWTS_2012_ILO.dta
...
global country "MWI" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2012"  /*time*/
global inputFile "MWI_SWTS_2012.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if a3==2         // Urban 
		replace ilo_geo=2 if a3==1         // Rural
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if  b02==1           // Male
		replace ilo_sex=2 if  b02==2           // Female
	...
	* Detailed
	gen ilo_edu_isced97=.
		...
		*replace ilo_edu_isced97=5 if                        // Upper secondary education
		replace ilo_edu_isced97=6 if  c12a== 6                      // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if c12a== 5                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if c12a== 4                         // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if  b06==4                                         // Single
		replace ilo_mrts_details=2 if  b06==1                                         // Married
		replace ilo_mrts_details=3 if   b06==5                                        // Union / cohabiting
		replace ilo_mrts_details=4 if  b06==3                                         // Widowed
		replace ilo_mrts_details=5 if  b06==2                                         // Divorced / separated
...
* Comment: 
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (b22==1 & b23==1 & b24==1 & b25==1 )                            
             replace ilo_dsb_details=2 if (b22==2 | b23==2 | b24==2 | b25==2 )                                                 
             replace ilo_dsb_details=3 if (b22==3 | b23==3 | b24==3 | b25==3 )                                                 
             replace ilo_dsb_details=4 if (b22==4 | b23==4 | b24==4 | b25==4 )
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c32a==1 | c32b==1 | c32c==1 | c32d==1 | ///
		c32e==1 | c32f==1 |c32g==1 ) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if c33==1 & ilo_wap==1										// Employed: temporary absent
...
* Comment:
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if d06==1 & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if d06==2 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if d06==3 & ilo_lfs==1          // Own-account workers
		 replace ilo_job1_ste_icse93=4 if d06==4 & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if d06==5 & ilo_lfs==1          // Contributing family workers
	...
	* 1-digit level
	...
	/*
		...
		*/
		...
		replace ilo_job1_eco_isic4=22 if industry==99 & ilo_lfs==1
...
gen ilo_job1_ocu_isco08=.                        
		...
		*replace ilo_job1_ocu_isco08=10 if (ilo_job1_ocu_isco08==0 & ilo_lfs==1)  // Armed forces
		 ...
		 replace ilo_job1_ocu_isco08=11 if inlist(occu,99,.) & ilo_lfs==1 
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if d09==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if d09==2 &  ilo_lfs==1               				// Temporary
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.	
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   d05==1 | ///
									   (inlist(d05,4) & ilo_job1_ste_icse93==1 & d13j==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((ilo_job1_ste_icse93==1 & d13j==1) | ///
										 (ilo_job1_ste_icse93==1 & inlist(d13j,3,997)& d13c==1 & d13d==1) | ///
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Monthly estimated wage (before deduction) based on the reference period (d18)
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = d14a      if ilo_job1_ste_aggregate==1 & d14a>=0
		replace ilo_job1_lri_ees=ilo_job1_lri_ees + d16a if ilo_job1_ste_aggregate==1 & d16a>=0 & d16==1
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = d27      if ilo_job1_ste_aggregate==2 & d27>=0
		 replace ilo_job1_lri_slf = ilo_job1_lri_slf + d29   if ilo_job1_ste_aggregate==2 & d29>=0 & d28==1
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(e05,7,8,9,10,11)          & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(e05,1,2)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(e05,3,4,5,6)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if e14==6          & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if (ilo_olf_reason==.|e05==99 & !e14==6) & ilo_lfs==3     // Not elsewhere classified
