...
* Comment: completed level of education always to be considered! 
	...
	gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if inlist(C03,0,95)
		replace ilo_edu_isced11=2 if inrange(C03,10,16)
		replace ilo_edu_isced11=3 if inrange(C03,17,23) 
		replace ilo_edu_isced11=4 if inrange(C03,24,33)
		replace ilo_edu_isced11=5 if C03==34 | inrange(C03,61,64)
		replace ilo_edu_isced11=6 if inlist(C03,43,83)
		replace ilo_edu_isced11=7 if inlist(C03,51,52,53,84)
		replace ilo_edu_isced11=8 if inlist(C03,65,71,91,92)
		replace ilo_edu_isced11=9 if C03==72
		replace ilo_edu_isced11=10 if C03==73
...
* Comment: 
		...
		gen ilo_edu_attendance=.
			replace ilo_edu_attendance=1 if inlist(C01,2,4,5)
			replace ilo_edu_attendance=2 if inlist(C01,1,3)
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if B09==1                             // Single
		replace ilo_mrts_details=2 if inlist(B09,2,3)                    // Married
		replace ilo_mrts_details=3 if B09==4                             // Union / cohabiting
		replace ilo_mrts_details=4 if B09==6                             // Widowed
		replace ilo_mrts_details=5 if inlist(B09,5,7)                    // Divorced / separated
...
* Comment: 
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (D01==1 | D02==1 | D03==1 | D04==1 | D05==1 | D06==1 | D07==1 | D08==1 | D09==1)
		replace ilo_lfs=2 if (ilo_lfs!=1 & I06==1 & (I01==1 | I05==4))
...
* Comment:
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if E22==1
		replace ilo_mjh=2 if E22==2
...
* Comment: 
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inlist(E16,5,6)
			replace ilo_job1_ste_icse93=2 if inlist(E16,1,3)
			replace ilo_job1_ste_icse93=3 if inlist(E16,2,4)
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if inlist(E16,7,8)
...
* Comment: ISIC Rev. 4 being used and initially indicated on 4-digits level --> keep only 2 digits level
	...
	* Economic activity indicated both for primary and secondary activity --> capture it for both
	...
	gen indu_code_prim=int(E15/100) if ilo_lfs==1 & E15!=0
...
* Comment: 
	...
	gen occ_code_prim=int(E11/100) if ilo_lfs==1 & E11!=0
...
* Comment: 
	...
	* Primary occupation
	 ...
	 gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(E17,1,2,5)
		replace ilo_job1_ins_sector=2 if inlist(E17,3,4,6,7,8,9)
...
* Comment: 
	...
	foreach var of varlist F01A F01B F02A F02B F03A F03B F04A F04B F05A F05B F06A F06B F07A F07B F09A F09B F10A F10B F11A F11B F12A F12B F13A F13B F14A F14B F15A F15B {
	
		replace `var'=. if `var'>24 // get rid of wrong values
	...
	* Actual hours worked 
		...
		* Primary job
		...
		egen ilo_job1_how_actual=rowtotal(F01B F02B F03B F04B F05B F06B F07B), m
			replace ilo_job1_how_actual=. if ilo_lfs!=1
			lab var ilo_job1_how_actual "Weekly hours actually worked in main job"
		
		* Secondary job 
		
		egen ilo_job2_how_actual=rowtotal(F09B F10B F11B F12B F13B F14B F15B), m
	...
	* Hours usually worked
		...
		* Primary job
		...
		egen ilo_job1_how_usual=rowtotal(F01A F02A F03A F04A F05A F06A F07A), m
			replace ilo_job1_how_usual=. if ilo_lfs!=1
			lab var ilo_job1_how_usual "Weekly hours usually worked in main job"
			
		* Secondary job
		
		egen ilo_job2_how_usual=rowtotal(F09A F10A F11A F12A F13A F14A F15A), m
...
* Comment: Report uses 35 hours as threshold (cf. page 62, chapter 4.2 "Time related underemployment")
			...
			* note that 2012 dataset contains a variable capturing the information on the working time agreement (E12). 
			* However, as it is based on a question asking the person whether the job is full-time or part-time, in order to have a more consistent time-series
			* use the same definition as for other time periods
...
* Comment: "Unspecified Duration" is being considered as unknown
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if G06==2
		replace ilo_job1_job_contract=2 if G06==1
...
* Comment: Registration --> "In process of registration" considered as not registered
	...
	* Useful questions: E17: Institutional sector 
					*	[no question]: Destination of production 
					*	H03: Bookkeeping (only asked to "other employer" and "other own account worker")
					*	H01: Registration(only asked to "other employer" and "other own account worker")
					*	[no question]: Location of workplace
					* 	E18: Size of the establishment (not being considered in the command, as new definition requires to have location of workplace in order to determine whether size is used)
					*	G01: Social security
					*	(E19E: Pension fund)
					*	(G03: Paid annual leave)
					*	(G04A: Paid sick leave)
	

	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=1 if (!inlist(E17,1,2,5,7,8) & H03!=1 & inlist(H01,2,3)) | (!inlist(E17,1,2,5,7,8) & H03!=1 & !inlist(H01,1,2,3) ///
										& ((ilo_job1_ste_aggregate==1 & G01!=1) | ilo_job1_ste_aggregate!=1))
		replace ilo_job1_ife_prod=2 if inlist(E17,1,2,5) | (!inlist(E17,1,2,5,7,8) & H03==1) | (!inlist(E17,1,2,5,7,8) & H03!=1 & H01==1) | ///
										(!inlist(E17,1,2,5,7,8) & H03!=1 & !inlist(H01,1,2,3) & ilo_job1_ste_aggregate==1 & G01==1)
		replace ilo_job1_ife_prod=3 if inlist(E17,7,8) | ilo_job1_ocu_isco88_2digits==62 | ilo_job1_eco_isic4_2digits==97
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_job1_ife_nature=.
		replace ilo_job1_ife_nature=1 if ilo_job1_ste_icse93==5 | (inlist(ilo_job1_ste_icse93,1,6) & G01!=1) | (inlist(ilo_job1_ste_icse93,2,3,4) & inlist(ilo_job1_ife_prod,1,3))
		replace ilo_job1_ife_nature=2 if (inlist(ilo_job1_ste_icse93,1,6) & G01==1) | (inlist(ilo_job1_ste_icse93,2,3,4) & ilo_job1_ife_prod==2)
...
* Comment: 
	...
	* Primary employment 
		...
		gen ilo_job1_lri_ees=G10 if ilo_job1_ste_aggregate==1
		...
		* Note that the revenue related to self-employment includes also the value of own consumption (cf. resolution from 1998)
		...
		gen ilo_job1_lri_slf=H07 if ilo_job1_ste_aggregate==2 & H07!=999999
...
* Comment: use F20 as a proxy for the availability criterion (indicating the number of hours that the person could have worked additionally)

	gen ilo_joball_tru=1 if ilo_job1_job_time==1 & F18==1 & !inlist(F20,0,.)
...
* Comment: Note that question does not only asks for how long the person has not been employed, but for how long the person has been without work and available to work
	 ...
	 gen ilo_dur_details=.
		replace ilo_dur_details=1 if I08==1
		replace ilo_dur_details=2 if I08==2 
		replace ilo_dur_details=3 if I08==3
		replace ilo_dur_details=4 if I08==4
		replace ilo_dur_details=5 if I08==5
		replace ilo_dur_details=6 if I08==6
...
* -------------------------------------------------------------------------------------------
*
* Comment: Variable I09 only allows to identify those unemployed individuals that were working within the last 12 months, but not those
...
* Comment: Degree of labour market attachment of persons outside the labour force
			* note that the questionnaire does not contain any information on the willingness to work

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if I01==1 & I06==2
		replace ilo_olf_dlma=2 if I01==2 & I06==1
...
* Comment: consider variables I02Last7Days I05YNotLook 
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if inlist(I05,1,3) | I07==2 
		replace ilo_olf_reason=2 if inlist(I05,2,5)
		replace ilo_olf_reason=3 if inlist(I07,3,4,5)
		replace ilo_olf_reason=4 if inlist(I07,1,6)
...
* Comment: 
		...
		gen ilo_dis=1 if ilo_lfs==3 & ilo_olf_reason==1 & I06==1
