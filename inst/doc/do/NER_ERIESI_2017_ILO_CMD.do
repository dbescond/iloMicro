* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Niger, 2017
* DATASET USED: NER, ERIESI, 2017
* Files created: Standard variables NER_ERIESI_2017_FULL.dta and NER_ERIESI_2017_ILO.dta
...
global country "NER" 
global source "ERIESI"
global time "2017"
global inputFile "NER_ERIESI_2017.dta"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if m3==1		// Male
		replace ilo_sex=2 if m3==2		// Female
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		...
		* replace ilo_edu_isced11=2 if 						// Early childhood education
		replace ilo_edu_isced11=3 if m21==1	  				// Primary education
		replace ilo_edu_isced11=4 if inlist(m21,2,3,4,5)	// Lower secondary education
		replace ilo_edu_isced11=5 if m21==6				    // Upper secondary education
		* replace ilo_edu_isced11=6 if						// Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if m21==7					// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if m21==8					// Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if inlist(m21,9,10)		// Master's or equivalent level
		replace ilo_edu_isced11=10 if m21==11				// Doctoral or equivalent level
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if m15==1					// Attending
		replace ilo_edu_attendance=2 if (m15==2 | m13==2)		// Not attending
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if m25==1					// Single
		replace ilo_mrts_details=2 if inlist(m25,2,3)			// Married
		* replace ilo_mrts_details=3 if 						// Union / cohabiting
		replace ilo_mrts_details=4 if m25==5					// Widowed
		replace ilo_mrts_details=5 if m25==4					// Divorced / separated
...
* Comment: Questions SE2 to SE6b for Employment. Questions SE7, SE9 and SE10 for Unemployment.
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (se2_empl==1 | inrange(se3_aut,1,9)) & ilo_wap==1					// Employed: ILO definition
		replace ilo_lfs=1 if (inrange(se5_rais,1,4) | se6a_qua==1 | se6b_rep==1) & ilo_wap==1	// Employed: temporary absent
		replace ilo_lfs=2 if (se7_r30j==1 & inlist(se9_disp,1,2) & ilo_lfs!=1 & ilo_wap==1)		// Unemployed: three criteria
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if as1a_act==2 & ilo_lfs==1		// One job only     
		replace ilo_mjh=2 if as1a_act==1 & ilo_lfs==1		// More than one job
		replace ilo_mjh=1 if as1a_act==. & ilo_lfs==1
...
* ------------------------------------------------------------------------------
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(ap3_cat,1,2,3,4,5,6) & ilo_lfs==1	// Employees
		 replace ilo_job1_ste_icse93=2 if ap3_cat==7 & ilo_lfs==1					// Employers
		 replace ilo_job1_ste_icse93=3 if ap3_cat==8 & ilo_lfs==1          			// Own-account workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if inlist(as4_ca1,1,2,3,4,5,6) & ilo_mjh==2	// Employees
		  replace ilo_job2_ste_icse93=2 if as4_ca1==7 & ilo_mjh==2					// Employers
		  replace ilo_job2_ste_icse93=3 if as4_ca1==8 & ilo_mjh==2					// Own-account workers
		  * replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2							// Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if as4_ca1==9 & ilo_mjh==2					// Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits=. 
	    replace ilo_job1_eco_isic4_2digits=int(ap2_bran/100) if ilo_lfs==1
   ...
   * Aggregate level
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits=. 
	    replace ilo_job2_eco_isic4_2digits=int(as3_bra1/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits=. 
	    replace ilo_job1_ocu_isco08_2digits=int(ap1_nom/10000) if ilo_lfs==1
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits=. 
	    replace ilo_job2_ocu_isco08_2digits=int(as2_pro1/10000) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(ap4_ent,1,2,5) & ilo_lfs==1		// Public
...
* ------------------------------------------------------------------------------
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    replace ilo_job1_how_actual=ap10c_pr if ilo_lfs==1
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual=.
	    replace ilo_job2_how_actual=as9c_heu if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if ap9a_per==1 & ilo_lfs==1   				// Permanent
		replace ilo_job1_job_contract=2 if inlist(ap9a_per,2,3) & ilo_lfs==1		// Temporary
...
/* Useful questions:
          - Institutional sector: AP4_ENT
		  - Private household identification: 
		  - Destination of production:
		  - Bookkeeping: AP6D_COM
		  - Registration: AP6A_FIS / AP6B_RC / AP6B_NIF / AP6B_INP
		  - Status in employment: AP3_CAT
		  - Social security contribution (Proxy: pension funds): AP16_PR1
		  - Place of work: AP7_LOCA
		  - Size: AP5_NBRE
		  - Paid annual leave: AP16_PR2
		  - Paid sick leave: AP16_PR3
*/
    * Social Security
	gen social_security=.
	    replace social_security=1 if ap16_pr1==3 & ilo_lfs==1		// Social security (proxy)
		replace social_security=2 if ap16_pr1==. & ilo_lfs==1		// No social security (proxy)
	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (ap4_ent==7 | ilo_job1_eco_isic4_2digits==63 | ilo_job1_eco_isic4_2digits==97)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & (inlist(ap4_ent,1,2,5) | inlist(ap6a_fis,1,2,3) | ap6b_rc==1 | ap6b_nif==1 | ap6b_inp==1 | inlist(ap6d_com,2,3))
...
* ------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if c2_raiso==1 & ilo_lfs==2				// Previously employed       
		replace ilo_cat_une=2 if c2_raiso==2 & ilo_lfs==2				// Seeking for the first time
...
* Comment: It should be based on C1_CHOM and C1_CHMO but the results are messy.
/*
	* Detailed categories		
    gen ilo_dur_details=.
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if se7_r30j==1 & se9_disp!=1 & se9_disp!=2 & ilo_lfs==3				// Seeking, not available
		replace ilo_olf_dlma = 2 if se7_r30j==2 & inlist(se9_disp,1,2) & ilo_lfs==3	    				// Not seeking, available
		replace ilo_olf_dlma = 3 if se7_r30j==2 & se9_disp!=1 & se9_disp!=2 & se10_emp==1 & ilo_lfs==3	// Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if se7_r30j==2 & se9_disp!=1 & se9_disp!=2 & se10_emp==2 & ilo_lfs==3	// Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(se8_rsm,1,2,3) & ilo_lfs==3			// Labour market 
			replace ilo_olf_reason=2 if inlist(se8_rsm,4,5) & ilo_lfs==3			// Other labour market reasons
			replace ilo_olf_reason=3 if inlist(se8_rsm,6,7,8,9,11) & ilo_lfs==3		// Personal/Family-related
			replace ilo_olf_reason=4 if se8_rsm==10 & ilo_lfs==3					// Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & inlist(se9_disp,1,2) & ilo_olf_reason==1
