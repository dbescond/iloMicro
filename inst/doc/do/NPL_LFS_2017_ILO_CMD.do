* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Nepal, 2017
* DATASET USED: NPL, LFS, 2017
* Files created: Standard variables NPL_LFS_2017_FULL.dta and NPL_LFS_2017_ILO.dta
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 11 September 2018
...
global source "LFS"
global time "2017"
global inputFile "NPL_LFS_2017.dta"
...
* Comment: Weights not adjusted for the moment. Need to check with NSO because hhoverall or hhprov need to be converted.

	gen ilo_wgt=.
	    replace ilo_wgt=hhoverall 
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if ur==1		// Urban 
		replace ilo_geo=2 if ur==2		// Rural
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if b01==1		// Male
		replace ilo_sex=2 if b01==2		// Female
...
* ------------------------------------------------------------------------------
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if (b21==2 | b22==17)		// No schooling
		replace ilo_edu_isced11=2 if inlist(b22,0,16)		// Early childhood education
		replace ilo_edu_isced11=3 if inlist(b22,1,2,3,4,5)  // Primary education
		replace ilo_edu_isced11=4 if inlist(b22,6,7,8)		// Lower secondary education
		replace ilo_edu_isced11=5 if inlist(b22,9,10,11)    // Upper secondary education
		replace ilo_edu_isced11=6 if inlist(b22,12,15)      // Post-secondary non-tertiary education
		* replace ilo_edu_isced11=7 if						// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if b22==13				// Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if b22==14				// Master's or equivalent level
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if b20==1					// Attending
		replace ilo_edu_attendance=2 if b20==2					// Not attending
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if b05==1					// Single
		replace ilo_mrts_details=2 if b05==2					// Married
		* replace ilo_mrts_details=3 if 						// Union / cohabiting
		replace ilo_mrts_details=4 if b05==3					// Widowed
		replace ilo_mrts_details=5 if inlist(b05,4,5)			// Divorced / separated
...
* Comment: Questions C01 to C09 for Employment. Questions G01, G02, G03, G7 and G09 for Unemployment.
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c01==1 | c04==2 | inlist(c05,1,2)) & ilo_wap==1				// Employed: ILO definition
		replace ilo_lfs=1 if (inlist(c07,1,2,3,4) | c08==1 | c09==1) & ilo_wap==1			// Employed: temporary absent
		replace ilo_lfs=2 if (g01==1 & g03!=13 & inlist(g09,1,2) & ilo_lfs!=1 & ilo_wap==1)	// Unemployed: three criteria
		replace ilo_lfs=2 if (g02==1 & inlist(g09,1,2) & ilo_lfs!=1 & ilo_wap==1)			// Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if d20==2 & ilo_lfs==1		// One job only     
		replace ilo_mjh=2 if d20==1 & ilo_lfs==1		// More than one job
		replace ilo_mjh=1 if d20==. & ilo_lfs==1
...
* ------------------------------------------------------------------------------
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(d03,1,2) & ilo_lfs==1			// Employees
		 replace ilo_job1_ste_icse93=2 if d03==3 & ilo_lfs==1					// Employers
		 replace ilo_job1_ste_icse93=3 if d03==4 & ilo_lfs==1          			// Own-account workers
		 * replace ilo_job1_ste_icse93=4 if & ilo_lfs==1          				// Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if d03==5 & ilo_lfs==1					// Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if inlist(d23,1,2) & ilo_mjh==2			// Employees
		  replace ilo_job2_ste_icse93=2 if d23==3 & ilo_mjh==2					// Employers
		  replace ilo_job2_ste_icse93=3 if d23==4 & ilo_mjh==2					// Own-account workers
		  * replace ilo_job2_ste_icse93=4 if  & ilo_mjh==2						// Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if d23==5 & ilo_mjh==2					// Contributing family workers
...
* Comment: To double check. Does not seem to follow ISIC Rev. 4. Need national classification. 
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits=. 
	    replace ilo_job1_eco_isic4_2digits=int(d12_nsic/100) if ilo_lfs==1
   ...
   * Aggregate level
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits=. 
	    replace ilo_job2_eco_isic4_2digits=int(d25_nsic/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits=. 
	    replace ilo_job1_ocu_isco08_2digits=int(d02_nsco/100) if ilo_lfs==1
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits=. 
	    replace ilo_job2_ocu_isco08_2digits=int(d22_nsco/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(d13,1,2) & ilo_lfs==1			// Public
...
* ------------------------------------------------------------------------------
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual=.
	    replace ilo_job1_how_usual=e01a if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    replace ilo_job1_how_actual=e02a if ilo_lfs==1
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual=.
	    replace ilo_job2_how_usual=e01b if ilo_mjh==2
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual=.
	    replace ilo_job2_how_actual=e02b if ilo_mjh==2
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual=.
	    replace ilo_joball_how_usual=e01d if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual=.
	    replace ilo_joball_how_actual=e02d if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if d05==1 & ilo_lfs==1         				// Permanent
		replace ilo_job1_job_contract=2 if d05==2 & ilo_lfs==1         				// Temporary
...
/* Useful questions:
          - Institutional sector: D13 / D14
		  - Private household identification: D11a
		  - Destination of production:
		  - Bookkeeping: D16
		  - Registration: D15
		  - Status in employment: D03
		  - Social security contribution (Proxy: pension funds): D08
		  - Place of work: D18
		  - Size: D17
		  - Paid annual leave: D09
		  - Paid sick leave: D10
*/
    * Social Security
	gen social_security=.
	    replace social_security=1 if d08==1 & ilo_lfs==1		// Social security (proxy)
		replace social_security=2 if d08==2 & ilo_lfs==1		// No social security (proxy)
	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (d11a==2 | ilo_job1_eco_isic4_2digits==63 | ilo_job1_eco_isic4_2digits==97)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & (inlist(d13,1,2) | d14==1 | d15==1 | d16==1)
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
		replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ((inlist(ilo_job1_ste_icse93,1,6) & (social_security==1 | (d08==3 & d09==1 & d10==1))) | (inlist(ilo_job1_ste_icse93,2,4) & ilo_job1_ife_prod==2))
...
* Comment: Currency -> Nepalese Rupees (1 US$ = 115 Nepalese Rupees in September 2018) 

    * MAIN JOB
	
	* Employees
	gen ilo_job1_lri_ees=.
	    replace ilo_job1_lri_ees=f01c if (f01b==3 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=f01c*(52/12) if (f01b==2 & ilo_job1_ste_aggregate==1)
		replace ilo_job1_lri_ees=f01c*(365/12) if (f01b==1 & ilo_job1_ste_aggregate==1)
			    replace ilo_job1_lri_ees=.     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf=.
	    replace ilo_job1_lri_slf=f01c if (f01b==3 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=f01c*(52/12) if (f01b==2 & ilo_job1_ste_aggregate==2)
		replace ilo_job1_lri_slf=f01c*(365/12) if (f01b==3 & ilo_job1_ste_aggregate==2)
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<=40 & e07==1 & e08==1 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		...
		replace ilo_cat_une=2 if h01==2 & ilo_lfs==2				// Seeking for the first time
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
		...
		replace ilo_dur_details=2 if g06==2 & ilo_lfs==2						// 1 to 3 months
		replace ilo_dur_details=3 if g06==3 & ilo_lfs==2						// 3 to 6 months
		replace ilo_dur_details=4 if g06==4 & ilo_lfs==2						// 6 to 12 months
		replace ilo_dur_details=5 if g06==5 & ilo_lfs==2						// 12 to 24 months
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level
	gen ilo_preveco_isic4_2digits=. 
	    replace ilo_preveco_isic4_2digits=int(h06_nsic/100) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* ------------------------------------------------------------------------------
	...
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits=. 
	    replace ilo_prevocu_isco08_2digits=h04_nsco if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if g01==1 & g03!=13 & g09==3 & ilo_lfs==3			// Seeking, not available
		replace ilo_olf_dlma = 2 if g01==2 & inlist(g09,1,2) & ilo_lfs==3	    	// Not seeking, available
		replace ilo_olf_dlma = 3 if g01==2 & g02==2	& g07==1 & g09==3 & ilo_lfs==3	// Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if g01==2 & g02==2	& g07==2 & ilo_lfs==3			// Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(g08,4,5,6,7) & ilo_lfs==3		// Labour market 
			replace ilo_olf_reason=2 if inlist(g08,1,2,3) & ilo_lfs==3			// Other labour market reasons
			replace ilo_olf_reason=3 if inlist(g08,8,9,10,11,13) & ilo_lfs==3	// Personal/Family-related
			replace ilo_olf_reason=4 if g08==12 & ilo_lfs==3					// Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & inlist(g09,1,2) & ilo_olf_reason==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sub=.
	    replace ilo_sub=1 if !inlist(ilo_lfs,1,2) & inlist(c05,3,4)
