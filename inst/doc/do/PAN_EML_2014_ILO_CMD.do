...
replace P3=-1000 if P3==0
foreach var of varlist _all {
capture replace `var'=. if `var'==0
}
replace P3=0 if P3==-1000
...
capture destring P36, replace
capture destring P29, replace
capture destring P31, replace
capture destring P31A, replace
capture destring P48, replace
capture destring P51, replace
...
if `Y'==2010 {
	recode P8 (60 70 11/15=1) (16 21/25=2) (26/44=3) (45/49 =4) (. 0 51/53=5) , gen(ilo_edu_aggregate)
}
else {
	recode P8 (1/15 21 22=1) (16 23 31/35=2) (36 41/46 51/54=3) (55/84 =4) (. 0 =5) , gen(ilo_edu_aggregate)
}
	replace ilo_edu_aggregate=5 if  !inrange(ilo_edu_aggregate,1,5)
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: this question is made to people aged 15 years and above
if `Y' <= 2015 {
	gen marital = P5
}
...
destring P5_CONYUGA, replace
	gen marital = P5_CONYUGA
}
	
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if marital==7                                     // Single
		replace ilo_mrts_details=2 if marital==4                                     // Married
		replace ilo_mrts_details=3 if marital==1                                     // Union / cohabiting
		replace ilo_mrts_details=4 if marital==6                                     // Widowed
		replace ilo_mrts_details=5 if inlist(marital,2,3,5)                          // Divorced / separated
...
if `Y'<2015 { 
	...
	* Classification aggregated level
		...
		* Secondary activity
		...
		gen todrop_length=length(P46)
		replace ilo_job2_eco_isic4_2digits = substr(ilo_job2_eco_isic4_2digits,1,1) if todrop_length==3
		drop todrop_length
...
* 1) Weekly hours ACTUALLY worked:
			gen ilo_job2_how_actual=P48 if (ilo_lfs==1) & ilo_mjh==2
					*to avoid missing values of workers that were temporary absent
					replace ilo_job2_how_actual=0 if ilo_lfs==1&ilo_job2_how_actual==.&P10_18==2& ilo_mjh==2
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* The variable that allows to know if they were available (number of available hours P51 is empty for that year)
if `Y'!=2012 {
		gen ilo_joball_tru=.
