* TITLE OF DO FILE: ILO Microdata Preprocessing code template - PerÃº
* DATASET USED: PerÃº HS (ENAHO)
* NOTES: 
* Files created: Standard variables on ENAHO PerÃº
...
cd "$inpath"
	...
	* note that there is a series of observations having no weight factor associated to them, as well as observations corresponding to individuals 
		...
		* "200. CaracterÃ­sticas de los miembros del hogar" and exclude those observations such as described there 
	...
	* for 2016: note also that while merging the different modules, questions p559* and p560* were dropped, as the datafile could not be properly imported due to its large size 
	* and in any case information related to personal consumption is not being considered for pre-processing. Should anyone wish to include this info, please refer to 
	* file used for merging data	
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if p209==6                                   // Single
		replace ilo_mrts_details=2 if p209==2                                   // Married
		replace ilo_mrts_details=3 if p209==1                                   // Union / cohabiting
		replace ilo_mrts_details=4 if p209==3                                   // Widowed
		replace ilo_mrts_details=5 if inlist(p209,4,5)                          // Divorced / separated
...
* Comment: availability criterion not considered for people seeking a job - put a note in FWK
			...
			* this technical document (Ficha tÃ©cnica http://iinei.inei.gob.pe/iinei/srienaho/Descarga/DocumentosMetodologicos/2014-55/FichaTecnica.pdf ) (pg. 17-18) 
...
cd "$outpath"
	...
	replace `var'=. if considered!=1 
		...
		drop indu_code_* occ_code_*  /*prev*_cod*/   monthly_* year registration
