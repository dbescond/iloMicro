* TITLE OF DO FILE: ILO Microdata Preprocessing code template - PERU, 2013
* DATASET USED: PERU, SWTS, 2013
* NOTES: 
* Files created: Standard variables PER_SWTS_2013_FULL.dta and PER_SWTS_2013_ILO.dta
...
global country "PER" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2013"  /*time*/
global inputFile "PER_SWTS_2013.dta" /*name of the input file in stata format*/
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: only urban area (S4:31)
	
	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(area,1,2)         // Urban 
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if p205==2            // Male
		replace ilo_sex=2 if p205==1            // Female
...
* Comment:
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  (p401==2 | p412==1)                     // No schooling
		replace ilo_edu_isced97=2 if  inlist(p412,2,3)                      // Pre-primary education
		replace ilo_edu_isced97=3 if  inlist(p412,4,5)                      // Primary education or first stage of basic education
		*replace ilo_edu_isced97=4 if                        // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if  inlist(p412,6,7)                    // Upper secondary education
		replace ilo_edu_isced97=6 if  inlist(p412,8,9)                      // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if  p412==10                      // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  p412==11                      // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if p402==1                       // Attending
		replace ilo_edu_attendance=2 if p402==2                       // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if p302== 6                                         // Single
		replace ilo_mrts_details=2 if p302==2                                          // Married
		replace ilo_mrts_details=3 if inlist(p302,1)                                         // Union / cohabiting
		replace ilo_mrts_details=4 if p302==3                                           // Widowed
		replace ilo_mrts_details=5 if inlist(p302,4,5)                                          // Divorced / separated
...
* Comment: 
    ...
    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=1 if p317_1==2 & p317_2==2 & p317_3==2 & p317_4==2  // No, no difficulty
		replace ilo_dsb_details=2 if p317_1_1==1 | p317_2_1==1 |  p317_3_1==1 |  p317_4_1==1  // Yes, some difficulty
		replace ilo_dsb_details=3 if p317_1_1==2 | p317_2_1==2 |  p317_3_1==2 |  p317_4_1==2    	                // Yes, a lot of difficulty
		replace ilo_dsb_details=4 if p317_1_1==3 | p317_2_1==3 |  p317_3_1==3 |  p317_4_1==3  		                // Cannot do it at all
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (p437_1==1 | p437_2==1 | p437_3==1 | p437_4==1 | ///
		p437_5==1 | p437_6==1 |p437_7==1 |p437_8==1 | p437_9==1) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if p438==1 & ilo_wap==1										// Employed: temporary absent
		replace ilo_lfs=2 if inlist(p601,1,2) & p613==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (p603==1 | p603_a==1) & p613==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
* Comment:
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(p506,3) & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if p506==1 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if p506==2 & ilo_lfs==1          // Own-account workers
		 replace ilo_job1_ste_icse93=4 if p506==4 & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if p506==5 & ilo_lfs==1          // Contributing family workers
...
* Comment: 
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = p530      if ilo_lfs==1 & p530<999
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if p508==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if p508==2 & ilo_lfs==1               				// Temporary
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds):p512_10
		  - Place of work:
		  - Size:
		  - Paid annual leave:p512_3
		  - Paid sick leave:p512_4
*/
    * Social Security:
		gen social_security=.
	    replace social_security=1 if p512_10==1   & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if p512_10==2    & ilo_lfs==1          // no social security (proxy)
	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	     *replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (inlist(ilo_job1_eco_isic4_2digits,97))
		                               
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((publicadmin==1) | ///
									   (p504==1) | ///
									   (p504==4 & ilo_job1_ste_icse93==1 & social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (ilo_job1_ste_icse93==1 & p512_3==1 & p512_4==1) | ///
...
* Comment: 
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = p514e_1      if ilo_job1_ste_aggregate==1 & inrange(p514e_1,0,5400) 
		 replace ilo_job1_lri_ees = 1000      if ilo_job1_ste_aggregate==1 & p514e_1==. &p515==1
		 replace ilo_job1_lri_ees = (5000+1000)/2      if ilo_job1_ste_aggregate==1 & p514e_1==. & p515==2
		 replace ilo_job1_lri_ees = (5000+9000)/2      if ilo_job1_ste_aggregate==1 & p514e_1==. & p515==3
		 replace ilo_job1_lri_ees = 9000      if ilo_job1_ste_aggregate==1 & p514e_1==. & p515==4
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = p524      if ilo_job1_ste_aggregate==2 & p524>=0
...
* Comment:
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_actual<=35  & p531==1 & p532>0  & ilo_lfs==1
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(p612,1,2)      & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if p612==3      & ilo_lfs==2                 // 1 to 3 months
		replace ilo_dur_details=3 if p612==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if p612==5      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if p612==6      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if p612==7      & ilo_lfs==2                 // 24 months or more
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  inlist(p601,1,2) &  p613==2 & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  p601==3 & p613==1 & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if p601==3  & p613==2 &	p604==1 & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if p601==3  & p613==2 &	p604==2 & ilo_lfs==3      // Not seeking, not available, not willing
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(p605,7,8,9,10,11)           & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(p605,1,2)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(p605,3,4,5,6)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if p614==6 & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if (inlist(p605,12) | ilo_olf_reason==.) & ilo_lfs==3     // Not elsewhere classified
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & p613==1  & ilo_olf_reason==1
