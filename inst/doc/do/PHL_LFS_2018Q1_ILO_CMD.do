...
global source "LFS"
global time "2018Q1"
global inputFile "lfsjan18-puf.dta"
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_wgt=ilo_WEIGTH		
		...
		if inlist(time,"2015Q2","2015Q3","2015Q4","2016Q3") {
...
* Comment: compare our numbers with variable "newempstat" (other variable considering labour force status, "cempst1", does not take into 
				...
				* NSO exlcudes overseas worker (whatever is inlist(c10_conwr,1,2,3) ) -- given that there is no info about them (and they fall into category "outside the labour force")
...
* Comment: Currency in the Philippines: Philippine peso
...
if time <= "2016Q1"{
	...
	gen ilo_job1_lri_ees=(ilo_PBASIC*100)*(365/12)
...
* Comment:	Question asks how much time ther person has spent looking for another job
			* note that highest value appearing is 98 weeks, which is less than 24 months - therefore no observations for the category "24 months or more" for the more detailed classification
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if ilo_FLWK==2
		replace ilo_cat_une=2 if ilo_FLWK==1
...
* -------------------------------------------------------------------------------------------
	...
	capture gen ilo_WLG=a07_wlng 
	capture gen	ilo_WLG=c37_willing

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if ilo_lfs!=1 & ilo_LOOK_FOR_WORK==1 & ilo_AVAILABLE_FOR_WORK==2
		replace ilo_olf_dlma=2 if ilo_lfs!=1 & ilo_LOOK_FOR_WORK==2 & ilo_AVAILABLE_FOR_WORK==1
		replace ilo_olf_dlma=3 if ilo_lfs!=1 & ilo_LOOK_FOR_WORK==2 & ilo_AVAILABLE_FOR_WORK==2 & ilo_WLG==1
		replace ilo_olf_dlma=4 if ilo_lfs!=1 & ilo_LOOK_FOR_WORK==2 & ilo_AVAILABLE_FOR_WORK==2 & ilo_WLG==2
	...
	drop ilo_WORK ilo_JOB ilo_LOOK_FOR_WORK	ilo_WLG		
