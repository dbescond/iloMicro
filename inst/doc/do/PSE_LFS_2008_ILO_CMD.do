...
global source "LFS"
global time "2008"
global input_file "LFS-2008-e.DTA"
...
* -------------------------------------------------------------------------------------------
	...
	* Comment: Completed level of education always to be considered !
		...
		gen ilo_edu_isced11=.
				* No schooling
			replace ilo_edu_isced11=1 if eduq==1 
				* Early childhood eduqation
			replace ilo_edu_isced11=2 if eduq==2 
				* Primary eduqation
			replace ilo_edu_isced11=3 if eduq==3
				* Lower secondary eduqation
			replace ilo_edu_isced11=4 if eduq==4
				* Upper secondary eduqation
			replace ilo_edu_isced11=5 if eduq==5
				* Post-secondary non-tertiary eduqation
			replace ilo_edu_isced11=6 if eduq==6
				* Short-cycle tertiary eduqation
			/* replace ilo_edu_isced11=7 if eduq==6 */
				* Bachelor's or equivalent level
			replace ilo_edu_isced11=8 if eduq==7
				* Master's or equivalent level
			replace ilo_edu_isced11=9 if (eduq==8 | eduq==9)
				* Doctoral or equivalent level 
			replace ilo_edu_isced11=10 if eduq==10
				* Not elsewhere classified
			replace ilo_edu_isced11=11 if ilo_edu_isced11==.
				label def isced_11_lab 1 "X - No schooling" 2 "0 - Early childhood eduqation" 3 "1 - Primary eduqation" 4 "2 - Lower secondary eduqation" 5 "3 - Upper secondary eduqation" ///
										6 "4 - Post-secondary non-tertiary eduqation" 7 "5 - Short-cycle tertiary eduqation" 8 "6 - Bachelor's or equivalent level" 9 "7 - Master's or equivalent level" ///
										10 "8 - Doctoral or equivalent level" 11 "9 - Not elsewhere classified"
				label val ilo_edu_isced11 isced_11_lab
				lab var ilo_edu_isced11 "eduqation (ISCED 11)"
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_edu_attendance=.
			replace ilo_edu_attendance=1 if atten==1
			replace ilo_edu_attendance=2 if atten!=1
...
* Comment: 
	...
	* Primary activity
		...
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if inrange(pw23,6,12) // 1 - Employees
			replace ilo_job1_ste_icse93=2 if inlist(pw23,1,2)  // 2 - Employers
			replace ilo_job1_ste_icse93=3 if inlist(pw23,3,4)  // 3 - Own-account workers
			/* replace ilo_job1_ste_icse93=4 if */
			replace ilo_job1_ste_icse93=5 if pw23==5 // 5 - Contributing family workers
...
* -------------------------------------------------------------------------------------------
	...
	* Primary occupation
	 ...
	 gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inrange(pw23,6,9)
...
* Comment: ??? with our definitions, there are not people considered as 'dis', relaxing the condition of olf==1, and considering 
**             that the individual does not seek for job, the result is higher. 
...
*/
	...
	gen ilo_dis=1 if ilo_lfs==3 & pw11==1 & pw14==2
		lab def ilo_dis_lab 1 "Discouraged job-seekers" 
		lab val ilo_dis ilo_dis_lab
		lab var ilo_dis "Discouraged job-seekers"
