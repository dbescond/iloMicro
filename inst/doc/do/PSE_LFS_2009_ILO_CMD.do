...
global source "LFS"
global time "2009"
global input_file "LFS-2009-e.DTA"
...
** Comment:  ISCO-88 classification used
	...
	* Two-digits level
			...
			** The variable ilo_job1_ocu_isco88 is only possible to compute at 1-digit level due to the information given by pw22. 
		...
		* Primary activity
		...
		gen occ_code_prim_1dig=int(pw22/10) if ilo_lfs==1 & !inrange(pw22,1,9)
			replace occ_code_prim_1dig = pw22 if ilo_lfs==1 & inrange(pw22,1,9)
...
COMMENT: Informality is not computed in 2009 because the procedure would be different from that followed in more recent years.
...
* Comment: ISCO-88 classification being used
	...
	* Reduce it to 1-digit level 
	...
	gen prevocu_cod=int(pw22/10) if ilo_lfs==2 & ilo_cat_une==1 & !inrange(pw22,1,9) & pw22!=99 
		replace prevocu_cod = pw22 if ilo_lfs==2 & ilo_cat_une==1 & inrange(pw22,1,9) 
