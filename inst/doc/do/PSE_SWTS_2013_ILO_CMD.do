* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Occupied Palestinian Territory, 2013
* DATASET USED: Occupied Palestinian Territory, SWTS, 2013
* NOTES: 
* Files created: Standard variables PSE_SWTS_2013_FULL.dta and PSE_SWTS_2013_ILO.dta
...
global country "PSE" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2013"  /*time*/
global inputFile "PSE_SWTS_2013.dta" /*name of the input file in stata format*/
...
* Comment:
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  c12a==1 | c01==2                     // No schooling
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		replace ilo_edu_isced97=3 if  c12a==2                      // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if  c12a==3                      // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if  c12a==4                      // Upper secondary education
		replace ilo_edu_isced97=6 if  c12a==5                      // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if  c12a==6                      // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  c12a==7                      // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if   c02==1                     // Attending
		replace ilo_edu_attendance=2 if   inlist(c02,2,3)                     // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if   b04==1                                        // Single
		replace ilo_mrts_details=2 if   b04==3                                        // Married
		replace ilo_mrts_details=3 if   b04==2                                        // Union / cohabiting
		replace ilo_mrts_details=4 if   b04==5                                        // Widowed
		replace ilo_mrts_details=5 if   inlist(b04,4,6)                                        // Divorced / separated
...
* Comment: 
    ...
    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=1 if b12==1 & b13==1 & b14==1 & b15==1  // No, no difficulty
		replace ilo_dsb_details=2 if b12==2 | b13==2 |  b14==2 |  b15==2  // Yes, some difficulty
		replace ilo_dsb_details=3 if b12==3 | b13==3 |  b14==3 |  b15==3    	                // Yes, a lot of difficulty
		replace ilo_dsb_details=4 if b12==4 | b13==4 |  b14==4 |  b15==4  		                // Cannot do it at all
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c28_1==1 | c28_2==1 | c28_3==1 | c28_4==1 | ///
		c28_5==1 | c28_6==1 |c28_7==1) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if c29==1 & !inlist(c30,13) & ilo_wap==1										// Employed: temporary absent
...
* Comment:
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if d08==1  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if d08==2 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if d08==3 & ilo_lfs==1          // Own-account workers
		 *replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if d08==4 & ilo_lfs==1          // Contributing family workers
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if d10==2 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if d10==1 & ilo_lfs==1               				// Temporary
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds): d14_10
		  - Place of work:
		  - Size:
		  - Paid annual leave: d14_3
		  - Paid sick leave: d14_4
*/
    * Social Security:
		gen social_security=.
	    replace social_security=1 if d14_10==1   & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if d14_10==2    & ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		...
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((publicadmin==1) | ///
									   (d05==1) | ///
									   (d05==4 & ilo_job1_ste_icse93==1 & social_security==1))		   
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (ilo_job1_ste_icse93==1 & d14_3==1 & d14_4==1) | ///
...
* Comment: 
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = d15 if ilo_job1_ste_aggregate==1 & d15>0
		replace ilo_job1_lri_ees = d15 + d17 if ilo_job1_ste_aggregate==1 & d15>0 & d16==2 & d17>0
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = d27 if ilo_job1_ste_aggregate==2 & d27>0
		 replace ilo_job1_lri_slf = d27 + d29 if ilo_job1_ste_aggregate==2 & d27>0 & d28==1 & d29>0
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  inlist(e01,1,2) &  e13==2 & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if  e01==3 & e13==1 & ilo_lfs==3	    // Not seeking, available
		replace ilo_olf_dlma = 3 if e01==3  & e13==2 &	e04==1 & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if e01==3  & e13==2 &	e04==2 & ilo_lfs==3      // Not seeking, not available, not willing
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(e05,8,9,10,11,12)           & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(e05,1,2)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(e05,3,4,5,6,7)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if e14==6 & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if (inlist(e05,13) | ilo_olf_reason==.) & ilo_lfs==3     // Not elsewhere classified
