* TITLE OF DO FILE: ILO Microdata Preprocessing code template - RUSSIAN FEDERATION, 2012
* DATASET USED: RUSSIAN FEDERATION, SWTS, 2012
* Files created: Standard variables RUS_SWTS_2012_FULL.dta and RUS_SWTS_2012_ILO.dta
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 08 August 2018
...
global source "SWTS"
global time "2012" 
global inputFile "RUS_SWTS_2012.dta"
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if highestlevel_comp_1==1     // No schooling
		*replace ilo_edu_isced97=2 if  highestlevel_comp_1==2      // Pre-primary education
		replace ilo_edu_isced97=3 if highestlevel_comp_1==2      // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if   highestlevel_comp_1==3     // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if    highestlevel_comp_1==4    // Upper secondary education
		*replace ilo_edu_isced97=6 if   highestlevel_comp_1==5   // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if   highestlevel_comp_1==6     // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if  highestlevel_comp_1==7      // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if inlist(marital_1,6,2)    // Single
		replace ilo_mrts_details=2 if marital_1==1    // Married
		replace ilo_mrts_details=3 if marital_1==4    // Union / cohabiting
		replace ilo_mrts_details=4 if  marital_1==3   // Widowed
		replace ilo_mrts_details=5 if  marital_1==5   // Divorced / separated
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (emp_lastw_1==1 | emp_lastw_2==1) & ilo_wap==1				   					// Employed: ILO definition
		replace ilo_lfs=1 if emp_temp_absent==1 & ilo_wap==1												// Employed: temporary absent
		replace ilo_lfs=2 if inlist(seekingjob,1,2) & availability_1==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Only aggregated categories

   * MAIN JOB

	* Aggregate categories 
	  gen ilo_job1_ste_aggregate=.
		  replace ilo_job1_ste_aggregate=1 if statusinemp_1==1   & ilo_lfs==1              // Employees
		  replace ilo_job1_ste_aggregate=2 if statusinemp_1==2 & ilo_lfs==1  // Self-employed
...
* ------------------------------------------------------------------------------
   ...
   * Aggregate level
   gen ilo_job1_eco_aggregate=.
	   replace ilo_job1_eco_aggregate=1 if industry==1
	   replace ilo_job1_eco_aggregate=2 if industry==3
	   replace ilo_job1_eco_aggregate=3 if industry==6
	   replace ilo_job1_eco_aggregate=4 if inlist(industry,2,4,5)
	   replace ilo_job1_eco_aggregate=5 if inrange(industry,7,14)
	   replace ilo_job1_eco_aggregate=6 if inrange(industry,15,21)
	   replace ilo_job1_eco_aggregate=7 if industry==22
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
		...
		*replace ilo_job1_ocu_isco08=10 if (ilo_job1_ocu_isco08==0 & ilo_lfs==1)                                      // Armed forces
		 ...
		 replace ilo_job1_ocu_isco08=11 if (ilo_job1_ocu_isco08==. | ilo_job1_ocu_isco08==999) & ilo_lfs==1
...
* ------------------------------------------------------------------------------ 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = hours_week_usual      if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual =  hours_week_actual     if ilo_lfs==1
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds): benefit_serv_j
		  - Place of work:
		  - Size:
		  - Paid annual leave: benefit_serv_c
		  - Paid sick leave: benefit_serv_d
*/
    * Social Security:
    * Social Security:
	*gen social_security=.
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.	         
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & informalsector==0
		replace ilo_job1_ife_prod=1 if ilo_lfs==1 & !inlist(ilo_job1_ife_prod,2,3) & informalsector==1
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & informalemp==0
	    replace ilo_job1_ife_nature=1 if ilo_lfs==1 & ilo_job1_ife_nature!=2 & informalemp!=0
...
* ------------------------------------------------------------------------------
	...
	* Employees
	 gen ilo_job1_lri_ees = .   
	    replace ilo_job1_lri_ees = cash_amount if ilo_job1_ste_aggregate==1 & cash_amount>0
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(5*52/12) if payment_period==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(52/12) if payment_period==2
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(26/12) if payment_period==3
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
		...
		replace ilo_job1_lri_slf=ilo_job1_lri_slf + amount_prodhh_selfemp if deduction==1 & amount_prodhh_selfemp>0
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_usual<= 35  & likeworkmore==1  & more_less_hours>0 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if  inlist(seekingjob,1,2) &  availability_1==2 & ilo_lfs==3     		// Seeking, not available
		replace ilo_olf_dlma = 2 if  seekingjob==3 & availability_1==1 & ilo_lfs==3	    				// Not seeking, available
		replace ilo_olf_dlma = 3 if seekingjob==3  & availability_1==2 &	wanttowork==1 & ilo_lfs==3  // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if seekingjob==3  & availability_1==2 &	wanttowork==2 & ilo_lfs==3  // Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=3 if inlist(reason_notseeking,3,4,5,6) & ilo_lfs==3             				// Personal/Family-related
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & availability_1==1 & ilo_olf_reason==1
