* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Rwanda, 2017
* DATASET USED: Rwanda LFS 2017
* Files created: Standard variables on Rwanda LFS 2017
* Authors: ILO / Department of Statistics / DPAU
* Starting Date: 02 August 2018
...
global source "LFS"
global time "2017Q1"
global inputFile "RWA_LFS_2017Q1.dta"
...
* Comment: Only persons aged 14+ are answering these questions. 

	gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if b02a==1									// No schooling
		replace ilo_edu_isced11=2 if b02a==2									// Early childhood education
		replace ilo_edu_isced11=3 if b02a==3									// Primary education
		replace ilo_edu_isced11=4 if b02a==4									// Lower secondary education
		replace ilo_edu_isced11=5 if b02a==5									// Upper secondary education
		* replace ilo_edu_isced11=6 if  										// Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if b02a==6 & inlist(b03,1,2,3,4)				// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if b02a==6 & inlist(b03,5,6) 					// Bachelor or equivalent
		replace ilo_edu_isced11=9 if b02a==6 & inlist(b03,7,8)					// Master's or equivalent level
		replace ilo_edu_isced11=10 if b02a==6 & b03==9							// Doctoral or equivalent level
		replace ilo_edu_isced11=11 if b02a==.									// Not elsewhere classified
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if b01==1				// Attending 
		replace ilo_edu_attendance=2 if b01==2				// Not attending 
		replace ilo_edu_attendance=3 if b01==.				// Not elsewhere classified 
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
		replace ilo_mrts_details=1 if a05==6				// Single
		replace ilo_mrts_details=2 if inlist(a05,1,2)		// Married
		replace ilo_mrts_details=3 if a05==3				// Union / cohabiting
		replace ilo_mrts_details=4 if a05==7				// Widowed
		replace ilo_mrts_details=5 if inlist(a05,4,5)		// Divorced / separated
...
* -------------------------------------------------------------------------------------------		
    ...
    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=1 if (a06==1 & a07==1 & a08==1 & a09==1 & a10==1)	// No, no difficulty
		replace ilo_dsb_details=2 if (a06==2 | a07==2 | a08==2 | a09==2 | a10==2)	// Yes, some difficulty
		replace ilo_dsb_details=3 if (a06==3 | a07==3 | a08==3 | a09==3 | a10==3)	// Yes, a lot of difficulty
		replace ilo_dsb_details=4 if (a06==4 | a07==4 | a08==4 | a09==4 | a10==4)	// Cannot do it at all
...
* -------------------------------------------------------------------------------------------
	 ...
	 gen ilo_lfs=.
		replace ilo_lfs=1 if (c01==1 | c02==1 | c03==1) & !inlist(c05,3,4) & ilo_wap==1					// Employed
		replace ilo_lfs=1 if ((c06==1 & inlist(c07,1,2,3)) | (c06==1 & (c08==1 | c09==1))) & ilo_wap==1	// Employed
		replace ilo_lfs=2 if ilo_lfs!=1 & (c19==1 | c20==1) & c21a!=8 & c25==1 & ilo_wap==1				// Unemployed
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_mjh=.
		replace ilo_mjh=2 if c10==1 & ilo_lfs==1
		replace ilo_mjh=1 if c10==2 & ilo_lfs==1
	...
	* MAIN JOB
		...
		* Detailed categories
			gen ilo_job1_ste_icse93=.
				...
				replace ilo_job1_ste_icse93=4 if d05==5					// Producer cooperatives
				replace ilo_job1_ste_icse93=5 if d05==6					// Contributing family workers
				replace ilo_job1_ste_icse93=6 if (d05==7 | d04==.)		// Not classifiable
	...
	* SECOND JOB
		...
		* Detailed categories
		gen ilo_job2_ste_icse93=.
				replace ilo_job2_ste_icse93=1 if inlist(e04,1,2)		// Employees
				replace ilo_job2_ste_icse93=2 if e04==3					// Employers
				replace ilo_job2_ste_icse93=3 if e04==4					// Own-account workers
				replace ilo_job2_ste_icse93=4 if e04==5					// Producer cooperatives
				replace ilo_job2_ste_icse93=5 if e04==6					// Contributing family workers
				replace ilo_job2_ste_icse93=6 if (e04==7 | e04==.)		// Not classifiable
...
* -------------------------------------------------------------------------------------------	  
		...
		* SECOND JOB
			...
			* ISIC Rev. 4 - 2 digits
						gen isic2dig_job2=int(e02b1/100)
...
* -------------------------------------------------------------------------------------------
	...
	* MAIN JOB
			gen occup_2dig=int(d01b2/100) if ilo_lfs==1 
		...
		* SECOND JOB
			gen occup_2dig_job2=int(e01b1/100) if ilo_mjh==2 
...
* -------------------------------------------------------------------------------------------		
	...
	* MAIN JOB
		gen ilo_job1_ins_sector=.
			replace ilo_job1_ins_sector=1 if inlist(d04,1,2,4,5) & ilo_lfs==1		// Public
	...
	* SECOND JOB
		gen ilo_job2_ins_sector=.
			replace ilo_job2_ins_sector=1 if inlist(e03,1,2,4,5) & ilo_mjh==2		// Public
...
* -------------------------------------------------------------------------------------------		
	...
	* Weekly hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    replace ilo_job1_how_actual=(c12a*c13a) if ilo_lfs==1
	...
	* Weekly hours USUALLY worked
	gen ilo_job1_how_usual=.
	    replace ilo_job1_how_usual=c11a if ilo_lfs==1
	...
	* Weekly hours ACTUALLY worked
    gen ilo_job2_how_actual=.
	    replace ilo_job2_how_actual=(c12b*c13b) if ilo_mjh==2
	...
	* Weekly hours USUALLY worked
	gen ilo_job2_how_usual=.
	    replace ilo_job2_how_usual=c11b if ilo_mjh==2
	...
	* Weekly hours ACTUALLY worked
	gen ilo_joball_how_actual=.
	    replace ilo_joball_how_actual=(ilo_job1_how_actual+ilo_job2_how_actual) if ilo_lfs==1 & ilo_mjh==2
		replace ilo_joball_how_actual=ilo_job1_how_actual if ilo_lfs==1 & ilo_joball_how_actual==.
	...
	* Weekly hours USUALLY worked
    gen ilo_joball_how_usual=.
		...
		replace ilo_joball_how_usual=ilo_job1_how_usual if ilo_lfs==1 & ilo_joball_how_usual==.
...
* -------------------------------------------------------------------------------------------			
	...
	/* Questions used: * D04 - Institutional sector
					   * D05 - Status in employment
					   * D21 - Bookkeeping
					   * D20 - Registration
					   * D09A - Social security coverage
					   * D10A - Paid annual leave
					   * D10B - Paid sick leave
 	*/

	* 1) Unit of production
	
		gen social_security=.
			replace social_security=1 if ((d09a==1 | (d09a==3 & d10a==1 & d10b==1)) & ilo_lfs==1)
	
		gen ilo_job1_ife_prod=.
			replace ilo_job1_ife_prod=2 if (inlist(d04,1,2,4,5) | d21==1 | d20==1)
			replace ilo_job1_ife_prod=3 if (d04==7 | ilo_job1_eco_isic4_2digits==97 | ilo_job1_ocu_isco08_2digits==63)
...
* -------------------------------------------------------------------------------------------
	...
	* Employees
		gen ilo_job1_lri_ees=.
			replace ilo_job1_lri_ees=d12a if ilo_job1_ste_aggregate==1 & d13==1
			replace ilo_job1_lri_ees=d12a * ((365/14)/12) if ilo_job1_ste_aggregate==1 & d13==2
			replace ilo_job1_lri_ees=d12a * ((365/7)/12) if ilo_job1_ste_aggregate==1 & d13==3
			replace ilo_job1_lri_ees=d12a * (365/12) if ilo_job1_ste_aggregate==1 & d13==4
			replace ilo_job1_lri_ees=. if d12a==. & ilo_job1_ste_aggregate==1
				lab var ilo_job1_lri_ees "Monthly earnings of employees in main job"

	* Self-employed
		gen ilo_job1_lri_slf=d18a1 if ilo_job1_ste_aggregate==2.
...
* Comment : To follow national definition : 40 hours and hours actually worked are used. 	
		...
		gen ilo_joball_tru=.
			replace ilo_joball_tru=1 if (ilo_lfs==1 & c17==1 & ilo_joball_how_actual<40)
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if f01==1 & ilo_lfs==2				// Previously employed       
		replace ilo_cat_une=2 if f01==2 & ilo_lfs==2				// Seeking for the first time
...
* -------------------------------------------------------------------------------------------
		...
		* Details
			gen ilo_dur_details=.
				replace ilo_dur_details=1 if c22==1 & ilo_lfs==2 				// Less than 1 month
				replace ilo_dur_details=2 if c22==2 & ilo_lfs==2				// 1 month to less than 3 months
				replace ilo_dur_details=3 if c22==3 & ilo_lfs==2				// 3 months to less than 6 months
				replace ilo_dur_details=4 if c22==4 & ilo_lfs==2				// 6 months to less than 12 months
				replace ilo_dur_details=5 if c22==5 & ilo_lfs==2				// 12 months to less than 24 months
				replace ilo_dur_details=6 if c22==6 & ilo_lfs==2				// 24 months or more
...
* -------------------------------------------------------------------------------------------		
	...
	* 2-digit level
	gen ilo_preveco_isic4_2digits=. 
	    replace ilo_preveco_isic4_2digits=int(f04b1/100) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* -------------------------------------------------------------------------------------------	
	...
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits=. 
	    replace ilo_prevocu_isco08_2digits=int(f03b1/100) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_olf_dlma=.
			replace ilo_olf_dlma=1 if (c19==1 | c20==1) & c21a!=8 & c25!=1 & ilo_lfs==3		// Seeking, not available
			replace ilo_olf_dlma=2 if c19==2 & c20==2 & c25==1 & ilo_lfs==3					// Not seeking, available
			replace ilo_olf_dlma=3 if c19==2 & c20==2 & c25!=1 & c23==1 & ilo_lfs==3		// Not seeking, not available, willing
			replace ilo_olf_dlma=4 if c19==2 & c20==2 & c25!=1 & c23==1 & ilo_lfs==3		// Not seeking, not available, not willing
...
* -------------------------------------------------------------------------------------------		
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if	inlist(c24,7,8,9,10,11) & ilo_lfs==3	// Labour market (discouraged)
			* replace ilo_olf_reason=2 if					 					// Other labour market reasons
			replace ilo_olf_reason=3 if inlist(c24,1,2,3,4,5,6) & ilo_lfs==3	// Personal/Family-related
			replace ilo_olf_reason=4 if inlist(c24,12,13) & ilo_lfs==3 			// Does not need/want to work
...
* -------------------------------------------------------------------------------------------		
		...
		gen ilo_dis=.
			replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & c25==1 & ilo_olf_reason==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sub=.
	    replace ilo_sub=1 if !inlist(ilo_lfs,1,2) & inlist(c05,3,4) & ilo_wap==1 
