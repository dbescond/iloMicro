* TITLE OF DO FILE: ILO Microdata Preprocessing code template - EL SALVADOR, 2012
* DATASET USED: EL SALVADOR, SWTS, 2012
* NOTES: 
* Files created: Standard variables SLV_SWTS_2012_FULL.dta and SLV_SWTS_2012_ILO.dta
...
global source "SWTS"
global time "2012"
global inputFile "SLV_SWTS_2012.dta"
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if  area==2        // Urban 
		replace ilo_geo=2 if  area==1        // Rural
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if highestlevel_comp==7 |ever_attend==2       // No schooling
		*replace ilo_edu_isced97=2 if                        					// Pre-primary education
		replace ilo_edu_isced97=3 if highestlevel_comp==1                		// Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if highestlevel_comp==2                     	// Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if highestlevel_comp==3                       // Upper secondary education
		replace ilo_edu_isced97=6 if highestlevel_comp==4                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if highestlevel_comp==5                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if highestlevel_comp==6                      	// Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if estadocivil==6                            // Single
		replace ilo_mrts_details=2 if estadocivil==2                            // Married
		replace ilo_mrts_details=3 if estadocivil==1                            // Union / cohabiting
		replace ilo_mrts_details=4 if  estadocivil==3                           // Widowed
		replace ilo_mrts_details=5 if  inlist(estadocivil,4,5)                  // Divorced / separated
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (oitt_s02_234==1 | oitt_s02_234_02==1 | oitt_s02_234_03==1 | oitt_s02_234_04==1 | ///
		oitt_s02_234_05==1 | oitt_s02_234_06==1 |oitt_s02_234_07==1 ) & ilo_wap==1				   				// Employed: ILO definition
		replace ilo_lfs=1 if oitt_s02_5_235==1 & !inlist(oitt_s02_5_236,13) & ilo_wap==1						// Employed: temporary absent
...
* Comment: 
	...
	* 2 digit level
	...
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = isic_occ_main     if ilo_lfs==1
...
* Comment:
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = isco08c2d if ilo_lfs==1 
		replace ilo_job1_ocu_isco08_2digits = .   if ilo_lfs!=1 | isco08c2d==99
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
	                                     ((ilo_job1_ste_icse93==1 & benefit_servj==1) | ///
										 (ilo_job1_ste_icse93==1 & inlist(benefit_servj,3,997)& benefit_servc==1 & benefit_servd==1) | ///
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(oitt_s03_7_412,1,2) & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if oitt_s03_7_412==3      & ilo_lfs==2                 // 1 to 3 months ********[C7:3905]
		replace ilo_dur_details=3 if oitt_s03_7_412==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if oitt_s03_7_412==5      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if oitt_s03_7_412==6      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if oitt_s03_7_412==7      & ilo_lfs==2                 // 24 months or more
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			...
			replace ilo_olf_reason=4 if oitt_s03_7_414==6 & ilo_lfs==3             								// Does not need/want to work
			replace ilo_olf_reason=5 if (inlist(reason_notseeking,13,99) | ilo_olf_reason==.) & ilo_lfs==3     // Not elsewhere classified
