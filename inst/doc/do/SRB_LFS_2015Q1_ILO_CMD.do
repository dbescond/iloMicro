* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Serbia, 2015Q1
* DATASET USED: Serbia LFS 2015 Q1
* NOTES:
...
global source "LFS"
global time "2015Q1"
global inputFile "SRB_LFS_2015_Q1"
...
*--- In 2016, the variable "tip" is a string variable with characters, then:   
   local A "year"
   gen `A'= to_drop_1
...
if (`A' == 2016 & `Q' == 1) {
  ...
  *---- for 2014 and 2015 where the variable "tip" is string but float
else{
...
*          It is mapped to ISCED11 following the structure of the education system in Serbia (2008)
*          http://www.ibe.unesco.org/sites/default/files/Serbia.pdf
*          answered only by 15+
	...
	gen ilo_edu_isced11=.
		...
		replace ilo_edu_isced11=4 if inlist(hatlevel,5,6)                       // Lower secondary education
		replace ilo_edu_isced11=5 if inlist(hatlevel,7,8)      		            // Upper secondary education
		replace ilo_edu_isced11=6 if hatlevel==9                	            // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if hatlevel==10 			                    // Short-cycle tertiary eucation
		replace ilo_edu_isced11=8 if hatlevel==11     				            // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if hatlevel==12	                            // Master's or equivalent level
		replace ilo_edu_isced11=10 if hatlevel==13		                        // Doctoral or equivalent level
...
* Comment: no information related to disability status
...
/*ilo_lfs: employment includes employed, employed people temporary absent and seasonal workers
(contract lay-off or off-season); unemployment follows the ILO definition of unemployment: 
a) people without work during the reference period, b) people currently available for 
work, and c) seeking work (including those who are not seeking because they have already found
a job which is about to start in less than 3 months) */

gen ilo_lfs=.
	    replace ilo_lfs=1 if (posplata==1 | pospoc==1 | posnepl==1 | pospolj==1) & ilo_wap==1                                              //Employed
		replace ilo_lfs=1 if (odsutvl==1 & (masine==1 | kancel==1 | reklama==1)) & ilo_wap==1                                              //Employed temporary absent self-employed and using one of machinery, place or pub  
		replace ilo_lfs=1 if (odsutvl==1 & (masine!=1 & kancel !=1 & reklama!=1) & uktrajods==1) & ilo_wap==1                              //Employed temporary absent self-employed not using previous but returning in less than 3 months
        ...
        replace ilo_lfs=1 if (odspomcl==1 & povratak==1 & uktrajods==1) & ilo_wap==1                                                       //Employed temporary absent unpaid worker and (<3 months)
		replace ilo_lfs=1 if (odsradnik==1 & garancijar==1 & inlist(nowkreas,5,6)) & ilo_wap==1                                            //Employed temporary absent unpaid worker and assurance to resume work and ill or maternity leave
		replace ilo_lfs=1 if ((inlist(razlnerad,1,3) & uktrajods==1) | (inlist(razlnerad,1,3) & uktrajods==2 & zarada50==3)) & ilo_wap==1  //Contract lay-off/off-season and (<3 months or >3months + 50% or more of the salary)
		replace ilo_lfs=1 if (odsradnik==1 & inlist(nowkreas,5,6) & uktrajods==1) & ilo_wap==1                                             //Reason of absence from work illness, injury or temporary disability (or) maternity leave, absence less than3 months
		replace ilo_lfs=1 if (odsradnik==1 & inlist(nowkreas,5,6) & uktrajods==2 & zarada50==3) & ilo_wap==1                               //Reason of absence from work illness, injury or temporary disability (or) maternity leave, absence more than 3 months + receiving 50% or more of the salary
  ...
  * MAIN JOB:
	...
	* Detailed categories:
		...
		gen ilo_job1_ste_icse93=.
			...
			replace ilo_job1_ste_icse93=2 if (inlist(stapro,1,2,6) & zapdrrad==1 & ilo_lfs==1)	                                          // Employers
			replace ilo_job1_ste_icse93=3 if (inlist(stapro,3,4,5) & ilo_lfs==1) | (inlist(stapro,1,2,6) & zapdrrad==2 & ilo_lfs==1)      // Own-account workers
   ...
   *TWO DIGIT LEVEL
		...
		*Correspondences at two digit level
		gen indu_code_prim=int(nace3d/10) if ilo_lfs==1 
		    replace indu_code_prim=. if indu_code_prim==0 & ilo_lfs==1
		...
		* economic activity is already at 2 digit level
		...
		gen ilo_job2_eco_isic4_2digits=nace2j2d if ilo_mjh==2
		    replace ilo_job2_eco_isic4_2digits=. if ilo_job2_eco_isic4_2digits==0
...
* Comment: Private sector includes: private registered/not registered and other ownership registered/not registered
...
/* Useful questions:
				* Institutional sector: svojina (for state-owned); ilo_job1_eco_isic4==20 (T: Activities of households as employers, etc --> ortogonal to svojina, then ok)
				* Destination of production: prodaja 
	...
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR		
			...
			gen ilo_job1_ife_prod=.
			    replace ilo_job1_ife_prod=2 if (svojina==3 & ilo_lfs==1) | (inlist(svojina,1,4) & ilo_lfs==1)
				replace ilo_job1_ife_prod=3 if (ilo_job1_eco_isic4==20) & ilo_lfs==1
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	      ...
	      gen ilo_job1_ife_nature=.
		      replace ilo_job1_ife_nature=1 if ilo_job1_ste_icse93==5 | (inlist(ilo_job1_ste_icse93,1,6) & social_security==2) | ///
			                                   (inlist(ilo_job1_ste_icse93,2,4) & ilo_job1_ife_prod==1) | (ilo_job1_ste_icse93==3 & prodaja==2) | ///
											   (ilo_job1_ste_icse93==3 & inlist(prodaja,.,1) & ilo_job1_ife_prod==1) 
			  replace ilo_job1_ife_nature=2 if (inlist(ilo_job1_ste_icse93,1,6) & social_security==1) | (inlist(ilo_job1_ste_icse93,2,4) & inlist(ilo_job1_ife_prod,2,3)) | ///
			                                   (ilo_job1_ste_icse93==3 & inlist(prodaja,.,1) & inlist(ilo_job1_ife_prod,2,3)) 
...
* Comment: the total net salary received in the previous month is computed merging the exact amount
*          for those that answered the question, and the mid-point of their interval for those who
*          did not want to answer the exact amount but answered following the table of intervals.
*          qtable note due to "net salary"
	 
	 *MAIN JOB
	  
	/*mid-point for those using the intervals*/
	           gen incdecil_mid=.
			       replace incdecil_mid=0 if inlist(incdecil,0,98)              // Just started working and did not receive a salary + did not have a salary for several months
				   ...
				   replace incdecil_mid=200000 if incdecil==10                  // 200000 RSD and more
				   replace incdecil_mid=. if incdecil==99                       // Refused to answer
	
	/*final amount*/
	           
			   gen incdecil_final=. 
			       replace incdecil_final=incdecil_tacno if ilo_lfs==1                       // Exact amount
				   replace incdecil_final=incdecil_mid if (incdecil_final==. & ilo_lfs==1)   // Mid-point of the interval for the rest
			   			   
    /* monthly labour related income*/
	         
	        *Employees
	           gen ilo_job1_lri_ees=.
			       replace ilo_job1_lri_ees=incdecil_final if (ilo_job1_ste_aggregate==1)    
				   lab var ilo_job1_lri_ees "Monthly earnings of employees in main job"
	 
	         *Self-employed
			   * quetion refers to net salary -> only for employees; no question is asked to self-employees
				   
	 *SECOND JOB
	 
	 /*no information related to income from the second job*/
...
* Comments: "Search not yet started" is being classified as "not elsewhere classied"
...
*          Not possible to keep two digit level because not all previously employed have informartion
*          on the economic activity at the last job.
   ...
   * ONE DIGIT LEVEL
		...
		gen ilo_preveco_isic4=.
			...
			replace ilo_preveco_isic4=22 if nacepr2d==. & (ilo_lfs==2 & ilo_cat_une==1) 
...
*           the one digit and aggregated level)
		...
		* ONE DIGIT LEVEL
		gen ilo_prevocu_isco08=.
		    replace ilo_prevocu_isco08=11 if ilo_prevocu_isco08_2digits==99 & ilo_lfs==2 & ilo_cat_une==1                                //Not elsewhere classified
		...
		* AGGREGATE
			gen ilo_prevocu_isco08_aggregate=.
				...
				replace ilo_prevocu_isco08_aggregate=7 if ilo_prevocu_isco08==11
					lab val ilo_prevocu_isco08_aggregate ocu_aggr_lab
		...
		* SKILL LEVEL
		    gen ilo_prevocu_isco08_skill=.
				...
				replace ilo_prevocu_isco08_skill=4 if inlist(ilo_prevocu_isco08,10,11)        // Not elsewhere classified
					lab val ilo_prevocu_isco08_skill ocu_skill_lab
				    lab var ilo_prevocu_isco08_skill "Previous occupation (Skill level)"

* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			General social protection ('ilo_gsp_unschemes') [to check]
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
...
* Comment:
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 4 if (inlist(seekwork,3,4) & availble==2 & wantwork==2 & ilo_lfs==3)	 // Not seeking, not available, not willing
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if	(inlist(seekreas,1,7) & ilo_lfs==3)          			//Labour market
		replace ilo_olf_reason=2 if	(inlist(seekreas,2,3,4,5) & ilo_lfs==3)             	//Personal/Family-related
		replace ilo_olf_reason=3 if (seekreas==6 & ilo_lfs==3)			                    //Does not need/want to work
		replace ilo_olf_reason=4 if (inlist(seekreas,8,9) & ilo_lfs==3)						//Not elsewhere classified
...
cd "$outpath"
		...
		drop to_drop to_drop_1 to_drop_2 year quarter
		
		/*Variables computed in-between*/
		drop lab isco08_2digits isco88_2digits isco08 isco88 isic4_2digits isic4 isic3_2digits isic3 indu_code_prim
		drop occ_code_prim social_security incdecil_mid incdecil_final occ_code_previ
