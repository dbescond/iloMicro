* TITLE OF DO FILE: ILO Microdata Preprocessing code template - SERBIA, 2015
* DATASET USED: SERBIA, SWTS, 2015
* NOTES: 
* Files created: Standard variables SRB_SWTS_2015_FULL.dta and SRB_SWTS_2015_ILO.dta
...
global country "SRB" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2015"  /*time*/
global inputFile "SRB_SWTS_2015.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_wgt=.
	    replace ilo_wgt= individual_weight
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(area_nov,2,3)         // Urban 
		replace ilo_geo=2 if area_nov==1         // Rural
...
* Comment:
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if (ever_attend==4 | highestlevel_comp==1) // No schooling
		*replace ilo_edu_isced11=2 if                        // Early childhood education
		...
		replace ilo_edu_isced11=5 if  inlist(highestlevel_comp,4,5)  // Upper secondary education
		...
		*replace ilo_edu_isced11=7 if                        // Short-cycle tertiary education
		...
		replace ilo_edu_isced11=10 if highestlevel_comp==9     // Doctoral or equivalent level
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  ever_attend==1                      // Attending
		replace ilo_edu_attendance=2 if  inlist(ever_attend,2,3)                      // Not attending
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual =hours_usual       if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual =hours_actual       if ilo_lfs==1
