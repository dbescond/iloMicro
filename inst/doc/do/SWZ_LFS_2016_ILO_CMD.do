* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Swaziland, 2016
* DATASET USED: Swaziland, Labour Force Survey, 2016
* NOTES: 
* Files created: Standard variables SWZ_LFS_2016_FULL.dta and SWZ_LFS_2016_ILO.dta
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if settlement==1		// Urban 
		replace ilo_geo=2 if settlement==2		// Rural
...
* ------------------------------------------------------------------------------
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if s1q4==1	// Male
		replace ilo_sex=2 if s1q4==2	// Female
...
* Comment: Highest level completed.
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if (s1q20==3 | s1q21==1)	// No schooling
		replace ilo_edu_isced11=2 if s1q21==2				// Early childhood education
		replace ilo_edu_isced11=3 if inrange(s1q21,3,9)		// Primary education
		replace ilo_edu_isced11=4 if inrange(s1q21,10,14)   // Lower secondary education
		replace ilo_edu_isced11=5 if s1q21==15				// Upper secondary education
		replace ilo_edu_isced11=6 if s1q21==16				// Post-secondary non-tertiary education
		* replace ilo_edu_isced11=7 if						// Short-cycle tertiary education
		replace ilo_edu_isced11=8 if inlist(s1q21,17,18)	// Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if s1q21==19				// Master's or equivalent level
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if s1q20==2				// Attending
		replace ilo_edu_attendance=2 if (s1q20==1 | s1q20==3)	// Not attending
...
* ------------------------------------------------------------------------------
    ...
    * Aggregate  	
	gen ilo_dsb_aggregate=.
	    replace ilo_dsb_aggregate=1 if (s1q1801==0 & s1q1802==0 & s1q1803==0 & s1q1804==0 & s1q1805==0 & s1q1806==0 & s1q1807==0 & s1q1808==0 & s1q1809==0 & s1q1810==0 & s1q1811==0 & s1q1812==0)	// Persons without disability
		replace ilo_dsb_aggregate=2 if (s1q1801==1 | s1q1802==1 | s1q1803==1 | s1q1804==1 | s1q1805==1 | s1q1806==1 | s1q1807==1 | s1q1808==1 | s1q1809==1 | s1q1810==1 | s1q1811==1 | s1q1812==1)	// Persons with disability
...
* Comment: Section 3 of the questionnaire
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (s3q1==1 | s3q2==1 | s3q3==1) & ilo_wap==1													// Employed: ILO definition
		replace ilo_lfs=1 if (s3q4==1 | s3q5==1 | s3q6==1) & (inlist(s3q7,1,2,3,4) | s3q8==1 | s3q9==1) & ilo_wap==1	// Employed: temporary absent
		replace ilo_lfs=2 if (s3q10==1 | s3q10b==1 | s3q10c==1) & (s3q14a==1 | s3q14b==1) & ilo_lfs!=1 & ilo_wap==1		// Unemployed: three criteria and available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (s4q1==2 & ilo_lfs==1)		// One job only     
		replace ilo_mjh=2 if (s4q1==1 & ilo_lfs==1)		// More than one job
...
* ------------------------------------------------------------------------------
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if (s4q5==1 & ilo_lfs==1)				// Employees
		 replace ilo_job1_ste_icse93=2 if (s4q5==2 & ilo_lfs==1)				// Employers
		 replace ilo_job1_ste_icse93=3 if (s4q5==3 & ilo_lfs==1)				// Own-account workers
		 replace ilo_job1_ste_icse93=4 if (s4q5==5 & ilo_lfs==1)				// Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if (s4q5==4 & ilo_lfs==1)				// Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if (inlist(s4q14,1,2) & ilo_lfs==1)	    // Public
...
* ------------------------------------------------------------------------------
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual=.
	    replace ilo_joball_how_usual=s4q17 if ilo_lfs==1
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual=.
	    replace ilo_joball_how_actual=s4q418 if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if (s4q11==2 & ilo_lfs==1)					// Permanent
		replace ilo_job1_job_contract=2 if (s4q11==1 & ilo_lfs==1)     				// Temporary
...
/* Useful questions:
		  - Status in employment: s4q5
		  - Institutional sector: s4q14
		  - Bookkeeping: s4q13c
		  - Registration: s4q12 / s4q13
		  - Social security contribution (Proxy: pension funds): s4q6
		  - Place of work: NA
		  - Size: s4q15
		  - Paid annual leave: s4q8a
		  - Paid sick leave: s4q8c
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if (s4q6==1 & ilo_lfs==1)  // Social security (proxy)
		replace social_security=2 if (s4q6!=1 & ilo_lfs==1)  // No social security (proxy)
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru=.
	    replace ilo_joball_tru=1 if (ilo_joball_how_usual<=35 & s4q23==1 & ilo_lfs==1)
...
* ------------------------------------------------------------------------------
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if (s3q12==1 & ilo_lfs==2)			// Less than 1 month
		replace ilo_dur_details=2 if (s3q12==2 & ilo_lfs==2)			// 1 to 3 months
		replace ilo_dur_details=3 if (s3q12==3 & ilo_lfs==2)			// 3 to 6 months
		replace ilo_dur_details=4 if (s3q12==4 & ilo_lfs==2)			// 6 to 12 months
		replace ilo_dur_details=5 if (s3q12==5 & ilo_lfs==2)			// 12 to 24 months
		replace ilo_dur_details=6 if (s3q12==56& ilo_lfs==2)			// 24 months or more
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if (s3q10==1 | s3q10b==1 | s3q10c==1) & (s3q14a!=1 & s3q14b!=1) & ilo_lfs==3	// Seeking, not available
		replace ilo_olf_dlma=2 if (s3q10!=1 & s3q10b!=1 & s3q10c!=1) & (s3q14a==1 | s3q14b==1) & ilo_lfs==3	// Not seeking, available
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if (inlist(s3q16,3,4) & ilo_lfs==3)        // Labour market 
			replace ilo_olf_reason=2 if (inlist(s3q16,1,2) & ilo_lfs==3)		// Other labour market reasons
			replace ilo_olf_reason=3 if (inlist(s3q16,5,6,7,9,10) & ilo_lfs==3)	// Personal/Family-related
			replace ilo_olf_reason=4 if (s3q16==8 & ilo_lfs==3)					// Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if (!inlist(ilo_lfs,1,2) & (s3q14a==1 | s3q14b==1) & ilo_olf_reason==1)
