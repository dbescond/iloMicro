...
* Comment: - Given the available classification only aggregate level is defined.
    ...
    * Aggregate
    gen ilo_edu_aggregate=.
		replace ilo_edu_aggregate=1 if education_new==0
...
*          job and who are available to work. It does not include future starters
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (b1_new==1 | b2_new==1) & ilo_wap==1				                    // Employed
		replace ilo_lfs=1 if (b8_new==1 | b9_new==1 | b10_new==1 | b11_new==1) & ilo_wap==1         // Employed: own-use production workers/volunteer work
		replace ilo_lfs=1 if (b3_new==1 & inlist(b4_new,1,2,3,4,5,6)) & ilo_wap==1				    // Employed: temporary absent (reasons)
		replace ilo_lfs=1 if (b3_new==1 & (b5_new==1 | b6_new==1 | b7_new==1)) & ilo_wap==1		    // Employed: temporary absent (further evaluation)
		replace ilo_lfs=2 if (f1_new==1 & f4_new==1) & ilo_lfs!=1 & ilo_wap==1			            // Unemployed: three criteria
...
* ------------------------------------------------------------------------------
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(c6_new,1,2,3,4) & ilo_lfs==1   // Employees
		 replace ilo_job1_ste_icse93=2 if c6_new==6 & ilo_lfs==1                // Employers
		 replace ilo_job1_ste_icse93=3 if c6_new==5 & ilo_lfs==1                // Own-account workers
		 replace ilo_job1_ste_icse93=4 if c6_new==7 & ilo_lfs==1                // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if c6_new==8 & ilo_lfs==1                // Contributing family workers
...
* Comment: - Public: government and parastatal sector.
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(c7_new,1,2) & ilo_lfs==1        // Public
...
* ------------------------------------------------------------------------------
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = e1_weektot if ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Not enough information to define it.
/* Useful questions:
          - Institutional sector: c7_new
		  - Private household identification: c7_new (option 7)
		  - Destination of production: not asked.
		  - Bookkeeping: not asked.
		  - Registration: not asked.
		  - Status in employment: not asked.
...
* ------------------------------------------------------------------------------ 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if f3_new<1 & ilo_lfs==2                      // Less than 1 month
		replace ilo_dur_details=2 if f3_new>=1 & f3_new<3 & ilo_lfs==2          // 1 to 3 months
		replace ilo_dur_details=3 if f3_new>=3 & f3_new<6 & ilo_lfs==2          // 3 to 6 months
		replace ilo_dur_details=4 if f3_new>=6 & f3_new<12 & ilo_lfs==2         // 6 to 12 months
		replace ilo_dur_details=5 if f3_new>=12 & f3_new<24 & ilo_lfs==2        // 12 to 24 months
		replace ilo_dur_details=6 if f3_new>=24 & ilo_lfs==2                    // 24 months or more
...
*            3 and 4 are not possible to be produced.
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if f1_new==1 & f4_new==2  & ilo_lfs==3         // Seeking, not available
		replace ilo_olf_dlma = 2 if f1_new==2 & f4_new==1 & ilo_lfs==3	        // Not seeking, available
