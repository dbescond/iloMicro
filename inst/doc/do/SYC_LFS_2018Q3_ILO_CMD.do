* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Seychelles, 2018Q3
* DATASET USED: Seychelles, LFS, 2018 Quarter 3
* NOTES: 
* Files created: Standard variables SYC_LFS_2018Q3_FULL.dta and SYC_LFS_2018Q3_ILO.dta
...
global source "LFS"
global time "2018Q3"
global inputFile "SYC_LFS_2018Q3.dta"
...
* Comment: Given the available classification only aggregate level is defined.

    destring b1 b2, replace force

    * Aggregate
    gen ilo_edu_aggregate=.
		replace ilo_edu_aggregate=1 if b1==0
		replace ilo_edu_aggregate=2 if b1==1
		replace ilo_edu_aggregate=3 if b1==2
		replace ilo_edu_aggregate=4 if inlist(b1,3,4,5,6,7) 
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if inlist(b2,1,2)							// Attending
		replace ilo_edu_attendance=2 if b2==3									// Not attending
...
* ------------------------------------------------------------------------------
    ...
    destring marital_status, replace force

	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if marital_status==1                            // Single
		replace ilo_mrts_details=2 if marital_status==2                            // Married
		replace ilo_mrts_details=3 if marital_status==3                            // Union / cohabiting
		replace ilo_mrts_details=4 if marital_status==6                            // Widowed
		replace ilo_mrts_details=5 if inlist(marital_status,4,5)                   // Divorced / separated
...
* ------------------------------------------------------------------------------
    ...
    destring a1 a2 a3 a4 a4 a5 a6, replace force

    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=1 if (a1==1 & a2==1 & a3==1 & a4==1 & a5==1 & a6==1)	// No, no difficulty
		replace ilo_dsb_details=2 if (a1==2 | a2==2 | a3==2 | a4==2 | a5==2 | a6==2)	// Yes, some difficulty
		replace ilo_dsb_details=3 if (a1==3 | a2==3 | a3==3 | a4==3 | a5==3 | a6==3)	// Yes, a lot of difficulty
		replace ilo_dsb_details=4 if (a1==4 | a2==4 | a3==4 | a4==4 | a5==4 | a6==4)	// Cannot do it at all
...
* Comment: Employment questions are based on variables C1 and C2. Unemployment questions are based on F1, F2, F3 and F4 (actively looking) and F11, F12 and F13 (available)

    destring c1a c1b c1c c2a c2b c2d f1 f2 f3 f4 f9 f10 f11 f12 f13, replace force
	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c1a==1 | c1b==1 | c1c==1) & ilo_wap==1													// Employed
		replace ilo_lfs=1 if (c2a==1 & (c2b!=2 | c2d==1)) & ilo_wap==1													// Employed: temporary absent (reasons)
		replace ilo_lfs=2 if (f1==1 | f2==1) & (f3!=3 | f4==1) & (f12==1 | f13==1) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (f9==1 & inlist(f10,1,2) & f11==1 & ilo_lfs!=1 & ilo_wap==1)								// Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (d1==2 & ilo_lfs==1)								// One job only     
		replace ilo_mjh=2 if (d1==1 & ilo_lfs==1)								// More than one job
...
* ------------------------------------------------------------------------------
    ...
    destring d4, replace force

   * MAIN JOB
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(d4,1,2,3,4) & ilo_lfs==1		// Employees
		 replace ilo_job1_ste_icse93=2 if d4==6 & ilo_lfs==1					// Employers
		 replace ilo_job1_ste_icse93=3 if d4==5 & ilo_lfs==1					// Own-account workers
		 replace ilo_job1_ste_icse93=4 if d4==7 & ilo_lfs==1					// Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if inlist(d4,8,9) & ilo_lfs==1			// Contributing family workers
...
* Comment: ISIC based on variable D12 has not yet been coded.
/*
    *---------------------------------------------------------------------------
	* ISIC REV 4
	*---------------------------------------------------------------------------
	
	gen econ_act_prim=int(isic/100) if c4_code!=9999
...
* Comment: ISCO based on variable D3 has not yet been coded.
/*
    *---------------------------------------------------------------------------
	* ISCO 08
	*---------------------------------------------------------------------------
	
	gen occu_prim = int(isco/100)
...
* Comment: Public: government and parastatal sector.

    destring d9, replace force
		
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(d9,1,2,5,6) & ilo_lfs==1        // Public
...
* ------------------------------------------------------------------------------		
    ...
    * MAIN JOB - Hours ACTUALLY worked
	gen ilo_job1_how_actual=.
	    replace ilo_job1_how_actual=e1a if ilo_lfs==1
    ...
    * SECOND JOB - Hours ACTUALLY worked
    gen ilo_job2_how_actual=.
	    replace ilo_job2_how_actual=e2b if ilo_mjh==2
    ...
    * ALL JOBS - Hours ACTUALLY worked
	gen ilo_joball_how_actual=.
		replace ilo_joball_how_actual=(e1a+e2b+e3c) if ilo_lfs==1 & ilo_mjh==2 & e3c!=.
		replace ilo_joball_how_actual=(e1a+e2b) if ilo_lfs==1 & ilo_mjh==2 & e3c==.
	    replace ilo_joball_how_actual=e1a if ilo_lfs==1 & ilo_mjh==1
...
* ------------------------------------------------------------------------------
    ...
    destring e4, replace force
	
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if e4==1 & ilo_lfs==1						// Full-time
		replace ilo_job1_job_time=1 if e4==2 & ilo_lfs==1						// Part-time
...
* ------------------------------------------------------------------------------
    ...
    destring d22, replace force

	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if (d22==1 & ilo_lfs==1)      				// Permanent
		replace ilo_job1_job_contract=2 if (inlist(d22,2,3,4,5) & ilo_lfs==1)		// Temporary
...
/* Useful questions:
          - Institutional sector: D9
		  - Destination of production: NA
		  - Bookkeeping: D25
		  - Registration: D23
		  - Status in employment: D4
		  - Social security contribution (Proxy: pension funds): D20
		  - Place of work: D6
		  - Size: D5
		  - Paid annual leave: D15
		  - Paid sick leave: D16
*/

    destring d20 d23 d25b_1 d25b_2 d25b_3 d25b_4, replace force

	* 1) Unit of production
	
		gen social_security=.
			replace social_security=1 if (d20==1 & ilo_lfs==1)
	
		gen ilo_job1_ife_prod=.
			replace ilo_job1_ife_prod=2 if (inlist(d9,1,2,5,6) | inlist(d23,1,2,3) | d25b_1==1 | d25b_2==1 | d25b_3==1 | d25b_4==1 | d20==1)
			replace ilo_job1_ife_prod=3 if (d9==7)
...
* Comment: - Threshold is set at 35 hours usually worked per week in main job.
    ...
    destring e4 e6 e7 e8, replace force

	gen ilo_joball_tru=.
	    replace ilo_joball_tru=1 if (ilo_job1_how_actual<=35 & e4==2 & e6==1 & e7==1 & e8!=0 & ilo_lfs==1)
...
* ------------------------------------------------------------------------------ 
    ...
    destring f6, replace force

	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if f6==1 & ilo_lfs==2							// Less than 1 month
		replace ilo_dur_details=2 if f6==2 & ilo_lfs==2							// 1 to 3 months
		replace ilo_dur_details=3 if f6==3 & ilo_lfs==2							// 3 to 6 months
		replace ilo_dur_details=4 if f6==4 & ilo_lfs==2							// 6 to 12 months
		replace ilo_dur_details=5 if f6==5 & ilo_lfs==2							// 12 to 24 months
		replace ilo_dur_details=6 if f6==6 & ilo_lfs==2							// 24 months or more
...
* ------------------------------------------------------------------------------
    ...
    destring f8, replace force

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma=1 if (f1==1 | f2==1) & (f3!=3 | f4==1) & f12!=1 & f13!=1 & ilo_lfs==3		// Seeking, not available
		replace ilo_olf_dlma=2 if f1==2 & f2==2 & (f12==1 | f13==1) & ilo_lfs==3	        			// Not seeking, available
		replace ilo_olf_dlma=3 if f1==2 & f2==2 & f8==1 & f12!=1 & f13!=1 & ilo_lfs==3					// Not seeking, not available, willing
		replace ilo_olf_dlma=4 if f1==2 & f2==2 & f8==2 & ilo_lfs==3									// Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
    ...
    destring f9 f14, replace force

	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if (inlist(f9,5,6,7) | inlist(f14,8,9)) & ilo_lfs==3			// Labour market 
		replace ilo_olf_reason=2 if (inlist(f9,1,2,3,4) | inlist(f14,1,2)) & ilo_lfs==3			// Other labour market reasons
		replace ilo_olf_reason=3 if (inlist(f9,8,8,10,11) | inlist(f14,3,4,5,7)) & ilo_lfs==3	// Personal/Family-related
		replace ilo_olf_reason=4 if (f9==12 | f14==6) & ilo_lfs==3								// Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & (f12==1 | f13==1) & ilo_olf_reason==1
