* TITLE OF DO FILE: ILO Microdata Preprocessing code template - TOGO, 2012
* DATASET USED: TOGO, SWTS, 2012
* NOTES: 
* Files created: Standard variables TGO_SWTS_2012_FULL.dta and TGO_SWTS_2012_ILO.dta
...
global source "SWTS"
global time "2012"
global inputFile "TGO_SWTS_2012.dta"
...
* ------------------------------------------------------------------------------ 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if urb_rur==1         // Urban 
		replace ilo_geo=2 if urb_rur==2         // Rural
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if c1==2  | c9==1                 // No schooling
		*replace ilo_edu_isced97=2 if                        		// Pre-primary education
		replace ilo_edu_isced97=3 if c9==2                       // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if c9==3                       // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if c9==4                       // Upper secondary education
		replace ilo_edu_isced97=6 if c9==5                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if c9==6                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if c9==7                       // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* ------------------------------------------------------------------------------
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if b3==1                                     // Single
		replace ilo_mrts_details=2 if inlist(b3,4,3)                            // Married
		replace ilo_mrts_details=3 if b3==2                                     // Union / cohabiting
		*replace ilo_mrts_details=4 if                                          // Widowed
		replace ilo_mrts_details=5 if b3==5                                     // Divorced / separated
...
* ------------------------------------------------------------------------------
    ...
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (b13==1 & b14==1 & b15==1 & b16==1 )                            
             replace ilo_dsb_details=2 if (b13==2 | b14==2 | b15==2 | b16==2 )                                                 
             replace ilo_dsb_details=3 if (b13==3 | b14==3 | b15==3 | b16==3 )                                                 
             replace ilo_dsb_details=4 if (b13==4 | b14==4 | b15==4 | b16==4 )
...
* ------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (c32_1==1 | c32_2==1 | c32_3==1 | c32_4==1 | ///
		c32_5==1 | c32_6==1 |c32_7==1 ) & ilo_wap==1				   					// Employed: ILO definition
		replace ilo_lfs=1 if c33==1 & !inlist(c34,13) & ilo_wap==1						// Employed: temporary absent
...
* ------------------------------------------------------------------------------
   ...
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if d5==1  & ilo_lfs==1          // Employees
		 replace ilo_job1_ste_icse93=2 if d5==2 & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if d5==3 & ilo_lfs==1          // Own-account workers
		 replace ilo_job1_ste_icse93=4 if d5==4 & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if d5==5 & ilo_lfs==1          // Contributing family workers
...
* ------------------------------------------------------------------------------
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = d36      if ilo_lfs==1
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if d7==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if d7==2 & ilo_lfs==1               				// Temporary
...
/* Useful questions:
		  ...
		  - Social security contribution (Proxy: pension funds):d13_10
		  - Place of work:
		  - Size:
		  - Paid annual leave:d13_03
		  - Paid sick leave:d13_04
*/
    * Social Security
	gen social_security=.
	    replace social_security=1 if d13_10==1 & ilo_lfs==1          // social security (proxy)
		replace social_security=2 if d13_10==2 & ilo_lfs==1          // no social security (proxy)
    ...
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   d3a==1 | ///
									   (inlist(d3a,4) & ilo_job1_ste_icse93==1 & social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (ilo_job1_ste_icse93==1 & d13_03==1 & d13_04==1) | ///
...
* ------------------------------------------------------------------------------
	...
	* Employees
	gen ilo_job1_lri_ees = .
		...
		replace ilo_job1_lri_ees = d15 + d18 if ilo_job1_ste_aggregate==1 & d15>0 & d16==2 & d18>0
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(5*52/12) if d20==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(52/12) if d20==2
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(26/12) if d20==3
	    replace ilo_job1_lri_ees = .     if ilo_lfs!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = d30 if ilo_job1_ste_aggregate==2 & d30>0
		 replace ilo_job1_lri_slf = d30 + d32 if ilo_job1_ste_aggregate==2 & d30>0 & d31==1 & d32>0
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru=.
	    replace ilo_joball_tru = 1 if ilo_job1_how_actual<= 35 & d37==1  & d38>0  & ilo_lfs==1
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(e5,7,8,9,10,11)           & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(e5,1,2)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(e5,3,4,5,6)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if e14==6 & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if (inlist(e5,12) | ilo_olf_reason==.) & ilo_lfs==3     // Not elsewhere classified
