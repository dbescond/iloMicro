...
** Comment: there is no ISCED mapping for TTO
		...
		gen ilo_edu_aggregate=.
			replace ilo_edu_aggregate=1 if inlist(reduc,1,2)
			replace ilo_edu_aggregate=2 if inrange(reduc,3,5)
			replace ilo_edu_aggregate=3 if inrange(reduc,6,10)
			replace ilo_edu_aggregate=4 if inrange(reduc,11,13)
			replace ilo_edu_aggregate=5 if inlist(reduc,14,16)
...
** Comment:  ISCO-88 classification used
	...
	* Two-digits level
		...
		* Main job
		...
		gen ilo_job1_ocu_isco88 = roccup if ilo_lfs==1
			replace ilo_job1_ocu_isco88=11 if inlist(roccup,88,99) & ilo_lfs==1
		...
		* Second job
			...
			** Despite the fact the variable p37 is given at 4-digit level, the ILO variable is only possible to compute at 1-digit level
			** because some second or third digits don't correspond to the international classification
			
			gen occ_code_sec_1dig=int(p37/1000) if ilo_mjh==2
 
		
		gen ilo_job2_ocu_isco88=occ_code_sec_1dig
			replace ilo_job2_ocu_isco88=10 if ilo_job2_ocu_isco88==0 & ilo_mjh==2
			replace ilo_job2_ocu_isco88=11 if ilo_job2_ocu_isco88==. & ilo_mjh==2
