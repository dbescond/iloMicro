* TITLE OF DO FILE: ILO Microdata Preprocessing code template - TUNISIA, 2013
* DATASET USED: TUNISIA, SWTS, 2013
* NOTES: 
* Files created: Standard variables TUN_SWTS_2013_FULL.dta and TUN_SWTS_2013_ILO.dta
...
global country "TUN" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2013"  /*time*/
global inputFile "TUN_SWTS_2013.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if milieu_men==1         // Urban 
		replace ilo_geo=2 if milieu_men==2         // Rural
...
* Comment:
    ...
    * Detailed				
    gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if  highestlevel_comp==1   | ever_attend==2                   // No schooling
		replace ilo_edu_isced11=2 if  highestlevel_comp==2                      // Early childhood education
		replace ilo_edu_isced11=3 if  highestlevel_comp==3                      // Primary education
		replace ilo_edu_isced11=4 if  highestlevel_comp==4                      // Lower secondary education
		replace ilo_edu_isced11=5 if  highestlevel_comp==5                      // Upper secondary education
		replace ilo_edu_isced11=6 if  highestlevel_comp==6                      // Post-secondary non-tertiary education
		replace ilo_edu_isced11=7 if  highestlevel_comp==7                      // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if  highestlevel_comp==8                      // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if  highestlevel_comp==9                      // Master's or equivalent level
		replace ilo_edu_isced11=10 if highestlevel_comp==10                      // Doctoral or equivalent level
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (emp_lastw_indep==1 | emp_lastw_sal==1 | emp_lastw_dom==1 | emp_lastw_contrmen==1 | ///
		emp_lastw_travparc==1 | emp_lastw_constrep==1 |emp_lastw_attr==1 ) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if emp_temp_absent==1 & ilo_wap==1										// Employed: temporary absent
		replace ilo_lfs=2 if inlist(seekingjob,1,2) & disponw==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (foundjob==1 | startbusiness==1) & disponw==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
