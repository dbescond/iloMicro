* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Tanzania, United Republic of, 2013
* DATASET USED: Tanzania, SWTS, United Republic of, 2013
* NOTES: 
* Files created: Standard variables TZA_SWTS_2013_FULL.dta and TZA_SWTS_2013_ILO.dta
...
global country "TZA" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2013"  /*time*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if residence==2         // Urban 
		replace ilo_geo=2 if residence==1         // Rural
...
* Comment:
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  (qc12highestlevel_comp==1 |  qc1ever_attend==2)                     // No schooling
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		replace ilo_edu_isced97=3 if qc12highestlevel_comp==2                       // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if qc12highestlevel_comp==3                       // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if qc12highestlevel_comp==4                       // Upper secondary education
		replace ilo_edu_isced97=6 if qc12highestlevel_comp==5                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if qc12highestlevel_comp==6                       // First stage of tertiary education (not leading directly to an advanced research qualification)
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  qc2currently_attend==1                      // Attending
		replace ilo_edu_attendance=2 if  inlist(qc2currently_attend,2,3)                      // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if  qb10marital==1                                         // Single
		replace ilo_mrts_details=2 if  qb10marital==3                                         // Married
		replace ilo_mrts_details=3 if  qb10marital==2                                         // Union / cohabiting
		replace ilo_mrts_details=4 if  qb10marital==5                                         // Widowed
		replace ilo_mrts_details=5 if  qb10marital==4                                         // Divorced / separated
...
* Comment: 
    ...
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (qb18diff_seeing==1 & qb19diff_hearing==1 & qb20diff_walking==1 & qb21diff_remembering==1 )                            
             replace ilo_dsb_details=2 if (qb18diff_seeing==2 | qb19diff_hearing==2 | qb20diff_walking==2 | qb21diff_remembering==2 )                                                 
             replace ilo_dsb_details=3 if (qb18diff_seeing==3 | qb19diff_hearing==3 | qb20diff_walking==3 | qb21diff_remembering==3 )                                                 
             replace ilo_dsb_details=4 if (qb18diff_seeing==4 | qb19diff_hearing==4 | qb20diff_walking==4 | qb21diff_remembering==4 )
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (qc32emp_lastw1==1 | qc32emp_lastw2==1 | qc32emp_lastw3==1 | qc32emp_lastw4==1 | ///
		qc32emp_lastw5==1 | qc32emp_lastw6==1 |qc32emp_lastw7==1 ) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if qc33emp_temp_absent==1 & !inlist(qc34reason_absent,13) & ilo_wap==1										// Employed: temporary absent
		replace ilo_lfs=2 if inlist(seekingjob,1,2) & availability==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (qd3afoun==1 | qe3bstar==1) & availability==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: available future starters
...
* Comment: 
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = qd4isic_occ_main     if ilo_lfs==1
...
* Comment:
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = qd2isco_occ_main    if ilo_lfs==1
...
* Comment: 
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = qd33hour       if ilo_lfs==1 & qd33hour<160
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if qd9duration_contract==1 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if qd9duration_contract==2 & ilo_lfs==1               				// Temporary
...
/* Useful questions:
		  ...
		  - Bookkeeping:
		  - Registration:qd5register_actv
		  - Status in employment: ilo_job1_ste_icse93
		  - Social security contribution (Proxy: pension funds): qd13benefit_serv_j
		  - Place of work:
		  - Size:
		  - Paid annual leave: qd13benefit_serv_c
		  - Paid sick leave: qd13benefit_serv_d
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if qd13benefit_serv_j==1 &    ilo_lfs==1          // social security (proxy)
		replace social_security=2 if qd13benefit_serv_j==2  &  ilo_lfs==1          // no social security (proxy)
	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR
	gen ilo_job1_ife_prod=.
	    *replace ilo_job1_ife_prod=3 if ilo_lfs==1 & (inlist(ilo_job1_eco_isic4_2digits,97) |  ilo_job1_ocu_isco08_2digits==63)
																   
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               (publicadmin==1 | ///
									   qd5register_actv==1 | ///
									   (inlist(qd5register_actv,4) & ilo_job1_ste_icse93==1 & social_security==1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	...
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (ilo_job1_ste_icse93==1 & qd13benefit_serv_c==1 & qd13benefit_serv_d==1) | ///
...
* Comment: 
	...
	* Employees
	gen ilo_job1_lri_ees = .
	    replace ilo_job1_lri_ees = qd14total_amount_wage if ilo_job1_ste_aggregate==1 & qd14total_amount_wage>0
		replace ilo_job1_lri_ees = qd14total_amount_wage + qd16deduction_amount if ilo_job1_ste_aggregate==1 & qd14total_amount_wage>0 & qd15deduction==2 & qd16deduction_amount>0
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(5*52/12) if qd18payment_period==1
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(52/12) if qd18payment_period==2
		replace ilo_job1_lri_ees = ilo_job1_lri_ees *(26/12) if qd18payment_period==3
	...
	* Self-employed
	gen ilo_job1_lri_slf = .
	    replace ilo_job1_lri_slf = qd27amount_profit_selfemp  
		replace ilo_job1_lri_slf = ilo_job1_lri_slf + qd29amount_prodhh_selfemp if ilo_job1_ste_aggregate==2 & qd28prodhh_selfemp==1 & qd29amount_prodhh_selfemp>0 
...
* Comment:
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_job1_how_actual<=35  & qd34like==1  & qd35hour>0 & ilo_lfs==1
...
* Comment: 
	...
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if inlist(qe12leng,1,2) & ilo_lfs==2                 // Less than 1 month
		replace ilo_dur_details=2 if qe12leng==3      & ilo_lfs==2                 // 1 to 3 months ********[C7:3905]
		replace ilo_dur_details=3 if qe12leng==4      & ilo_lfs==2                 // 3 to 6 months
		replace ilo_dur_details=4 if qe12leng==5      & ilo_lfs==2                 // 6 to 12 months
		replace ilo_dur_details=5 if qe12leng==6      & ilo_lfs==2                 // 12 to 24 months
		replace ilo_dur_details=6 if qe12leng==7      & ilo_lfs==2                 // 24 months or more
...
* Comment: 
	...
	gen ilo_olf_dlma=.
		...
		replace ilo_olf_dlma = 3 if seekingjob==3  & availability==2 &	qe4want==1 & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if seekingjob==3  & availability==2 &	qe4want==2 & ilo_lfs==3      // Not seeking, not available, not willing
...
* Comment: 
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(qe5reaso,7,8,9,10,11)           & ilo_lfs==3             // Labour market 
			replace ilo_olf_reason=2 if inlist(qe5reaso,1,2)          & ilo_lfs==3             // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(qe5reaso,3,4,5,6)          & ilo_lfs==3             // Personal/Family-related
			replace ilo_olf_reason=4 if qe14reas==6 & ilo_lfs==3             // Does not need/want to work
			replace ilo_olf_reason=5 if (inlist(qe5reaso,99) | ilo_olf_reason==.) & ilo_lfs==3     // Not elsewhere classified
