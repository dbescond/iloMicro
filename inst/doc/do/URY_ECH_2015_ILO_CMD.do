* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Uruguay, 2017
* DATASET USED: Uruguay ECH 2017
* NOTES: 
* Files created: Standard variables URY_ECH_2017_FULL.dta and URY_ECH_2017_ILO.dta
...
global source "ECH"
global time "2015"
global inputFile "Personas_2015_Terceros.dta"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Urban includes Montevideo, urban regions with 5000 inhabitants and 
*            urban regions with less than 5000 inhabitants. 
	
	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(region_4,1,2,3)          // Urban 
		replace ilo_geo=2 if region_4==4                     // Rural
...
* Comment: 
	...
	gen ilo_sex=.
	    replace ilo_sex=1 if e26==1            // Male
		replace ilo_sex=2 if e26==2            // Female
...
*          - The highest level finished is considered. All educational levels 
*            are asked using different questions and therefore all those variables
*            that have information on the educational attendance or its 
*            finalization are used.
	...
	*---------------------------------------------------------------------------
	...
	gen ilo_edu_isced97=.
	    replace ilo_edu_isced97=1 if e49==2                                                                                                                                         // No schooling
		replace ilo_edu_isced97=3 if e197_1==1 & inlist(e201_1,0,2) & inlist(e212_1,0,2) & inlist(e215_1,0,2) & inlist(e218_1,0,2) & inlist(e221_1,0,2) & inlist(e224_1,0,2)        // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if e201_1==1 & inlist(e212_1,0,2) & inlist(e215_1,0,2) & inlist(e218_1,0,2) & inlist(e221_1,0,2) & inlist(e224_1,0,2)                             // Lower secondary or second stage of basic education
		*replace ilo_edu_isced97=5 if                                                                                                                                               // Upper secondary education
		replace ilo_edu_isced97=6 if e212_1==1 & inlist(e215_1,0,2) & inlist(e218_1,0,2) & inlist(e221_1,0,2) & inlist(e224_1,0,2)                                                  // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if (e215_1==1 | e218_1==1 | e221_1==1) & inlist(e224_1,0,2)                                                                                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if e224_1==1                                                                                                                                      // Second stage of tertiary education (leading to an advanced research qualification)
		replace ilo_edu_isced97=2 if ilo_edu_isced97==. & e49==1                                                                                                                    // Pre-primary education
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - The highest level finished is considered. All educational levels 
*            are asked using different questions and therefore all those variables
*            that have information on the educational attendance or its 
*            finalization are used.

	gen ilo_edu_attendance=.
	    replace ilo_edu_attendance=1 if e238==1		                            // Attending: Educación inicial
		replace ilo_edu_attendance=1 if e193==1		                            // Attending: Educación preescolar
		replace ilo_edu_attendance=1 if e197==1		                            // Attending: Educación primaria
		replace ilo_edu_attendance=1 if e201==1		                            // Attending: Educación media
		replace ilo_edu_attendance=1 if e215==1		                            // Attending: Magistario o profesorado
		replace ilo_edu_attendance=1 if e218==1		                            // Attending: Ensenanza universitaria o similar
		replace ilo_edu_attendance=1 if e221==1		                            // Attending: Ensenanza terciaria no universitaria
		replace ilo_edu_attendance=1 if e224==1		                            // Attending: Posgrado
		replace ilo_edu_attendance=1 if e212==1		                            // Attending: Esenanza técnica
...
/*	
 ...
 if to_drop < "2016" {	
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if e36==6                                    // Single
		replace ilo_mrts_details=2 if e35==1                                    // Married
		replace ilo_mrts_details=3 if inlist(e35,2,3)                           // Union / cohabiting
		replace ilo_mrts_details=4 if inlist(e36,4,5)                           // Widowed
		replace ilo_mrts_details=5 if inrange(e36,1,3)                          // Divorced / separated
...
if to_drop >= "2016" {	
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if e36==6                                    // Single
		replace ilo_mrts_details=2 if inlist(e35,4,5)                           // Married
		replace ilo_mrts_details=3 if inlist(e35,2,3,6,7)                       // Union / cohabiting
		replace ilo_mrts_details=4 if inlist(e36,4,5)                           // Widowed
		replace ilo_mrts_details=5 if inrange(e36,1,3)                          // Divorced / separated
		replace ilo_mrts_details=6 if ilo_mrts_details==.			            // Not elsewhere classified
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment:  - Employment differs from national estimates due to the age coverage
*            (the national statistical office includes 14+).
*           - Unemployment includes those who were not in employment, available 
*             and seeking during the past week or during the past four weeks; 
*             it also includes available future  starters.
		
	gen ilo_lfs=.
		replace ilo_lfs=1 if (f66==1 | (f67==1 & inlist(f261,1,3)) | f68==1) & ilo_wap==1   // Employed/Employed temporary absent
...
* Comment: 
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (f70==1) & ilo_lfs==1             // One job only     
		replace ilo_mjh=2 if (f70>=2) & ilo_lfs==1             // More than one job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Social programmes' workers are classified as employees (main and 
*            second job). 
   ...
   * ICSE 1993
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  ...
		  replace ilo_job2_ste_icse93=4 if f92==3 & ilo_mjh==2                  // Members of producers' cooperatives 
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification is based on ISIC Rev.4 (4 digit-level available).
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = int(f72_2/100) if ilo_lfs==1
   ...
   * Aggregate level
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = int(f91_2/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification is based on ISCO-08 (4 digit-level available).
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = int(f71_2/100) if ilo_lfs==1
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring f90_2, replace force
	
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = int(f90_2/100) if ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Public: Public employees. Private: the rest. 
	...
	* SECOND JOB:	
	gen ilo_job2_ins_sector=.
		replace ilo_job2_ins_sector=1 if f73==2 & ilo_mjh==2                    // Public
...
/* Useful questions:
		  ...
		  - Bookkeeping: f262 (only asked to own-account workers)
		  - Registration: f263 (only asked to own-account workers)
		  - Status in employment: ilo_job1_ste_aggregate
		  - Social security contribution (Proxy: pension funds): f82 (contribution to pension funds)
		  - Place of work: f78
		  - Size: f77
		  - Paid annual leave: f267
		  - Paid sick leave: f268
*/ 
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR		
			...
			gen ilo_job1_ife_prod=.
				...
				replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
											   ...
											   (f73!=2 & f262==1) | ///
											   (f73!=2 & f262!=1 & f263==1) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate==1 & f82==1) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate==1 & f82!=1 & f78==1 & inlist(f77,3,4,5,6,7)) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate!=1 & f78==1 & inlist(f77,3,4,5,6,7)))
  			    replace ilo_job1_ife_prod=1 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
				                               ((f73!=2 & f262!=1 & f263==2) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate==1 & f82!=1 & f78==1 & inlist(f77,0,1,2)) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate==1 & f82!=1 & f78!=1) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate!=1 & f78==1 & inlist(f77,0,1,2)) | ///
											   (f73!=2 & f262!=1 & f263==0 & ilo_job1_ste_aggregate!=1 & f78!=1))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT 
	      ...
	      gen ilo_job1_ife_nature=.
		      replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
											  ...
											  (inlist(ilo_job1_ste_icse93,1,6) & f82==0 & f267==1 & f268==1) | ///
			  ...
			  replace ilo_job1_ife_nature=1 if ilo_lfs==1 & ///
			                                  ((inlist(ilo_job1_ste_icse93,1,6) & f82==2) | ///
											  (inlist(ilo_job1_ste_icse93,1,6) & f82==0 & inlist(f267,0,2)) | ///
											  (inlist(ilo_job1_ste_icse93,1,6) & f82==0 & f267==1 & inlist(f268,0,2)) | ///
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = f85 if ilo_lfs==1
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual = .
	    replace ilo_job2_how_usual = f98 if ilo_mjh==2
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if f116==1 & ilo_lfs==2                           // Previously employed       
		replace ilo_cat_une=2 if f116==2 & ilo_lfs==2                           // Seeking for the first time
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Original question refers to the number of weeks seeking for a job.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - National classification is based on ISIC Rev.4 (4 digit-level are available).

    *---------------------------------------------------------------------------
	* ISIC REV 4
	*---------------------------------------------------------------------------
	destring f120_2, replace force
	
	* 2-digit level
	gen ilo_preveco_isic4_2digits = . 
	    replace ilo_preveco_isic4_2digits = int(f120_2/100) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
* Comment: - National classification is based on ISCO-08 (4 digit-level available).
	...
	* ISCO 08
	*---------------------------------------------------------------------------
	destring f119_2, replace force
	
	* 2-digit level 
    gen ilo_prevocu_isco08_2digits = . 
	    replace ilo_prevocu_isco08_2digits = int(f119_2/100) if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
...
*           - Two questions related to looking/seeking for a job: during the past
*             week or during the past 4 weeks.
*           - Not available includes: he/she was available but not recently or 
*             not available.
