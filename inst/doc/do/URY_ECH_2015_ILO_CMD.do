* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Uruguay, 2015
* DATASET USED: Uruguay LFS 2015
* NOTES: 
...
global source "ECH"
global time "2015"
global inputFile "Personas_2015_Terceros"
...
*            educational attendance or its finalization are used.
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - The highest level finished is considered. All educational levels are asked using 
*            different questions and therefore all those variables that have information on the 
*            educational attendance or its finalization are used.
...
*             starters.
	...
	gen ilo_lfs=.
		replace ilo_lfs=1 if (f66==1 | (f67==1 & inlist(f261,1,3)) | f68==1) & ilo_wap==1   // Employed
...
* Comment: - National classification is based on ISIC Rev.4 (4 digit-level available).
  ...
  * ISIC Rev. 4 - 2 digit-level
  ...
  gen ilo_job1_eco_isic4_2digits = int(f72_2/100) if ilo_lfs==1
  ...
  * ISIC Rev. 4 - 2 digit-level
  ...
  gen ilo_job2_eco_isic4_2digits = int(f91_2/100) if ilo_mjh==2
...
* Comment: - National classification is based on ISCO-08 (4 digit-level available).
    ...
    * MAIN JOB	   
    * ISCO-08 2 digits-level
    destring f71_2, replace
   
    gen ilo_job1_ocu_isco08_2digits = int(f71_2/100) if ilo_lfs==1
	    lab val ilo_job1_ocu_isco08_2digits isco08_2digits
    ...
    * ISCO-08 2 digits-level
    ...
    gen ilo_job2_ocu_isco08_2digits = int(f90_2/100) if ilo_lfs==1
	    lab val ilo_job2_ocu_isco08_2digits isco08_2digits
	    lab var ilo_job2_ocu_isco08_2digits "Occupation (ISCO-08), 2 digit level in second job"
		
    * One digit-level
	gen ilo_job2_ocu_isco08=.
	    replace ilo_job2_ocu_isco08=int(ilo_job2_ocu_isco08_2digits/10) if ilo_lfs==1     // The rest of the occupations
		replace ilo_job2_ocu_isco08=10 if (ilo_job2_ocu_isco08==0 & ilo_lfs==1)           // Armed forces
				lab val ilo_job2_ocu_isco08 isco08
				lab var ilo_job2_ocu_isco08 "Occupation (ISCO-08) in second job"
	...
	* Aggregate:			
    gen ilo_job2_ocu_aggregate=.
	    ...
	    replace ilo_job2_ocu_aggregate=7 if ilo_job2_ocu_isco08==11
			    ...
			    lab var ilo_job2_ocu_aggregate "Occupation (Aggregate) in second job"	
	...
	* Skill level
	gen ilo_job2_ocu_skill=.
		...
		replace ilo_job2_ocu_skill=4 if inlist(ilo_job2_ocu_isco08,10,11)        // Not elsewhere classified
			    ...
			    lab var ilo_job2_ocu_skill "Occupation (Skill level) in second job"
...
/*	Useful questions:
			...
			- Households identification: ilo_job1_eco_isic4_2digits==97 / ilo_job1_ocu_isco08_2digits==63 (to check: around 7% when using weights)
...
* Comment: - National classification is based on ISIC Rev.4 (4 digit-level are available).
  ...
  * ISIC Rev. 4 - 2 digit-level
  ...
  gen ilo_preveco_isic4_2digits = int(f120_2/100) if ilo_lfs==2 & ilo_cat_une==1
...
* Comment: - National classification is based on ISCO-08 (4 digit-level available).
    ...
    * ISCO-08 2 digits-level
    ...
    gen ilo_prevocu_isco08_2digits = int(f119_2/100) if ilo_lfs==2 & ilo_cat_une==1
	    lab val ilo_prevocu_isco08_2digits isco08_2digits
