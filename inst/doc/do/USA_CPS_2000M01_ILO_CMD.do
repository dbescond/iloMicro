...
global source "CPS"
global time "2000M01"
global inputFile "jan00pub_2000b.dta"
...
* PWCMPWGT Composited Final Weight.  Used to create BLS's published labor force statistics (4 implied decimal places)
	...
	destring NWCMPWGT, replace
   gen ilo_wgt=.
       replace ilo_wgt = NWCMPWGT 
...
* PRCOW1 CLASS OF WORKER RECODE - JOB 1
	...
	gen ilo_ste = PRCOW1 // only 200-2002 else PRCOW1

	* Detailed categories:
		gen ilo_job1_ste_icse93=.
			replace ilo_job1_ste_icse93=1 if (inlist(ilo_ste, 1, 2, 3, 4) & ilo_lfs==1)   		    // Employees
			* replace ilo_job1_ste_icse93=2 if (ilo_ste==1 & ilo_lfs==1)	            // Employers
			replace ilo_job1_ste_icse93=3 if (ilo_ste== 5 & ilo_lfs==1)      			// Own-account workers
			* replace ilo_job1_ste_icse93=4                                         // Members of producers’ cooperatives
			replace ilo_job1_ste_icse93=5 if (ilo_ste==6 & ilo_lfs==1)	            // Contributing family workers
	...
	* Aggregate categories 
		gen ilo_job1_ste_aggregate=.
			replace ilo_job1_ste_aggregate=1 if (inlist(ilo_ste, 1, 2, 3, 4) & ilo_lfs==1)			// Employees
			replace ilo_job1_ste_aggregate=2 if (inlist(ilo_ste,5,6) & ilo_lfs==1)	// Self-employed
			replace ilo_job1_ste_aggregate=3 if (ilo_ste==. & ilo_lfs==1)			// Not elsewhere classified
...
* PEIO1OCD OCCUPATION CODE FOR PRIMARY JOB.
	...
	gen census = PEIO1OCD * 10   // only 2000 - 2002 else PEIO1OCD 
	destring census, replace
	replace census = census * 10   
...
capture replace ilo_job1_ocu_isco08_2digits = 52 if census == 20                            // 11 / 13 / 14 / 52 - Sales workers
capture replace ilo_job1_ocu_isco08_2digits = 11 if census == 30                            // 11 - Chief executives, senior officials and legislators
capture replace ilo_job1_ocu_isco08_2digits = 12 if census == 60                            // 11 / 12 - Administrative and commercial managers
capture replace ilo_job1_ocu_isco08_2digits = 11 if census == 425                            // 11 - Chief executives, senior officials and legislators
capture replace ilo_job1_ocu_isco08_2digits = 14 if census == 430                            // 11 / 12 / 13 / 14 - Hospitality, retail and other services managers
...
capture replace ilo_job1_ocu_isco08_2digits = 13 if census == 120                            // 12 / 13 - Production and specialised services managers
...
capture replace ilo_job1_ocu_isco08_2digits = 71 if census == 220                            // 13 / 71 - Building and related trades workers, excluding electricians
...
capture replace ilo_job1_ocu_isco08_2digits = 32 if census == 3260                            // 22 / 32 - Health associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 24 if census == 650                            // 23 / 24 - Business and administration professionals
...
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 620                            // 24 / 33 - Business and administration associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 630                            // 24 / 33 - Business and administration associate professionals
...
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 730                            // 24 / 33 - Business and administration associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 24 if census == 735                            // 24 - Business and administration professionals
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 740                            // 24 / 33 - Business and administration associate professionals
...
capture replace ilo_job1_ocu_isco08_2digits = 26 if census == 2850                            // 24 / 26 - Legal, social and cultural professionals
capture replace ilo_job1_ocu_isco08_2digits = 42 if census == 4710                            // 24 / 33 / 42 - Customer services clerks
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 4850                            // 24 / 33 - Business and administration associate professionals
...
capture replace ilo_job1_ocu_isco08_2digits = 35 if census == 1030                            // 25 / 35 - Information and communications technicians
capture replace ilo_job1_ocu_isco08_2digits = 25 if census == 1060                            // 25 - Information and communications technology professionals
capture replace ilo_job1_ocu_isco08_2digits = 35 if census == 1100                            // 25 / 35 - Information and communications technicians
capture replace ilo_job1_ocu_isco08_2digits = 35 if census == 1105                            // 25 / 35 - Information and communications technicians
...
capture replace ilo_job1_ocu_isco08_2digits = 51 if census == 2760                            // 26 / 34 / 51 - Personal service workers
...
capture replace ilo_job1_ocu_isco08_2digits = 54 if census == 3720                            // 31 / 54 - Protective services workers
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8040                            // 31 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8630                            // 31 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 53 if census == 3640                            // 32 / 53 - Personal care workers
capture replace ilo_job1_ocu_isco08_2digits = 32 if census == 3645                            // 32 - Health associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 51 if census == 3648                            // 32 / 51 - Personal service workers
capture replace ilo_job1_ocu_isco08_2digits = 32 if census == 3650                            // 32 - Health associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 53 if census == 3655                            // 32 / 53 - Personal care workers
capture replace ilo_job1_ocu_isco08_2digits = 75 if census == 6010                            // 32 / 33 / 75 - Food processing, wood working, garment and other craft and related trades workers
...
capture replace ilo_job1_ocu_isco08_2digits = 54 if census == 3710                            // 33 / 54 - Protective services workers
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 3820                            // 33 - Business and administration associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 54 if census == 3850                            // 33 / 54 - Protective services workers
...
capture replace ilo_job1_ocu_isco08_2digits = 42 if census == 4830                            // 33 / 42 - Customer services clerks
...
capture replace ilo_job1_ocu_isco08_2digits = 52 if census == 4960                            // 33 / 42 / 52 - Sales workers
capture replace ilo_job1_ocu_isco08_2digits = 52 if census == 4965                            // 33 / 42 / 52 - Sales workers
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 5000                            // 33 - Business and administration associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 43 if census == 5120                            // 33 / 43 - Numerical and material recording clerks
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 5220                            // 33 - Business and administration associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 42 if census == 5250                            // 33 / 42 - Customer services clerks
capture replace ilo_job1_ocu_isco08_2digits = 33 if census == 5500                            // 33 - Business and administration associate professionals
capture replace ilo_job1_ocu_isco08_2digits = 43 if census == 5610                            // 33 / 43 - Numerical and material recording clerks
...
capture replace ilo_job1_ocu_isco08_2digits = 44 if census == 2440                            // 34 / 44 - Other clerical support workers
...
capture replace ilo_job1_ocu_isco08_2digits = 35 if census == 2960                            // 34 / 35 - Information and communications technicians
...
capture replace ilo_job1_ocu_isco08_2digits = 54 if census == 3920                            // 34 / 54 - Protective services workers
...
capture replace ilo_job1_ocu_isco08_2digits = 51 if census == 4010                            // 34 / 51 - Personal service workers
capture replace ilo_job1_ocu_isco08_2digits = 53 if census == 4320                            // 34 / 44 / 51 / 53 - Personal care workers
...
capture replace ilo_job1_ocu_isco08_2digits = 43 if census == 5165                            // 42 / 43 - Numerical and material recording clerks
capture replace ilo_job1_ocu_isco08_2digits = 44 if census == 5210                            // 42 / 44 - Other clerical support workers
...
capture replace ilo_job1_ocu_isco08_2digits = 44 if census == 5420                            // 42 / 44 - Other clerical support workers
capture replace ilo_job1_ocu_isco08_2digits = 44 if census == 5540                            // 42 / 44 - Other clerical support workers
...
capture replace ilo_job1_ocu_isco08_2digits = 93 if census == 5620                            // 43 / 93 - Labourers in mining, construction, manufacturing and transport
...
capture replace ilo_job1_ocu_isco08_2digits = 53 if census == 4610                            // 44 / 51 / 53 - Personal care workers
...
capture replace ilo_job1_ocu_isco08_2digits = 96 if census == 5510                            // 44 / 83 / 93 / 96 - Refuse workers and other elementary workers
...
capture replace ilo_job1_ocu_isco08_2digits = 52 if census == 4120                            // 51 / 52 - Sales workers
...
capture replace ilo_job1_ocu_isco08_2digits = 91 if census == 4220                            // 51 / 91 - Cleaners and helpers
...
capture replace ilo_job1_ocu_isco08_2digits = 95 if census == 4650                            // 51 / 95 - Street and related sales and service workers
...
capture replace ilo_job1_ocu_isco08_2digits = 94 if census == 4130                            // 52 / 94 - Food preparation assistants
...
capture replace ilo_job1_ocu_isco08_2digits = 95 if census == 4950                            // 52 / 95 - Street and related sales and service workers
...
capture replace ilo_job1_ocu_isco08_2digits = 75 if census == 4210                            // 61 / 75 - Food processing, wood working, garment and other craft and related trades workers
capture replace ilo_job1_ocu_isco08_2digits = 62 if census == 6000                            // 61 / 62 - Market-oriented skilled forestry, fishery and hunting workers
capture replace ilo_job1_ocu_isco08_2digits = 62 if census == 6005                            // 61 / 62 - Market-oriented skilled forestry, fishery and hunting workers
capture replace ilo_job1_ocu_isco08_2digits = 62 if census == 6020                            // 61 / 62 - Market-oriented skilled forestry, fishery and hunting workers
capture replace ilo_job1_ocu_isco08_2digits = 92 if census == 6100                            // 62 / 63 / 92 - Agricultural, forestry and fishery labourers
capture replace ilo_job1_ocu_isco08_2digits = 63 if census == 6110                            // 62 / 63 - Subsistence farmers, fishers, hunters and gatherers
capture replace ilo_job1_ocu_isco08_2digits = 92 if census == 6120                            // 62 / 92 - Agricultural, forestry and fishery labourers
capture replace ilo_job1_ocu_isco08_2digits = 83 if census == 6130                            // 62 / 83 - Drivers and mobile plant operators
...
capture replace ilo_job1_ocu_isco08_2digits = 74 if census == 6540                            // 71 / 74 - Electrical and electronic trades workers
...
capture replace ilo_job1_ocu_isco08_2digits = 74 if census == 7000                            // 71 / 72 / 73 / 74 - Electrical and electronic trades workers
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8810                            // 71 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7920                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7930                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7940                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7950                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7960                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8000                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8010                            // 72 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8020                            // 72 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8220                            // 72 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 75 if census == 8330                            // 73 / 75 - Food processing, wood working, garment and other craft and related trades workers
capture replace ilo_job1_ocu_isco08_2digits = 73 if census == 8550                            // 73 - Handicraft and printing workers
capture replace ilo_job1_ocu_isco08_2digits = 75 if census == 8710                            // 73 / 75 - Food processing, wood working, garment and other craft and related trades workers
capture replace ilo_job1_ocu_isco08_2digits = 73 if census == 8750                            // 73 - Handicraft and printing workers
capture replace ilo_job1_ocu_isco08_2digits = 82 if census == 8910                            // 73 / 82 - Assemblers
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8920                            // 73 / 75 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7830                            // 75 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7840                            // 75 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7850                            // 75 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 7855                            // 75 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8460                            // 75 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8720                            // 75 / 81 - Stationary plant and machine operators
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8730                            // 75 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 81 if census == 8850                            // 75 / 81 - Stationary plant and machine operators
...
capture replace ilo_job1_ocu_isco08_2digits = 96 if census == 9620                            // 93 / 96 - Refuse workers and other elementary workers
