...
global source "LFS"
global time "2009"
global inputFile "LFS_2009-September_E"
...
* Comment:
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if residence==1
		replace ilo_geo=2 if residence==2
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Even though question 14 asks for the "highest grade of technical training and
...
*            for job/job application.
    ...
    gen ilo_lfs=.
	    replace ilo_lfs=1 if (q17==1 | q18==1) & ilo_wap==1                                                                    // Employed
		replace ilo_lfs=1 if ((q19==1 & inlist(q21,1,2,3,4,5)) | q19==2) & ilo_wap==1                                          // Employed: temporary absent in the past 7 days
		replace ilo_lfs=1 if ((q20==1 & inlist(q21,1,2,3,4,5)) | (q20==1 & inlist(q21,6,7,8,9,10) & q22==1)) & ilo_wap==1      // Employed: temporary absent 
		replace ilo_lfs=1 if (q20==2 & q22==1) & ilo_wap==1                                                                    // Employed: temporary absent
		replace ilo_lfs=2 if ilo_lfs!=1 & (q23==1 & q26==1) & ilo_wap==1                                                       // Unemployed
		replace ilo_lfs=2 if ilo_lfs!=1 & q28==5 & ilo_wap==1                                                                  // Unemployed: future starters
...
* Comment: 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if q58==3 & ilo_lfs==1
...
*            information on whether they are paid or not.
	...
	* Detailled categories:
	  gen ilo_job1_ste_icse93=.
		  replace ilo_job1_ste_icse93=1 if q47==4	& ilo_lfs==1			    // Employees
		  replace ilo_job1_ste_icse93=2 if q47==1	& ilo_lfs==1			    // Employers
		  replace ilo_job1_ste_icse93=3 if q47==2	& ilo_lfs==1			    // Own-account workers
		  replace ilo_job1_ste_icse93=4 if q47==5	& ilo_lfs==1			    // Producer cooperatives
		  replace ilo_job1_ste_icse93=5 if q47==3	& ilo_lfs==1	           	// Contributing family workers
	...
	* Detailled categories:
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if q66==4	& ilo_mjh==2			    // Employees
		  replace ilo_job2_ste_icse93=2 if q66==1	& ilo_mjh==2			    // Employers
		  replace ilo_job2_ste_icse93=3 if q66==2	& ilo_mjh==2			    // Own-account workers
		  replace ilo_job2_ste_icse93=4 if q66==5	& ilo_mjh==2			    // Producer cooperatives
		  replace ilo_job2_ste_icse93=5 if q66==3	& ilo_mjh==2	           	// Contributing family workers
...
* Comment: - National classification is based on ISIC 4 at a 2 digit-level.
  ...
  * MAIN JOB:
  * ISIC Rev. 4 - 2 digit
	gen isic2dig = int(q43/10) if ilo_lfs==1 
  ...
  * SECOND JOB:
  * ISIC Rev. 4 - 2 digit
	gen isic2dig2 = int(q63/10) if ilo_mjh==2
	    replace isic2dig2=. if isic2dig2==0
...
*          - It can be mapped to ISCO 08 at 1 digit level.
   ...
   * SECOND JOB:
   	 gen occup_1dig2=int(q59/100) if ilo_mjh==2 & q59>0
...
* Comment: - Public: state
	...
	* MAIN JOB:
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if q42==5 & ilo_lfs==1				    // Public
	...
	* SECOND JOB:
	gen ilo_job2_ins_sector=.
		replace ilo_job2_ins_sector=1 if q62==5 & ilo_mjh==2				    // Public
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* Comment: - There's no information available on the usual hours of work in the second job nor
*            in all jobs.
		
	* MAIN JOB:

	* Hours USUALLY worked in main job	
	gen ilo_job1_how_usual = q53 if q53!=-1 & ilo_lfs==1
	...
	gen ilo_job1_how_usual_bands=.
		...
		replace ilo_job1_how_usual_bands=. if ilo_lfs!=1
		   	    lab def how_bands_lab 1 "No hours worked" 2 "01-14" 3 "15-29" 4 "30-34" 5 "35-39" 6 "40-48" 7 "49+" 8 "Not elsewhere classified"		
	...
	* Hours ACTUALLY worked in main job	
	gen ilo_job1_how_actual=q54 if q54!=-1 & ilo_lfs==1
	...
	* Hours ACTUALLY worked in secondary job
    gen ilo_job2_how_actual = q72 if q72!=-1 & ilo_mjh==2
	...
	* Hours ACTUALLY worked all jobs
    gen ilo_joball_how_actual = q75 if q75!=-1 & ilo_lfs==1
...
*           for main job (because usual hours of work for all job is not available) which is 42.
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------			
* Comment: - Permanent: Indefinite-term labour contract.
	
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if q48==1 & ilo_lfs==1                     // Permanent
		replace ilo_job1_job_contract=2 if inlist(q48,2,3,4,5) & ilo_lfs==1        // Temporary
