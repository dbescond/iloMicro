* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Viet Nam, 2012
* DATASET USED: Viet Nam LFS 2012
* NOTES: 
* Files created: Standard variables on LFS Viet Nam 2012
...
global source "LFS"
global time "2012"
global inputFile "LFS-2012-final_E.dta"
...
*		     classified under "Not elsewhere classified".
	...
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if q13==0  					// No schooling
		replace ilo_edu_isced97=2 if q13==1 					// Pre-primary education
		replace ilo_edu_isced97=3 if q13==2  		            // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if q13==3  		            // Lower secondary or second stage of basic education
		replace ilo_edu_isced97=5 if inlist(q13,4,5,6,7)	    // Upper secondary education
		*replace ilo_edu_isced97=6 if                         	// Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if inlist(q13,8,9)	        // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if q13== 10    				// Second stage of tertiary education (leading to an advanced research qualification)
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - The question is only asked to people aged 15 and more. So people below the age of 15
*          are under "Not elsewhere classified". 

	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if q11==1			// Attending 
		replace ilo_edu_attendance=2 if q11==2 			// Not attending 
		replace ilo_edu_attendance=3 if q11==.      	// Not elsewhere classified 
...
* Comment: There is no information on union/cohabiting
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if q14==1                                     // Single
		replace ilo_mrts_details=2 if q14==2                                     // Married
		*replace ilo_mrts_details=3 if                                          // Union / cohabiting
		replace ilo_mrts_details=4 if q14==3                                     // Widowed
		replace ilo_mrts_details=5 if q14==4                                     // Divorced / separated
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Employed people is based on the variable difined by the NSO 
*          - Future starters must be included in unemployment. Question 23 groups together
...
*          are also in unemployment. 
    ...
    gen ilo_lfs=.
	    replace ilo_lfs=1 if q15==1 & ilo_wap==1
		replace ilo_lfs=1 if q16==1 & ilo_wap==1
		replace ilo_lfs=1 if q17==2 & ilo_wap==1
		replace ilo_lfs=1 if q19!=. & ilo_wap==1                                //Incluiding all reasons ("waiting for season" / "preparation for a new job")
		replace ilo_lfs=1 if q20==1 & ilo_wap==1
		replace ilo_lfs=2 if ilo_lfs!=1 & ((q21==1 & q24==1) | q23==5) & ilo_wap==1
...
* Comment: 
	...
	gen ilo_mjh=.
		replace ilo_mjh=1 if inlist(q57,3,.) & ilo_lfs==1
		replace ilo_mjh=2 if inlist(q57,1,2) & ilo_lfs==1
	...
	* MAIN JOB:
		...
		* Detailled categories:
			gen ilo_job1_ste_icse93=.
				replace ilo_job1_ste_icse93=1 if q43==4	& ilo_lfs==1			        // Employees
				replace ilo_job1_ste_icse93=2 if q43==1	& ilo_lfs==1			        // Employers
				replace ilo_job1_ste_icse93=3 if q43==2	& ilo_lfs==1			        // Own-account workers
				replace ilo_job1_ste_icse93=4 if q43==5	& ilo_lfs==1			        // Producer cooperatives
				replace ilo_job1_ste_icse93=5 if q43==3	& ilo_lfs==1	           		// Contributing family workers
	...
	* SECOND JOB:
		...
		* Detailled categories:
			gen ilo_job2_ste_icse93=.
				replace ilo_job2_ste_icse93=1 if q64==4	& ilo_mjh==2			        // Employees
				replace ilo_job2_ste_icse93=2 if q64==1	& ilo_mjh==2			        // Employers
				replace ilo_job2_ste_icse93=3 if q64==2	& ilo_mjh==2			        // Own-account workers
				replace ilo_job2_ste_icse93=4 if q64==5	& ilo_mjh==2			        // Producer cooperatives
				replace ilo_job2_ste_icse93=5 if q64==3	& ilo_mjh==2	        		// Contributing family workers
...
*          classification is identic to ISIC Rev. 4.
   ...
   * MAIN JOB:
	...
	* ISIC Rev. 4 - 2 digit
	gen isic2dig = int(q40/100) if ilo_lfs==1 
   ...
   * ISIC Rev. 4 - 2 digit
   gen isic2dig_job2 = int(q61/100) if ilo_mjh==2
...
*            It can be mapped to ISCO 08 at 1 digit level.  		
   ...
   * MAIN JOB:
   	 gen occup_1dig=int(q37/1000) if ilo_lfs==1 & q37>0
  ...
  * SECOND JOB:
	gen occup_1dig_job2=int(q58/1000) if ilo_mjh==2 & q58>0
...
* Comment: - Public: state agency, state productive unit, state entreprise
	...
	* MAIN JOB:
	  gen ilo_job1_ins_sector=.
		  replace ilo_job1_ins_sector=1 if inrange(q38,5,7) & ilo_lfs==1			// Public
	...
	* SECOND JOB:
	  gen ilo_job2_ins_sector=.
		  replace ilo_job2_ins_sector=1 if inrange(q59,5,7) & ilo_mjh==2			// Public
...
* Comment: 
	...
	* 1) Weekly hours ACTUALLY worked:
	     gen ilo_job1_how_actual=q53 if ilo_lfs==1
	...
	* 2) Weekly hours USUALLY worked:
		 gen ilo_job1_how_usual=q54 if ilo_lfs==1
	...
	* 1) Weekly hours ACTUALLY worked:
         gen ilo_job2_how_actual=q72 if ilo_mjh==2
	...
	* 1) Weekly hours ACTUALLY worked:
		 gen ilo_joball_how_actual=q75 if ilo_lfs==1
...
*        (because usual hours of work is not available) for all jobs which is 46
	   
	   gen ilo_job1_job_time=.
    	   replace ilo_job1_job_time=1 if ilo_job1_how_usual<46 
		   replace ilo_job1_job_time=2 if ilo_job1_how_usual>=46 & ilo_job1_how_usual!=.
...
*          (oral, no contract). As around 60% of people answer either oral agreement or no 
...
* Comment: 
            ...
            /*	Useful questions:
			- Institutional sector: q38
			- Destination of production: not asked.
			- Bookkeeping: not asked
			- Registration: q41a & q41b (considered registered if answer's yes to both)
			- Status in employment: ilo_job1_ste_aggregate
			- Social security contribution: q46c (pay for social insurance)
			- Place of work: q47 
			- Size: q42 (1st category is 1 to 20; Question: is it to use this filter?)
			- Paid annual leave: q46 -> receive paid public holidays/leaves?
    ...
    * Social security (to be dropped afterwards): 
            gen social_security=.
			    replace social_security=1 if (q46c==1 & ilo_lfs==1)             // Pay for social insurance
				replace social_security=2 if (q46c==2 & ilo_lfs==1)             // No pay for social insurance
				replace social_security=. if (social_security==. & ilo_lfs==1)
    * Registration 
            gen registration=.
			    replace registration=1 if (q41a==1 & q41b==1) & ilo_lfs==1      // Business registration and Tax code registration
				replace registration=2 if (q41a==2 | q41b==2) & ilo_lfs==1      // No business registration and no tax code registraiton
	...
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR		
			...
			gen ilo_job1_ife_prod=.
				...
				replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
				                              ((inlist(q38,5,6,7)) | /// 
											  (!inlist(q38,5,6,7) & registration==1) | ///
											  (!inlist(q38,5,6,7) & registration==. & ilo_job1_ste_aggregate==1 & social_security==1))
			    replace ilo_job1_ife_prod=1 if ilo_lfs==1 & ilo_job1_ife_prod!=2 & ilo_job1_ife_prod!=3 & ///
				                              ((!inlist(q38,5,6,7) & registration==2) | ///
											  (!inlist(q38,5,6,7) & registration==. & ilo_job1_ste_aggregate==1 & inlist(social_security,2,.)) | ///
											  (!inlist(q38,5,6,7) & registration==. & ilo_job1_ste_aggregate!=1))
...
*           - Unit: local currency (dongs)
			...
			* MAIN JOB:
			  gen wage=.
			      replace wage = q50 if q50>=0 & ilo_job1_ste_aggregate==1
				  replace wage = wage*1000
			 
			  gen other_payments=.
			      replace other_payments = q52 if q52>=0 & ilo_job1_ste_aggregate==1
			...
			* SECOND JOB:
			  gen wage2=.
			      replace wage2 = q70 if q70>=0 & ilo_job2_ste_aggregate==1
    			  replace wage2 = wage2 *1000
			
			  gen other_payments2=.
			      replace other_payments2 = q71 if q71>=0 & ilo_job2_ste_aggregate==1 
...
*          - Only the total hours ACTUALLY worked are available.
		   ...
		   gen ilo_joball_tru=.
			   replace ilo_joball_tru=1 if ilo_lfs==1 & q77==1 & q78==1 & q79>0 & ilo_joball_how_actual<35
...
* Comment: 
		...
		gen ilo_cat_une=.
			replace ilo_cat_une=1 if ilo_lfs==2 & q29==1			            // Previously employed
			replace ilo_cat_une=2 if ilo_lfs==2 & q29==2			            // Seeking first job
			replace ilo_cat_une=3 if ilo_lfs==2 & q29!=1 & q29!=2	            // Unkown
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: Question 27 asks the unemployed "How long have you been looking OR AVAILABLE to work?". 
...
*          only the aggregated variable can be created.	
	...
	* Aggregate:
	...
	gen ilo_dur_aggregate=.
		replace ilo_dur_aggregate=1 if inlist(q27,1,2) & ilo_lfs==2 	        // Less than 6 months
		replace ilo_dur_aggregate=2 if inlist(q27,3,4) & ilo_lfs==2	        	// 6 months to less than 12 months
		replace ilo_dur_aggregate=3 if q27==5 & ilo_lfs==2	        			// 12 months or more
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
* Commente:  National classification follow ISIC Rev.4

	* ISIC Rev. 4 - 2 digit
	  gen indu_code_une_2dig=int(q36/100) 
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
* Comment: - The country uses its own classification, can be mapped to ISCO 08 at 1 digit.
	     
  * MAIN JOB:
    gen prev_occup=int(q32/1000) if q32>0
...
*             are not possible to be produced.	
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if q21==1 & q24==2 & ilo_lfs==3 				// Seeking, not available
		replace ilo_olf_dlma = 2 if q21==2 & q24==1 & ilo_lfs==3				// Not seeking, available
...
* Comment:
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if	inlist(q23,2,3) & ilo_lfs==3   		    // Labour market
			replace ilo_olf_reason=2 if inlist(q23,4,6) & ilo_lfs==3            // Other labour market reasons
			replace ilo_olf_reason=3 if	inlist(q23,8,9) & ilo_lfs==3	        // Personal/Family-related
			replace ilo_olf_reason=4 if q23==1 & ilo_lfs==3						// Does not need/want to work
...
* Comment:
	...
	gen ilo_dis=1 if ilo_lfs==3 & q24==1 & ilo_olf_reason==1
...
cd "$outpath"
		...
		drop to_drop to_drop_1 to_drop_2 year_ quarter_
		
		/*Only age bands used*/
		drop ilo_age 
		
		/*Variables computed in-between*/
		drop isic2dig isic2dig_job2 occup_1dig occup_1dig_job2 social_security registration
		drop wage other_payments wage2 other_payments2 indu_code_une_2dig prev_occup
