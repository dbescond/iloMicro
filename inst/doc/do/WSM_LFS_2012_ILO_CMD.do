...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Only available at the aggregated level.
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Original question is only asked to persons aged 15 years or more; thus, the rest 
*            of the observations (or who didn't reply to the questions used here) are classified 
*            under "Not elsewhere classified".

	gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if (b_3==1)		                // Attending
		replace ilo_edu_attendance=2 if (b_3==2)	                    // Not attending
...
** the majority of "no elsewhere classied" observations are related to people aged below 15.
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if a_8==1                                           // Single
		replace ilo_mrts_details=2 if a_8==2                                          // Married
		*replace ilo_mrts_details=3 if a_8==                                          // Union / cohabiting
		replace ilo_mrts_details=4 if a_8==4                                          // Widowed
		replace ilo_mrts_details=5 if a_8==3                                          // Divorced / separated
...
*          - To measure disability status, the questions asking whether the person has any
*            difficulties seeing, hearing, walking, remembering and washing all over or dressing
*            (self-caring) are used.
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Employment comprises people who answered positively to questions about work for
*            at least one hour during the last 7 days, those employed temporary absent (not
*            including future starters or seasonal workers) and subsistence farmers (following
*            the national definitions).
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=1 if ilo_lfs!=1 & (inlist(e_2,6310,6320,6330,6340) | e_4==9700)     // Employed (subsistence farmers)
		replace ilo_lfs=2 if ilo_lfs!=1 & h_1==1 & h_7==1 & ilo_wap==1                      // Unemployment
		replace ilo_lfs=2 if ilo_lfs!=1 & h_7==1 & h_1==2 & h_4==1 & ilo_wap==1             // Unemplyment (future starters)
		replace ilo_lfs=3 if (ilo_lfs!=1 & ilo_lfs!=2 & ilo_wap==1) 						// Outside the labour force
				label define ilo_lfs 1 "Employed" 2 "Unemployed" 3 "Outside the Labour Force"
				label value ilo_lfs ilo_lfs
...
*          - Hours worked in other jobs/activities is used as a proxy.
	...
	gen ilo_mjh=.
		replace ilo_mjh=2 if !inlist(f_1b,0,.) & ilo_lfs==1                     // Hours worked in other job/activity greater than cero => more than one job
		replace ilo_mjh=1 if ilo_mjh!=2 & ilo_lfs==1                            // Only one job
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - National classification corresponds to ISIC Rev.4 - 4 digits

 
	* MAIN JOB:
	* ISIC Rev. 4 - 2 digit
	  gen ilo_job1_eco_isic4_2digits = int(e_4/100) if ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
* ------------------------------------------------------------------------------------------
* Comment: - National classification corresponds to ISCO08 - 2 digits

	* MAIN JOB:
    * ISCO 08 - 2 digits:
	  gen ilo_job1_ocu_isco08_2digits = int(e_2/100) if ilo_lfs==1
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* Comment: - Public: government and public/state-owned enterprise
*          - Private: the rest

	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inlist(e_7,1,2) & ilo_lfs==1                                 
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* Comment: - Information on the hours actually worked by subsitence farmers is gathered in 
*            section K. No information on the hours usually worked is available for them.
*          - Workers are asked how many hours they worked in "any other jobs/activities" in 
*            question f_1b, but hours worked only in the second job cannot be isolated, and
*            thus not computed.

   * MAIN JOB
   * 1) Weekly hours ACTUALLY worked:
   egen job1_how_actual1 = rowtotal(f_2am f_2bm f_2cm f_2dm f_2em f_2fm f_2gm), m        // Employment without subsistence farmers
   egen job1_how_actual2 = rowtotal(k1_hrs k2_hrs k3_hrs k4_hrs k5_hrs k6_hrs), m        // Subsistence farmers
   
   gen ilo_job1_how_actual = job1_how_actual1 if ilo_lfs==1
       replace ilo_job1_how_actual = job1_how_actual2 if ilo_job1_how_actual==. & ilo_lfs==1
   ...
   * ALL JOBS
   * 1) Weekly hours ACTUALLY worked:
   egen joball_how_actual1 = rowtotal(f_2am f_2ao f_2bm f_2bo f_2cm f_2co f_2dm f_2do f_2em f_2eo f_2fm f_2fo f_2gm f_2go), m   // Employment without subsistence farmers
   egen joball_how_actual2 = rowtotal(k1_hrs k2_hrs k3_hrs k4_hrs k5_hrs k6_hrs), m                                             // Subsistence farmers
   
   gen ilo_joball_how_actual = joball_how_actual1 if ilo_lfs==1
       replace ilo_joball_how_actual = joball_how_actual2 if ilo_joball_how_actual==. & ilo_lfs==1
               lab var ilo_joball_how_actual "Weekly hours actually worked in all job"
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - According to the LFS final report, the normal weekly working hours is 40 (median
*            of the sample =48 => threshold used is 40.
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - In addition to "unlimited duration" and "limited duration" there is another 
*            possible answer "unspecified duration". This category is classified under 
*            "limited duration" as the respondants are asked the same followign questions as
*            those answering "limited duration".
	...
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR	
			...
			gen ilo_job1_ife_prod=.
				...
				replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
				                               ((inlist(e_7,1,2,4))| ///
											   (inlist(e_7,3,5,6,7,8,.) & e_8==1)| ///
											   (inlist(e_7,3,5,6,7,8,.) & inlist(e_8,4,9,.) & ilo_job1_ste_icse93==1 & social_security==1)| ///
											   (inlist(e_7,3,5,6,7,8,.) & inlist(e_8,4,9,.) & ilo_job1_ste_icse93==1 & social_security!=1 & e_6==3 & inlist(e_5,3,4,5,6,7))| ///
											   (inlist(e_7,3,5,6,7,8,.) & inlist(e_8,4,9,.) & ilo_job1_ste_icse93!=1 & e_6==3 & inlist(e_5,3,4,5,6,7)))
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - Questions e_22 to e_25 refer to labour related income of the employees. They are
*            asked how much they earned in cash and in kind last time they were paid. In addition,
*            they are asked what's the periodicity of the payments (i.e 1 month, 1 fortnight, 
*            1 week, 1 day or other).
*          - In the case of self-employment, questions e_25a and e_25b refer to total earnings
*            in cash and in kind (after deduction expenses) in a period of 2 weeks for all 
*            respondants to this quesiton.
*          - To convert daily and weekly income into monthly income, the following factors were 
*            used:
*			- monthly income = (365/12)*daily income
*			- monthly income = (((365/7)/12)/2) *fortnight income
*			- monthly income = ((365/7)/12)*weekly income*/
* NOTES: For employees => 1. main job currently held (T11:142)
*   					  2. including payments in kind (T34:400)
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------			
* Comment: - There is no questions asking directly the workers if they are AVAILABLE to work 
*            additional hours (Note: T35:2416).
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
* Comment: - For days lost due to occupational injuries: codes 98 ("don't know") and 99 ("expecting
*            never return to work") are recoded as missing.
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
* Comment: - category 2 ("no") in question h_3 ("did ___ want to work or try to start a business
*            in the last 7 days") is used here to define category 3 ("does not need/want to work").
	...
	gen ilo_olf_reason=.
		...
		replace ilo_olf_reason=3 if (h_3==2 & !inlist(ilo_olf_reason,1,2) & ilo_lfs==3)	   //Does not need/want to work
...
cd "$outpath"
		...
		drop job1_how_actual1 job1_how_actual2 joball_how_actual1 joball_how_actual2 social_security cash_month kind_month cash_month_slf kind_month_slf
