...
global source "LFS"
global time "2017"
global inputFile "nSamoa_LFS2017_v6"
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - No informaiton available.	
...
*            over or dressing (self-caring) and communicating with others are used.
	...
	gen ilo_dsb_aggregate=.
		replace ilo_dsb_aggregate=2 if inlist(a1f,3,4)	                        // With disability
		replace ilo_dsb_aggregate=2 if inlist(a1g,3,4)	                        // With disability
		replace ilo_dsb_aggregate=2 if inlist(a1h,3,4)	                        // With disability
		replace ilo_dsb_aggregate=2 if inlist(a1i,3,4)	                        // With disability
		replace ilo_dsb_aggregate=2 if inlist(a1j,3,4)	                        // With disability
		replace ilo_dsb_aggregate=2 if inlist(a1k,3,4)                          // With disability
...
* Comment: 
	...
	gen ilo_lfs=.
		...
		replace ilo_lfs=1 if (d1b==1 & (d3a==1 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1                        // Employed
		replace ilo_lfs=1 if (d1c==1 & (d3a==1 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1                        // Employed
		replace ilo_lfs=1 if (d2a==1 & inrange(d2c,1,4) & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1
		replace ilo_lfs=1 if (d2a==1 & d2d==1 & (d3a==1 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1
		replace ilo_lfs=1 if (d2a==1 & inlist(d2c,5,6,7,8,9,11) & d2f==1) & ilo_wap==1
		replace ilo_lfs=1 if (d2b==1 & inrange(d2c,1,4) & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1
		replace ilo_lfs=1 if (d2b==1 & d2d==1 & (d3a==1 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1
		replace ilo_lfs=1 if (d2b==1 & inlist(d2c,5,6,7,8,9,11) & d2f==1) & ilo_wap==1
		
		
		
		

		replace ilo_lfs=1 if (d2a==1 | d2b==1) & ilo_wap==1						// Employed: temporary absent
...
* Comment: 
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits =      if ilo_lfs==2 & ilo_cat_une==1
                * labels already defined for main job
                lab val ilo_job1_eco_isic4_2digits eco_isic4_2digits
                lab var ilo_job1_eco_isic4_2digits "Previous economic activity (ISIC Rev. 4), 2 digits level"
