* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Samoa, 2017
* DATASET USED: Samoa, LFS, 2017
* NOTES: 
* Files created: Standard variables WSM_LFS_2017_FULL.dta and WSM_LFS_2017_ILO.dta
...
global source "LFS"
global time "2017"
global inputFile "nSamoa_LFS2017_v3"
...
cd "$inpath"
	...
	* 1) deleting ID (not used) otherwise there would be two variables with
	*    same name
	drop ID
	rename *, lower  
...
* ------------------------------------------------------------------------------
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if region==1							// Urban 
		replace ilo_geo=2 if inlist(region,2,3,4)				// Rural
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Asked to those aged 3 years old or more and therefore the rest are
*            classified under "level not stated".
*          - Given the lack of detailed mapping only aggregate version is defined.
*          - Highest level completed (combination between those currently
*            attending and those not attending but did it in the past).

    * Aggregate
    gen ilo_edu_aggregate=.
		replace ilo_edu_aggregate=1 if (b1e==1 & (inrange(b1f,0,11) | b1f==17)) | (b1g==0) | (b1g==1 & (inrange(b1i,0,7) | inlist(b1i,88,98)))
		replace ilo_edu_aggregate=2 if (b1e==1 & inlist(b1f,12,13)) | (b1g==1 & (inrange(b1i,8,11) | inlist(b1i,20,21)))
		replace ilo_edu_aggregate=3 if (b1e==1 & inrange(b1f,14,16)) | (b1g==1 & inrange(b1i,12,14))
		replace ilo_edu_aggregate=4 if (b1g==1 & inrange(b1i,15,19))
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Question is asked to those aged 3 years old or more and therefore
*            those below three are classified under "Not elsewhere classified".
			
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if b1e==1                                  // Attending
		replace ilo_edu_attendance=2 if b1e==2                                  // Not attending
...
*/
	...
	* Aggregate
	gen ilo_mrts_aggregate=.
	    replace ilo_mrts_aggregate=1 if inrange(a1l,2,4)                    // Single / Widowed / Divorced / Separated
		replace ilo_mrts_aggregate=2 if a1l==1                              // Married / Union / Cohabiting
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Only asked to people aged 15 years and over and therefore those aged
*            below are classified under "no difficulty".
*          - To measure disability status, questions asking whether the person has
*            any difficulties seeing, hearing, walking, remembering, washing all
*            over or dressing (self-caring) and communicating with others are used.
*          - The aggregate variable includes into people with disability those with
*            a lot of difficulty or cannot do it at all in the detail variable

    gen ilo_dsb_details=.
	    replace ilo_dsb_details=2 if a1f==2     	                            // Some difficulty
		replace ilo_dsb_details=2 if a1g==2		                                // Some difficulty
		replace ilo_dsb_details=2 if a1h==2		                                // Some difficulty
		replace ilo_dsb_details=2 if a1i==2		                                // Some difficulty
		replace ilo_dsb_details=2 if a1j==2		                                // Some difficulty
		replace ilo_dsb_details=2 if a1k==2		                                // Some difficulty
		replace ilo_dsb_details=3 if a1f==3	    	                            // A lot of difficulty
		replace ilo_dsb_details=3 if a1g==3		                                // A lot of difficulty
		replace ilo_dsb_details=3 if a1h==3		                                // A lot of difficulty
		replace ilo_dsb_details=3 if a1i==3		                                // A lot of difficulty
		replace ilo_dsb_details=3 if a1j==3		                                // A lot of difficulty
		replace ilo_dsb_details=3 if a1k==3		                                // A lot of difficulty
		replace ilo_dsb_details=4 if a1f==4  		                            // Cannot do it at all
		replace ilo_dsb_details=4 if a1g==4		                                // Cannot do it at all
		replace ilo_dsb_details=4 if a1h==4		                                // Cannot do it at all
		replace ilo_dsb_details=4 if a1i==4		                                // Cannot do it at all
		replace ilo_dsb_details=4 if a1j==4		                                // Cannot do it at all
		replace ilo_dsb_details=4 if a1k==4		                                // Cannot do it at all
	...
	gen ilo_dsb_aggregate=.
		replace ilo_dsb_aggregate=2 if inlist(ilo_dsb_details,3,4)	            // With disability
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Employment follows the skip pattern proposed on the questionnaire, 
*            which is based on the ILO's 19th ICLS. It excludes own-use production
*            workers
*          - Unemployment comprises those following the ILO's three criteria and 
*            available future starters.

	
	gen ilo_lfs=.
        replace ilo_lfs=1 if (d1a==1) & ilo_wap==1                                                              // Employed
		replace ilo_lfs=1 if (d1b==1 & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1                        // Employed
		replace ilo_lfs=1 if (d1c==1 & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1                        // Employed
		replace ilo_lfs=1 if (d2a==1 & inrange(d2c,1,4) & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1     // Employed
		replace ilo_lfs=1 if (d2a==1 & d2d==1 & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1               // Employed
		replace ilo_lfs=1 if (d2a==1 & inlist(d2c,5,6,7,8,9,11) & d2f==1) & ilo_wap==1                          // Employed 
		replace ilo_lfs=1 if (d2b==1 & inrange(d2c,1,4) & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1     // Employed
		replace ilo_lfs=1 if (d2b==1 & d2d==1 & (d3a==3 | inlist(d3c,1,2) | d3d==1)) & ilo_wap==1               // Employed
		replace ilo_lfs=1 if (d2b==1 & inlist(d2c,5,6,7,8,9,11) & d2f==1) & ilo_wap==1                          // Employed
		replace ilo_lfs=1 if (d2b==2 & (d3b==1 & inlist(d3c,1,2))) & ilo_wap==1                                 // Employed
		replace ilo_lfs=1 if (d2b==2 & (d3b==1 & d3d==1)) & ilo_wap==1                                          // Employed
		replace ilo_lfs=2 if (ilo_lfs!=1 & (g1a==1 | g1b==1) & (g1k==1 | g1l==1)) & ilo_wap==1			        // Unemployed: three criteria
		replace ilo_lfs=2 if (ilo_lfs!=1 & (g1c==1 & inlist(g1d,1,2)) & (g1k==1 | g1l==1)) & ilo_wap==1			// Unemployed: available future starters
...
* ------------------------------------------------------------------------------
    ...
    gen ilo_mjh=.
		replace ilo_mjh=1 if (e1a==2) & ilo_lfs==1                              // One job only     
		replace ilo_mjh=2 if (e1a==1) & ilo_lfs==1                              // More than one job
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Apprentices/Interns are classified under employees with pay in cash 
*            or in-kind.

   * MAIN JOB:
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if (inlist(e1d,3,4) & (e1y_1==1 | e1y_3==1 | e1y_4==1 | e1y_5==1)) & ilo_lfs==1  // Employees
		 replace ilo_job1_ste_icse93=2 if (e1d==1 & e1u==1) & ilo_lfs==1                                                // Employers
		 replace ilo_job1_ste_icse93=3 if (e1d==1 & (e1r==1 | e1t==2 | e1u==2)) & ilo_lfs==1                            // Own-account workers
		 * replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1                                                               // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if (e1d==2 & (e1s_1==1 | e1y_2==1 | e1y_6==1)) & ilo_lfs==1                      // Contributing family workers (spouses operating in joint business)
		 replace ilo_job1_ste_icse93=5 if (e1d==5 & e1f==1) & ilo_lfs==1                                                // Contributing family workers 
		 replace ilo_job1_ste_icse93=5 if (e1d==5 & e1f==2 & (e1y_2==1 | e1y_6==1)) & ilo_lfs==1                        // Contributing family workers
	...
	* ICSE 1993
	  gen ilo_job2_ste_icse93=.
		  replace ilo_job2_ste_icse93=1 if (inlist(e2c,1,2)) & ilo_mjh==2       // Employees
		  replace ilo_job2_ste_icse93=2 if (e2c==3) & ilo_mjh==2                // Employers
		  replace ilo_job2_ste_icse93=3 if (e2c==4) & ilo_mjh==2                // Own-account workers
		  replace ilo_job2_ste_icse93=4 if (e2c==7) & ilo_mjh==2                // Members of producers' cooperatives 
		  replace ilo_job2_ste_icse93=5 if (inlist(e2c,5,6)) & ilo_mjh==2       // Contributing family workers
...
* Comment: 
    ...
    * MAIN JOB:
    ***********
	gen econ_act_prim = int(e1cb/100)
	...
	* 2-digit level
	gen ilo_job1_eco_isic4_2digits = . 
	    replace ilo_job1_eco_isic4_2digits = econ_act_prim if ilo_lfs==1
   ...
   * Aggregate level
    ...
    * SECOND JOB:
    *************
    gen econ_act_secu = int(e2bb/100)
	...
	* 2-digit level
	gen ilo_job2_eco_isic4_2digits = . 
	    replace ilo_job2_eco_isic4_2digits = econ_act_secu if ilo_mjh==2
...
* Comment:
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco08_2digits = . 
	    replace ilo_job1_ocu_isco08_2digits = ocu_act_prim if ilo_lfs==1
	...
	* 1-digit level 				
	gen ilo_job1_ocu_isco08=.
	    replace ilo_job1_ocu_isco08=11 if inlist(ilo_job1_ocu_isco08_2digits,.,64) & ilo_lfs==1                          // Not elsewhere classified
	...
	* 2-digit level 
    gen ilo_job2_ocu_isco08_2digits = . 
	    replace ilo_job2_ocu_isco08_2digits = ocu_act_secu  if ilo_mjh==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Public: government or a state owned enterprise.
		
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if e1e==1  & ilo_lfs==1                   // Public
...
* ------------------------------------------------------------------------------
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = f1a8m if ilo_lfs==1
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = ilo_job1_how_actual if ilo_lfs==1 & f1b==1
		replace ilo_job1_how_usual = f1e1 if ilo_job1_how_usual==. & ilo_lfs==1 & f1b==2
	...
	* Hours ACTUALLY worked
    gen ilo_job2_how_actual = .
	    replace ilo_job2_how_actual = f1a9s if ilo_mjh==2
	...
	* Hours USUALLY worked
	gen ilo_job2_how_usual = .
	    replace ilo_job2_how_usual = ilo_job2_how_actual if ilo_mjh==2 & f1b==1
		replace ilo_job2_how_usual = f1e2 if ilo_job2_how_usual==. & ilo_mjh==2 & f1b==2
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual = .
	    replace ilo_joball_how_actual = f1a11 if ilo_lfs==1
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual = .
	    replace ilo_joball_how_usual = ilo_joball_how_actual if ilo_lfs==1 & f1b==1
		replace ilo_joball_how_usual = f1e4 if ilo_joball_how_usual==. & ilo_lfs==1 & f1b==2
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Threshold is set at 35 hours worked per week.
			
	gen ilo_job1_job_time=.
		replace ilo_job1_job_time=2 if ilo_joball_how_usual>=35  & ilo_lfs==1                             // Full-time
		replace ilo_job1_job_time=1 if ilo_joball_how_usual<35 & ilo_joball_how_usual!=. & ilo_lfs==1     // Part-time
...
* ------------------------------------------------------------------------------
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if e1p==2 & ilo_job1_ste_aggregate==1               		// Permanent
		replace ilo_job1_job_contract=2 if inlist(e1p,1,3) & ilo_job1_ste_aggregate==1              // Temporary
...
* Comment: 
/* Useful questions:
          - Institutional sector: e1e / also used -> Incorporated company: e1g
		  - Private household identification: ilo_job1_eco_isic4_2digits==97 / e1e option 6.
		  - Destination of production: not asked.
		  - Bookkeeping: e1i
		  - Registration: e1h
		  - Status in employment: ilo_job1_ste_aggregate
		  - Social security contribution (Proxy: pension funds): e1k (npf contributions)
		  - Place of work: e1j
		  - Size: e1n (threshold set at 5 persons)
		  - Paid annual leave: e1l
		  - Paid sick leave:e1m
*/
    * Social Security:
	gen social_security=.
	    replace social_security=1 if e1k==1 & ilo_lfs==1                        // social security (proxy)
		replace social_security=2 if e1k==2 & ilo_lfs==1                        // no social security (proxy) + other + DK + NA + Not asked
	
    * 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR

	gen ilo_job1_ife_prod=.
	    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & ///
		                               ((ilo_job1_eco_isic4_2digits==97) | e1e==6)
		replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ilo_job1_ife_prod!=3 & ///
		                               ((inlist(e1e,1,4,5)) | ///
									   (inlist(e1e,2,3,.) & e1g==1) | ///
									   (inlist(e1e,2,3,.) & e1g!=1 & e1i==1) | ///
									   (inlist(e1e,2,3,.) & e1g!=1 & e1i!=1 & e1h==1) | ///
									   (inlist(e1e,2,3,.) & e1g!=1 & e1i!=1 & e1h==. & ilo_job1_ste_aggregate==1 & social_security==1) | ///
									   (inlist(e1e,2,3,.) & e1g!=1 & e1i!=1 & e1h==. & ilo_job1_ste_aggregate==1 & inlist(social_security,2,.) & e1j==4 & inlist(e1n,3,4,5,6)) | ///
									   (inlist(e1e,2,3,.) & e1g!=1 & e1i!=1 & e1h==. & ilo_job1_ste_aggregate!=1 & e1j==4 & inlist(e1n,3,4,5,6)))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	gen ilo_job1_ife_nature=.
	    replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ///
										 ...
										 (inlist(ilo_job1_ste_icse93,1,6) & social_security==. & e1l==1 & e1m==1) | ///
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Payments in cash and in kind in main job and second job.

    ***********
    * MAIN JOB:
    ***********
	* In cash and kind
	egen cash_kind_prim = rowtotal(e1ab1 e1ab2), m
	
	gen cash_kind_prim_m = . 
	    replace cash_kind_prim_m = cash_kind_prim * (365/12) if e1aa==2       // daily
	    replace cash_kind_prim_m = cash_kind_prim * 4        if e1aa==3       // once a week
		replace cash_kind_prim_m = cash_kind_prim * 2        if e1aa==4       // every two weeks
		replace cash_kind_prim_m = cash_kind_prim            if e1aa==5       // every month
	
	* Employees
	gen ilo_job1_lri_ees = cash_kind_prim_m if ilo_job1_ste_aggregate==1
	    replace ilo_job1_lri_ees = . if ilo_job1_ste_aggregate!=1
		        lab var ilo_job1_lri_ees "Monthly earnings of employees - main job"
	
	* Self-employed
	gen ilo_job1_lri_slf = cash_kind_prim_m if ilo_job1_ste_aggregate==2
    ...
    *************
	* In cash and kind
	egen cash_kind_secu = rowtotal(e2h1 e2h2), m
		
	gen cash_kind_secu_m = . 
	    replace cash_kind_secu_m = cash_kind_secu * (365/12) if e2g==2       // daily
	    replace cash_kind_secu_m = cash_kind_secu * 4        if e2g==3       // once a week
		replace cash_kind_secu_m = cash_kind_secu * 2        if e2g==4       // every two weeks
		replace cash_kind_secu_m = cash_kind_secu            if e2g==5       // every month
		
	
	* Employees
	gen ilo_job2_lri_ees = .
	    replace ilo_job2_lri_ees = cash_kind_secu_m if ilo_job2_ste_aggregate==1
    ...
    * Self-employed		
	gen ilo_job2_lri_slf = .
	    replace ilo_job2_lri_slf =  cash_kind_secu_m if ilo_job2_ste_aggregate==2
...
* ------------------------------------------------------------------------------
	...
	gen ilo_joball_tru = .
	    replace ilo_joball_tru = 1 if ilo_joball_how_usual<35  & f1g==1 & f1h==1 & ilo_lfs==1
...
*-------------------------------------------------------------------------------
*-------------------------------------------------------------------------------
* Comment: - Injuries or illnesses within the past 12 months while working

	 gen ilo_joball_oi_case = .
		 replace ilo_joball_oi_case = 1 if i1a==1 & ilo_lfs==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: - Note that the question "duration of not having a work or job search"
*            is asked after question 'looking for a job/start a business', but even 
*            though it should be answered by all of those answering yes to this 
*            question, it was only recorded (maybe wrongly (?)) for those looking 
*            to start a new business (18 out of 668 unemployed people). Thus, it
*            is not defined.

/*
	* Detailed categories		
    gen ilo_dur_details=.
	    replace ilo_dur_details=1 if g1f==1 & ilo_lfs==2                        // Less than 1 month
		replace ilo_dur_details=2 if g1f==2 & ilo_lfs==2                        // 1 to 3 months
		replace ilo_dur_details=3 if g1f==3 & ilo_lfs==2                        // 3 to 6 months
		replace ilo_dur_details=4 if g1f==4 & ilo_lfs==2                        // 6 to 12 months
		replace ilo_dur_details=5 if g1f==5 & ilo_lfs==2                        // 12 to 24 months
		replace ilo_dur_details=6 if g1f==6 & ilo_lfs==2                        // 24 months or more
...
* ------------------------------------------------------------------------------
	...
	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if (g1a==1 | g1b==1) & (g1k==2 & g1l==2) & ilo_lfs==3               // Seeking, not available
		replace ilo_olf_dlma = 2 if (g1a==2 & g1b==2) & (g1k==1 | g1l==1) & ilo_lfs==3	             // Not seeking, available
		replace ilo_olf_dlma = 3 if (g1a==2 & g1b==2) & (g1k==2 & g1l==2) & g1i==1 & ilo_lfs==3      // Not seeking, not available, willing
		replace ilo_olf_dlma = 4 if (g1a==2 & g1b==2) & (g1k==2 & g1l==2) & g1i==2 & ilo_lfs==3      // Not seeking, not available, not willing
...
* ------------------------------------------------------------------------------
		...
		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inlist(g1j,3,4,5) & ilo_lfs==3          // Labour market 
			replace ilo_olf_reason=2 if inlist(g1j,1,2) & ilo_lfs==3            // Other labour market reasons
			replace ilo_olf_reason=3 if inlist(g1j,6,7,8,9) & ilo_lfs==3        // Personal/Family-related
			replace ilo_olf_reason=4 if g1j==4 & ilo_lfs==3                     // Does not need/want to work
...
* ------------------------------------------------------------------------------
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & (g1k==1 | g1l==1) & ilo_olf_reason==1
...
cd "$outpath"
	...
	drop econ_act_prim econ_act_secu ocu_act_prim ocu_act_secu social_security cash_kind_prim cash_kind_prim_m cash_kind_secu cash_kind_secu_m
	compress
