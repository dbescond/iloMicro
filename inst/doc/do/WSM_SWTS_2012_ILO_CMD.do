...
* DATASET USED: SAMOA, SWTS, 2012
* NOTES: 
* Files created: Standard variables WSM_SWTS_2012_FULL.dta and WSM_SWTS_2012_ILO.dta
...
global country "WSM" /*ref_area: ISO 3 Code from the workflow*/
global source "SWTS"  /*survey: Acronym from the workflow*/
global time "2012"  /*time*/
global inputFile "WSM_SWTS_2012.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_geo=.
	    replace ilo_geo=1 if inlist(qhregion,1,2)         // Urban 
		replace ilo_geo=2 if inlist(qhregion,3,4)         // Rural
...
* Comment:
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if  c10==1   | b_5==2                   // No schooling
		*replace ilo_edu_isced97=2 if                        // Pre-primary education
		replace ilo_edu_isced97=3 if c10==2                       // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if  c10==3                      // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if c10==4                       // Upper secondary education
		replace ilo_edu_isced97=6 if c10==5                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if c10==6                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if c10==7                       // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if  c1==1                      // Attending
		replace ilo_edu_attendance=2 if  inlist(c1,2,3)                      // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
		...
		replace ilo_mrts_details=2 if  b4==2                                         // Married
		*replace ilo_mrts_details=3 if                                           // Union / cohabiting
		replace ilo_mrts_details=4 if b4==4                                          // Widowed
		replace ilo_mrts_details=5 if b4==3                                          // Divorced / separated
...
* Comment: 
    ...
    * Detailed
       gen ilo_dsb_details=.
             replace ilo_dsb_details=1 if (a_9==1 & a_10==1 & a_11==1 & a_12==1 & a_13==1)                            
             replace ilo_dsb_details=2 if (a_9==2 | a_10==2 | a_11==2 | a_12==2 | a_13==2 )                                                 
             replace ilo_dsb_details=3 if (a_9==3 | a_10==3 | a_11==3 | a_12==3 | a_13==3 )                                                 
             replace ilo_dsb_details=4 if (a_9==4 | a_10==4 | a_11==4 | a_12==4 | a_13==4)
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (d_1a==1 | d_1b==1 | d_1c==1 | employed==1) & ilo_wap==1				   // Employed: ILO definition
		replace ilo_lfs=1 if d_2==1 & !inlist(d_3,12) & ilo_wap==1										// Employed: temporary absent
		replace ilo_lfs=2 if inlist(h_1,1,2) & h_7==1 & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
...
* Comment: 
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = f2      if ilo_lfs==1
...
* Comment: 
	...
	gen ilo_job1_job_contract=.
		replace ilo_job1_job_contract=1 if e_16==2 & ilo_lfs==1               				// Permanent
		replace ilo_job1_job_contract=2 if e_16==1 &ilo_lfs==1               				// Temporary
