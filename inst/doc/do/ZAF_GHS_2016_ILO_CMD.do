...
global inputFile "GHS-2016-PERSON_F1.dta" /*name of the input file in stata format*/
...
*                    2. Tradition
*                    3. Farms
destring geotype, replace
 	
	gen ilo_geo=.
	    replace ilo_geo=1 if geotype==1                  // Urban 
		replace ilo_geo=2 if inlist(geotype,2,3)         // Rural
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: Classification ISCE 97 is being used
	...
	* Detailed
	gen ilo_edu_isced97=.
		replace ilo_edu_isced97=1 if q15hiedu==98                       // No schooling
		replace ilo_edu_isced97=2 if q15hiedu==0                       // Pre-primary education
		replace ilo_edu_isced97=3 if inrange(q15hiedu,1,7)                       // Primary education or first stage of basic education
		replace ilo_edu_isced97=4 if inrange(q15hiedu,8,9)                       // Lower secondary education or second stage of basic education
		replace ilo_edu_isced97=5 if inrange(q15hiedu,10,15)                       // Upper secondary education
		replace ilo_edu_isced97=6 if inrange(q15hiedu,16,23)                       // Post-secondary non-tertiary education
		replace ilo_edu_isced97=7 if inrange(q15hiedu,24,28)                       // First stage of tertiary education (not leading directly to an advanced research qualification)
		replace ilo_edu_isced97=8 if q15hiedu==29                       // Second stage of tertiary education (leading to an advanced research qualification)                               
...
* Comment: 
    ...
    gen ilo_edu_attendance=.
		replace ilo_edu_attendance=1 if q110atte==1                             // Attending
		replace ilo_edu_attendance=2 if q110atte==2                             // Not attending
...
* Comment: 
	...
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if inlist(q12amarst,6,7)                     // Single
		replace ilo_mrts_details=2 if q12amarst==1                              // Married
		replace ilo_mrts_details=3 if q12amarst==2                              // Union / cohabiting
		replace ilo_mrts_details=4 if q12amarst==5                              // Widowed
		replace ilo_mrts_details=5 if inlist(q12amarst,3,4)                     // Divorced / separated
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: revised details
/*
    * Detailed
	gen ilo_dsb_details=.
	    replace ilo_dsb_details=1 if q29see==1 | q29hea==1 | q29wal==1 |  q29rem==1 |  q29sel==1 |  q29com==1                    // No, no difficulty
		replace ilo_dsb_details=2 if q29see==2 | q29hea==2 | q29wal==2 |  q29rem==2 |  q29sel==2 |  q29com==2	                // Yes, some difficulty
		replace ilo_dsb_details=3 if q29see==3 | q29hea==3 | q29wal==3 |  q29rem==3 |  q29sel==3 |  q29com==3	                // Yes, a lot of difficulty
		replace ilo_dsb_details=4 if q29see==4 | q29hea==4 | q29wal==4 |  q29rem==4	|  q29sel==4 |  q29com==4 		                // Cannot do it at all
...
*/
    * Aggregate  	
	gen ilo_dsb_aggregate=.
	    replace ilo_dsb_aggregate=1 if  disab==0                    // Persons without disability
		replace ilo_dsb_aggregate=2 if 	disab==1                    // Persons with disability
...
* Comment: there is no information on future starters
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q41awge==1 | q41bbus==1 | q41cvol==1) & ilo_wap==1						// Employed: ILO definition
		replace ilo_lfs=1 if (q41dret==1) & ilo_wap==1						                           // Employed: temporary absent
		replace ilo_lfs=2 if (q46alook==1 & q46daccjob==1) & ilo_lfs!=1 & ilo_wap==1			       // Unemployed: three criteria
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: there is information on wages (q42asto  q42bsp q42msal) but there is no information on Status in Employment
/*
    * MAIN JOB
