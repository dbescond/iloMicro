...
global time "2007Q3"  /*time*/
global inputFile "LFS_16WORKER!092007_F1.dta" /*name of the input file in stata format*/
...
* Comment: 
	...
	gen ilo_lfs=.
        replace ilo_lfs=1 if (q21bpaid==1 | q21aownb==1 | q21dunpd==1) & ilo_wap==1						            // Employed: ILO definition
		replace ilo_lfs=1 if (q22havew==1) & ilo_wap==1						                                        // Employed: temporary absent
		replace ilo_lfs=2 if (q38lookw==1 & q36accep==1 & inlist(q37whnst,1,2)) & ilo_lfs!=1 & ilo_wap==1			// Unemployed: three criteria
		replace ilo_lfs=2 if (q32ynotw==1) & ilo_lfs!=1 & ilo_wap==1			                                    // Unemployed: available future starters
...
* ------------------------------------------------------------------------------
* Comment: to revise
* Comment: the number of employers and own-account workers has been computed takinf into account the number of employees

   * MAIN JOB
   * ICSE 1993
	 gen ilo_job1_ste_icse93=.
		 replace ilo_job1_ste_icse93=1 if inlist(q43mwork,1,2) & ilo_lfs==1                                   // Employees
		 replace ilo_job1_ste_icse93=2 if inlist(q43mwork,3,4) & inrange(q416nrwo,2,6)  & ilo_lfs==1          // Employers
		 replace ilo_job1_ste_icse93=3 if inlist(q43mwork,3,4) & q416nrwo==1 & ilo_lfs==1                     // Own-account workers
		* replace ilo_job1_ste_icse93=4 if  & ilo_lfs==1          // Members of producers' cooperatives
		 replace ilo_job1_ste_icse93=5 if q43mwork==5 & ilo_lfs==1          // Contributing family workers
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: vables indus or q42indus (but do not match) (to check better)
/*
    * MAIN JOB
	...
	* 2-digit level 
    gen ilo_job1_ocu_isco88_2digits = . 
	    replace ilo_job1_ocu_isco88_2digits = int(q41occup/100) if ilo_lfs==1
		 replace ilo_job1_ocu_isco88_2digits = . if ilo_job1_ocu_isco88_2digits==99 & ilo_lfs==1
...
* Comment: 
	...
	gen ilo_job1_ins_sector=.
		replace ilo_job1_ins_sector=1 if inrange(q414typb,1,4) & ilo_lfs==1    // Public
...
* Comment: 
	...
	* Hours USUALLY worked
	gen ilo_job1_how_usual = .
	    replace ilo_job1_how_usual = q425ahrs if ilo_lfs==1 & !inlist(q425ahrs,888,999)
	...
	* Hours ACTUALLY worked
	gen ilo_job1_how_actual = .
	    replace ilo_job1_how_actual = q424ahrs if ilo_lfs==1 & !inlist(q424ahrs,888,999)
	...
	* Hours USUALLY worked
    gen ilo_joball_how_usual = .
	    replace ilo_joball_how_usual = q425chrs if ilo_lfs==1 & !inlist(q425chrs,888,999)
	...
	* Hours ACTUALLY worked
	gen ilo_joball_how_actual = .
	    replace ilo_joball_how_actual = q424chrs if ilo_lfs==1 & !inlist(q424chrs,888,999)
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: there is a variable for type of business sector (formal/informal): q422sect
/* Useful questions:
          - Institutional sector:  q414typb  
		  - Private household identification: check
...
* Comment:
	...
	gen ilo_cat_une=.
		replace ilo_cat_une=1 if q312ever==1 & ilo_lfs==2                     // Previously employed       
		replace ilo_cat_une=2 if q312ever==2 & ilo_lfs==2                     // Seeking for the first time
...
* Comment: 
    ...
    * Aggregate categories
	gen ilo_dur_aggregate=.
		replace ilo_dur_aggregate=1 if (inrange(q310tims,1,5) & ilo_lfs==2)     // Less than 6 months
		replace ilo_dur_aggregate=2 if (q310tims==6 & ilo_lfs==2)               // 6 to 12 months
		replace ilo_dur_aggregate=3 if (inlist(q310tims,7,8) & ilo_lfs==2)      // 12 months or more
...
* Comment: 
	...
	* 2-digit level 
    gen ilo_prevocu_isco88_2digits = . 
	    replace ilo_prevocu_isco88_2digits = int(q314occu/100) if ilo_lfs==2 & ilo_cat_une==1 & q314occu!=9999
                 * labels already defined for main job
...
* ------------------------------------------------------------------------------
*!!! CHECK!!
* Comment: there is a skyp patter in the question q36accep, if the person says that s/he will not accept a job, then s/he is not asked q37whnst
* Comment: there are so many missing (78%)
** see questions q34wrkre q35whyno

	gen ilo_olf_dlma=.
		replace ilo_olf_dlma = 1 if q38lookw==1 & !inlist(q37whnst,1,2) & ilo_lfs==3      // Seeking, not available
		replace ilo_olf_dlma = 2 if q38lookw==2 & inlist(q37whnst,1,2)  & ilo_lfs==3	    // Not seeking, available
		*replace ilo_olf_dlma = 3 if q38lookw==2 & inlist(q37whnst,1,2)		      & ilo_lfs==3      // Not seeking, not available, willing
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: TO CHECK (also with other do files)
* high amount of NEC

		gen ilo_olf_reason=.
			replace ilo_olf_reason=1 if inrange(q311rsnn,6,10) & ilo_lfs==3     // Labour market 
			replace ilo_olf_reason=2 if q311rsnn==1 & ilo_lfs==3                // Other labour market reasons
			replace ilo_olf_reason=3 if inrange(q311rsnn,2,5) & ilo_lfs==3      // Personal/Family-related
...
* Comment:
	...
	gen ilo_dis=.
	    replace ilo_dis=1 if !inlist(ilo_lfs,1,2) & inlist(q37whnst,1,2) & ilo_olf_reason==1
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: TO DO!! THERE IS INFORMATION
/*
	gen ilo_sub=.
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: no information on education attendance
/*
