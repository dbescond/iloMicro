...
* -------------------------------------------------------------------------------------------			
	...
	*recall that Q410INCOMETAX might explain discrepancies
	gen todrop_registration=1 if (Q413VAT==1|Q414TAX==1|Q410INCOMETAX==1) 							// theoretical 3 value node/ +1 registered national level/ 0 not registered national level, other, DK / 0 not asked NA
		replace todrop_registration=0 if (Q413VAT==2|Q414TAX==2|Q413VAT==3|Q414TAX==3|Q410INCOMETAX==2|Q410INCOMETAX==3)&todrop_registration!=1
		replace todrop_registration=-1 if todrop_registration==.
		
	gen todrop_SS=1 if Q46PENSION==1|Q48UIF==1|Q49MEDICAL==1  								// theoretical 3 value node/ +1 Social security/ 0 don't know, Other, Not asked; NA / -1 No social security 

		replace todrop_SS=0 if   (Q46PENSION==.|Q46PENSION==3)&(Q48UIF==.|Q48UIF==3)&(Q49MEDICAL==.|Q49MEDICAL==3) &  todrop_SS!=1
		replace todrop_SS=-1 if (Q46PENSION==2|Q48UIF==2|Q49MEDICAL==2) &  todrop_SS==.
	...
	gen todrop_paidleave=1 if Q47PDLEAVE==1							// theoretical 2 value node/ COMPLICATION!
		replace todrop_paidleave=0 if todrop_paidleave==.
	
	capture confirm variable Q47B1PDSICK
	
	if !_rc {
                       gen todrop_paidsick=0 if Q47B1PDSICK==1	
               }
               else {
                       gen todrop_paidsick=.
    }
									// theoretical 2 value node/ 
	...
	*** End informality****************************************
	***********************************************************
* to solve small problems of scope
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_olf_reason=.
		replace ilo_olf_reason=1 if	(inlist(Q38RSNNOTSEEK,7,8,10,11) & ilo_lfs==3)					//Labour market
		replace ilo_olf_reason=2 if	(inlist(Q38RSNNOTSEEK,1,2) & ilo_lfs==3)							//Other Labour market
		replace ilo_olf_reason=3 if	(inlist(Q38RSNNOTSEEK,3,4,5,6,13)  & ilo_lfs==3)						//Personal/Family-related
		replace ilo_olf_reason=4 if (inlist(Q38RSNNOTSEEK,14,15)  & ilo_lfs==3)						//Does not need/want to work
		replace ilo_olf_reason=5 if (inlist(Q38RSNNOTSEEK,16,12,9)  & ilo_lfs==3)						//Not elsewhere classified
