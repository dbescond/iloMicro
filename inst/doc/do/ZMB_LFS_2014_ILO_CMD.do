* TITLE OF DO FILE: ILO Microdata Preprocessing code template - Zambia LFS
* DATASET USED: Zambia LFS 2014
* NOTES:
...
global source "LFS"
global time "2014"
global inputFile "ZM_LFS_2014.dta"
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_geo = .
		replace ilo_geo = 1 if region == 2	// 1 - Urban
		replace ilo_geo = 2 if  region == 1	// 2 - Rural
...
** Comment: The higher category stated is Master	

gen ilo_edu_isced11=.
		replace ilo_edu_isced11=1 if b2 == 2                                     // No schooling
		replace ilo_edu_isced11=2 if b6 == 0                                     // Early childhood education
		replace ilo_edu_isced11=3 if inrange(b6,1,7)                             // Primary education 
		replace ilo_edu_isced11=4 if inlist(b6,8,9)                             // Lower secondary education
		replace ilo_edu_isced11=5 if inlist(b6,10,11)                           // Upper secondary education
		replace ilo_edu_isced11=6 if b6 == 12                                   // Post-secodary non-tertiary education
		replace ilo_edu_isced11=7 if inlist(b6,13,14)                           // Short-cycle tertiary education
		replace ilo_edu_isced11=8 if b6 == 15                                   // Bachelor's or equivalent level
		replace ilo_edu_isced11=9 if b6 == 16                                  // Master's or equivalent level
		*replace ilo_edu_isced11=10 if                                // Doctoral or equivalent level
		replace ilo_edu_isced11=11 if ilo_edu_isced11 == . & b2 == 1                      // Not elsewhere classified 
...
* -------------------------------------------------------------------------------------------
		...
		replace ilo_edu_attendance = 1 if b3 == 1                        // 1 - Attending
		replace ilo_edu_attendance = 2 if b3 == 2               //  2 - Not attending
		replace ilo_edu_attendance = 3 if  b3 == -9                    // 3 - Not elsewhere classified
...
gen ilo_lfs =.
		...
		** EMPLOYED
		...
		replace ilo_lfs = 1 if inrange(c1,1,6) & ilo_wap == 1
		replace ilo_lfs = 1 if c2==1 & ilo_wap == 1
		*replace ilo_lfs = 1 if c3 == 1 & ilo_wap == 1 // according to the definitions on the report, they take into account own-production services/goods. 
													  // however the result does not match
													  // including OP the result is higher, and lower than their objective without including it
		...
		** all persons in LF who are completely jobless and are AVAILABLE AND ACTIVELY LOOKING FOR WORK. 
		** included are also those not working, not looking but available.
		
		replace ilo_lfs = 2 if ilo_lfs !=1 & g1 == 1 & g3 == 1 &  ilo_wap == 1 
		
		*replace ilo_lfs = 2 if ilo_lfs != 1  & c1 == 8 & c2 == 2 & g1 == 1 &  ilo_wap == 1 // not working but looking for work/business and available
		*replace ilo_lfs = 2 if ilo_lfs != 1  & c1 == 10 & c2 == 2 &  ilo_wap == 1 // according to their definition, they also consider as unemployed people who are not working, not looking for a job but available. 
		*replace ilo_lfs = 2 if ilo_lfs != 1  & g5==1 &  ilo_wap == 1 // future job starters
...
** - 1. One job only
	...
	replace ilo_mjh = 1 if  inlist(d18,-9,2,.) & ilo_lfs == 1

** - 2. More than one job

	replace ilo_mjh = 2 if d18 == 1 & ilo_lfs == 1
...
** Comments: - it is supossed from the questionnaire that apprentices and interns are paid (see section f). 
**			- volunteers (not classifiable by status??)
...
* Detailed categories:
		...
		gen ilo_job1_ste_icse93 = .
			...
			replace ilo_job1_ste_icse93 = 1 if inrange(d4,1,3)   & ilo_lfs == 1   	// Employees
			
			replace ilo_job1_ste_icse93 = 2 if d4 == 4 & ilo_lfs==1	            // Employers
			
			replace ilo_job1_ste_icse93 = 3 if d4 == 5 & ilo_lfs==1         // Own-account workers
			
			*replace ilo_job1_ste_icse93 = 4 if       			                    // Members of producers cooperatives

			replace ilo_job1_ste_icse93 = 5 if d4 == 7 & ilo_lfs==1	            // Contributing family workers
			
			replace ilo_job1_ste_icse93 = 6 if  d4 == 6 & ilo_lfs==1                // Workers not classifiable by status
...
* ISIC Rev. 4
	...
	* MAIN JOB:
	gen indu_code_prim =.
	replace indu_code_prim = int(d2/100)
 ...
 * MAIN JOB	
	gen occ_code_prim=.
	    replace occ_code_prim=int(d1/100)
         
    * 2-digit level
	gen ilo_job1_ocu_isco08_2digits = occ_code_prim if ilo_lfs==1
	replace ilo_job1_ocu_isco08_2digits = 61 if occ_code_prim == 1 & ilo_job1_eco_isic4_2digits == 1 & ilo_lfs==1
	replace ilo_job1_ocu_isco08_2digits = 01 if occ_code_prim == 1 & ilo_job1_eco_isic4_2digits == 84  & ilo_lfs==1
...
***** MAIN JOB *****
		...
		gen ilo_job1_ins_sector = .
			replace ilo_job1_ins_sector = 1 if inrange(d13,1,4) & ilo_lfs == 1 	// Public
			replace ilo_job1_ins_sector = 2 if inrange(d13,5,8) & ilo_lfs == 1    // Private
...
** Comment: according to the report of the Zambia LFS 2014, the define as part-time workers those who
**          worked less than 8 hours/day or 40 hours/week,


** DOUBT: variable d5 ??

**VARIABLES: d5, e2a, e2b

** Comment: variables e2a and e2b refer to hours per day. 
...
***** MAIN JOB *****
		...
		replace ilo_job1_job_time = 1 if e2a < 8 & ilo_lfs == 1 				// 1 - Part-time
		replace ilo_job1_job_time = 2 if e2a >= 8  & ilo_lfs == 1 				// 2 - Full-time
...
gen ilo_job2_job_time = .
		replace ilo_job2_job_time = 1 if e2b < 8 & ilo_lfs == 1 				// 1 - Part-time
		replace ilo_job2_job_time = 2 if e2b >= 8  & ilo_lfs == 1 				// 2 - Full-time
		replace ilo_job2_job_time = 3 if ilo_job2_job_time == . & ilo_lfs == 1 		// 3 - Unknown
			*lab def job_time_lab 1 "1 - Part-time" 2 "2 - Full-time" 3 "3 - Unknow"
			    lab values ilo_job2_job_time job_time_lab
			    lab var ilo_job2_job_time "Job (Working time arrangement)"


* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*		1. 	Hours of work actually worked ('ilo_job1_how_actual') NO DONE
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*		2. 	Hours of work usually worked ('ilo_job1_how_usual')   NO DONE
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
...
*** MAIN JOB
		...
		gen ilo_job1_how_usual = e2a if ilo_lfs==1
		replace ilo_job1_how_usual = . if ilo_job1_how_usual == -9
...
*** DOUBT: limit for scope, lfs/mjh?

gen ilo_job2_how_usual = e2b if ilo_lfs==1
		replace ilo_job2_how_usual = . if ilo_job2_how_usual == -9
...
** DOUBT: fixed period contract ??? part-time???
** written oral (d12)???

*** main job
gen ilo_job1_job_contract=.

		replace ilo_job1_job_contract=1 if inlist(d5,1,3) & ilo_job1_ste_aggregate==1 // 1 - Permanent
		replace ilo_job1_job_contract=2 if inlist(d5,2,4,5) & ilo_job1_ste_aggregate==1 // 2 - Temporary
...
*/
	...
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR		
			...
			gen ilo_job1_ife_prod=.
			    replace ilo_job1_ife_prod=3 if ilo_lfs==1 & ((c1==7) |(ilo_job1_eco_isic4_2digits==97) | ///    *** there is no variable to indentify the destination of the production. 
				                                (ilo_job1_ocu_isco08_2digits==63))
				replace ilo_job1_ife_prod=2 if ilo_lfs==1 & ((inrange(d13,1,4)) | ///
				                               *(c1!=7 & !inrange(d13,1,4)) & bookkeping) | ///
											   (c1!=7 & !inrange(d13,1,4) &  !inlist(d14,4,5)) | ///
											   (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate==1 & d6==1) | ///
											   (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate==1 & d6!=1 & inlist(d16,1,2) & inlist(d15,2,3)) | ///
											   (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate!=1 &   inlist(d16,1,2) & inlist(d15,2,3)) 
											   
											 
			    replace ilo_job1_ife_prod=1 if ilo_lfs==1 & (!inlist(ilo_job1_ife_prod,3,2)) & ///
				                               (c1!=7 & !inrange(d13,1,4)  & !inlist(d14,4,5)) | ///
				                               (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate==1 & d6!=1  & !inlist(d16,1,2)) | ///
											   (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate==1 & d6!=1  & inlist(d16,1,2) & !inlist(d15,2,3)) | ///
											   (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate!=1 & !inlist(d16,1,2)) | ///
											   (c1!=7 & !inrange(d13,1,4) &  inlist(d14,5,.) & ilo_job1_ste_aggregate!=1 & inlist(d16,1,2) & !inlist(d15,2,3)))
	...
	* 2) NATURE OF JOB: FORMAL/INFORMAL EMPLOYMENT
	      ...
	      gen ilo_job1_ife_nature=.
		      replace ilo_job1_ife_nature=2 if ilo_lfs==1 & ((inlist(ilo_job1_ste_icse93,1,6) & d6==1) | ///
			                                   (inlist(ilo_job1_ste_icse93,1,6) & inlist(d6,-9,.) & d7==1 & d8==1) | ///
											   (inlist(ilo_job1_ste_icse93,2,4) & ilo_job1_ife_prod==2) | ///
											   (c1!=7 & ilo_job1_ste_icse93==3 & ilo_job1_ife_prod==2))
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Monthly related income ('ilo_job1_lri_ees' and 'ilo_job1_lri_slf')   !!! SELF!!!
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
...
*** Employees	
	...
	*** fa5c: it is related to paid employment in their main job (paid employee, apprendice, and intern)
	gen ilo_job1_lri_ees=.
	 replace ilo_job1_lri_ees =fa5c if ilo_job1_ste_aggregate == 1                
		   lab var ilo_job1_lri_ees "Monthly earnings of employees in all jobs"
		   

		   
** Comment: -- SEVERAL VARIABLES FOR INCOME IN SLF. 
** DOUBT: negative values >> set to zero??
** missings: -9


*** Self-employed			   
	gen ilo_job1_lri_slf=.
	 replace ilo_job1_lri_slf = fb6b if ilo_job1_ste_aggregate == 2                
		   lab var ilo_job1_lri_slf "Monthly labour related income of self-employed in all jobs"
...
* -------------------------------------------------------------------------------------------		
		...
		gen ilo_joball_tru = .
		replace ilo_joball_tru = 1 if  e3 == 1 & e4 != . & ilo_job1_how_usual < 40
...
* -------------------------------------------------------------------------------------------
		...
		gen ilo_cat_une = .
		replace ilo_cat_une = 1 if h1 == 1 & ilo_lfs == 2				// 1 - Unemployed previously employed
		
** Comment: - g3 / g6 ?? 		
		replace ilo_cat_une = 2 if h1 == 2 & g3 == 1 & ilo_lfs == 2	// 2 - Unemployed seeking their first job
...
* -------------------------------------------------------------------------------------------		.	
		...
		replace ilo_olf_dlma = 1 if ilo_lfs == 3 & c1 == 9			        // Seeking (actively), not available
		replace ilo_olf_dlma = 2 if ilo_lfs == 3 & c1 == 10 			                            // Not seeking, available
		replace ilo_olf_dlma = 3 if ilo_lfs == 3 & c1 == 11	                    // Not seeking, not available, willing  
		replace ilo_olf_dlma = 4 if ilo_lfs == 3 & c1 == 16 & g1 == 2			                // Not seeking, not available, not willing  
...
gen ilo_olf_reason = .
			replace ilo_olf_reason = 1 if inrange(g5,10,15) & ilo_lfs==3   // Labour market
			replace ilo_olf_reason=2 if inlist(g5,2,3) & ilo_lfs==3      // Other labour market reasons
			replace ilo_olf_reason = 3 if inrange(g5,4,9)   & ilo_lfs==3    // Personal/Family-related
			replace ilo_olf_reason=4 if g5 == 16  & ilo_lfs==3		      // Does not need/want to work
			*replace ilo_olf_reason=5 if g5 ==  & ilo_lfs==3	      // Not elsewhere classified
...
* -------------------------------------------------------------------------------------------		
	...
	gen ilo_dis=1 if (ilo_lfs==3 & g1 == 1 & ilo_olf_reason==1)
