...
global source "LFS"
global time "2017Q1"
global inputFile "ZMB_LFS_2017_Q1.dta"
...
gen ilo_lfs =.
		...
		** EMPLOYED
		...
		replace ilo_lfs = 1 if (c2a==1 & inrange(c2c,1,4)) &  ilo_wap == 1 
		replace ilo_lfs = 1 if (c2a==1 & !inlist(c2c,1,2,3,4,11) & c2d ==1) &  ilo_wap == 1 
		replace ilo_lfs = 1 if (c2a==1 & !inlist(c2c,1,2,3,4,11) & c2d ==2 & c2e==1) &  ilo_wap == 1 
	    replace ilo_lfs = 1 if (c2b==1 & inrange(c2c,1,4)) &  ilo_wap == 1 
		replace ilo_lfs = 1 if (c2b==1 & !inlist(c2c,1,2,3,4,11) & c2d ==1) &  ilo_wap == 1 
		replace ilo_lfs = 1 if (c2b==1 & !inlist(c2c,1,2,3,4,11) & c2d ==2 & c2e==1) &  ilo_wap == 1  //this is not included in the characteristic for them!
		replace ilo_lfs = 1 if (inlist(c3a,1,2) & inlist(c3b,1,2)) &  ilo_wap == 1  //different as well
		replace ilo_lfs = 1 if (c3b==1 & inlist(c3b,1,2)) & ilo_wap == 1 
...
* ISIC Rev. 4
   ...
   * Aggregate level
	...
	* Comment: Information only for the main job????
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Occupation ('ilo_job1_ocu_isco08') RDO!
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*			Institutional sector of economic activities ('ilo_job1_ins_sector') RDO!
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------		
...
***** MAIN JOB *****
		...
		gen ilo_job1_ins_sector = .
			...
			replace ilo_job1_ins_sector = 2 if inrange(d5,5,7) & ilo_lfs == 1    // Private
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*		1. 	Hours of work actually worked ('ilo_job1_how_actual')  MIRAR LO DE 99
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
...
*** SECOND JOB
** Comment: the information is on to ALL OTHER JOBS
...
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------
*		2. 	Hours of work usually worked ('ilo_job1_how_usual')   REVISAR EL -9
* -------------------------------------------------------------------------------------------
* -------------------------------------------------------------------------------------------	
...
/* Useful questions:
			...
			DOUBT:
			- Destination of production: h1 == 1  ?? own-use production work 
...
*/
   ...
   ** DOUBT: inlist(d5,5,6,7,.)?? .???
   ** DOUBT: size >> only possible to establish below 5
	...
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR		
			...
			gen ilo_job1_ife_prod=.
				...
				replace ilo_job1_ife_prod=2 if ilo_lfs==1 & inrange(d5,1,4) | ///
				                               (d5!=8 & inlist(d5,5,6,7,.) & d21==1) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==1) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate==1 & d10==1) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate==1 & d10!=1 & inlist(d4,1,2) & inrange(d6,3,7)) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate!=1 & inlist(d4,1,2) & inrange(d6,3,7))
			    replace ilo_job1_ife_prod=1 if ilo_lfs==1 & (!inlist(ilo_job1_ife_prod,3,2)) & ///
				                               ((d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20!=1) | ///
				                               (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate==1 & d10!=1 & !inlist(d4,1,2)) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate==1 &d10!=1 & inlist(d4,1,2) & !inrange(d6,3,7)) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate!=1 & !inlist(d4,1,2)) | ///
											   (d5!=8 & inlist(d5,5,6,7,.) & d21!=1 & d20==. & ilo_job1_ste_aggregate!=1 & inlist(d4,1,2) & !inrange(d6,3,7)))
...
*** DOUBT: the information is related to all jobs?????
...
gen ilo_job1_lri_slf=.
          replace ilo_job1_lri_slf =  fb3 if (fb1==1 & ilo_job1_ste_aggregate==2)                          // Month
  		  replace ilo_job1_lri_slf =  fb3*(2) if (fb1==2 & ilo_job1_ste_aggregate==2)                      // Two weeks
		  replace ilo_job1_lri_slf =  fb3*(52/12) if (fb1==3 & ilo_job1_ste_aggregate==2)                  // Week
		  replace ilo_job1_lri_slf =  fb3*(365/12) if (fb1==4 & ilo_job1_ste_aggregate==2)                 // Day
		  replace ilo_job1_lri_slf =  fb3*(8760/12) if (fb1==5 & ilo_job1_ste_aggregate==2)                // Hourly
		  replace ilo_job1_lri_slf =  fb3*(1/12) if (fb1==6 & ilo_job1_ste_aggregate==2)                   // One year
...
*--------------------------------------------------------------------------------------------
	...
	gen ilo_joball_oi_case = .
		replace ilo_joball_oi_case = 1 if i1==1 
			lab def lab_ilo_joball_oi_case 1 "Time-related underemployed" 
			lab val ilo_joball_oi_case lab_joball_oi_case
			lab var ilo_joball_oi_case "Time-related underemployed"
