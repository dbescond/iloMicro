...
** Comment: The higher category stated is Master's degree - Only persons aged 5+ are answering	
...
* ------------------------------------------------------------------------------
* ------------------------------------------------------------------------------
* Comment: marital status is asked to the population aged 12 years and above. 
	
	* Detailed
	gen ilo_mrts_details=.
	    replace ilo_mrts_details=1 if a5==1                                     // Single
		replace ilo_mrts_details=2 if inlist(a5,3,4)                            // Married
		replace ilo_mrts_details=3 if a5==2                                     // Union / cohabiting
		replace ilo_mrts_details=4 if a5==7                                     // Widowed
		replace ilo_mrts_details=5 if inlist(a5,5,6)                            // Divorced / separated
...
* -------------------------------------------------------------------------------------------
	...
	gen ilo_lfs=.
		...
		** EMPLOYED
		...
		replace ilo_lfs=1 if (c1a==1 & ilo_wap==1) 	
		replace ilo_lfs=1 if (c2d==1 & ilo_wap==1)
		replace ilo_lfs=1 if (c2e==1 & ilo_wap==1)
		replace ilo_lfs=1 if (c3a==3 & ilo_wap==1)
		replace ilo_lfs=1 if (inlist(c3c,1,2) & ilo_wap==1)
	
		** UNEMPLOYED : The definition includes those who are unemployed, available and actively looking for a job. 	  
	
		replace ilo_lfs=2 if ilo_lfs!=1 & g1==1 & inlist(g5,1,2,3) & ilo_wap==1 
...
/* Useful questions:
			...
			- Destination of production: ** d5==8 >> Household - This category for question d5 that does not appear in the questionnaire (d5==8). 
			- Bookkeeping: d21 
			- Registration: d20
...
*/
	...
	* 1) UNIT OF PRODUCTION: FORMAL/INFORMAL SECTOR		
			...
			gen ilo_job1_ife_prod=.
				...
				replace ilo_job1_ife_prod=2 if ilo_lfs==1 & (inrange(d5,1,4) | (inlist(d5,5,6,7,.) & d21==1) | (inlist(d5,5,6,7,.) & d21!=1 & d20==1))
											  
			    replace ilo_job1_ife_prod=1 if ilo_lfs==1 & !inlist(ilo_job1_ife_prod,3,2) &  inlist(d5,5,6,7,.) & d21!=1 & d20!=1
...
** Comment: Coffi, clearly there is something wrong in the microdata as only persons in employment or outside the labour force answer to G3 ?!?
