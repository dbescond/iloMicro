...
*** This code refers to the ONE extra level tabulation cases for Working-age Population.
...
if !_rc {
	capture confirm variable ilo_wap //Check if the variable ilo_wap exists. This is used for the condition.
		if !_rc {	
			di in green "Variables $LEVEL1 & ilo_sex exist! Variable ilo_wap also exists (used for the condition)!"
			//The next "test" checks if the main variable is empty.
			egen TEST_No_Emptya=count($LEVEL1) if ilo_wap==1
			egen TEST_No_Empty=mean(TEST_No_Emptya)
			if TEST_No_Empty!=0 & TEST_No_Empty!=. {
				gen condition = 1 if ilo_wap==1 & $LEVEL1!=. //The tabulations are calculated based on this condition.
			...
			drop TEST_No_Emptya TEST_No_Empty
		}
	else {
		log using "$LIBMPLOGS\LOG_for_${country}_${source}_${time}.smcl", append
