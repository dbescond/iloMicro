...
*** This code refers to the TWO extra levels tabulation cases for Working-age Population.
*DRY stands for "Don't Repeat Yourself!" :)
capture confirm variable $LEVEL1 //Check if the variable exists
if !_rc {
	di in green "Both variables $LEVEL1 & ilo_sex exist!"
		...
		if !_rc {
			...
			if !_rc {	
				di in green "Variables $LEVEL1, $LEVEL2 & ilo_sex exist! Variable ilo_wap also exists (used for the condition)!"
				//The next "test" checks if the main variables are empty.
				egen TEST_No_Empty1a=count($LEVEL1) if ilo_wap==1
				egen TEST_No_Empty1=mean(TEST_No_Empty1a)
				egen TEST_No_Empty2a=count($LEVEL2) if ilo_wap==1
				egen TEST_No_Empty2=mean(TEST_No_Empty2a)
				if TEST_No_Empty1!=0 & TEST_No_Empty2!=0 {
					gen condition = 1 if ilo_wap==1 & $LEVEL2!=.
