...
if !_rc {
	...
	if !_rc {
		...
		if !_rc {	
			...
			if !_rc {
				di in green "Variables $hours & ilo_sex exist! Variables ilo_lfs, ilo_job1_ste_aggregate & ilo_wap also exist (used for the condition)!"
				//The next "test" checks if the main variable is empty.
				egen TEST_No_Emptya=count($hours) if ilo_lfs==1 & ilo_wap==1 & ilo_job1_ste_aggregate==1 & $hours!=0
