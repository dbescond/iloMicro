...
if !_rc {
	...
	if !_rc {
		...
		if !_rc {	
			...
			if !_rc {
				di in green "Variables $hours, $LEVEL1 & ilo_sex exist! Variables ilo_lfs & ilo_wap also exist (used for the condition)!"
				//The next "test" checks if the main variable is empty.
				egen TEST_No_EmptyHRa=count($hours) if ilo_lfs==1 & ilo_wap==1 & $hours!=0
				egen TEST_No_EmptyHR=mean(TEST_No_EmptyHRa)
				egen TEST_No_Empty1a=count($LEVEL1) if ilo_lfs==1 & ilo_wap==1 & $hours!=0
