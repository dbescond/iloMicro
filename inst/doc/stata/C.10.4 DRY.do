	...
	if !_rc {
		...
		if !_rc {	
			...
			if !_rc {
				...
				if !_rc {
					di in green "Variables $hours, $LEVEL1 & ilo_sex exist! Variables ilo_lfs, ilo_ste_aggregate & ilo_wap also exist (used for the condition)!"
					//The next "test" checks if the main variable is empty.
					egen TEST_No_EmptyHRa=count($hours) if ilo_lfs==1 & ilo_wap==1 & ilo_job1_ste_aggregate==1 & $hours!=0
					egen TEST_No_EmptyHR=mean(TEST_No_EmptyHRa)
					egen TEST_No_Empty1a=count($LEVEL1) if ilo_lfs==1 & ilo_wap==1 & ilo_job1_ste_aggregate==1 & $hours!=0
					egen TEST_No_Empty1=mean(TEST_No_Empty1a)
					if TEST_No_EmptyHR!=0 & TEST_No_Empty1!=0 {
						gen condition = 1 if ilo_lfs==1 & ilo_wap==1 & ilo_job1_ste_aggregate==1 & $hours!=0 //The tabulations are calculated based on this condition.
						do "$LIBMPDO\C.0 Master for ONE extra level tabulation - HOURS.do"
						drop condition
					}
					else {
						log using "$LIBMPLOGS\LOG_for_${country}_${source}_${time}.smcl", append
						di in red "Variables $hours and/or $LEVEL1 is EMPTY for the observations that meet the condition."
						log close
					}
					drop TEST_No_EmptyHR TEST_No_EmptyHRa TEST_No_Empty1 TEST_No_Empty1a
				}
				else {
					log using "$LIBMPLOGS\LOG_for_${country}_${source}_${time}.smcl", append
