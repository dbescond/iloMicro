...
*** This code refers to the TWO extra levels tabulation cases for Labour Force.
...
if !_rc {
	...
	if !_rc {
		...
		if !_rc {
			...
			if !_rc {	
				di in green "Variables $LEVEL1, $LEVEL2 & ilo_sex exist! Variables ilo_lfs & ilo_wap also exist (used for the condition)!"
				//The next "test" checks if the main variables are empty.
				egen TEST_No_Empty1a=count($LEVEL1) if (ilo_lfs==1|ilo_lfs==2) & ilo_wap==1
				egen TEST_No_Empty1=mean(TEST_No_Empty1a)
				egen TEST_No_Empty2a=count($LEVEL2) if (ilo_lfs==1|ilo_lfs==2) & ilo_wap==1
				egen TEST_No_Empty2=mean(TEST_No_Empty2a)
				if TEST_No_Empty1!=0 & TEST_No_Empty2!=0 {
					gen condition = 1 if (ilo_lfs==1|ilo_lfs==2) & ilo_wap==1 & $LEVEL2!=.
					do "$LIBMPDO\C.0 Master for TWO extra level tabulation.do"
					drop condition
				}
				else {
				...
				drop TEST_No_Empty1a TEST_No_Empty1 TEST_No_Empty2a TEST_No_Empty2
			}
			else {
				log using "$LIBMPLOGS\LOG_for_${country}_${source}_${time}.smcl", append
