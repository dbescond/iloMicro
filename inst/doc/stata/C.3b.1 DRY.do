...
*** This code refers to the ONE extra level tabulation cases for Employees.
...
if !_rc {
	...
	if !_rc {
		...
		if !_rc {
			...
			if !_rc {
				di in green "Variables $LEVEL1 & ilo_sex exist! Variables ilo_lfs, ilo_wap & ilo_job1_ste_aggregate also exist (used for the condition)!"
				//The next "test" checks if the main variable is empty.
				egen TEST_No_Emptya=count($LEVEL1) if ilo_lfs==1 & ilo_wap==1 & ilo_job1_ste_aggregate==1
				egen TEST_No_Empty=mean(TEST_No_Emptya)
				if TEST_No_Empty!=0 & TEST_No_Empty!=. {
					gen condition = 1 if ilo_lfs==1 & ilo_wap==1 & ilo_job1_ste_aggregate==1 & $LEVEL1!=. //The tabulations are calculated based on this condition.
