...
*** This code refers to the ONE extra level tabulation cases for Unemployed previously employed.
...
if !_rc {
	...
	if !_rc {
		...
		if !_rc {	
			capture confirm variable ilo_cat_une //Check if the variable ilo_cat_une exists. This is used for the condition.
			if !_rc {
				di in green "Variables $LEVEL1 & ilo_sex exist! Variables ilo_lfs, ilo_cat_une & ilo_wap also exist (used for the condition)!"
				//The next "test" checks if the main variable is empty.
				egen TEST_No_Emptya=count($LEVEL1) if ilo_lfs==2 & ilo_wap==1 & ilo_cat_une==1
				egen TEST_No_Empty=mean(TEST_No_Emptya)
				if TEST_No_Empty!=0 & TEST_No_Empty!=. {
					gen condition = 1 if ilo_lfs==2 & ilo_wap==1 & ilo_cat_une==1 & $LEVEL1!=. //The tabulations are calculated based on this condition.
				...
				log using "$LIBMPLOGS\LOG_for_${country}_${source}_${time}.smcl", append
				di in red "Variable ilo_cat_une doesn't exist."
				log close
			}
		}
		else {
