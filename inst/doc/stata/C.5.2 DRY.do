...
*** This code refers to the TWO extra levels tabulation cases for Economically Inactive People.
	...
	if !_rc {
		...
		if !_rc {
			...
			if !_rc {	
				...
				egen TEST_No_Empty1a=count($LEVEL1) if ilo_lfs==3 & ilo_wap==1
				egen TEST_No_Empty1=mean(TEST_No_Empty1a)
				egen TEST_No_Empty2a=count($LEVEL2) if ilo_lfs==3 & ilo_wap==1
				egen TEST_No_Empty2=mean(TEST_No_Empty2a)
				if TEST_No_Empty1!=0 & TEST_No_Empty2!=0 {
					gen condition = 1 if ilo_lfs==3 & ilo_wap==1 & $LEVEL2!=.
